...
void handle_teleport(struct char_data *ch, struct char_data *tar, int location) {
    ...
    if (success == true) { /* We have made it. */
        act("@w$n@w appears in an instant out of nowhere!@n", true, ch, NULL, NULL, TO_ROOM);
        if (DRAGGING(ch) && !IS_NPC(DRAGGING(ch))) {
            char_from_room(DRAGGING(ch));
            char_to_room(DRAGGING(ch), IN_ROOM(ch));
            act("@w$n@w appears in an instant out of nowhere being dragged by $N!@n", true, DRAGGING(ch), NULL, ch,
                TO_NOTVICT);
        }
        if (GRAPPLING(ch) && !IS_NPC(GRAPPLING(ch))) {
            char_from_room(GRAPPLING(ch));
            char_to_room(GRAPPLING(ch), IN_ROOM(ch));
            act("@w$n@w appears in an instant out of nowhere being grappled by $N!@n", true, GRAPPLING(ch), NULL, ch,
                TO_NOTVICT);
        }
        if (CARRYING(ch)) {
            char_from_room(CARRYING(ch));
            char_to_room(CARRYING(ch), IN_ROOM(ch));
            act("@w$n@w appears in an instant out of nowhere being carried by $N!@n", true, CARRYING(ch), NULL, ch,
                TO_NOTVICT);
        }
        if (GRAPPLED(ch) && !IS_NPC(GRAPPLED(ch))) {
            char_from_room(GRAPPLED(ch));
            char_to_room(GRAPPLED(ch), IN_ROOM(ch));
            act("@w$n@w appears in an instant out of nowhere being grappled by $N!@n", true, GRAPPLED(ch), NULL, ch,
                TO_NOTVICT);
        }
        if (DRAGGING(ch) && IS_NPC(DRAGGING(ch))) {
            act("@WYou stop dragging @C$N@W!@n", true, ch, NULL, DRAGGING(ch), TO_CHAR);
            act("@C$n@W stops dragging @c$N@W!@n", true, ch, NULL, DRAGGING(ch), TO_ROOM);
...
ACMD(do_carry) {
    ...
    if (CARRYING(ch)) { /* Already carrying someone. Put them down. Simple and clean. */
        ...
        if (GET_PC_WEIGHT(vict) + IS_CARRYING_W(vict) > CAN_CARRY_W(ch)) {
            ...
            act("@WYou pick up @C$N@W and put $M over your shoulder.@n", true, ch, NULL, vict, TO_CHAR);
            act("@C$n@W picks up $c$N@W and puts $M over $s shoulder.@n", true, ch, NULL, vict, TO_NOTVICT);
            if (SITS(vict)) {
...
void carry_drop(struct char_data *ch, int type) {
    ...
    switch (type) {
        case 0: /* Awww we were gentle >.> */
            act("@WYou gently set @C$N@W down on the ground.@n", true, ch, NULL, vict, TO_CHAR);
            act("@C$n @Wgently sets you down on the ground.@n", true, ch, NULL, vict, TO_VICT);
            act("@C$n @Wgently sets @c$N@W down on the ground.@n", true, ch, NULL, vict, TO_NOTVICT);
            break;
        case 1: /* We're not super nice. */
            act("@WYou set @C$N@W hastily onto the ground.@n", true, ch, NULL, vict, TO_CHAR);
            act("@C$n @Wsets you hastily onto the ground.@n", true, ch, NULL, vict, TO_VICT);
            act("@C$n @Wsets @c$N@W hastily onto the ground.@n", true, ch, NULL, vict, TO_NOTVICT);
            break;
        case 2: /* Uh oh we dropped them from being hit! */
            act("@WYou have @C$N@W knocked out of your arms and onto the ground!@n", true, ch, NULL, vict, TO_CHAR);
            act("@WYou are knocked out of @C$n's@W arms and onto the ground!@n", true, ch, NULL, vict, TO_VICT);
            act("@C$n @Whas @c$N@W knocked out of $s arms and onto the ground!@n", true, ch, NULL, vict, TO_NOTVICT);
            break;
        case 3: /* Uh oh they are being extracted! */
            act("@WYou stop carrying @C$N@W for some reason.@n", true, ch, NULL, vict, TO_CHAR);
            act("@C$n @Wstops carrying you for some reason.@n", true, ch, NULL, vict, TO_VICT);
            act("@C$n @Wstops carrying @c$N@W for some reason.@n", true, ch, NULL, vict, TO_NOTVICT);
            break;
    }
    CARRYING(ch) = NULL;
...
ACMD(do_land) {
    ...
    if (landing != -1) {
        ...
        snprintf(sendback, MAX_INPUT_LENGTH, "@C$n@Y flies down through the atmosphere toward @G%s@Y!@n", blah);
        act(sendback, true, ch, NULL, NULL, TO_ROOM);
        char_from_room(ch);
        char_to_room(ch, real_room(landing));
        int zone = 0;
        ...
        act("$n comes down from high above in the sky and quickly lands on the ground.", true, ch, NULL, NULL,
            TO_ROOM);
        return;
    }
}
...
static int has_flight(struct char_data *ch) {
    ...
    if (AFF_FLAGGED(ch, AFF_FLYING) &&
        ((ch_getMaxKI(ch) * ch->energy)) >= (GET_LEVEL(ch) + (GET_MAX_MANA(ch) / (GET_LEVEL(ch) * 30))) && !IS_ANDROID(ch) &&
        !IS_NPC(ch)) {
        return (1);
    }
    if (AFF_FLAGGED(ch, AFF_FLYING) && ((ch_getMaxKI(ch) * ch->energy)) < (GET_LEVEL(ch) + (GET_MAX_MANA(ch) / (GET_LEVEL(ch) * 30))) &&
        !IS_ANDROID(ch) && !IS_NPC(ch)) {
        act("@WYou crash to the ground, too tired to fly anymore!@n", true, ch, NULL, NULL, TO_CHAR);
        act("@W$n@W crashes to the ground!@n", true, ch, NULL, NULL, TO_ROOM);
   ...
   */
    ...
    if (((SECT(IN_ROOM(ch)) == SECT_WATER_NOSWIM) || (SECT(EXIT(ch, dir)->to_room) == SECT_WATER_NOSWIM)) &&
        ...
        if (IS_KANASSAN(ch) && !has_flight(ch)) {
            act("@CYou swim swiftly.@n", true, ch, NULL, NULL, TO_CHAR);
            act("@c$n@C swims swiftly.@n", true, ch, NULL, NULL, TO_ROOM);
            ...
            if (!check_swim(ch)) {
                ...
                act("@CYou swim through the cold water.@n", true, ch, NULL, NULL, TO_CHAR);
                act("@c$n@C swim through the cold water.@n", true, ch, NULL, NULL, TO_ROOM);
                WAIT_STATE(ch, PULSE_1SEC);
            }
        }
    }
    ...
    if (SUNKEN(IN_ROOM(ch)) ||
        ...
        if (!has_o2(ch) &&
            ((group_bonus(ch, 2) != 10 && ((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) / 200) || (group_bonus(ch, 2) == 10 &&
                                                                                                         ((ch_getMaxKI(
                                                                                                                 ch) *
                                                                                                           ch->energy)) <
                                                                                         GET_MAX_MANA(ch) / 800))) {
            if (GET_HIT(ch) >= (ch_getEffMaxPL(ch)) / 20) {
                send_to_char(ch, "@RYou struggle to breath!@n\r\n");
                int64_t amt = ch_getMaxPL(ch) / 20;
                int64_t result;
                result = ch_decCurHealth(ch, amt, 0);
            }
            if (GET_HIT(ch) < (ch_getEffMaxPL(ch)) / 20) {
        ...
        if (!has_o2(ch) &&
            ...
            if (group_bonus(ch, 2) == 10) {
                int64_t amt = ch_getMaxKI(ch) / 800;
                int64_t result;
                result = ch_decCurKI(ch, amt, 0);
            } else {
                int64_t amt = ch_getMaxKI(ch) / 200;
                int64_t result;
                result = ch_decCurKI(ch, amt, 0);
            }
        }
    ...
    if (AFF_FLAGGED(ch, AFF_FLYING) && (((ch_getMaxKI(ch) * ch->energy)) < flight_cost) && !IS_ANDROID(ch)) {
        int64_t result;
        result = ch_decCurKI(ch, flight_cost, 0);
        act("@WYou crash to the ground, too tired to fly anymore!@n", true, ch, NULL, NULL, TO_CHAR);
        act("@W$n@W crashes to the ground!@n", true, ch, NULL, NULL, TO_ROOM);
        REMOVE_BIT_AR(AFF_FLAGS(ch), AFF_FLYING);
    ...
    if (((ch_getCurST(ch))) < need_movement && !AFF_FLAGGED(ch, AFF_FLYING) && !IS_NPC(ch)) {
    ...
    if (EXIT(ch, dir)->dcskill != 0) {
        if (EXIT(ch, dir)->dcmove > roll_skill(ch, EXIT(ch, dir)->dcskill)) {
            ...
            if (!ADM_FLAGGED(ch, ADM_WALKANYWHERE) && !IS_NPC(ch) && !AFF_FLAGGED(ch, AFF_FLYING)) {
                int64_t result;
                result = ch_decCurST(ch, need_movement, 0);
            }
            return (0);
        } else {
    ...
    if (AFF_FLAGGED(ch, AFF_SNEAK) && !IS_NPC(ch)) {
        ...
        if (GET_SKILL(ch, SKILL_MOVE_SILENTLY)) {
            ...
            act(buf2, true, ch, NULL, NULL, TO_ROOM | TO_SNEAKRESIST);
    ...
    was_in = IN_ROOM(ch);
    if (DRAGGING(ch)) {
        act("@C$n@w drags @c$N@w with $m.@n", true, ch, NULL, DRAGGING(ch), TO_ROOM);
    }
    if (CARRYING(ch)) {
        act("@C$n@w carries @c$N@w with $m.@n", true, ch, NULL, CARRYING(ch), TO_ROOM);
    }
    ...
    act("$n arrives from $T.", true, ch, NULL, buf2, TO_ROOM | TO_SNEAKRESIST);
    if (FIGHTING(ch)) {
    ...
    if (DRAGGING(ch)) {
        act("@wYou drag @C$N@w with you.@n", true, ch, NULL, DRAGGING(ch), TO_CHAR);
        act("@C$n@w drags @c$N@w with $m.@n", true, ch, NULL, DRAGGING(ch), TO_ROOM);
        char_from_room(DRAGGING(ch));
        char_to_room(DRAGGING(ch), IN_ROOM(ch));
        if (SITS(DRAGGING(ch))) {
    ...
    if (CARRYING(ch)) {
        act("@wYou carry @C$N@w with you.@n", true, ch, NULL, CARRYING(ch), TO_CHAR);
        act("@C$n@w carries @c$N@w with $m.@n", true, ch, NULL, CARRYING(ch), TO_ROOM);
        char_from_room(CARRYING(ch));
        char_to_room(CARRYING(ch), IN_ROOM(ch));
    ...
    if (ch->desc != NULL) {
        ...
        if (AFF_FLAGGED(ch, AFF_SNEAK) && !IS_NPC(ch) && GET_SKILL(ch, SKILL_MOVE_SILENTLY) &&
            ...
            act("@c$n@w makes a noise revealing $s sneaking!@n", true, ch, NULL, NULL, TO_ROOM | TO_SNEAKRESIST);
            reveal_hiding(ch, 0);
            REMOVE_BIT_AR(AFF_FLAGS(ch), AFF_SNEAK);
        }
    }
    ...
    if (ROOM_EFFECT(IN_ROOM(ch)) == 6 || ROOM_EFFECT(was_in) == 6) {
        if (!IS_DEMON(ch) && !AFF_FLAGGED(ch, AFF_FLYING) && group_bonus(ch, 2) != 14) {
            act("@rYour legs are burned by the lava!@n", true, ch, NULL, NULL, TO_CHAR);
            act("@R$n@r's legs are burned by the lava!@n", true, ch, NULL, NULL, TO_ROOM);
            if (IS_NPC(ch) && IS_HUMANOID(ch) && rand_number(1, 2) == 2) {
                do_fly(ch, NULL, 0, 0);
            }
            int64_t amt = ch_getEffMaxPL(ch) / 20;
            int64_t result;
            result = ch_decCurHealth(ch, amt, 0);
            if (GET_HIT(ch) <= 0) {
                act("@rYou have burned to death!@n", true, ch, NULL, NULL, TO_CHAR);
                act("@R$n@r has burned to death!@n", true, ch, NULL, NULL, TO_ROOM);
                die(ch, NULL);
            }
        }
        if (DRAGGING(ch) && !IS_DEMON(DRAGGING(ch))) {
            act("@R$N@r gets burned!@n", true, ch, NULL, DRAGGING(ch), TO_CHAR);
            act("@R$N@r gets burned!@n", true, ch, NULL, DRAGGING(ch), TO_ROOM);
            int64_t amt = ch_getEffMaxPL(DRAGGING(ch)) / 20;
            int64_t result;
            result = ch_decCurHealth(DRAGGING(ch), amt, 0);
            if (GET_HIT(DRAGGING(ch)) < 0) {
                act("@rYou have burned to death!@n", true, DRAGGING(ch), NULL, NULL, TO_CHAR);
                act("@R$n@r has burned to death!@n", true, DRAGGING(ch), NULL, NULL, TO_ROOM);
...
ACMD(do_move) {
    ...
    if (GET_COND(ch, DRUNK) > 4 && (rand_number(1, 9) + GET_COND(ch, DRUNK)) >= rand_number(14, 20)) {
        ...
        act("@C$n@W wobbles around before falling on $s ass@n.", true, ch, NULL, NULL, TO_ROOM);
   ...
   */
    ...
    if (!IS_NPC(ch)) {
        ...
        if (ROOM_GRAVITY(IN_ROOM(ch)) == 10 && (ch_getEffMaxPL(ch)) <= 10000 && !IS_BARDOCK(ch) && !IS_NPC(ch)) {
            send_to_char(ch, "The gravity slows you down some.\r\n");
            WAIT_STATE(ch, PULSE_1SEC);
        }
        if (ROOM_GRAVITY(IN_ROOM(ch)) == 20 && (ch_getEffMaxPL(ch)) <= 30000) {
            send_to_char(ch, "The gravity slows you down some.\r\n");
            WAIT_STATE(ch, PULSE_2SEC);
        }
        if (ROOM_GRAVITY(IN_ROOM(ch)) == 30 && (ch_getEffMaxPL(ch)) <= 100000) {
            send_to_char(ch, "The gravity slows you down some.\r\n");
            WAIT_STATE(ch, PULSE_3SEC);
        }
        if (ROOM_GRAVITY(IN_ROOM(ch)) == 40 && (ch_getEffMaxPL(ch)) <= 200000) {
            send_to_char(ch, "The gravity slows you down some.\r\n");
            WAIT_STATE(ch, PULSE_3SEC);
        }
        if (ROOM_GRAVITY(IN_ROOM(ch)) == 50 && (ch_getEffMaxPL(ch)) <= 300000) {
            send_to_char(ch, "The gravity slows you down some.\r\n");
            WAIT_STATE(ch, PULSE_3SEC);
        }
        if (ROOM_GRAVITY(IN_ROOM(ch)) == 100 && (ch_getEffMaxPL(ch)) <= 500000) {
            send_to_char(ch, "The gravity slows you down some.\r\n");
            WAIT_STATE(ch, PULSE_3SEC);
        }
        if (ROOM_GRAVITY(IN_ROOM(ch)) == 200 && (ch_getEffMaxPL(ch)) <= 1000000) {
            send_to_char(ch, "The gravity slows you down some.\r\n");
            WAIT_STATE(ch, PULSE_3SEC);
        }
        if (ROOM_GRAVITY(IN_ROOM(ch)) == 300 && (ch_getEffMaxPL(ch)) <= 8000000) {
            send_to_char(ch, "The gravity slows you down some.\r\n");
            WAIT_STATE(ch, PULSE_3SEC);
        }
        if (ROOM_GRAVITY(IN_ROOM(ch)) == 400 && (ch_getEffMaxPL(ch)) <= 15000000) {
            send_to_char(ch, "The gravity slows you down some.\r\n");
            WAIT_STATE(ch, PULSE_3SEC);
        }
        if (ROOM_GRAVITY(IN_ROOM(ch)) == 500 && (ch_getEffMaxPL(ch)) <= 25000000) {
            send_to_char(ch, "The gravity slows you down some.\r\n");
            WAIT_STATE(ch, PULSE_4SEC);
        }
        if (ROOM_GRAVITY(IN_ROOM(ch)) == 1000 && (ch_getEffMaxPL(ch)) <= 35000000) {
            send_to_char(ch, "The gravity slows you down some.\r\n");
            WAIT_STATE(ch, PULSE_5SEC);
        }
        if (ROOM_GRAVITY(IN_ROOM(ch)) == 5000 && (ch_getEffMaxPL(ch)) <= 100000000) {
            send_to_char(ch, "The gravity slows you down some.\r\n");
            WAIT_STATE(ch, PULSE_5SEC);
        }
        if (ROOM_GRAVITY(IN_ROOM(ch)) == 10000 && (ch_getEffMaxPL(ch)) <= 200000000) {
            send_to_char(ch, "The gravity slows you down some.\r\n");
            WAIT_STATE(ch, PULSE_5SEC);
        }
        if (ROOM_FLAGGED(IN_ROOM(ch), ROOM_SPACE) && GET_ADMLEVEL(ch) < 1) {
            ...
            act("@wYou slowly pull yourself along with your arm...@n", true, ch, NULL, NULL, TO_CHAR);
            act("@C$n@w slowly pulls $mself along with one arm...@n", true, ch, NULL, NULL, TO_ROOM);
            if (GET_LIMBCOND(ch, 2) < 50) {
                ...
                if (GET_LIMBCOND(ch, 1) <= 0) {
                    act("@RYour left arm falls apart!@n", true, ch, NULL, NULL, TO_CHAR);
                    act("@r$n's@R left arm falls apart!@n", true, ch, NULL, NULL, TO_ROOM);
                }
            }
            WAIT_STATE(ch, PULSE_5SEC);
            ...
            if (GET_LIMBCOND(ch, 1) < 50) {
                ...
                if (GET_LIMBCOND(ch, 1) <= 0) {
                    act("@RYour right arm falls apart!@n", true, ch, NULL, NULL, TO_CHAR);
                    act("@r$n's@R right arm falls apart!@n", true, ch, NULL, NULL, TO_ROOM);
                }
            }
            WAIT_STATE(ch, PULSE_5SEC);
        } else if ((GET_LIMBCOND(ch, 3) <= 0 && GET_LIMBCOND(ch, 4) <= 0) && !AFF_FLAGGED(ch, AFF_FLYING)) {
            act("@wYou slowly pull yourself along with your arms...@n", true, ch, NULL, NULL, TO_CHAR);
            act("@C$n@w slowly pulls $mself along with one arms...@n", true, ch, NULL, NULL, TO_ROOM);
            if (GET_LIMBCOND(ch, 2) < 50) {
                send_to_char(ch, "@RYour left arm is damaged by the forced use!@n\r\n");
                GET_LIMBCOND(ch, 2) -= rand_number(1, 5);
            ...
            act("@wYou hop on one leg...@n", true, ch, NULL, NULL, TO_CHAR);
            act("@C$n@w hops on one leg...@n", true, ch, NULL, NULL, TO_ROOM);
            if (GET_LIMBCOND(ch, 4) < 50) {
                ...
                if (GET_LIMBCOND(ch, 4) <= 0) {
                    act("@RYour left leg falls apart!@n", true, ch, NULL, NULL, TO_CHAR);
                    act("@r$n's@R left leg falls apart!@n", true, ch, NULL, NULL, TO_ROOM);
                }
            }
            WAIT_STATE(ch, PULSE_2SEC);
            ...
            if (GET_LIMBCOND(ch, 3) < 50) {
                ...
                if (GET_LIMBCOND(ch, 3) <= 0) {
                    act("@RYour right leg falls apart!@n", true, ch, NULL, NULL, TO_CHAR);
                    act("@r$n's@R right leg falls apart!@n", true, ch, NULL, NULL, TO_ROOM);
                }
            }
            WAIT_STATE(ch, PULSE_2SEC);
        } else if (GET_POS(ch) == POS_RESTING) {
            act("@wYou crawl on your hands and knees.@n", true, ch, NULL, NULL, TO_CHAR);
            act("@C$n@w crawls on $s hands and knees.@n", true, ch, NULL, NULL, TO_ROOM);
            if (SITS(ch)) {
            ...
            act("@wYou shuffle on your hands and knees.@n", true, ch, NULL, NULL, TO_CHAR);
            act("@C$n@w shuffles on $s hands and knees.@n", true, ch, NULL, NULL, TO_ROOM);
            if (SITS(ch)) {
                struct obj_data *chair = SITS(ch);
                SITTING(chair) = NULL;
                SITS(ch) = NULL;
...
static void do_doorcmd(struct char_data *ch, struct obj_data *obj, int door, int scmd) {
            ...
            if (obj) {
                if ((obj) && GET_OBJ_TYPE(obj) == ITEM_HATCH && (vehicle)) {
                    ...
                    if (GET_OBJ_VNUM(obj) > 19199) {
                        ...
                        act("@wYou open @c$p@w.", true, ch, obj, NULL, TO_CHAR);
                        act("@C$n@w opens @c$p@w.", true, ch, obj, NULL, TO_ROOM);
            ...
            if (obj) {
                if ((obj) && GET_OBJ_TYPE(obj) == ITEM_HATCH && (vehicle)) {
                    ...
                    if (GET_OBJ_VNUM(obj) > 19199) {
                        ...
                        act("@wYou close @c$p@w.", true, ch, obj, NULL, TO_CHAR);
                        act("@C$n@w closes @c$p@w.", true, ch, obj, NULL, TO_ROOM);
    ...
    if (!obj || IN_ROOM(obj) != NOWHERE)
        act(buf, false, ch, obj, obj ? NULL : EXIT(ch, door)->keyword, TO_ROOM);
...
static int ok_pick(struct char_data *ch, obj_vnum keynum, int pickproof, int dclock, int scmd, struct obj_data *hatch) {
    ...
    if (keynum == NOTHING) {
        ...
        act("@c$n@w puts a set of lockpick tools away.@n", true, ch, NULL, NULL, TO_ROOM);
...
ACMD(do_gen_door) {
    ...
    if ((obj) || (door >= 0)) {
        ...
        if (!(DOOR_IS_OPENABLE(ch, obj, door)))
            act("You can't $F that!", false, ch, NULL, cmd_door[subcmd], TO_CHAR);
...
static int do_simple_enter(struct char_data *ch, struct obj_data *obj, int need_specials_check) {
    ...
    act("$n enters $p.", true, ch, obj, NULL, TO_ROOM | TO_SNEAKRESIST);
    ...
    if (GET_OBJ_TYPE(obj) == ITEM_PORTAL)
        act("$n arrives from $p.", false, ch, obj, NULL, TO_ROOM | TO_SNEAKRESIST);
    else
        act("$n arrives from outside.", false, ch, NULL, NULL, TO_ROOM | TO_SNEAKRESIST);
    if (DRAGGING(ch)) {
        act("@wYou drag @C$N@w with you.@n", true, ch, NULL, DRAGGING(ch), TO_CHAR);
        act("@C$n@w drags @c$N@w with $m.@n", true, ch, NULL, DRAGGING(ch), TO_ROOM);
        if (!AFF_FLAGGED(DRAGGING(ch), AFF_KNOCKED) && !AFF_FLAGGED(DRAGGING(ch), AFF_SLEEP) && rand_number(1, 3)) {
...
static int do_simple_leave(struct char_data *ch, struct obj_data *obj, int need_specials_check) {
    ...
    act("$n leaves $p.", true, ch, vehicle, NULL, TO_ROOM | TO_SNEAKRESIST);
    ...
    if (vehicle) {
        act("$n arrives from inside $p.", true, ch, vehicle, NULL, TO_ROOM | TO_SNEAKRESIST);
    } else {
        act("$n arrives from inside", true, ch, NULL, NULL, TO_ROOM | TO_SNEAKRESIST);
    }
    if (DRAGGING(ch)) {
        act("@wYou drag @C$N@w with you.@n", true, ch, NULL, DRAGGING(ch), TO_CHAR);
        act("@C$n@w drags @c$N@w with $m.@n", true, ch, NULL, DRAGGING(ch), TO_ROOM);
    ...
    if (ch->desc != NULL) {
        act(obj->action_description, true, ch, obj, NULL, TO_CHAR);
        look_at_room(IN_ROOM(ch), ch, 0);
    }
...
static void handle_fall(struct char_data *ch) {
    ...
    while (EXIT(ch, 5) && SECT(IN_ROOM(ch)) == SECT_FLYING) {
        ...
        if (!EXIT(ch, 5) || SECT(IN_ROOM(ch)) != SECT_FLYING) {
            act("@r$n slams into the ground!@n", true, ch, NULL, NULL, TO_ROOM);
            int64_t amt = ch_getEffMaxPL(ch) / 20;
            int64_t result;
            result = ch_decCurHealth(ch, amt, 1);

            act("@rYou slam into the ground!@n", true, ch, NULL, NULL, TO_CHAR);
            look_at_room(IN_ROOM(ch), ch, 0);
        } else {
            act("@r$n pummets down toward the ground below!@n", true, ch, NULL, NULL, TO_ROOM);
        }
    }
    if (SECT(IN_ROOM(ch)) == SECT_WATER_NOSWIM && !CARRIED_BY(ch) && !IS_KANASSAN(ch)) {
        if (((ch_getCurST(ch))) >= ((ch_getCurCarriedWeight(ch)))) {
            act("@bYou swim in place.@n", true, ch, NULL, NULL, TO_CHAR);
            act("@C$n@b swims in place.@n", true, ch, NULL, NULL, TO_ROOM);
            int64_t amt = ch_getCurCarriedWeight(ch);
            int64_t result;
            result = ch_decCurST(ch, amt, 0);
            act("@RYou are drowning!@n", true, ch, NULL, NULL, TO_CHAR);
            act("@C$n@b gulps water as $e struggles to stay above the water line.@n", true, ch, NULL, NULL,
                TO_ROOM);
            if (GET_HIT(ch) - ((ch_getEffMaxPL(ch)) / 3) <= 0) {
                act("@rYou drown!@n", true, ch, NULL, NULL, TO_CHAR);
                act("@R$n@r drowns!@n", true, ch, NULL, NULL, TO_ROOM);
                die(ch, NULL);
                int64_t result1;
                result1 = ch_decCurHealthPercent(ch, 1, 1);
            } else {
                int64_t result1;
                result1 = ch_decCurHealthPercent(ch, .33, 0);
            }
        }
    }
...
static int check_swim(struct char_data *ch) {
    ...
    if (ROOM_FLAGGED(IN_ROOM(ch), ROOM_SPACE)) {
        int64_t space_cost = (GET_MAX_MANA(ch) / 1000) + (((ch_getCurCarriedWeight(ch))) / 2);
        if (ch_getMaxKI(ch) * ch->energy >= space_cost)
            can = true;
        int64_t result1;
        result1 = ch_decCurKI(ch, space_cost, 0);
        if (!can) send_to_char(ch, "You do not have enough ki to fly through space. You are drifting helplessly.\r\n");
        return can;
    } else {
        int64_t swim_cost = ((ch_getCurCarriedWeight(ch))) - 1;
        if (ch_getCurST(ch) >= swim_cost)
            can = true;
        int64_t result;
        result = ch_decCurST(ch, swim_cost, 0);
...
ACMD(do_fly) {
    ...
    if (!*arg) {
        if (AFF_FLAGGED(ch, AFF_FLYING) && SECT(IN_ROOM(ch)) != SECT_FLYING && SECT(IN_ROOM(ch)) != SECT_SPACE) {
            act("@WYou slowly settle down to the ground.@n", true, ch, NULL, NULL, TO_CHAR);
            act("@W$n slowly settles down to the ground.@n", true, ch, NULL, NULL, TO_ROOM);
            REMOVE_BIT_AR(AFF_FLAGS(ch), AFF_FLYING);
            GET_ALT(ch) = 0;
            return;
        }
        ...
        if (AFF_FLAGGED(ch, AFF_FLYING) && SECT(IN_ROOM(ch)) == SECT_FLYING) {
            act("@WYou begin to plummet to the ground!@n", true, ch, NULL, NULL, TO_CHAR);
            act("@W$n starts to pummet to the ground below!@n", true, ch, NULL, NULL, TO_ROOM);
            REMOVE_BIT_AR(AFF_FLAGS(ch), AFF_FLYING);
            GET_ALT(ch) = 0;
            handle_fall(ch);
            return;
        ...
        if (AFF_FLAGGED(ch, AFF_FLYING) && SECT(IN_ROOM(ch)) == SECT_SPACE) {
            act("@WYou let yourself drift aimlessly through space.@n", true, ch, NULL, NULL, TO_CHAR);
            act("@W$n starts to drift slowly.!@n", true, ch, NULL, NULL, TO_ROOM);
            REMOVE_BIT_AR(AFF_FLAGS(ch), AFF_FLYING);
            GET_ALT(ch) = 0;
            return;
        }
        if (((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) / 100 && !IS_ANDROID(ch)) {
            send_to_char(ch, "You do not have the ki to fly.");
            return;
        } else {
            reveal_hiding(ch, 0);
            act("@WYou slowly take off into the sky.@n", true, ch, NULL, NULL, TO_CHAR);
            act("@W$n slowly takes off into the sky.@n", true, ch, NULL, NULL, TO_ROOM);
            if (SITS(ch)) {
    ...
    if (!strcasecmp("high", arg)) {
        if (((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) / 100 && !IS_ANDROID(ch)) {
            ...
            reveal_hiding(ch, 0);
            act("@WYou rocket high into the sky.@n", true, ch, NULL, NULL, TO_CHAR);
            act("@W$n rockets high into the sky.@n", true, ch, NULL, NULL, TO_ROOM);
            if (SITS(ch)) {
                SITTING(SITS(ch)) = NULL;
                SITS(ch) = NULL;
            }
    ...
    if (!strcasecmp("space", arg)) {
        ...
        if (((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) / 10 && !IS_ANDROID(ch)) {
        ...
        if (ROOM_FLAGGED(IN_ROOM(ch), ROOM_EARTH)) {
            ...
            act("@C$n blasts up from the atmosphere below and then comes to a stop.@n", true, ch, NULL, NULL,
                TO_ROOM);
...
ACMD(do_stand) {
    ...
    switch (GET_POS(ch)) {
        ...
        case POS_SITTING:
            ...
            act("$n clambers to $s feet.", true, ch, NULL, NULL, TO_ROOM);
            if (SITS(ch)) {
                if (CAN_WEAR(SITS(ch), ITEM_WEAR_TAKE) && GET_OBJ_TYPE(SITS(ch)) != ITEM_CHAIR &&
                    ...
                    act("You pick up $p.", true, ch, SITS(ch), NULL, TO_CHAR);
                    act("$n picks up $p.", true, ch, SITS(ch), NULL, TO_ROOM);
                }
                chair = SITS(ch);
                SITTING(chair) = NULL;
                SITS(ch) = NULL;
        ...
        case POS_RESTING:
            ...
            act("$n stops resting, and clambers to $s feet.", true, ch, NULL, NULL, TO_ROOM);
            if (SITS(ch)) {
    ...
    if (!*arg) {
        switch (GET_POS(ch)) {
            case POS_STANDING:
                ...
                act("$n sits down.", false, ch, NULL, NULL, TO_ROOM);
                GET_POS(ch) = POS_SITTING;
                break;
            case POS_SITTING:
            ...
            default:
                ...
                act("$n stops floating around, and sits down.", true, ch, NULL, NULL, TO_ROOM);
                GET_POS(ch) = POS_SITTING;
                break;
        }
    } else {
        ...
        switch (GET_POS(ch)) {
            case POS_STANDING:
                reveal_hiding(ch, 0);
                act("You sit down on $p.", false, ch, chair, NULL, TO_CHAR);
                act("$n sits down on $p.", false, ch, chair, NULL, TO_ROOM);
                GET_POS(ch) = POS_SITTING;
                SITS(ch) = chair;
                SITTING(chair) = ch;
                break;
    ...
    if (!*arg) {
        ...
        switch (GET_POS(ch)) {
            case POS_STANDING:
                ...
                act("$n lays down and rests.", true, ch, NULL, NULL, TO_ROOM);
        ...
        switch (GET_POS(ch)) {
            case POS_STANDING:
                reveal_hiding(ch, 0);
                act("You lay down and rest on $p.", true, ch, chair, NULL, TO_CHAR);
                act("$n lays down and rests on $p.", true, ch, chair, NULL, TO_ROOM);
                SITS(ch) = chair;
                SITTING(chair) = ch;
                GET_POS(ch) = POS_RESTING;
                break;
...
ACMD(do_sleep) {
    ...
    if (!*arg) {
        ...
        switch (GET_POS(ch)) {
            ...
            case POS_STANDING:
                reveal_hiding(ch, 0);
                act("You lay down on $p and sleep.", false, ch, chair, NULL, TO_CHAR);
                act("$n lays down on $p and sleeps.", false, ch, chair, NULL, TO_ROOM);
                /* Fury Mode Loss for halfbreeds */
...
ACMD(do_wake) {
    ...
    if (*arg) {
        if (GET_POS(ch) == POS_SLEEPING)
            ...
            act("$E's in pretty bad shape!", false, ch, NULL, vict, TO_CHAR);
            ...
            act("You are awakened by $n.", false, ch, NULL, vict, TO_VICT | TO_SLEEP);
            GET_POS(vict) = POS_SITTING;
            if (DRAGGED(vict)) {
                act("@WYou stop dragging @C$N@W!@n", true, DRAGGED(vict), NULL, vict, TO_CHAR);
                act("@C$n@W stops dragging @c$N@W!@n", true, DRAGGED(vict), NULL, vict, TO_ROOM);
    ...
    if (AFF_FLAGGED(ch, AFF_SLEEP))
        ...
        if (DRAGGED(ch)) {
            act("@WYou stop dragging @C$N@W!@n", true, DRAGGED(ch), NULL, ch, TO_CHAR);
            act("@C$n@W stops dragging you!@n", true, DRAGGED(ch), NULL, ch, TO_VICT);
            act("@C$n@W stops dragging @c$N@W!@n", true, DRAGGED(ch), NULL, ch, TO_NOTVICT);
...
ACMD(do_follow) {
    ...
    if (AFF_FLAGGED(ch, AFF_CHARM) && (ch->master)) {
        act("But you only feel like following $N!", false, ch, NULL, ch->master, TO_CHAR);
