...
static void shopping_buy(char *arg, struct char_data *ch, struct char_data *keeper, vnum shop_nr) {
    ...
    strlcpy(tempstr, times_message(ch->carrying, NULL, bought), sizeof(tempstr));

    snprintf(tempbuf, sizeof(tempbuf), "$n buys %s.", tempstr);
    act(tempbuf, false, ch, obj, NULL, TO_ROOM);
...
static void shopping_sell(char *arg, struct char_data *ch, struct char_data *keeper, vnum shop_nr) {
    ...
    strlcpy(tempstr, times_message(NULL, name, sold), sizeof(tempstr));
...
SPECIAL(shop_keeper) {
    ...
    if (CMD_IS("steal")) {
        ...
        if (!SHOP_ALLOW_STEAL(shop_nr)) {
            ...
            act(argm, false, ch, NULL, keeper, TO_CHAR);
            act(argm, false, ch, NULL, keeper, TO_ROOM);
