...
void do_start(struct char_data *ch) {
    ...
    if (IS_ANDROID(ch) && PLR_FLAGGED(ch, PLR_SENSEM)) {
        ...
        int64_t amt2 = rand_number(400, 500);
        int64_t result2;
        result2 = ch_gainBasePL(ch, amt2, false);
        int64_t amt1 = rand_number(400, 500);
        int64_t result1;
        result1 = ch_gainBaseST(ch, amt1, false);
        int64_t amt = rand_number(400, 500);
        int64_t result;
        result = ch_gainBaseKI(ch, amt, false);
    }
...
void advance_level(struct char_data *ch, int whichclass) {
    ...
    if (GET_LEVEL(ch) >= 1) {
        ...
        if (!IS_HUMAN(ch)) {
            add_hp = (((ch_getBasePL(ch))) * 0.01) * pl_percent;
        } else if (IS_HUMAN(ch)) {
            add_hp = ((((ch_getBasePL(ch))) * 0.01) * pl_percent) * 0.8;
        }
        add_mana = (((ch->baseki)) * 0.01) * ki_percent;
        add_move = (((ch_getBaseST(ch))) * 0.01) * st_percent;
    ...
    if (GET_LEVEL(ch) > 1) {
        ...
        int64_t amt = rand_number(1, 20);
        int64_t result;
        result = ch_gainBasePL(ch, amt, false);
    ...
    int64_t result2;
    result2 = ch_gainBasePL(ch, add_hp, true);
    int64_t result;
    result = ch_gainBaseKI(ch, add_mana, true);
    int64_t result1;
    result1 = ch_gainBaseST(ch, add_move, true);
    int nhp = add_hp;
    int nma = add_mana;
    int nmo = add_move;
    add_hp = nhp;
    ...
    if (howlong) {
        ...
        for (i = 0; i < NUM_CLASSES; i++) {
            ptr += snprintf(ptr, sizeof(str) - (ptr - str), "%s%s%s%s%s%d", buf, buf2, buf,
                            (howlong == 2 ? sen_getName(i) : sen_getAbbr(i)), buf3,
        ...
        rank = GET_CLASS_RANKS(ch, GET_CLASS(ch));
        snprintf(ptr, sizeof(str) - (ptr - str), "%s%d%s", sen_getNameLower(ch->chclass),
...
const char* sen_getName(int sen_id) {
    switch(sen_id) {
        case CLASS_ROSHI:
            return "Roshi";
        case CLASS_PICCOLO:
...
const char* sen_getNameLower(int sen_id) {
    static char name[30];
    strcpy(name, sen_getName(sen_id));
    str_lower(name);
    return name;
}

const char* sen_getAbbr(int sen_id) {
    switch(sen_id) {
        case CLASS_ROSHI:
            return "Ro";
        case CLASS_PICCOLO:
...
int sen_findName(char *arg) {
    for(int i = 0; i < NUM_CLASSES; i++) {
        if(!strcasecmp(sen_getName(i), arg)) return i;
    }

    return CLASS_UNDEFINED;
}

bool sen_isPC(int sen_id) {
    return sen_id >= 0 && sen_id <= 13;
}

bool sen_raceOK(int sen_id, int race_id) {
    switch (sen_id) {
        case CLASS_ANDSIX:
...
bool sen_rppCost(int sen_id, int race_id) {
    switch (sen_id) {
        case CLASS_KABITO:
            if (race_id != RACE_KAI) {
                return 10;
...
int sen_gravTolerance(int sen_id) {
    switch (sen_id) {
        case CLASS_BARDOCK:
            return 10;
        default:
...
room_vnum sen_startRoom(int sen_id) {
    switch (sen_id) {
        case CLASS_ROSHI:
            return 1130;
        case CLASS_KABITO:
...
room_vnum sen_location(int sen_id) {
    switch (sen_id) {
        case CLASS_ROSHI:
            return 1131;
        case CLASS_KABITO:
...
const char* sen_style(int sen_id) {
    switch(sen_id) {
        case CLASS_ROSHI:
            return "Kame Arts";
        case CLASS_PICCOLO:
            return "Demon Taijutsu";
        case CLASS_KRANE:
            return "Crane Arts";
        case CLASS_NAIL:
            return "Tranquil Palm";
        case CLASS_BARDOCK:
            return "Brutal Beast";
        case CLASS_GINYU:
            return "Flaunted Style";
        case CLASS_FRIEZA:
            return "Frozen Fist";
        case CLASS_TAPION:
            return "Shadow Grappling";
        case CLASS_ANDSIX:
            return "Iron Hand";
        case CLASS_DABURA:
            return "Devil Dance";
        case CLASS_KABITO:
            return "Gentle Fist";
        case CLASS_JINTO:
            return "Star's Radiance";
        case CLASS_TSUNA:
            return "Sacred Tsunami";
        case CLASS_KURZAK:
            return "Adaptive Taijutsu";
        default:
            return "Like a Bum";
    }
}