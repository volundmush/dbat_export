    ...
    if (!GET_SKILL(ch, SKILL_SPIRITCONTROL)) {
        ...
        if (AFF_FLAGGED(ch, AFF_SPIRITCONTROL)) {
            ...
            if (((ch_getCurST(ch))) < cost) {
                ...
                act("@y$n@Y seems to concentrate hard for a moment.@n", true, ch, NULL, NULL, TO_ROOM);
...
ACMD(do_multiform) {
    ...
    static struct char_data* multis[30];
    int mcount = 0;
    ...
    if (!strcasecmp(arg, "split")) {
        ...
        if (GET_SKILL(ch, SKILL_MULTIFORM) < roll) {
            act("@YYou focus your ki into your body while concentrating on the image of your body splitting into two. @yYou lose your concentration and fail to split though...@n",
                true, ch, NULL, NULL, TO_CHAR);
            act("@y$n@Y seems to concentrate really hard for a moment, before relaxing.@n", true, ch, NULL, NULL,
                TO_ROOM);
            int64_t result;
            result = ch_decCurST(ch, cost, 0);
            int64_t result1;
...
static void generate_multiform(struct char_data *ch, int count) {
    static char blamo[MAX_INPUT_LENGTH];
    static char clone_name[MAX_STRING_LENGTH], clone_sdesc[MAX_STRING_LENGTH], clone_ldesc[MAX_STRING_LENGTH];
    ...
    snprintf(clone_name, MAX_STRING_LENGTH, "%s's Clone", ch->name);
    snprintf(clone_sdesc, MAX_STRING_LENGTH, "%s's @CClone@n", ch->name);
    snprintf(clone_ldesc, MAX_STRING_LENGTH, "%s's @CClone@w is standing here.@n\n", ch->name);
...
static void resolve_song(struct char_data *ch) {
    ...
    if (instrument == 0) {
        ...
        act("@c$n@C stops playing $s song.@n", true, ch, NULL, NULL, TO_ROOM);
        GET_SONG(ch) = 0;
        return;
    }
    ...
    if (skill > diceroll) {
        ...
        act("@CYou continue playing your song.@n", true, ch, NULL, NULL, TO_CHAR);
        act(buf, true, ch, NULL, NULL, TO_ROOM);
    } else {
        act("@CYou mess up a portion of the song, but continue playing.@n", true, ch, NULL, NULL, TO_CHAR);
        act("@c$n@C messes up a portion of $s song, but continues to play.@n", true, ch, NULL, NULL, TO_ROOM);
        return;
    }
                    ...
                    if ((AFF_FLAGGED(ch, AFF_GROUP) && AFF_FLAGGED(vict, AFF_GROUP)) || vict == ch) {
                        if (ch == vict->master || ch->master == vict || ch->master == vict->master || vict == ch) {
                            if (skill > diceroll) {
                                ...
                                int64_t result2;
                                result2 = ch_incCurHealth(vict, restore, true);
                                int64_t result;
                                result = ch_incCurST(vict, restore * .5, true);
                                ...
                                act("@c$n@C continues playing $s ocarina!@n", true, ch, NULL, vict, TO_NOTVICT);
                                improve_skill(ch, SKILL_MYSTICMUSIC, 2);
                                int64_t amt = (ch_getMaxKI(ch) * .0003) + skill;
                                int64_t result1;
                    ...
                    if (AFF_FLAGGED(ch, AFF_GROUP) && AFF_FLAGGED(vict, AFF_GROUP)) {
                        if (ch == vict->master || ch->master == vict || ch->master == vict->master) {
                            ...
                            if (!IS_NPC(vict)) {
                                ...
                                int64_t amt = ch_getPercentOfMaxKI(ch, .001) + skill;
                                int64_t result;
                ...
                if (vict == ch || (AFF_FLAGGED(ch, AFF_GROUP) && AFF_FLAGGED(vict, AFF_GROUP))) {
                    if (ch == vict->master || ch->master == vict || ch->master == vict->master || vict == ch) {
                        if (skill > diceroll) {
                            ...
                            int64_t amt = ch_getPercentOfMaxKI(ch, .02) + skill;
                            int64_t result;
...
ACMD(do_song) {
    ...
    if (GET_SONG(ch)) {
        act("@cYou stop playing your ocarina.@n", true, ch, NULL, NULL, TO_CHAR);
        act("@c$n stops playing their ocarina.@n", true, ch, NULL, NULL, TO_ROOM);
        GET_SONG(ch) = 0;
        return;
    } else {
        ...
        if (!*arg) {
            ...
            if (modifier == 0) {
                ...
                if (modifier == 3 && ((ch_getMaxKI(ch) * ch->energy)) < cost) {
                    ...
                    return;
                } else if (modifier == 8 && (ch_getCurKI(ch)) < cost) {
                    send_to_char(ch, "@wYou do not have enough ki to power the instrument for that song!@n\r\n");
                    return;
                    ...
                    result = ch_decCurKI(ch, cost, 0);
                    return;
                } else if (modifier == 50 && (ch_getCurKI(ch)) < cost) {
                    send_to_char(ch, "@wYou do not have enough ki to power the instrument for that song!@n\r\n");
                    return;
                    ...
                    result = ch_decCurKI(ch, cost, 0);
                    return;
                } else if (modifier == 20 && (ch_getCurKI(ch)) < cost) {
                    send_to_char(ch, "@wYou do not have enough ki to power the instrument for that song!@n\r\n");
                    return;
...
ACMD(do_moondust) {
    ...
    cost += ((ch_getMaxLF(ch))) * 0.02;
    heal = cost * 3;

    if (GET_HIT(ch) >= (ch_getEffMaxPL(ch)) * 0.8) {
        cost = cost * 0.5;
    }
    ...
    int64_t result2;
    result2 = ch_incCurHealth(ch, heal, true);
    int64_t result1;
    ...
    for (vict = world[IN_ROOM(ch)].people; vict; vict = next_v) {
        ...
        if (AFF_FLAGGED(vict, AFF_GROUP)) {
            if (ch->master == vict->master || vict->master == ch || ch->master == vict) {
                int64_t result;
                result = ch_incCurHealth(vict, heal, true);
                act("@CYou breathe in the dust and are healed by it somewhat!@n", true, vict, NULL, NULL,
                    TO_CHAR);
                act("@c$n@C breathes in the dust and is healed somewhat!@n", true, vict, NULL, NULL, TO_ROOM);
...
ACMD(do_shell) {
    ...
    if (AFF_FLAGGED(ch, AFF_SHELL)) {
        act("@mYou quickly absorb the armor carapace covering your body back inside.@n", true, ch, NULL, NULL,
            TO_CHAR);
        act("@M$n's@m armored carapce retreats back to its original size.@n", true, ch, NULL, NULL, TO_ROOM);
        REMOVE_BIT_AR(AFF_FLAGS(ch), AFF_SHELL);
        return;
    }

    if (((ch_getCurST(ch))) < GET_MAX_MOVE(ch) * 0.2) {
        ...
        act("@M$n@m crouches down and after a few moments of straining $s body's carapace armor starts to grow thicker and extends to cover all parts of $s body!@n",
            true, ch, NULL, NULL, TO_ROOM);
        int64_t result;
        result = ch_decCurSTPercent(ch, .2, 0);
...
ACMD(do_liquefy) {
    ...
    if (((ch_getMaxKI(ch) * ch->energy)) < (GET_MAX_MANA(ch) * 0.002) + 150) {
    ...
    if (!strcasecmp(arg, "hide")) {
        ...
        if (axion_dice(0) > GET_LEVEL(ch)) {
            ...
            act("@m$n@M's body starts to become loose and sag, but $e seems to return normal a moment later.@n", true,
                ch, NULL, NULL, TO_ROOM);
            int64_t amt = (GET_MAX_MANA(ch) * .002) + 150;
            int64_t result;
            result = ch_decCurKI(ch, amt, 0);
        ...
        if (!*arg2) {
            ...
            if (((ch_getMaxKI(ch) * ch->energy)) < (GET_MAX_MANA(ch) * 0.10) + 150) {
                ...
                int64_t result;
                result = ch_decCurKI(ch, amt, 0);
                int64_t dmg = (ch_getEffMaxPL(ch)) * 0.08;
...
ACMD(do_defend) {
    ...
    if (!*arg && GET_DEFENDING(ch) == NULL) {
        ...
        act("@YYou stop defending @y$N@Y.@n", true, ch, NULL, GET_DEFENDING(ch), TO_CHAR);
        act("@y$n@Y stops defending you.@n", true, ch, NULL, GET_DEFENDING(ch), TO_VICT);
        act("@y$n@Y stops defending @y$N@Y.@n", true, ch, NULL, GET_DEFENDING(ch), TO_NOTVICT);
    ...
    if (!(vict = get_char_vis(ch, arg, NULL, FIND_CHAR_ROOM))) {
        ...
        act("@YYou start defending @y$N@Y.@n", true, ch, NULL, vict, TO_CHAR);
        act("@y$n@Y starts defending you.@n", true, ch, NULL, vict, TO_VICT);
        act("@y$n@Y starts defending @y$N@Y.@n", true, ch, NULL, vict, TO_NOTVICT);
...
ACMD(do_fish) {
    ...
    if (!strcasecmp(arg, "cast")) {
        ...
        if (!PLR_FLAGGED(ch, PLR_FISHING)) {
            ...
            reveal_hiding(ch, 0);
            act("@CYou begin reeling the fish in slowly.@n", true, ch, NULL, NULL, TO_CHAR);
            act("@c$n@C begins to reel the line on $s pole in.@n", true, ch, NULL, NULL, TO_ROOM);
        ...
        if (!GET_EQ(ch, WEAR_WIELD2)) {
            ...
            if (GET_OBJ_TYPE(pole) != ITEM_FISHPOLE) {
                ...
                if (!(bait = get_obj_in_list_vis(ch, arg2, NULL, ch->carrying))) {
                    ...
                    reveal_hiding(ch, 0);
                    act("@CYou carefully apply the $p@C to your hook.@n", true, ch, bait, NULL, TO_CHAR);
                    act("@c$n@C carefully applies $p@C to $s fishing pole's hook.@n", true, ch, bait, NULL, TO_ROOM);
        ...
        if (!PLR_FLAGGED(ch, PLR_FISHING)) {
            ...
            reveal_hiding(ch, 0);
            act("@CYou reel in your line and stop fishing.@n", true, ch, NULL, NULL, TO_CHAR);
            act("@c$n@C reels in $s fishing line and stops fishing.@n", true, ch, NULL, NULL, TO_ROOM);
            ...
            if (PLR_FLAGGED(i, PLR_FISHING) && has_pole(i) == true) {
                ...
                if (GET_FISHD(ch) <= 0 && GET_FISHSTATE(ch) == FISH_REELING) { /* We've caught it */
                    ...
                    if (GET_FISHSTATE(ch) == FISH_REELING && rand_number(1, 100) <= 80) {
                        ...
                        act("@CYou reel the line on your pole some.@n", true, ch, NULL, NULL, TO_CHAR);
                        act("@c$n@C reels the line on $s pole slowly.@n", true, ch, NULL, NULL, TO_ROOM);
                        ...
                        act("@CYou struggle as the fish fights against your attempts to reel it in!@n", true, ch,
                            NULL, NULL, TO_CHAR);
                        act("@c$n@C struggles with the fish on the end of $s pole!@n", true, ch, NULL, NULL,
                            TO_ROOM);
                        ...
                        act("@CYou feel the line go slack and realize you've lost the fish! You reel your line back in...@n",
                            true, ch, NULL, NULL, TO_CHAR);
                        act("@c$n@C frowns and then begins to reel in $s line.@n", true, ch, NULL, NULL, TO_ROOM);
                        GET_FISHD(ch) = 0;
                        GET_FISHSTATE(ch) = FISH_NOFISH;
                        REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_FISHING);
                        if (has_pole(ch) == true) {
                        ...
                        act("@CYou feel as if the fish has stopped biting...@n", true, ch, NULL, NULL, TO_CHAR);
                        GET_FISHSTATE(ch) = FISH_NOFISH;
                        ...
                        act("@CYou feel a fish biting on your line! Better @Ghook@C it!@n", true, ch, NULL, NULL,
                            TO_CHAR);
...
static void catch_fish(struct char_data *ch, int quality) {
    ...
    act("@CYou manage to pull a $p@C from the water and onto the ground in front of you!@n", true, ch, fish, NULL,
        TO_CHAR);
    act("@c$n@C manages to pull a $p@C from the water and onto the ground in front of $m!@n", true, ch, fish, NULL,
        TO_ROOM);
...
ACMD(do_scry) {
    ...
    if (GET_PRACTICES(ch, GET_CLASS(ch)) < cost) {
        ...
        act("@GYou see @C$n@G begin to focus, and then without warning, your mind is flooded painfully with images, energy and information. The data streams in a mad torrent through your psyche, and just when you think snapping is possible, the voice of @C$n@G comes to you and eases and guides you. You see images of potential futures, information not yet known, knowledge yet undiscovered. Though you could not fully  grasp what is to come, you feel more prepared at facing the unknown.@n",
            true, ch, NULL, vict, TO_VICT);
        act("@C$n@W appears to be performing some sort of ritual or something with @c$N@W.@n", true, ch, NULL, vict,
            TO_NOTVICT);
        int64_t boost = GET_INT(ch) * 0.5;

        int64_t amt2 = (ch_getBasePL(vict) * .01) * boost;
        int64_t result2;
        result2 = ch_gainBasePL(vict, amt2, false);
        int64_t amt = (vict->baseki * .01) * boost;
        int64_t result;
        result = ch_gainBaseKI(vict, amt, false);
        int64_t amt1 = (ch_getBaseST(vict) * .01) * boost;
        int64_t result1;
        ...
        if (GET_LEVEL(ch) < 100) {
            ...
            ch_gainBaseAllPercent(ch, .025, true);
            ...
            if (GET_OBJ_VNUM(obj) == 1306) {
                if (axion_dice(0) > GET_CON(ch)) {
                    if (!IS_ANDROID(ch) && !IS_DEMON(ch) && !IS_ICER(ch)) {
                        reveal_hiding(ch, 0);
                        int64_t amt = ((GET_MAX_MOVE(ch) * 0.005) + 20) * GET_OBJ_COST(obj);
                        int64_t result;
                        result = ch_decCurST(ch, amt, 0);
                        act("@RYou choke on the the burning hot @Da@Ws@wh@Dc@Wl@wo@Du@Wd@R!@n", true, ch, NULL,
                            NULL, TO_CHAR);
                        act("@r$n@R chokes on the burning hot @Da@Ws@wh@Dc@Wl@wo@Du@Wd@R!@n", true, ch, NULL,
                            NULL, TO_ROOM);
                    }
                    if (!IS_ANDROID(ch) && !IS_DEMON(ch) && !IS_NPC(ch)) {
                        if (!PLR_FLAGGED(ch, PLR_EYEC) && !AFF_FLAGGED(ch, AFF_BLIND)) {
                            reveal_hiding(ch, 0);
                            act("@DYour eyes sting from the hot ash! You can't see!@n", true, ch, NULL, NULL,
                                TO_CHAR);
                            act("@r$n@D eyes appear to have been hurt by the ash!@n", true, ch, NULL, NULL,
                                TO_ROOM);
                            int duration = 1;
    ...
    if (!GET_SKILL(ch, SKILL_BUILD)) {
        ...
        if (!(obj = get_obj_in_list_vis(ch, arg, NULL, ch->carrying))) {
            ...
            if (!wearable_obj(obj)) {
                ...
                if (((ch_getCurST(ch))) < GET_OBJ_WEIGHT(obj) + (GET_MAX_MOVE(ch) / 40)) {
                    ...
                    if (GET_OBJ_SIZE(obj) == SIZE_SMALL) {
                        ...
                        act("@WYou carefully adjust the size of @c$p@W.@n", true, ch, obj, NULL, TO_CHAR);
                        act("@C$n@W carefully adjusts the size of @c$p@W.@n", true, ch, obj, NULL, TO_ROOM);
                        GET_OBJ_SIZE(obj) = SIZE_SMALL;
                        int64_t amt = GET_OBJ_WEIGHT(obj) + (GET_MAX_MOVE(ch) / 40);
                        int64_t result;
                        result = ch_decCurST(ch, amt, 0);
                    }
...
ACMD(do_amnisiac) {
    ...
    if (cost > ((ch_getMaxKI(ch) * ch->energy))) {
        send_to_char(ch, "You do not have enough ki!\r\n");
        return;
    } else if (perc < chance) {
        act("@WYou attempt to grab @C$N@W to kiss $M, but $E evades!@n", true, ch, NULL, vict, TO_CHAR);
...
ACMD(do_metamorph) {
    ...
    int64_t result;
    result = ch_decCurKI(ch, cost / 2, 0);
    if (perc < chance) {
        act("@WYou focus your energies and prepare your @RDark Metamorphisis@W but screw up your focus!@n", true, ch,
            NULL, NULL, TO_CHAR);
        act("@WA dark @Rred@W glow starts to surround @C$n@W, but it fades quickly.@n", true, ch, NULL, NULL,
            TO_ROOM);
        ...
        int64_t result1;
        result1 = ch_incCurHealthPercent(ch, .6, true);
        return;
    }
...
ACMD(do_hydromancy) {
    ...
    if (!strcasecmp(arg, "spike")) {
        ...
        if (GET_OBJ_WEIGHT(obj) + ((ch_getCurCarriedWeight(ch))) <= CAN_CARRY_W(ch))
            ...
            act("@C$n@w drops an ice spike.@n", true, ch, NULL, NULL, TO_ROOM);
        ...
        if (CAN_GO(ch, attempt)) {
            ...
            if (skill < chance) {
                act("@BUsing your ki you attempt to create a rush of water! @RYou fail!@n", true, ch, NULL, NULL,
                    TO_CHAR);
                act("@b$n@B seems to attempt to create water with $s ki! @RHowever, $e fails!@n", true, ch, NULL,
                    NULL, TO_ROOM);
                int64_t result;
                result = ch_decCurKI(ch, cost, 0);
                WAIT_STATE(ch, PULSE_2SEC);
                ...
                sprintf(bunn, "@B$n@B uses $s ki to create a rush of water flooding away toward the @C%s@B!@n",
                        dirs[attempt]);
                act(bun, true, ch, NULL, NULL, TO_CHAR);
                act(bunn, true, ch, NULL, NULL, TO_ROOM);
                ...
                for (vict = world[IN_ROOM(ch)].people; vict; vict = next_v) {
                    ...
                    if (!can_kill(ch, vict, NULL, 1)) {
                        act("@CYou are protected from the water!@n", true, vict, NULL, NULL, TO_VICT);
                        act("@C$n@C is protected from the water!@n", true, vict, NULL, NULL, TO_ROOM);
                    } else if (IS_KANASSAN(vict)) {
                        act("@CYou effortlessly swim against the current.@n", true, vict, NULL, NULL, TO_CHAR);
                        act("@C$n@C effortlessly swims against the current.@n", true, vict, NULL, NULL, TO_ROOM);
                    } else if (GET_SKILL_BASE(vict, SKILL_BALANCE) >= axion_dice(-10)) {
                        act("@CYou manage to keep your balance and are not swept away!@n", true, vict, NULL, NULL,
                            TO_CHAR);
                        act("@C$n@C manages to keep $s balance and is not swept away!@n", true, vict, NULL, NULL,
                            TO_ROOM);
                    } else if (AFF_FLAGGED(ch, AFF_FLYING)) {
                        act("@CYou fly above the rushing waters and are untouched.@n", true, vict, NULL, NULL,
                            TO_CHAR);
                        act("@C$n@C flies above the rushing waters and is untouched.@n", true, vict, NULL, NULL,
                            TO_ROOM);
                    } else {
                        act("@cYou are caught by the rushing waters and sent tumbling away!@n", true, vict, NULL,
                            NULL, TO_CHAR);
                        act("@c$n@c is caught by the rushing waters and sent tumbling away!@n", true, vict, NULL,
                            NULL, TO_ROOM);
...
ACMD(do_kanso) {
    ...
    if (((ch_getMaxKI(ch) * ch->energy)) < cost) { /* Not enough ki */
        send_to_char(ch, "You do not have enough ki.\r\n");
        return;
    }
    ...
    if (skill < dice) { /* Failed the technique, user's fault */
        act("You close your eyes and focus, before bounding effortlessly toward $N. Closing the distance, you place your hands on $N's chest but nothing happens!\r\n",
            true, ch, NULL, vict, TO_CHAR); /* Message Character $n sees */
        ...
        /* Heal the user */
        int64_t amt = (ch_getEffMaxPL(ch) * .01) * dam;
        int64_t result1;
        ...
        if (skill > pdice && !AFF_FLAGGED(vict, AFF_PARA)) { /* Paralyze them too */
            act("@R$N@W is paralyzed by the attack!@n", true, ch, NULL, vict,
                TO_CHAR); /* Message Character $n sees */
            act("@RYou are paralyzed by the attack!@n", true, ch, NULL, vict, TO_VICT); /* Message Vict $N Sees */
            act("@R$N@Wis paralyzed by the attack!@n", true, ch, NULL, vict,
        ...
        if (GET_ROOM_VNUM(IN_ROOM(ch)) == 17875) {
            ...
            for (d = descriptor_list; d; d = d->next) {
                ...
                if (IN_ARENA(d->character)) {
                    ...
                    act(buf, true, ch, NULL, d->character, TO_CHAR);
                    found = true;
                }
            }
        ...
        if (num < 0) {
            ...
            if (found == true) {
                act("@wYou start watching the action surrounding that particular fighter in the arena.@n", true, ch,
                    NULL, NULL, TO_CHAR);
                act("@C$n@w starts watching the action in the arena.@n", true, ch, NULL, NULL, TO_ROOM);
        ...
        if (found == false) {
            ...
            if (!strcasecmp(arg2, "head")) {
                if (prob <= perc) {
                    ...
                    act("@WYou attempt to weave the bundle and manage to create $p@W!@n", true, ch, weaved, NULL,
                        TO_CHAR);
...
ACMD(do_adrenaline) {
    ...
    if (!IS_ARLIAN(ch) && (!IS_BIO(ch) || (IS_BIO(ch) && (GET_GENOME(ch, 0) != 6 && GET_GENOME(ch, 1) != 6)))) {
        ...
        if (!*arg || !*arg2) {
            ...
            if ((ch_getCurST(ch) - (ch_getBasePL(ch) * percent)) < 0) {
                send_to_char(ch, "You do not have enough stamina to trade for adrenaline!\r\n");
                return;
            }

            int64_t trade = ch_getBaseST(ch) * percent;
            ...
            if (!strcasecmp(arg, "pl")) {
                act("@GYou focus your mind and begin to overwork your powerful adrenal glands and your wounds begin to heal!@n",
                    true, ch, NULL, NULL, TO_CHAR);
                act("@g$n@G seems to concentrate and $s wounds begin to heal!@n", true, ch, NULL, NULL, TO_ROOM);

                if (GET_HIT(ch) + trade > (ch_getEffMaxPL(ch)))
                    send_to_char(ch, "Some of your stamina was wasted because your powerlevel maxed out.\r\n");
                int64_t result2;
                result2 = ch_incCurHealth(ch, trade, true);
                int64_t result;
                result = ch_decCurST(ch, trade, 0);

            } else if (!strcasecmp(arg, "ki")) {
                act("@GYou focus your mind and begin to overwork your powerful adrenal glands and you feel your ki replenish!@n",
                    true, ch, NULL, NULL, TO_CHAR);
                act("@g$n@G seems to concentrate and $e appears energized!@n", true, ch, NULL, NULL, TO_ROOM);

                if (((ch_getMaxKI(ch) * ch->energy)) + trade > GET_MAX_MANA(ch))
                    send_to_char(ch, "Some of your stamina was wasted because your ki maxed out.\r\n");
                int64_t result1;
                result1 = ch_incCurKI(ch, trade, true);
                int64_t result;
                result = ch_decCurST(ch, trade, 0);
            }
        } /* End inner else */
...
ACMD(do_obstruct) {
    ...
    if (!EXIT(ch, dir)) {
        ...
        if (strength > (ch_getEffMaxPL(ch)) * 20) {
            strength = (ch_getEffMaxPL(ch)) + (strength / 20);
        } else if (strength > (ch_getEffMaxPL(ch)) * 15) {
            strength = (ch_getEffMaxPL(ch)) + (strength / 15);
        } else if (strength > (ch_getEffMaxPL(ch)) * 10) {
            strength = (ch_getEffMaxPL(ch)) + (strength / 10);
        } else if (strength > (ch_getEffMaxPL(ch)) * 5) {
            strength = (ch_getEffMaxPL(ch)) + (strength / 5);
        } else if (strength > (ch_getEffMaxPL(ch)) * 2) {
            strength = (ch_getEffMaxPL(ch)) + (strength / 2);
        }
...
ACMD(do_dimizu) {
    ...
    if (ROOM_EFFECT(IN_ROOM(ch)) < 0) {
        ...
        if (((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) / 12) {
            send_to_char(ch, "You do not have enough ki to perform the technique.\r\n");
            return;
        } else if (skill < prob) {
...
ACMD(do_spoil) {
    ...
    if (type == 0) {
        act("@C$n@W reaches down and @rtears@W the head off of @R$p@W!@n", true, ch, obj, NULL, TO_ROOM);
        act("@WYou reach down and @rtear@W the head off of @R$p@W!@n", true, ch, obj, NULL, TO_CHAR);
    } else if (type == 1) {
        act("@C$n@W reaches down and @rcuts@W the head off of @R$p@W!@n", true, ch, obj, NULL, TO_ROOM);
        act("@WYou reach down and @rcut@W the head off of @R$p@W!@n", true, ch, obj, NULL, TO_CHAR);
    } else if (type == 2) {
        act("@C$n@W reaches down and @rcuts@W the head off of @R$p@W!@n", true, ch, obj, NULL, TO_ROOM);
        act("@WYou reach down and @rcut@W the head off of @R$p@W!@n", true, ch, obj, NULL, TO_CHAR);
    }
