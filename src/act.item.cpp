        ...
        if (!strcasecmp(arg2, "water")) {
            ...
            if (found == false) {
                ...
                act("@GAs you go to water @g$p@G you end up sloppily wasting about half of it on the ground.@n", true,
                    ch, obj, NULL, TO_CHAR);
                act("@g$n@G takes a bottle of grow water and sloshes some of it on @g$p@G.@n", true, ch, obj, NULL,
                    TO_ROOM);
                ...
                act("@GYou calmly and expertly pour the grow water on @g$p@G.@n", true, ch, obj, NULL, TO_CHAR);
                act("@g$n@G calmly and expertly pours some grow water on @g$p@G.@n", true, ch, obj, NULL, TO_ROOM);
                int64_t result;
                result = ch_decCurST(ch, cost, 0);
            ...
            if (found == false) {
                ...
                act("@GAs you go to harvest @g$p@G you end up cutting it in half instead!@n", true, ch, obj, NULL,
                    TO_CHAR);
                ...
                act("@GYou calmly and expertly harvest @g$p@G.@n", true, ch, obj, NULL, TO_CHAR);
                act("@g$n@G calmly and expertly harvests @g$p@G.@n", true, ch, obj, NULL, TO_ROOM);
                int64_t result;
                result = ch_decCurST(ch, cost, 0);
            ...
            if (found == false) {
                ...
                act("@GYou calmly dig up @g$p@G.@n", true, ch, obj, NULL, TO_CHAR);
                act("@g$n@G calmly digs up @g$p@G.@n", true, ch, obj, NULL, TO_ROOM);
                int64_t result;
                result = ch_decCurST(ch, cost, 0);
                obj_from_room(obj);
                obj_to_char(obj, ch);
            ...
            if (found == false) {
                ...
                act("@GYou dig a proper sized hole and plant @g$p@G in it.@n", true, ch, obj, NULL, TO_CHAR);
                act("@g$n@G digs a proper sized hole in a planter and plants @g$p@G in it.@n", true, ch, obj, NULL,
                    TO_ROOM);
                obj_from_char(obj);
            ...
            if (!OBJ_FLAGGED(obj, ITEM_MATURE)) {
                ...
                act("@GYou end up shredding @g$p@G with your clumsy unskilled hands.@n", true, ch, obj, NULL,
                    TO_CHAR);
                act("@g$n@G grabs a hold of @g$p@G and shreds it in an attempt to pick it.@n", true, ch, obj, NULL,
                    TO_ROOM);
                return;
            } else {
                act("@GYou grab a hold of @g$p@G and carefully pick it out of the soil.@n", true, ch, obj, NULL,
                    TO_CHAR);
                act("@g$n@G grabs a hold of @g$p@G and carefully picks it out of the soil.@n", true, ch, obj, NULL,
                    TO_ROOM);
                obj_from_room(obj);
                obj_to_char(obj, ch);
                int64_t result;
...
ACMD(do_twohand) {
    ...
    if (!GET_EQ(ch, WEAR_WIELD1) && !PLR_FLAGGED(ch, PLR_THANDW)) {
        ...
        act("$n stops wielding $s weapon with both hands.", true, ch, NULL, NULL, TO_ROOM);
        REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_THANDW);
        return;
    } else {
        send_to_char(ch, "You grab your weapon with both hands.\r\n");
        act("$n starts wielding $s weapon with both hands.", true, ch, NULL, NULL, TO_ROOM);
    ...
    if (!strcasecmp(arg, "list")) {
        ...
        for (obj = world[auct_room].contents; obj; obj = next_obj) {
            ...
            if (obj) {
                ...
                list++;
                if (GET_AUCTIME(obj) + 86400 > time(NULL) && GET_CURBID(obj) <= -1) {
                    send_to_char(ch,
                ...
                } else if (GET_AUCTIME(obj) + 86400 > time(NULL) && GET_CURBID(obj) > -1) {
                    send_to_char(ch,
        ...
        if (!obj2) {
            ...
            if (GET_SKILL(ch, SKILL_APPRAISE) < rand_number(1, 101)) {
                ...
                act("@c$n@w looks stumped about something they viewed on their scouter screen.@n", true, ch, NULL,
                    NULL, TO_ROOM);
                WAIT_STATE(ch, PULSE_2SEC);
                return;
            } else {
                send_to_char(ch, "You look at images of the object on your scouter.\r\n");
                act("@c$n@w looks at something on their scouter screen.@n", true, ch, NULL, NULL, TO_ROOM);
                send_to_char(ch, "@c------------------------------------------------------------------------\n");
        ...
        if (!obj2) {
            ...
            return;
        } else if (GET_AUCTIME(obj2) + 86400 <= time(NULL)) {
...
ACMD(do_assemble) {
    ...
    for (tools = ch->carrying; tools; tools = next_obj) {
        ...
        if (GET_OBJ_VNUM(tools) == 386 && GET_OBJ_VAL(tools, VAL_ALL_HEALTH) > 0) {
            tool = tools;
            act("@WYou open up your toolkit and take out the necessary tools.@n", true, ch, NULL, NULL, TO_CHAR);
            act("@C$n@W opens up $s toolkit and takes out the necessary tools.@n", true, ch, NULL, NULL, TO_ROOM);
        }
    }
    ...
    if (strcasecmp(argument, "campfire")) {
        ...
        if (GET_SKILL(ch, SKILL_SURVIVAL) <= roll) {
            ...
            if (tool && rand_number(1, 3) == 3 && GET_OBJ_VAL(tool, VAL_ALL_HEALTH) > 0) {
                ...
                act("@RYour toolset is looking a bit more worn.@n", true, ch, NULL, NULL, TO_CHAR);
...
static void perform_put(struct char_data *ch, struct obj_data *obj,
    ...
    if (OBJ_FLAGGED(cont, ITEM_FORGED)) {
        act("$P is forged and won't hold anything.", false, ch, NULL, cont, TO_CHAR);
        return;
    }
...
static int can_take_obj(struct char_data *ch, struct obj_data *obj) {
    if (!(CAN_WEAR(obj, ITEM_WEAR_TAKE))) {
        act("$p: you can't take that!", false, ch, obj, NULL, TO_CHAR);
        return (0);
    } else if (IS_CARRYING_N(ch) >= CAN_CARRY_N(ch)) {
        act("$p: your arms are full!", false, ch, obj, NULL, TO_CHAR);
        return (0);
    } else {
        if ((((ch_getCurCarriedWeight(ch))) + GET_OBJ_WEIGHT(obj)) > CAN_CARRY_W(ch)) {
            act("$p: you can't carry that much weight.", false, ch, obj, NULL, TO_CHAR);
            return (0);
        } else if ((GET_OBJ_WEIGHT(obj) + ROOM_GRAVITY(IN_ROOM(ch))) + (ch_getCurCarriedWeight(ch)) > CAN_CARRY_W(ch)) {
            act("$p: you can't carry that much weight because of the gravity.", false, ch, obj, NULL, TO_CHAR);
            return (0);
        }
    }
    return (1);
...
static void get_check_money(struct char_data *ch, struct obj_data *obj) {
    ...
    if (GET_GOLD(ch) + value > GOLD_CARRY(ch)) {
        ...
        act("@w$n @wdrops some onto the ground.@n", false, ch, NULL, NULL, TO_ROOM);
        extract_obj(obj);
        int diff = 0;
        ...
        if (SITS(ch) && GET_OBJ_VNUM(SITS(ch)) > 603 && GET_OBJ_VNUM(SITS(ch)) < 608 &&
            ...
            if (OBJ_FLAGGED(obj, ITEM_HOT)) {
                if (GET_BONUS(ch, BONUS_FIREPROOF) <= 0 && !IS_DEMON(ch)) {
                    int64_t result;
                    result = ch_decCurHealthPercent(ch, .25, 0);
                    ...
                    act("@RYou are burned by it!@n", true, ch, NULL, NULL, TO_CHAR);
                    act("@R$n@R is burned by it!@n", true, ch, NULL, NULL, TO_ROOM);
                }
            }
            if (IS_NPC(ch)) {
                item_check(obj, ch);
...
static void get_from_container(struct char_data *ch, struct obj_data *cont,
    ...
    if (OBJVAL_FLAGGED(cont, CONT_CLOSED))
        ...
        if (!found) {
            if (obj_dotmode == FIND_ALL)
                act("$p seems to be empty.", false, ch, cont, NULL, TO_CHAR);
            else {
                char buf[MAX_STRING_LENGTH];
...
int perform_get_from_room(struct char_data *ch, struct obj_data *obj) {
    ...
    if (can_take_obj(ch, obj) && get_otrigger(obj, ch)) {
        ...
        act("$n gets $p.", true, ch, obj, NULL, TO_ROOM);

        if (OBJ_FLAGGED(obj, ITEM_HOT)) {
            if (GET_BONUS(ch, BONUS_FIREPROOF) <= 0 && !IS_DEMON(ch)) {
                int64_t result;
                result = ch_decCurHealthPercent(ch, .25, 1);
                if (GET_BONUS(ch, BONUS_FIREPRONE) > 0) {
                    int64_t result1;
                    result1 = ch_decCurHealthPercent(ch, 1, 1);
                }
    ...
    if (amount <= 0)
        ...
        if (mode != SCMD_JUNK) {
            ...
            if (mode == SCMD_DONATE) {
                ...
                obj_to_room(obj, RDR);
                act("$p suddenly appears in a puff of orange smoke!", 0, NULL, obj, NULL, TO_ROOM);
            } else {
                char buf[MAX_STRING_LENGTH];
    ...
    switch (mode) {
        case SCMD_DROP:
            if (!OBJ_FLAGGED(obj, ITEM_UNBREAKABLE) && ROOM_EFFECT(IN_ROOM(ch)) == 6) {
                act("$p melts in the lava!", false, ch, obj, NULL, TO_CHAR);
                act("$p melts in the lava!", false, ch, obj, NULL, TO_ROOM);
                extract_obj(obj);
            } else if (ROOM_EFFECT(IN_ROOM(ch)) == 6) {
                act("$p plops down on some cooled lava!", false, ch, obj, NULL, TO_CHAR);
                act("$p plops down on some cooled lava!", false, ch, obj, NULL, TO_ROOM);
                obj_to_room(obj, IN_ROOM(ch));
                if (GET_ADMLEVEL(ch) > 0) {
                    send_to_imm("IMM DROP: %s dropped %s in room [%d]", GET_NAME(ch), obj->short_description,
                                GET_ROOM_VNUM(IN_ROOM(obj)));
...
static void perform_give(struct char_data *ch, struct char_data *vict,
    ...
    if ((GET_OBJ_WEIGHT(obj) + ROOM_GRAVITY(IN_ROOM(vict))) + ((ch_getCurCarriedWeight(vict))) > CAN_CARRY_W(vict)) {
        act("$E can't carry that much weight because of the gravity.", false, ch, NULL, vict, TO_CHAR);
        if (IS_NPC(ch)) {
            act("$n@n drops $p because you can't carry anymore.", true, ch, obj, vict, TO_VICT);
            act("$n@n drops $p on the ground since $N's unable to carry it.", true, ch, obj, vict, TO_NOTVICT);
            obj_from_char(obj);
    ...
    if (OBJ_FLAGGED(obj, ITEM_HOT)) {
        if (GET_BONUS(vict, BONUS_FIREPROOF) <= 0 && !IS_DEMON(vict)) {
            ...
            act("@RYou are burned by it!@n", true, vict, NULL, NULL, TO_CHAR);
            act("@R$n@R is burned by it!@n", true, vict, NULL, NULL, TO_ROOM);
        }
    }
...
ACMD(do_drink) {
    ...
    if (!*arg && !IS_NPC(ch)) {
        ...
        switch (SECT(IN_ROOM(ch))) {
            ...
            case SECT_UNDERWATER:
                ...
                if (GET_SKILL(ch, SKILL_WELLSPRING) && ((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) && wasthirsty <= 30 &&
                    subcmd != SCMD_SIP) {

                    int64_t amt = ((GET_MAX_MANA(ch) * 0.005) + (GET_WIS(ch) * rand_number(80, 100))) *
            ...
            default:
                if (!SUNKEN(IN_ROOM(ch))) {
                    ...
                    send_to_char(ch, "You take a refreshing drink from the surrounding water.\r\n");
                    gain_condition(ch, THIRST, 1);
                    if (GET_SKILL(ch, SKILL_WELLSPRING) && !ch_isFullKI(ch) && wasthirsty <= 30 && subcmd != SCMD_SIP) {
                        int64_t amt = ((GET_MAX_MANA(ch) * 0.005) + (GET_WIS(ch) * rand_number(80, 100))) *
                                      GET_SKILL(ch, SKILL_WELLSPRING);
                        if (ch_incCurKI(ch, amt, true) == ch_getMaxKI(ch)) {
                            send_to_char(ch, "You feel your ki return to full strength.\r\n");
                        } else {
    ...
    if (IS_NPC(ch)) {     /* Cannot use GET_COND() on mobs. */
        act("$n@w drinks from $p.", true, ch, temp, NULL, TO_ROOM);
        obj_from_char(temp);
        extract_obj(temp);
        return;
    }
    ...
    if (subcmd == SCMD_DRINK) {
        ...
        if (temp->action_description)
            act(temp->action_description, true, ch, temp, NULL, TO_CHAR);
    ...
    if (GET_FOODR(ch) == 0 && subcmd != SCMD_SIP) {
        int64_t amt = (ch_getMaxST(ch) / 100) * amount;
        int64_t result;
...
! */
        ...
        act("$n chokes and utters some strange sounds.", true, ch, NULL, NULL, TO_ROOM);
...
ACMD(do_eat) {
    ...
    if (subcmd == SCMD_EAT) {
        act("You eat some of $p.", false, ch, food, NULL, TO_CHAR);
        if (food->action_description)
            act(food->action_description, false, ch, food, NULL, TO_CHAR);
        act("$n eats some of $p.", true, ch, food, NULL, TO_ROOM);
    } else {
    ...
    if (GET_FOODR(ch) == 0 && subcmd != SCMD_TASTE) {
        int64_t amt = (ch_getMaxST(ch) / 100) * amount;
        int64_t result1;
        result1 = ch_incCurST(ch, amt, true);
        GET_FOODR(ch) = 2;
        if (OBJ_FLAGGED(food, ITEM_YUM)) {
            int64_t result;
            result = ch_incCurSTPercent(ch, .25, true);
        }
        send_to_char(ch, "You feel rejuvinated by it.\r\n");
    }
    ...
    if (GET_OBJ_VNUM(food) >= MEAL_START && GET_OBJ_VNUM(food) <= MEAL_LAST && GET_COND(ch, HUNGER) < 48 &&
        ...
        if (!GET_OBJ_VAL(food, VAL_FOOD_POISON) && GET_HIT(ch) < (ch_getEffMaxPL(ch)) && subcmd != SCMD_TASTE) {
            int64_t suppress = ((ch_getEffMaxPL(ch)) * 0.01) * GET_SUPPRESS(ch);
            if (GET_WEIGHT(food) < 6) {
                int64_t result;
                result = ch_incCurHealthPercent(ch, .05, true);
            } else {
                int64_t result;
    ...
    if (GET_OBJ_VAL(food, VAL_FOOD_POISON) && !ADM_FLAGGED(ch, ADM_NOPOISON)) {
        ...
        act("$n coughs and utters some strange sounds.", false, ch, NULL, NULL, TO_ROOM);
    ...
    if (subcmd == SCMD_EAT) {
        if (foob >= GET_OBJ_VAL(food, VAL_FOOD_FOODVAL)) {
            ...
            if (GET_OBJ_VNUM(food) == 53) {
                ...
                result1 = ch_incCurST(ch, amt, true);
                int64_t result;
                result = ch_incCurLFPercent(ch, .01, true);
            ...
            if (GET_OBJ_VNUM(food) == 95) {
                ...
                result1 = ch_incCurST(ch, amt, true);
                int64_t result;
                result = ch_incCurLFPercent(ch, .02, true);
                if (OBJ_FLAGGED(food, ITEM_FORGED)) {
                    send_to_char(ch, "This is a forgery. You gain nothing!\r\n");
                } else {
                    send_to_char(ch, "You feel more energetic!\r\n");
            ...
            if (GET_OBJ_VNUM(food) == 95) {
                ...
                result1 = ch_incCurST(ch, amt, true);
                int64_t result;
                result = ch_incCurLFPercent(ch, .03, true);
                if (OBJ_FLAGGED(food, ITEM_FORGED)) {
                    send_to_char(ch, "This is a forgery. You gain nothing!\r\n");
                } else {
                    send_to_char(ch, "You feel more energetic!\r\n");
...
/* Rillao: transloc, add new transes here */
    ...
    switch (type) {
        case -1: {
            st = MIN(300000L, (((ch_getBaseST(ch))) / 1200) + rand_number(GET_LEVEL(ch), GET_LEVEL(ch) * 2));
        }
            pl = MIN(300000L, (((ch_getBasePL(ch))) / 1200) + rand_number(GET_LEVEL(ch), GET_LEVEL(ch) * 2));
            ki = MIN(300000L, (((ch->baseki)) / 1200) + rand_number(GET_LEVEL(ch), GET_LEVEL(ch) * 2));
            break;
        case 0: {
            st = MIN(500000L, (((ch_getBaseST(ch))) / 1200) + rand_number(GET_LEVEL(ch), GET_LEVEL(ch) * 2));
        }
            pl = MIN(500000L, (((ch_getBasePL(ch))) / 1200) + rand_number(GET_LEVEL(ch), GET_LEVEL(ch) * 2));
            ki = MIN(500000L, (((ch->baseki)) / 1200) + rand_number(GET_LEVEL(ch), GET_LEVEL(ch) * 2));
            break;
        case 1: {
            st = MIN(1200000L, (((ch_getBaseST(ch))) / 1000) + rand_number(GET_LEVEL(ch), GET_LEVEL(ch) * 2));
        }
            pl = MIN(1200000L, (((ch_getBasePL(ch))) / 1000) + rand_number(GET_LEVEL(ch), GET_LEVEL(ch) * 2));
            ki = MIN(1200000L, (((ch->baseki)) / 1000) + rand_number(GET_LEVEL(ch), GET_LEVEL(ch) * 2));
            break;
        case 2: {
            st = MIN(1500000L, (((ch_getBaseST(ch))) / 900) + rand_number(GET_LEVEL(ch), GET_LEVEL(ch) * 2));
        }
            pl = MIN(1500000L, (((ch_getBasePL(ch))) / 900) + rand_number(GET_LEVEL(ch), GET_LEVEL(ch) * 2));
            ki = MIN(1500000L, (((ch->baseki)) / 900) + rand_number(GET_LEVEL(ch), GET_LEVEL(ch) * 2));
            break;
    }
    int64_t result2;
    ...
    int64_t result1;
    result1 = ch_gainBaseST(ch, st, true);
...
ACMD(do_pour) {
    ...
    if (subcmd == SCMD_FILL) {
        ...
        if (!*arg2) {        /* no 2nd argument */
            act("What do you want to fill $p from?", false, ch, to_obj, NULL, TO_CHAR);
            return;
        }
        ...
        if (GET_OBJ_TYPE(from_obj) != ITEM_FOUNTAIN && !OBJ_FLAGGED(from_obj, ITEM_BROKEN)) {
            act("You can't fill something from $p.", false, ch, from_obj, NULL, TO_CHAR);
            return;
        } else if (GET_OBJ_TYPE(from_obj) == ITEM_FOUNTAIN && OBJ_FLAGGED(from_obj, ITEM_BROKEN)) {
            act("You can't fill something from a broken fountain.", false, ch, from_obj, NULL, TO_CHAR);
            return;
        }
    }
    if (GET_OBJ_VAL(from_obj, VAL_DRINKCON_HOWFULL) == 0) {
        act("The $p is empty.", false, ch, from_obj, NULL, TO_CHAR);
        return;
    }
...
ACMD(do_wear) {
    ...
    if (dotmode == FIND_ALL) {
        for (obj = ch->carrying; obj; obj = next_obj) {
            next_obj = obj->next_content;
            if (CAN_SEE_OBJ(ch, obj) && (where = find_eq_pos(ch, obj, NULL)) >= 0) {
                if (GET_LEVEL(ch) < GET_OBJ_LEVEL(obj)) {
                    ...
                    act("$p: it seems to be broken.", false, ch, obj, NULL, TO_CHAR);
                } else if (OBJ_FLAGGED(obj, ITEM_FORGED)) {
                    act("$p: it seems to be fake...", false, ch, obj, NULL, TO_CHAR);
                } else {
                    items_worn++;
    ...
    if (!*arg)
        ...
        if (GET_OBJ_TYPE(obj) == ITEM_LIGHT) {
            ...
            if (GET_OBJ_VAL(obj, VAL_LIGHT_HOURS) > 0 || GET_OBJ_VAL(obj, VAL_LIGHT_HOURS) < 0) {
                act("@wYou light $p@w.", true, ch, obj, NULL, TO_CHAR);
                act("@C$n@w lights $p@w.", true, ch, obj, NULL, TO_ROOM);
            }
            if (GET_OBJ_VAL(obj, VAL_LIGHT_HOURS) == 0) {
                act("@wYou try to light $p@w but it is burnt out.", true, ch, obj, NULL, TO_CHAR);
                act("@C$n@w tries to light $p@w but nothing happens.", true, ch, obj, NULL, TO_ROOM);
            }
        } else {
...
void perform_remove(struct char_data *ch, int pos) {
     ...
     */
        ...
        act("You can't remove $p, it must be CURSED!", false, ch, obj, NULL, TO_CHAR);
    else if (IS_CARRYING_N(ch) >= CAN_CARRY_N(ch))
        act("$p: your arms are full!", false, ch, obj, NULL, TO_CHAR);
    else {
        ...
        if (previous > GET_HIT(ch)) {
            ...
            act(drop, false, ch, obj, NULL, TO_CHAR);
        }
    }
}
...
ACMD(do_sac) {
    ...
    act("$n sacrifices $p.", false, ch, j, NULL, TO_ROOM);
...
/*  abil = MAX(0, MIN(100, GET_STR(ch)));
  ...
  while (abil > 30) {
    ...
    abil = ((ch_getEffMaxPL(ch)) / 200) + (GET_STR(ch) * 50);
    total = 1;
    return (total * abil);
}
