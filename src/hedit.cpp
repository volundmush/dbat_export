...
static void hedit_setup_new(struct descriptor_data *d) {
    ...
    snprintf(buf3, MAX_INPUT_LENGTH, "<<X<< Put helpfile keywords here in caps");
            ...
            switch (*arg) {
                ...
                case 'Y':
                    if (OLC_HELP(d)->keywords == NULL) {
                        ...
                        *buf = '\0';
                        snprintf(buf, MAX_INPUT_LENGTH, "%s", OLC_HELP(d)->keywords);
...
ACMD(do_hindex) {
    ...
    len = snprintf(buf, MAX_STRING_LENGTH, "Help index entries based on '%s':\r\n", argument);
    for (i = 0; i < top_of_helpt; i++) {
        num++;
