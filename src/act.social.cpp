...
ACMD(do_action) {
    ...
    if (!vict) {
        if (action->char_obj_found) {
            ...
            if (targ) {
                act(action->char_obj_found, action->hide, ch, targ, NULL, TO_CHAR);
                act(action->others_obj_found, action->hide, ch, targ, NULL, TO_ROOM);
                return;
            }
        }
        if (action->not_found)
    ...
    if (GET_POS(vict) < action->min_victim_position)
        act("$N is not in a proper position for that.", false, ch, NULL, vict, TO_CHAR | TO_SLEEP);
    else {
        if (*part) {
                    ...
                    case 0:
                        if (GET_SEX(ch) == SEX_MALE) {
                            if (GET_SEX(victim) == SEX_MALE)
                                act("$n accuses you of fighting like a woman!", false, ch, NULL, victim, TO_VICT);
                            else
                                act("$n says that women can't fight.", false, ch, NULL, victim, TO_VICT);
                        } else {        /* Ch == Woman */
                            if (GET_SEX(victim) == SEX_MALE)
                        ...
                        break;
                    case 1:
                        act("$n calls your mother a bitch!", false, ch, NULL, victim, TO_VICT);
                        break;
                    default:
                        act("$n tells you to get lost!", false, ch, NULL, victim, TO_VICT);
                        break;
                }            /* end switch */
...
ACMD(do_gmote) {
    ...
    if ((act_nr = find_action(cmd)) < 0) {
        snprintf(buf, sizeof(buf), "@D[@BOOC@D: @g%s %s@n@D]", GET_ADMLEVEL(ch) < 1 ? ch->desc->user : GET_NAME(ch),
                 argument);
        act(buf, false, ch, NULL, vict, TO_GMOTE);
        return;
    }
