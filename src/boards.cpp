...
struct board_info *load_board(obj_vnum board_vnum) {
    struct board_info *temp_board = NULL;
    struct board_msg *bmsg = NULL;
    struct obj_data *obj = NULL;
    struct stat st;
    struct board_memory *memboard = NULL, *list = NULL;
...
void show_board(obj_vnum board_vnum, struct char_data *ch) {
    ...
    *name = '\0';
    ...
    if (bnum == 3092) {
        GET_BOARD(ch, 0) = time(NULL);
        /* Mort board */
    }
    if (bnum == 3099) {
        GET_BOARD(ch, 3) = time(NULL);
        /* Duo's board */
    }
    if (bnum == 3098) {
        GET_BOARD(ch, 1) = time(NULL);
        /* Imm board */
    }
    if (bnum == 3090) {
        GET_BOARD(ch, 4) = time(NULL);
        /* Builder board */
    }
    save_char(ch);
    return;
...
void write_board_message(obj_vnum board_vnum, struct char_data *ch, char *arg) {
    ...
    act("@C$n@w starts writing on the board.@n", true, ch, NULL, NULL, TO_ROOM);
    skip_spaces(&arg);
    ...
    MESG_TIMESTAMP(message) = time(NULL);
    ...
    if (board_vnum == 3092) {
        BOARDNEWMORT = time(NULL);
    }
    if (board_vnum == 3098) {
        BOARDNEWIMM = time(NULL);
    }
    if (board_vnum == 3099) {
        BOARDNEWDUO = time(NULL);
    }
    if (board_vnum == 3090) {
        BOARDNEWBUI = time(NULL);
    }
