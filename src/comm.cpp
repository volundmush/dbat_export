...
void game_loop(socklen_t cmmother_desc) {
    ...
    while (!circle_shutdown) {
        ...
        if (descriptor_list == NULL) {
            ...
            if (select(top_desc + 1, &input_set, (fd_set *) NULL, (fd_set *) NULL, NULL) < 0) {
                if (errno == EINTR)
        ...
        gettimeofday(&before_sleep, (struct timezone *) NULL); /* current time */
        ...
        for (d = descriptor_list; d; d = next_d) {
            ...
            if (d->character) {
                ...
                if (STATE(d) == CON_PLAYING && GET_WAS_IN(d->character) != NOWHERE) {
                    ...
                    act("$n has returned.", true, d->character, NULL, NULL, TO_ROOM);
                }
    ...
    if (d->showstr_count) {
        ...
        *prompt = '\0';
        ...
        if (PRF_FLAGGED(d->character, PRF_DISPAUTO) && GET_LEVEL(d->character) >= 500 && len < sizeof(prompt)) {
            ...
            if (GET_HIT(ch) << 2 < (ch_getEffMaxPL(ch))) {
                count = snprintf(prompt + len, sizeof(prompt) - len, "PL: %" I64T " ", GET_HIT(ch));
                if (count >= 0)
                    len += count;
            }
            if (((ch_getCurST(ch))) << 2 < GET_MAX_MOVE(ch) && len < sizeof(prompt)) {
                count = snprintf(prompt + len, sizeof(prompt) - len, "STA: %" I64T " ", ((ch_getCurST(ch))));
                if (count >= 0)
                    len += count;
            }
            ...
            if (!PRF_FLAGGED(d->character, PRF_DISPERC)) {
                if (PRF_FLAGGED(d->character, PRF_DISPHP) && len < sizeof(prompt)) {
                    char *col = "n";
                    struct char_data *ch = d->character;
                    if (ch_getMaxPL(ch) > ch_getMaxPLTrans(ch))
                        col = "g";
                    else {
                        if (ch_isWeightedPL(ch))
                            col = "m";
                        else {
                            if (ch_getCurHealthPercent(ch) > .5)
                                col = "c";
                            else if (ch_getCurHealthPercent(ch) > .1)
                                col = "y";
                            else if (ch_getCurHealthPercent(ch) <= .1)
                                col = "r";
                        }
                    ...
                    if ((count = snprintf(prompt + len, sizeof(prompt) - len, "@D[@RPL@n@Y: @%s%s@D]@n", col,
                                          add_commas(ch_getCurPL(ch)))) > 0)
                        len += count;
                }
            } else if (PRF_FLAGGED(d->character, PRF_DISPHP)) {

                struct char_data *ch = d->character;
                double perc = ((double) ch_getCurHealth(ch) / (double) ch_getMaxPLTrans(ch)) * 100;
                char *col = "n";
                if (perc > 100)
                    col = "g";
                else if (perc >= 70)
            ...
            if (!PRF_FLAGGED(d->character, PRF_DISPERC)) {
                if (PRF_FLAGGED(d->character, PRF_DISPKI) && len < sizeof(prompt) &&
                    ((ch_getMaxKI(d->character) * d->character->energy)) > GET_MAX_MANA(d->character) / 2) {
                    count = snprintf(prompt + len, sizeof(prompt) - len, "@D[@CKI@Y: @c%s@D]@n", add_commas(
                            ((ch_getMaxKI(d->character) * d->character->energy))));
                    if (count >= 0)
                        len += count;
                } else {
                    if (PRF_FLAGGED(d->character, PRF_DISPKI) && len < sizeof(prompt) &&
                        ((ch_getMaxKI(d->character) * d->character->energy)) > GET_MAX_MANA(d->character) / 10) {
                        count = snprintf(prompt + len, sizeof(prompt) - len, "@D[@CKI@Y: @y%s@D]@n", add_commas(
                                ch_getCurKI(d->character)));
                        if (count >= 0)
                            len += count;
                    } else if (PRF_FLAGGED(d->character, PRF_DISPKI) && len < sizeof(prompt) &&
                               (ch_getCurKI(d->character)) <= GET_MAX_MANA(d->character) / 10) {
                ...
                int64_t power = ((ch_getMaxKI(d->character) * d->character->energy)), maxpower = GET_MAX_MANA(d->character);
                int perc = 0;
                if (power <= 0) {
                    power = 1;
                }
            ...
            if (!PRF_FLAGGED(d->character, PRF_DISPERC)) {
                if (PRF_FLAGGED(d->character, PRF_DISPMOVE) && len < sizeof(prompt) &&
                    ((ch_getCurST(d->character))) > GET_MAX_MOVE(d->character) / 2) {
                    count = snprintf(prompt + len, sizeof(prompt) - len, "@D[@GSTA@Y: @c%s@D]@n", add_commas(
                            ((ch_getCurST(d->character)))));
                    if (count >= 0)
                        len += count;
                } else {
                    if (PRF_FLAGGED(d->character, PRF_DISPMOVE) && len < sizeof(prompt) &&
                        ((ch_getCurST(d->character))) > GET_MAX_MOVE(d->character) / 10) {
                ...
                int64_t power = ((ch_getCurST(d->character))), maxpower = GET_MAX_MOVE(d->character);
                int perc = 0;
                if (power <= 0) {
                    power = 1;
                }
...
ssize_t perform_socket_write(socklen_t desc, const char *txt, size_t length) {
    ssize_t result = write(desc, txt, length);
...
void close_socket(struct descriptor_data *d) {
    ...
    if (d->character) {
        ...
        if (IS_PLAYING(d) || STATE(d) == CON_DISCONNECT) {
            ...
            act("$n has lost $s link.", true, link_challenged, NULL, NULL, TO_ROOM);
