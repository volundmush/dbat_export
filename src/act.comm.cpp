    ...
    *buf2 = '\0';
    ...
    if (!*argument) {
        ...
        if (!sch) {
            ...
            act(buf2, true, ch, NULL, sch, TO_VICT);
            act(buf, true, ch, NULL, sch, TO_NOTVICT);
        }
    ...
    if (!*argument)
        ...
        if (AFF_FLAGGED(k, AFF_GROUP) && (k != ch) && AWAKE(k)) {
            if (CONFIG_ENABLE_LANGUAGES) {
                ...
                act(blah, true, ch, NULL, k, TO_VICT);
            }
        }
        for (f = k->followers; f; f = f->next)
            if (AFF_FLAGGED(f->follower, AFF_GROUP) && (f->follower != ch) && AWAKE(f->follower)) {
                ...
                if (CONFIG_ENABLE_LANGUAGES) {
                    ...
                    act(blah, true, ch, NULL, f->follower, TO_VICT | TO_SLEEP);
                }
            }
...
static int is_tell_ok(struct char_data *ch, struct char_data *vict) {
    ...
    if (ch == vict)
        ...
        act("$E's linkless at the moment.", false, ch, NULL, vict, TO_CHAR | TO_SLEEP);
    else if (PLR_FLAGGED(vict, PLR_WRITING))
        act("$E's writing a message right now; try again later.", false, ch, NULL, vict, TO_CHAR | TO_SLEEP);
...
ACMD(do_spec_comm) {
    ...
    if (!*buf || !*buf2)
        ...
        if (!IS_NPC(ch) && PRF_FLAGGED(ch, PRF_NOREPEAT)) {
            ...
            act(blum, true, ch, NULL, vict, TO_CHAR);
        }
...
ACMD(do_write) {
    ...
    if (obj) {                /* then there IS a board! */
        ...
        act("$n begins to write a note on $p.", true, ch, obj, NULL, TO_ROOM);
        return;
    }
            ...
            if (ADM_FLAGGED(ch, ADM_TELLALL)) {
                for (d = descriptor_list; d; d = d->next)
                    if (STATE(d) == CON_PLAYING && d->character)
                        act(buf, false, ch, NULL, d->character, TO_VICT);
            } else
    ...
    *buf2 = '\0';
    ...
    for (i = descriptor_list; i; i = i->next) {
        if (STATE(i) == CON_PLAYING && i != ch->desc && i->character &&
            ...
            msg = act(buf1, false, ch, NULL, i->character, TO_VICT | TO_SLEEP);
...
ACMD(do_qcomm) {
    ...
    if (!*argument)
        ...
        if (PRF_FLAGGED(ch, PRF_NOREPEAT))
            ...
            act(buf, false, ch, NULL, argument, TO_CHAR);
        } else
            act(argument, false, ch, NULL, argument, TO_CHAR);
        ...
        for (i = descriptor_list; i; i = i->next)
            if (STATE(i) == CON_PLAYING && i != ch->desc && PRF_FLAGGED(i->character, PRF_QUEST))
                act(buf, 0, ch, NULL, i->character, TO_VICT | TO_SLEEP);
    }
}
