...
void act_to_room(char *str, room_data *room) {
     ...
     */
    act(str, false, room->people, NULL, NULL, TO_ROOM);
    act(str, false, room->people, NULL, NULL, TO_CHAR);
}
...
WCMD(do_wload) {
    ...
    if (is_abbrev(arg1, "mob")) {
        ...
        if (SCRIPT(room)) { /* It _should_ have, but it might be detached. */
            ...
            snprintf(buf, MAX_INPUT_LENGTH, "%c%d", UID_CHAR, GET_ID(mob));
...
const struct wld_command_info wld_cmd_info[] = {
        {"RESERVED",     NULL,      0},/* this must be first -- for specprocs */
