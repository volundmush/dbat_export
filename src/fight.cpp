    ...
    if (ch->followers) {
        for (k = ch->followers; k; k = next) {
            ...
            if (!AFF_FLAGGED(k->follower, AFF_GROUP)) {
                ...
                if (type == 0) {
                    int64_t result;
                    result = ch_incCurLFPercent(k->follower, .25, true);
                    send_to_char(k->follower, "@CIncensed by the death of your comrade your life force swells!@n");
                    return (true);
                } else if (type == 1) {
                    int64_t result;
                    result = ch_incCurLFPercent(k->follower, .4, true);
                    send_to_char(k->follower, "@CIncensed by the death of your comrade your life force swells!@n");
                    return (true);
                } else if (type == 2) {
                    if (IS_ROSHI(ch)) {
...
void mutant_limb_regen(struct char_data *ch) {
    if (GET_LIMBCOND(ch, 1) > 0 && GET_LIMBCOND(ch, 1) < 50) {
        act("The bones in your right arm have mended them selves.", true, ch, NULL, NULL, TO_CHAR);
        act("$n starts moving $s right arm gingerly for a moment.", true, ch, NULL, NULL, TO_ROOM);
        GET_LIMBCOND(ch, 1) = 100;
    } else if (GET_LIMBCOND(ch, 1) <= 0) {
        act("Your right arm begins to grow back very quickly. Within moments it is whole again!", true, ch, NULL,
            NULL, TO_CHAR);
        act("$n's right arm starts to regrow! Within moments the arm is whole again!.", true, ch, NULL, NULL,
            TO_ROOM);
        GET_LIMBCOND(ch, 1) = 100;
    }
    if (GET_LIMBCOND(ch, 2) > 0 && GET_LIMBCOND(ch, 2) < 50) {
        act("The bones in your left arm have mended them selves.", true, ch, NULL, NULL, TO_CHAR);
        act("$n starts moving $s left arm gingerly for a moment.", true, ch, NULL, NULL, TO_ROOM);
    ...
    if (GET_LIMBCOND(ch, 3) > 0 && GET_LIMBCOND(ch, 3) < 50) {
        act("The bones in your right leg have mended them selves.", true, ch, NULL, NULL, TO_CHAR);
        act("$n starts moving $s right leg gingerly for a moment.", true, ch, NULL, NULL, TO_ROOM);
    ...
    if (GET_LIMBCOND(ch, 4) > 0 && GET_LIMBCOND(ch, 4) < 50) {
        act("The bones in your left leg have mended them selves.", true, ch, NULL, NULL, TO_CHAR);
        act("$n starts moving $s left leg gingerly for a moment.", true, ch, NULL, NULL, TO_ROOM);
...
static int pick_n_throw(struct char_data *ch, char *buf) {
    ...
    for (cont = world[IN_ROOM(ch)].contents; cont; cont = cont->next_content) {
        if (GET_OBJ_WEIGHT(cont) <= CAN_CARRY_W(ch) + IS_CARRYING_W(ch)) {
            snprintf(buf2, MAX_INPUT_LENGTH, "%s", cont->name);
            do_get(ch, buf2, 0, 0);
            snprintf(buf3, MAX_INPUT_LENGTH, "%s %s", buf2, buf);
...
static void mob_attack(struct char_data *ch, char *buf) {
    ...
    if (axion_dice(-10) > 90 && ch_getCurHealthPercent(ch) <= .5 && !MOB_FLAGGED(ch, MOB_POWERUP) &&
        GET_MOB_VNUM(ch) != 25 && !(IS_ANDROID(ch) || IS_ANIMAL(ch))) {
        do_powerup(ch, NULL, 0, 0);
        return;
    }

    if (((ch_getMaxKI(ch) * ch->energy)) >= GET_MAX_MANA(ch) * 0.05 && IS_HUMANOID(ch) && (!IS_DRAGON(ch) || dragonpass == true)) {
        if (ch->mobcharge <= 0 && rand_number(1, 10) >= 8) {
            act("@wAn aura flares up around @R$n@w!@n", true, ch, NULL, NULL, TO_ROOM);
            ch->mobcharge += 1;
            if (GET_LEVEL(ch) > 80) {
                ch->mobcharge += 1;
            }
        } else if (ch->mobcharge <= 5) {
            act("@wThe aura burns brighter around @R$n@w!@n", true, ch, NULL, NULL, TO_ROOM);
            ch->mobcharge += 1;
            if (GET_LEVEL(ch) > 80) {
                ch->mobcharge += 1;
            }
        } else if (ch->mobcharge == 6) {
            act("@wThe aura around @R$n@w flashes!@n", true, ch, NULL, NULL, TO_ROOM);
            ch->mobcharge += 1;
            special = 100;
        }
    }
    ...
    if (IS_HUMANOID(ch) && dragonpass == true) { /* Is a humanoid mob */
        if (AFF_FLAGGED(ch, AFF_PARALYZE)) {
            ...
            if (GET_CLASS(ch) == CLASS_SHADOWDANCER && rand_number(1, 3) == 3) {
                snprintf(buf2, MAX_INPUT_LENGTH, "ass %s", buf);
                do_throw(ch, buf2, 0, 0);
            } else if (IS_ANDROID(ch) && MOB_FLAGGED(ch, MOB_REPAIR) && GET_HIT(ch) <= (ch_getEffMaxPL(ch)) * 0.5 &&
                       rand_number(1, 20) >= 16) {
                do_srepair(ch, NULL, 0, 0);
            } else if (IS_ANDROID(ch) && MOB_FLAGGED(ch, MOB_ABSORB) && rand_number(1, 20) >= 19) {
                do_absorb(ch, buf2, 0, 0);
            } else if ((IS_BIO(ch) || IS_MAJIN(ch)) && GET_HIT(ch) <= (ch_getEffMaxPL(ch)) * 0.5 &&
                       rand_number(1, 20) >= 17) {
                do_regenerate(ch, "25", 0, 0);
            } else if (IS_NAMEK(ch) && GET_HIT(ch) <= (ch_getEffMaxPL(ch)) * 0.5 && rand_number(1, 20) == 20) {
                do_regenerate(ch, "25", 0, 0);
...
void fight_stack() {
    ...
    for (tch = character_list; tch; tch = tch->next) {
        ...
        if (IS_NPC(ch) && MOB_FLAGGED(ch, MOB_POWERUP) && axion_dice(0) >= 90) {
            ...
            } else if (GET_HIT(ch) >= (ch_getEffMaxPL(ch)) / 2) {
                act("@g$n@G continues powering up as torrents of energy crackle within $s aura.@n", true, ch, NULL,
                    NULL, TO_ROOM);
                int64_t result;
                result = ch_incCurHealthPercent(ch, .1, true);
            } else if (GET_HIT(ch) > (ch_getEffMaxPL(ch)) / 4) {
                act("@g$n@G powers up as a steady aura around $s body grow brighter.@n", true, ch, NULL, NULL,
                    TO_ROOM);
                int64_t result;
                result = ch_incCurHealthPercent(ch, .125, true);
            } else if (GET_HIT(ch) > 0) {
                act("@g$n@G powers up, as a weak aura flickers around $s body.@n", true, ch, NULL, NULL, TO_ROOM);
                int64_t result;
                result = ch_incCurHealthPercent(ch, .2, true);
            }
        }
        ...
        if (FIGHTING(ch) && DRAGGING(ch)) {
            act("@WYou are forced to stop dragging @C$N@W!@n", true, ch, NULL, DRAGGING(ch), TO_CHAR);
            act("@C$n@W is forced to stop dragging @c$N@W!@n", true, ch, NULL, DRAGGING(ch), TO_ROOM);
            DRAGGED(DRAGGING(ch)) = NULL;
            DRAGGING(ch) = NULL;
        }

        if (GET_LIFEPERC(ch) > 0 && ch->health < (double) GET_LIFEPERC(ch) / 100 && ((ch_getCurLF(ch))) > 0 &&
            !IS_ANDROID(ch)) {
            if (rand_number(1, 15) >= 14) {
                if (((ch_getCurLF(ch))) >= ((ch_getMaxLF(ch))) * 0.05 || AFF_FLAGGED(ch, AFF_HEALGLOW) || (IS_KANASSAN(ch) &&
                                                                                                           ((ch_getCurLF(
                                                                                                                   ch))) >=
                                                                                                     ((ch_getMaxLF(
                                                                                                             ch))) *
                                                                                                     0.03)) {
                    int64_t refill = 0, lfcost = ((ch_getMaxLF(ch))) * 0.05;
                    if (GET_BONUS(ch, BONUS_DIEHARD) > 0 &&
                        (!IS_MUTANT(ch) || (GET_GENOME(ch, 0) != 2 && GET_GENOME(ch, 1) != 2))) {
                        refill = ((ch_getMaxLF(ch))) * 0.1;
                    } else if (GET_BONUS(ch, BONUS_DIEHARD) > 0 && IS_MUTANT(ch) &&
                               (GET_GENOME(ch, 0) == 2 || GET_GENOME(ch, 1) == 2)) {
                        refill = ((ch_getMaxLF(ch))) * 0.17;
                    } else if (IS_MUTANT(ch) && (GET_GENOME(ch, 0) == 2 || GET_GENOME(ch, 1) == 2)) {
                        refill = ((ch_getMaxLF(ch))) * 0.12;
                    } else if (IS_KANASSAN(ch)) {
                        lfcost = ((ch_getMaxLF(ch))) * 0.03;
                        refill = ((ch_getMaxLF(ch))) * 0.03;
                    } else {
                        refill = ((ch_getMaxLF(ch))) * 0.05;
                    }
                    int64_t result1;
                    result1 = ch_incCurHealth(ch, refill, true);
                    if (!AFF_FLAGGED(ch, AFF_HEALGLOW)) {
                        int64_t result;
                        result = ch_decCurLF(ch, lfcost, 0);
                    }
                } else {
        ...
        if (!AFF_FLAGGED(ch, AFF_POSITION)) {
            if (roll_balance(ch) > axion_dice(0) && rand_number(1, 10) >= 7) {
                if (FIGHTING(ch)) {
                    if (!AFF_FLAGGED(FIGHTING(ch), AFF_POSITION)) {
                        act("@YYou manage to move into an advantageous position!@n", true, ch, NULL, NULL,
                            TO_CHAR);
                        act("@y$n@Y manages to move into an advantageous position!@n", true, ch, NULL, NULL,
                            TO_ROOM);
            ...
            if (roll_balance(ch) < axion_dice(-30) || GET_POS(ch) < POS_STANDING) {
                act("@YYou are moved out of your position!@n", true, ch, NULL, NULL, TO_CHAR);
                act("@y$n@Y is moved out of $s position!@n", true, ch, NULL, NULL, TO_ROOM);
                REMOVE_BIT_AR(AFF_FLAGS(ch), AFF_POSITION);
            }
        }
        if (GRAPPLING(ch) && GRAPTYPE(ch) == 2 && rand_number(1, 11) >= 8) {
            if (((ch_getCurST(((ch)->grappling)))) >= GET_MAX_MOVE(GRAPPLING(ch)) / 8) {
                act("@WYou choke @C$N@W!@n", true, ch, NULL, GRAPPLING(ch), TO_CHAR);
                act("@C$n@W chokes YOU@W!@n", true, ch, NULL, GRAPPLING(ch), TO_VICT);
                act("@C$n@W chokes @c$N@W!@n", true, ch, NULL, GRAPPLING(ch), TO_NOTVICT);
                int64_t amt = ch_getMaxST(GRAPPLING(ch)) / 8;
                int64_t result;
                result = ch_decCurST(GRAPPLING(ch), amt, 0);
            } else {
                act("@WYou choke @C$N@W, and $E passes out!@n", true, ch, NULL, GRAPPLING(ch), TO_CHAR);
                act("@C$n@W chokes YOU@W, and you pass out!@n", true, ch, NULL, GRAPPLING(ch), TO_VICT);
                act("@C$n@W chokes @c$N@W, and $E passes out!@n", true, ch, NULL, GRAPPLING(ch), TO_NOTVICT);
            ...
            act("@WYou crush @C$N@W some more!@n", true, ch, NULL, GRAPPLING(ch), TO_CHAR);
            act("@C$n@W crushes YOU@W some more!@n", true, ch, NULL, GRAPPLING(ch), TO_VICT);
            act("@C$n@W crushes @c$N@W some more!@n", true, ch, NULL, GRAPPLING(ch), TO_NOTVICT);
            int64_t damg = GET_STR(ch) * (10 + ((ch_getEffMaxPL(ch)) * 0.005));
        ...
        if (IS_HALFBREED(ch) && PLR_FLAGGED(ch, PLR_FURY)) {
            ...
            if (GET_RMETER(ch) >= 1000) {
                ...
                int64_t result2;
                result2 = ch_incCurHealthPercent(ch, .15, true);
                int64_t result1;
                result1 = ch_incCurKIPercent(ch, .15, true);
                int64_t result;
                result = ch_incCurSTPercent(ch, .15, true);
        ...
        if (!IS_NPC(ch) && IS_TRANSFORMED(ch) && !IS_ICER(ch) && IS_NONPTRANS(ch)) {
            ...
            if (ch_getCurST(ch) < GET_MAX_MOVE(ch) / 60) {
                if (!(tier == 1 && PLR_FLAGGED(ch, PLR_FPSSJ))) {
                    act("@mExhausted of stamina, your body forcibly reverts from its form.@n", true, ch, NULL,
                        NULL, TO_CHAR);
                    act("@C$n @wbreathing heavily, reverts from $s form, returning to normal.@n", true, ch, NULL,
                        NULL, TO_ROOM);
                    if (GET_KAIOKEN(ch) < 1)
                        do_kaioken(ch, "0", 0, 0);
            ...
            if (ch_getCurST(ch) >= GET_MAX_MOVE(ch) / 800 && PLR_FLAGGED(ch, PLR_TRANS1)) {
                if (!PLR_FLAGGED(ch, PLR_FPSSJ)) {
                    if (IS_SAIYAN(ch) && ((ch_getCurLF(ch))) >= ((ch_getMaxLF(ch))) * 0.7) {
                        int64_t amt = ch_getMaxST(ch) / 900;
                        int64_t result;
                        result = ch_decCurST(ch, amt, 0);
                    } else {
                        int64_t amt = ch_getMaxST(ch) / 800;
                        int64_t result;
                        result = ch_decCurST(ch, amt, 0);
                    }
                }
            } else {
                if (ch_getCurST(ch) >= GET_MAX_MOVE(ch) / 600 && PLR_FLAGGED(ch, PLR_TRANS2) && !IS_KONATSU(ch) &&
                    !IS_KAI(ch) && !IS_NAMEK(ch)) {
                    if (IS_SAIYAN(ch) && ((ch_getCurLF(ch))) >= ((ch_getMaxLF(ch))) * 0.7) {
                        int64_t amt = ch_getMaxST(ch) / 700;
                        int64_t result;
                        result = ch_decCurST(ch, amt, 0);
                    } else {
                        int64_t amt = ch_getMaxST(ch) / 600;
                        int64_t result;
                        result = ch_decCurST(ch, amt, 0);
                    }
                } else if (ch_getCurST(ch) >= GET_MAX_MOVE(ch) / 500 && PLR_FLAGGED(ch, PLR_TRANS2)) {
                    int64_t amt = ch_getMaxST(ch) / 500;
                    int64_t result;
                    result = ch_decCurST(ch, amt, 0);
                } else if (ch_getCurST(ch) >= GET_MAX_MOVE(ch) / 400 && PLR_FLAGGED(ch, PLR_TRANS3) && !IS_SAIYAN(ch)) {
                    int64_t amt = ch_getMaxST(ch) / 400;
                    int64_t result;
                    result = ch_decCurST(ch, amt, 0);
                } else if (ch_getCurST(ch) >= GET_MAX_MOVE(ch) / 250 && PLR_FLAGGED(ch, PLR_TRANS3)) {
                    if (IS_SAIYAN(ch) && ((ch_getCurLF(ch))) >= ((ch_getMaxLF(ch))) * 0.7) {
                        int64_t amt = ch_getMaxST(ch) / 300;
                        int64_t result;
                        result = ch_decCurST(ch, amt, 0);
                    } else {
                        int64_t amt = ch_getMaxST(ch) / 250;
                        int64_t result;
                        result = ch_decCurST(ch, amt, 0);
                    }
                } else if (ch_getCurST(ch) >= GET_MAX_MOVE(ch) / 200 && PLR_FLAGGED(ch, PLR_TRANS4) && !IS_SAIYAN(ch)) {
                    int64_t amt = ch_getMaxST(ch) / 200;
                    int64_t result;
                    result = ch_decCurST(ch, amt, 0);
                } else if (ch_getCurST(ch) >= GET_MAX_MOVE(ch) / 170 && PLR_FLAGGED(ch, PLR_TRANS4)) {
                    if (IS_SAIYAN(ch) && ((ch_getCurLF(ch))) >= ((ch_getMaxLF(ch))) * 0.7) {
                        int64_t amt = ch_getMaxST(ch) / 240;
                        int64_t result;
                        result = ch_decCurST(ch, amt, 0);
                    } else {
                        int64_t amt = ch_getMaxST(ch) / 170;
                        int64_t result;
                        result = ch_decCurST(ch, amt, 0);
                    }
                }
        ...
        if (IS_NPC(ch) && GET_HIT(ch) < (ch_getEffMaxPL(ch)) / 10 && GET_HIT(ch) > 0 && FIGHTING(ch) &&
        ...
        if (!IS_NPC(ch) && PLR_FLAGGED(ch, PLR_DISGUISED) && GET_SKILL(ch, SKILL_DISGUISE) < rand_number(1, 125)) {
            ...
            act("@W$n's@W disguise comes off because of $s swift movements!@n", false, ch, NULL, NULL, TO_ROOM);
        }
        if (IS_NPC(ch) && AFF_FLAGGED(ch, AFF_BLIND) && rand_number(1, 200) >= 190) {
            act("@W$n@W is no longer blind.@n", false, ch, NULL, NULL, TO_ROOM);
            REMOVE_BIT_AR(AFF_FLAGS(ch), AFF_BLIND);
        }

        if (AFF_FLAGGED(ch, AFF_KNOCKED) && rand_number(1, 200) >= 195) {
            ch_cureStatusKnockedOut(ch, true);
            if (IS_NPC(ch) && rand_number(1, 20) >= 12) {
                act("@W$n@W stands up.@n", false, ch, NULL, NULL, TO_ROOM);
                GET_POS(ch) = POS_STANDING;
            }
        }
        ...
        if (FIGHTING(ch) && IS_NPC(ch) && !MOB_FLAGGED(ch, MOB_DUMMY)) {
            ...
            if (AFF_FLAGGED(FIGHTING(ch), AFF_FLYING) && !IS_HUMANOID(ch) && !AFF_FLAGGED(ch, AFF_FLYING) &&
                ...
                if (rand_number(1, 30) >= 22 && !block_calc(ch)) {
                    act("$n@G flees in terror and you lose sight of $m!", true, ch, NULL, NULL, TO_ROOM);
                    while (ch->carrying)
            ...
            if (AFF_FLAGGED(FIGHTING(ch), AFF_FLYING) && IS_HUMANOID(ch) && GET_LEVEL(ch) <= 10) {
                if (rand_number(1, 30) >= 22 && !block_calc(ch)) {
                    act("$n@G turns and runs away. You lose sight of $m!", true, ch, NULL, NULL, TO_ROOM);
                    while (ch->carrying)
                        extract_obj(ch->carrying);
                    extract_char(ch);
                    continue;
            ...
            if (IS_AFFECTED(ch, AFF_PARA) && IS_NPC(ch) && GET_INT(ch) + 10 < rand_number(1, 60)) {
                act("@yYou fail to overcome your paralysis!@n", true, ch, NULL, NULL, TO_CHAR);
                act("@Y$n @ystruggles with $s paralysis!@n", true, ch, NULL, NULL, TO_ROOM);
                continue;
            }
        ...
        if (PLR_FLAGGED(ch, PLR_POWERUP) && rand_number(1, 3) == 3) {
            ...
            if (GET_HIT(ch) >= (ch_getEffMaxPL(ch)) && ((ch_getMaxKI(ch) * ch->energy)) >= GET_MAX_MANA(ch) / 20 &&
                GET_PREFERENCE(ch) != PREFERENCE_KI) {
                if (((ch_getMaxKI(ch) * ch->energy)) >= GET_MAX_MANA(ch) * 0.5) {
                    int64_t raise = GET_MAX_MOVE(ch) * 0.02;
                    int64_t result;
                    result = ch_incCurST(ch, raise, true);
                }
                ch_restoreHealth(ch, false);
                int64_t amt = ch_getMaxKI(ch) / 20;
                int64_t result1;
                result1 = ch_decCurKI(ch, amt, 0);
                dispel_ash(ch);
                act("@RYou have reached your maximum!@n", true, ch, NULL, NULL, TO_CHAR);
                act("@R$n stops powering up in a flash of light!@n", true, ch, NULL, NULL, TO_ROOM);
                ...
                if (GET_HIT(ch) >= (ch_getEffMaxPL(ch)) && ((ch_getMaxKI(ch) * ch->energy)) >= (GET_MAX_MANA(ch) * 0.0375) + 1 &&
                           GET_PREFERENCE(ch) == PREFERENCE_KI) {
                    if ((ch_getCurKI(ch)) >= (GET_MAX_MANA(ch) * 0.0375) + 1) {
                        int64_t raise = GET_MAX_MOVE(ch) * 0.02;
                        int64_t result;
                        result = ch_incCurST(ch, raise, true);
                    }
                    ch_restoreHealth(ch, false);
                    int64_t amt = (GET_MAX_MANA(ch) * 0.0375) + 1;
                    int64_t result1;
                    result1 = ch_decCurKI(ch, amt, 0);
                    dispel_ash(ch);
                    act("@RYou have reached your maximum!@n", true, ch, NULL, NULL, TO_CHAR);
            ...
            if (((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) / 20 && GET_PREFERENCE(ch) != PREFERENCE_KI) {
                int64_t amt = ch_getMaxKI(ch) / 20;
                int64_t result;
                result = ch_decCurKI(ch, amt, 0);
                act("@RYou have run out of ki.@n", true, ch, NULL, NULL, TO_CHAR);
                act("@R$n stops powering up in a flash of light!@n", true, ch, NULL, NULL, TO_ROOM);
                send_to_sense(0, "You sense someone stop powering up", ch);
                sprintf(buf3, "@D[@GBlip@D]@r Rising Powerlevel Final@D: [@Y%s@D]", add_commas(GET_HIT(ch)));
                send_to_scouter(buf3, ch, 1, 0);
                ...
                if (((ch_getMaxKI(ch) * ch->energy)) < (GET_MAX_MANA(ch) * 0.0375) + 1 && GET_PREFERENCE(ch) == PREFERENCE_KI) {
                    int64_t amt = (GET_MAX_MANA(ch) * 0.0375) + 1;
                    int64_t result;
                    result = ch_decCurKI(ch, amt, 0);
                    act("@RYou have run out of ki.@n", true, ch, NULL, NULL, TO_CHAR);
            ...
            if (GET_HIT(ch) < (ch_getEffMaxPL(ch)) && ((GET_PREFERENCE(ch) != PREFERENCE_KI &&
                                                        ((ch_getMaxKI(ch) * ch->energy)) >= GET_MAX_MANA(ch) / 20) ||
                                                       (GET_PREFERENCE(ch) == PREFERENCE_KI &&
                                                        ((ch_getMaxKI(ch) * ch->energy)) >= (GET_MAX_MANA(ch) * 0.0375) + 1))) {
                int64_t result1;
                result1 = ch_incCurHealthPercent(ch, .1, true);
                if (GET_PREFERENCE(ch) != PREFERENCE_KI) {
                    int64_t amt = ch_getMaxKI(ch) / 20;
                    int64_t result;
                    result = ch_decCurKI(ch, amt, 0);
                } else {
                    int64_t amt = ch_getMaxKI(ch) * .0375;
                    int64_t result;
                    result = ch_decCurKI(ch, amt, 0);
                }
                if (((ch_getMaxKI(ch) * ch->energy)) >= GET_MAX_MANA(ch) * 0.5) {
                ...
                if ((ch_getEffMaxPL(ch)) < 50000) {
                    act("@RYou continue to powerup, as wind billows out from around you!@n", true, ch, NULL, NULL,
                        TO_CHAR);
                    act("@R$n continues to powerup, as wind billows out from around $m!@n", true, ch, NULL, NULL,
                        TO_ROOM);
                } else if ((ch_getEffMaxPL(ch)) < 500000) {
                    act("@RYou continue to powerup, as the ground splits beneath you!@n", true, ch, NULL, NULL,
                        TO_CHAR);
                    act("@R$n continues to powerup, as the ground splits beneath $m!@n", true, ch, NULL, NULL,
                        TO_ROOM);
                } else if ((ch_getEffMaxPL(ch)) < 5000000) {
                    act("@RYou continue to powerup, as the ground shudders and splits beneath you!@n", true, ch,
                        NULL, NULL, TO_CHAR);
                    act("@R$n continues to powerup, as the ground shudders and splits beneath $m!@n", true, ch, NULL,
                        NULL, TO_ROOM);
                } else if ((ch_getEffMaxPL(ch)) < 50000000) {
                    act("@RYou continue to powerup, as a huge depression forms beneath you!@n", true, ch, NULL,
                        NULL, TO_CHAR);
                    act("@R$n continues to powerup, as a huge depression forms beneath $m!@n", true, ch, NULL,
                        NULL, TO_ROOM);
                } else if ((ch_getEffMaxPL(ch)) < 100000000) {
                    act("@RYou continue to powerup, as the entire area quakes around you!@n", true, ch, NULL,
                        NULL, TO_CHAR);
                    act("@R$n continues to powerup, as the entire area quakes around $m!@n", true, ch, NULL, NULL,
                        TO_ROOM);
                    ...
                    act("@RYou continue to powerup, as the very air around you crackles and burns!@n", true, ch,
                        NULL, NULL, TO_CHAR);
                    act("@R$n continues to powerup, as the very air around $m crackles and burns!@n", true, ch, NULL,
                        NULL, TO_ROOM);
                }
        ...
        if ((GET_POS(ch) == POS_SLEEPING || GET_POS(ch) == POS_RESTING) &&
            ...
            int64_t result;
            result = ch_incCurKI(ch, GET_CHARGE(ch), true);
            GET_CHARGE(ch) = 0;
            GET_CHARGETO(ch) = 0;
        }
        ...
        if (GET_CHARGE(ch) >= ch_getMaxKI(ch) / 2) {
        ...
        if (!PLR_FLAGGED(ch, PLR_CHARGE) && rand_number(1, 40) >= 38 && !FIGHTING(ch) &&
            ...
            if (GET_CHARGE(ch) >= GET_MAX_MANA(ch) / 100) {
                ...
                switch (rand_number(1, 3)) {
                    case 1:
                        act("$n@w's aura flickers weakly.@n", true, ch, NULL, NULL, TO_ROOM);
                        break;
                    case 2:
                        act("$n@w's aura sheds energy.@n", true, ch, NULL, NULL, TO_ROOM);
                        break;
                    case 3:
                        act("$n@w's aura flickers brightly before growing dimmer.@n", true, ch, NULL, NULL,
                            TO_ROOM);
                        break;
                    default:
                        act("$n@w's aura shrinks some.@n", true, ch, NULL, NULL, TO_ROOM);
                        break;
                }
                ...
                act("$n@w's aura fades away dimmly.@n", true, ch, NULL, NULL, TO_ROOM);
                GET_CHARGE(ch) = 0;
            }
        }
        ...
        if (PLR_FLAGGED(ch, PLR_CHARGE)) {
            ...
            if (((ch_getMaxKI(ch) * ch->energy)) <= 0) {
                send_to_char(ch, "You can not charge anymore, you have charged all your energy!\r\n");
                act("$n@w's aura grows calm.@n", true, ch, NULL, NULL, TO_ROOM);
                REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_CHARGE);
            } else {
                if (((GET_MAX_MANA(ch) * 0.01) * perc) >= ((ch_getMaxKI(ch) * ch->energy))) {
                    send_to_char(ch, "You have charged the last that you can.\r\n");
                    act("$n@w's aura @Yflashes@w spectacularly, rushing upwards in torrents!@n", true, ch, NULL, NULL,
                        TO_ROOM);
                    GET_CHARGE(ch) += (ch_getCurKI(ch));
                    int64_t result;
                    result = ch_decCurKIPercent(ch, 1, 0);
                    GET_CHARGETO(ch) = 0;
                    REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_CHARGE);
                } else {
                    if (GET_CHARGE(ch) >= GET_CHARGETO(ch)) {
                        send_to_char(ch, "You have already reached the maximum that you wished to charge.\r\n");
                        act("$n@w's aura burns steadily.@n", true, ch, NULL, NULL, TO_ROOM);
                        GET_CHARGETO(ch) = 0;
                        REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_CHARGE);
                    } else if (GET_CHARGE(ch) + (((GET_MAX_MANA(ch) * 0.01) * perc) + 1) >= GET_CHARGETO(ch)) {
                        int64_t result;
                        result = ch_decCurKI(ch, GET_CHARGETO(ch) - GET_CHARGE(ch), 0);
                        GET_CHARGE(ch) = GET_CHARGETO(ch);
                        send_to_char(ch, "You stop charging as you reach the maximum that you wished to charge.\r\n");
                        act("$n@w's aura flares up brightly and then burns steadily.@n", true, ch, NULL, NULL,
                            TO_ROOM);
                        GET_CHARGETO(ch) = 0;
                        REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_CHARGE);
                    } else {
                        int64_t amt = ((GET_MAX_MANA(ch) * 0.01) * perc) + 1;
                        int64_t result;
                        result = ch_decCurKI(ch, amt, 0);
                        GET_CHARGE(ch) += ((GET_MAX_MANA(ch) * 0.01) * perc) + 1;
                        switch (rand_number(1, 3)) {
                            case 1:
                                act("$n@w's aura ripples magnificantly while growing brighter!@n", true, ch, NULL,
                                    NULL, TO_ROOM);
                                send_to_char(ch, "Your aura grows bright as you charge more ki.\r\n");
                                break;
                            case 2:
                                act("$n@w's aura ripples with power as it grows larger!@n", true, ch, NULL, NULL,
                                    TO_ROOM);
                                send_to_char(ch, "Your aura ripples with power as you charge more ki.\r\n");
                                break;
                            case 3:
                                act("$n@w's aura throws sparks off violently!.@n", true, ch, NULL, NULL, TO_ROOM);
                        ...
                        if (GET_CHARGE(ch) >= GET_CHARGETO(ch)) {
                            ...
                            act("$n@w's aura burns brightly and then evens out.@n", true, ch, NULL, NULL, TO_ROOM);
                            REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_CHARGE);
                            GET_CHARGETO(ch) = 0;
                        }
                    }
...
static void make_pcorpse(struct char_data *ch) {
    ...
    if (AFF_FLAGGED(ch, AFF_ASHED)) {
        act("@WSome ashes fall off the corpse.@n", true, ch, NULL, NULL, TO_ROOM);
...
void raw_kill(struct char_data *ch, struct char_data *killer) {
    ...
    if (IS_NPC(ch) && !MOB_FLAGGED(ch, MOB_DUMMY)) {
        int shadowed = false;
        int64_t result;
        result = ch_decCurHealthPercent(ch, 1, 0);
        ...
        enum DeathSpecial death_type = Afterlife;
        char_data *receiver = ch;
        if (ROOM_FLAGGED(IN_ROOM(receiver), ROOM_PAST))
            death_type = Past;
        else {
            if (ch_in_room_range(ch, 17900, 17999))
                death_type = Northran;
            else {
                char_data *receiver1 = ch;
                if (GET_LEVEL(receiver1) < 9)
                    death_type = Newbie;
            }
        }
        ...
        switch (death_type) {
            case Afterlife: {
                ...
                ch_teleport_to(ch, 6000);
                break;
            case Northran: {
                ch_restore(ch, false);
            }
                ch_teleport_to(ch, 17900);
                android_lose = false;
            ...
            case Newbie: {
                ...
                room_vnum rnum = sen_startRoom(ch->chclass);
                ch_teleport_to(ch, rnum);
...
void die(struct char_data *ch, struct char_data *killer) {
    if (!IS_NPC(ch)) {
        ...
        if ((IS_MAJIN(ch) || IS_BIO(ch)) &&
            (((ch_getCurLF(ch))) >= ((ch_getMaxLF(ch))) * 0.75 || (PLR_FLAGGED(ch, PLR_SELFD2) &&
                                                                   ((ch_getCurLF(ch))) >= ((ch_getMaxLF(ch))) * 0.5))) {
            int64_t result;
            result = ch_decCurLFPercent(ch, 2, -1);
            int64_t result1;
        ...
        if (PLR_FLAGGED(ch, PLR_IMMORTAL)) {
            act("@c$n@w disappears right before dying. $n appears to be immortal.@n", true, ch, NULL, NULL,
                TO_CHAR);
            act("@c$n@w disappears right before dying. $n appears to be immortal.@n.", true, ch, NULL, NULL,
                TO_ROOM);
            int64_t result2;
            result2 = ch_decCurHealthPercent(ch, 1, 1);
            int64_t result1;
            result1 = ch_decCurKIPercent(ch, 1, 1);
            int64_t result;
        ...
        if (!AFF_FLAGGED(ch, AFF_SPIRIT) && !ROOM_FLAGGED(IN_ROOM(ch), ROOM_PAST) && GET_LEVEL(ch) > 8) {
            if (GET_ROOM_VNUM(IN_ROOM(ch)) >= 2002 && GET_ROOM_VNUM(IN_ROOM(ch)) <= 2011) {
                ...
                if (killer != NULL && IS_NPC(killer)) {
                    GET_DTIME(ch) = time(NULL) + 28800;
                    GET_DCOUNT(ch) += 1;
                } else if (killer != NULL && !IS_NPC(killer)) {
                    GET_DTIME(ch) = time(NULL) + 1123200;
                    ...
                    if (GET_DCOUNT(ch) <= 0) {
                        ...
                        GET_DTIME(ch) = time(NULL) + 43200;
                    } else if (GET_DCOUNT(ch) <= 3) {
                        GET_DTIME(ch) = time(NULL) + 86400;
                    } else if (GET_DCOUNT(ch) <= 5) {
                        GET_DTIME(ch) = time(NULL) + 172800;
                    } else if (GET_DCOUNT(ch) > 5) {
                        GET_DTIME(ch) = time(NULL) + 604800;
                    }
                    GET_DCOUNT(ch) += 1;
                }
            }
...
static void perform_group_gain(struct char_data *ch, int base, struct char_data *victim) {
    ...
    if (!IS_NPC(ch)) {
        if (GET_LEVEL(ch) >= 100 && (ch_getEffMaxPL(ch)) * .025 >= (ch_getEffMaxPL(victim))) {
            share *= .05;
        } else if ((ch_getEffMaxPL(ch)) * .025 >= (ch_getEffMaxPL(victim))) {
            share = 1;
        } else if ((ch_getEffMaxPL(ch)) * .05 >= (ch_getEffMaxPL(victim))) {
            share *= .05;
        } else if ((ch_getEffMaxPL(ch)) * .1 >= (ch_getEffMaxPL(victim))) {
            share *= .1;
        } else if ((ch_getEffMaxPL(ch)) * .15 >= (ch_getEffMaxPL(victim))) {
            share *= .15;
        } else if ((ch_getEffMaxPL(ch)) * .25 >= (ch_getEffMaxPL(victim))) {
            share *= .25;
        } else if ((ch_getEffMaxPL(ch)) * .5 >= (ch_getEffMaxPL(victim))) {
            share *= .5;
        } else if ((ch_getEffMaxPL(ch)) * .9 >= (ch_getEffMaxPL(victim))) {
            share *= .65;
        } else if ((ch_getEffMaxPL(ch)) >= (ch_getEffMaxPL(victim))) {
            share *= .7;
        }
    }
    ...
    if (group_bonus(ch, 2) == 2) {
        ...
        int64_t result1;
        result1 = ch_incCurKIPercent(ch, .04, true);
        send_to_char(ch, "You receive a bonus from your group's leader! @D[@G4%s Ki Regenerated!@D]@n\r\n", "%");
    } else if (group_bonus(ch, 2) == 6) {
        int64_t result1;
        result1 = ch_incCurKIPercent(ch, .02, true);
        int64_t result;
        result = ch_incCurSTPercent(ch, .02, true);
        int64_t result2;
        result2 = ch_incCurHealthPercent(ch, .02, true);
        ...
        if (IS_ANDROID(ch)) {
            if (PLR_FLAGGED(leader, PLR_ABSORB)) {
                ...
                int64_t result;
                result = ch_incCurHealthPercent(ch, .02, true);
            ...
            int64_t result2;
            result2 = ch_incCurHealthPercent(ch, .01, true);
            int64_t result1;
            result1 = ch_incCurKIPercent(ch, .01, true);
            int64_t result;
            result = ch_incCurSTPercent(ch, .01, true);
        }
    } else if (group_bonus(ch, 2) == 11) {
        int64_t result;
        result = ch_incCurSTPercent(ch, .04, true);
...
void group_gain(struct char_data *ch, struct char_data *victim) {
    ...
    for (f = k->followers; f; f = f->next)
        if (AFF_FLAGGED(f->follower, AFF_GROUP) && IN_ROOM(f->follower) == IN_ROOM(ch)) {
            if (!ch_isWeightedPL(f->follower)) {
                tot_levels += GET_LEVEL(f->follower);
                tot_members++;
            } else if ((ch_getEffMaxPL(f->follower)) >= (ch_getEffMaxPL(ch)) * 0.5) {
                tot_levels += GET_LEVEL(f->follower);
                tot_members++;
            }
        }
    ...
    if (AFF_FLAGGED(k, AFF_GROUP) && IN_ROOM(k) == IN_ROOM(ch)) {
     ...
     } else if (k != ch && (ch_getEffMaxPL(k)) >= (ch_getEffMaxPL(ch)) * 0.5) {
      perform_group_gain(k, base, victim);
     } else if (k == ch && (ch_getEffMaxPL(k)) >= GET_MAX_HIT(ch) * 0.5) {
      perform_group_gain(k, base, victim);
     } else {
      if (k == ch) {
    ...
    for (f = k->followers; f; f = f->next) {
        if (AFF_FLAGGED(f->follower, AFF_GROUP) && IN_ROOM(f->follower) == IN_ROOM(ch)) {
            //if ((f->ch_getEffMaxPL(follower)) >= GET_MAX_HIT(ch) * 0.5)
