...
static struct bfs_queue_struct *bfs_queue_head = NULL, *bfs_queue_tail = NULL;
...
ACMD(do_sradar) {
    ...
    if (noship == false) {
        if (!strcasecmp(arg, "earth") || !strcasecmp(arg, "Earth")) {
            ...
            snprintf(planet, 20, "Buoy One");
        } else if (!strcasecmp(arg, "buoy2") && GET_RADAR2(ch) > 0) {
            int rad = GET_RADAR2(ch);
            dir = find_first_step(IN_ROOM(vehicle), real_room(rad));
            snprintf(planet, 20, "Buoy Two");
        } else if (!strcasecmp(arg, "buoy3") && GET_RADAR3(ch) > 0) {
            int rad = GET_RADAR3(ch);
            dir = find_first_step(IN_ROOM(vehicle), real_room(rad));
            snprintf(planet, 20, "Buoy Three");
        } else {
...
ACMD(do_radar) {
    room_rnum room = 0, dir, num = 0, fcount = 0;
    ...
    if (IS_NPC(ch)) {
        ...
        act("$n holds up a dragon radar and pushes its button.", false, ch, NULL, NULL, TO_ROOM);
        while (num < 20000) {
...
ACMD(do_track) {
    ...
    if (!*arg && !FIGHTING(ch)) {
        ...
        act("You look at $N@n intently for a moment.", true, ch, NULL, vict, TO_CHAR);
        act("$n looks at you intently for a moment.", true, ch, NULL, vict, TO_VICT);
        act("$n looks at $N@n intently for a moment.", true, ch, NULL, vict, TO_NOTVICT);
        if (!IS_ANDROID(vict)) {
    ...
    if ((GET_SKILL_BASE(ch, SKILL_SENSE) == 100) &&
        ...
        switch (dir) {
            ...
            case BFS_ALREADY_THERE:
                act("You look at $N@n intently for a moment.", true, ch, NULL, vict, TO_CHAR);
                act("$n looks at you intently for a moment.", true, ch, NULL, vict, TO_VICT);
                act("$n looks at $N intently for a moment.", true, ch, NULL, vict, TO_NOTVICT);
                if (!IS_ANDROID(vict)) {
                    if (GET_ALIGNMENT(vict) > 50 && GET_ALIGNMENT(vict) < 200) {
                        send_to_char(ch, "You sense slightly pure and good ki from them.\r\n");
                    } else if (GET_ALIGNMENT(vict) > 200 && GET_ALIGNMENT(vict) < 500) {
