...
ACMD(do_evolve) {
    ...
    plcost += molt_threshold(ch) * 0.65 + (ch_getBasePL(ch) * 0.15);
    kicost += (molt_threshold(ch) * 0.50) + (ch->baseki * 0.22);
    stcost += (molt_threshold(ch) * 0.50) + (ch_getBaseST(ch) * 0.15);
    ...
    if (!*arg) {
        ...
        if (plcost > molt_threshold(ch)) {
            ...
            int64_t plgain = ((ch_getBasePL(ch))) * 0.01;
            ...
            int64_t result;
            result = ch_gainBasePL(ch, plgain, false);
        ...
        if (kicost > molt_threshold(ch)) {
            ...
            int64_t result;
            result = ch_gainBaseKI(ch, kigain, false);
        ...
        if (stcost > molt_threshold(ch)) {
            ...
            int64_t stgain = ((ch_getBaseST(ch))) * 0.01;
            ...
            int64_t result;
            result = ch_gainBaseST(ch, stgain, false);
...
static void search_room(struct char_data *ch) {
    ...
    if (((ch_getCurST(ch))) < GET_MAX_MOVE(ch) * 0.001) {
    ...
    reveal_hiding(ch, 0);
    act("@y$n@Y begins searching the room carefully.@n", true, ch, NULL, NULL, TO_ROOM);
    ...
    for (vict = world[IN_ROOM(ch)].people; vict; vict = next_v) {
        ...
        if (AFF_FLAGGED(vict, AFF_HIDE) && vict != ch) {
            ...
            if (perc * bonus >= prob * terrain) { /* Found them. */
                act("@YYou find @y$N@Y hiding nearby!@n", true, ch, NULL, vict, TO_CHAR);
                act("@y$n@Y has found your hiding spot!@n", true, ch, NULL, vict, TO_VICT);
                act("@y$n@Y has found @y$N's@Y hiding spot!@n", true, ch, NULL, vict, TO_NOTVICT);
    ...
    for (obj = world[IN_ROOM(ch)].contents; obj; obj = obj->next_content) {
        if (OBJ_FLAGGED(obj, ITEM_BURIED) && perc * bonus > rand_number(50, 200)) {
            act("@YYou uncover @y$p@Y, which had been burried here.@n", true, ch, obj, NULL, TO_CHAR);
            act("@y$n@Y uncovers @y$p@Y, which had burried here.@n", true, ch, obj, NULL, TO_ROOM);
            REMOVE_BIT_AR(GET_OBJ_EXTRA(obj), ITEM_BURIED);
            found++;
        }
    }
    int64_t result;
    result = ch_decCurSTPercent(ch, .001, 0);
...
ACMD(do_mimic) {
    ...
    if (!*arg) {
        ...
        for (int i = 0; i < NUM_RACES;i++) {
            if(!race_CanBeMimiced(i)) continue;
            if (count == 2) {
                send_to_char(ch, "%s\n", race_getName(i));
                count = 0;
            } else {
        ...
        if (ch->mimic) {
            send_to_char(ch, "You currently Mimic a %s", race_getName(ch->mimic+1));
        }
    ...
    if (!strcasecmp(arg, "stop")) {
        ...
        act("@mYou concentrate for a moment and release the illusion that was mimicing another race.@n", true, ch,
            NULL, NULL, TO_CHAR);
        act("@M$n@m concentrates for a moment and SUDDENLY $s appearance changes some what!@n", true, ch, NULL,
            NULL, TO_ROOM);
        ch->mimic = 0;
    }

    int race = race_findName(arg);
    ...
    if (race == ch->mimic+1) {
        ...
        if (race && ((ch_getMaxKI(ch) * ch->energy)) < cost) {
...
ACMD(do_kyodaika) {
    ...
    if (GET_GENOME(ch, 0) == 0) {
        act("@GYou growl as your body grows to ten times its normal size!@n", true, ch, NULL, NULL, TO_CHAR);
        act("@g$n@G growls as $s body grows to ten times its normal size!@n", true, ch, NULL, NULL, TO_ROOM);
        ...
        act("@GYou growl as your body shrinks to its normal size!@n", true, ch, NULL, NULL, TO_CHAR);
        act("@g$n@G growls as $s body shrinks to its normal size!@n", true, ch, NULL, NULL, TO_ROOM);
        send_to_char(ch, "@cStrength@D: @C-5\r\n@cSpeed@D: @c+2@n\r\n");
        ch->real_abils.str -= 5;
        ch->real_abils.cha += 2;
        GET_GENOME(ch, 0) = 0;
...
ACMD(do_shuffle) {
    ...
    act("$n shuffles their deck.", true, ch, NULL, NULL, TO_ROOM);
...
ACMD(do_hand) {
    ...
    if (!strcasecmp("look", arg)) {
        ...
        act("$n looks at $s hand.", true, ch, NULL, NULL, TO_ROOM);
        if (count == 0) {
            send_to_char(ch, "No cards.");
            act("There were no cards.", true, ch, NULL, NULL, TO_ROOM);
        } else if (count > 7) {
            act("You have more than seven cards in your hand.", true, ch, NULL, NULL, TO_CHAR);
            act("$n has more than seven cards in $s hand.", true, ch, NULL, NULL, TO_ROOM);
        } else {
            char buf[200];
        ...
        for (obj = ch->carrying; obj; obj = next_obj) {
            ...
            if (obj) {
                count += 1;
                act("$p", true, ch, obj, NULL, TO_ROOM);
            }
        }
        if (count == 0) {
            act("No cards.", true, ch, NULL, NULL, TO_ROOM);
        }
        if (count > 7) {
            act("You have more than seven cards in your hand.", true, ch, NULL, NULL, TO_CHAR);
            act("$n has more than seven cards in $s hand.", true, ch, NULL, NULL, TO_ROOM);
...
ACMD(do_post) {
    ...
    if (!*arg2) {
        ...
        act("@WYou post $p@W on a nearby structure.@n", true, ch, obj, NULL, TO_CHAR);
        act("@C$n@W posts $p@W on a nearby structure.@n", true, ch, obj, NULL, TO_ROOM);
        obj_from_char(obj);
...
ACMD(do_play) {
    ...
    act("You play $p on your table.", true, ch, obj, NULL, TO_CHAR);
    act("$n plays $p on $s table.", true, ch, obj, NULL, TO_ROOM);
    obj_from_char(obj);
    obj_to_obj(obj, obj2);
}
...
ACMD(do_intro) {
    ...
    if (readIntro(vict, ch) == 2) {
        ...
        act("You decide to call $M, $N.", true, ch, NULL, vict, TO_CHAR);
        act("$n seems to decide something about you.", true, ch, NULL, vict, TO_VICT);
        act("$n seems to decide something about $N.", true, ch, NULL, vict, TO_NOTVICT);
        return;
    }
}
...
static void diag_char_to_char(struct char_data *i, struct char_data *ch) {
    ...
    int64_t hit = GET_HIT(i), max = (ch_getEffMaxPL(i));
...
static void look_at_char(struct char_data *i, struct char_data *ch) {
    ...
    if (found && (!IS_NPC(ch) && !PRF_FLAGGED(ch, PRF_NOEQSEE))) {
        ...
        if (!PLR_FLAGGED(i, PLR_DISGUISED)) {
            act("$n is using:", false, i, NULL, ch, TO_VICT);
        } else {
            act("The disguised person is using:", false, i, NULL, ch, TO_VICT);
        }
    ...
    if (ch != i && ((GET_SKILL(ch, SKILL_KEEN) && AFF_FLAGGED(ch, AFF_SNEAK)) || GET_ADMLEVEL(ch))) {
        found = false;
        act("\r\nYou attempt to peek at $s inventory:", false, i, NULL, ch, TO_VICT);
        if (CAN_SEE(i, ch))
            act("$n tries to evaluate what you have in your inventory.", true, ch, NULL, i, TO_VICT);
        if (GET_SKILL(ch, SKILL_KEEN) > axion_dice(0) && (!IS_NPC(i) || GET_ADMLEVEL(ch) > 1)) {
            ...
            act("You are unsure about $s inventory.", false, i, NULL, ch, TO_VICT);
            if (CAN_SEE(i, ch))
                act("$n didn't seem to get a good enough look.", true, ch, NULL, i, TO_VICT);
            improve_skill(ch, SKILL_KEEN, 1);
            return;
        }
        if (!found) {
...
static void list_one_char(struct char_data *i, struct char_data *ch) {
    ...
    if (IS_NPC(i) && i->long_descr && GET_POS(i) == GET_DEFAULT_POS(i) && !FIGHTING(i)) {
        ...
        if (IS_NPC(i) && GET_HIT(i) >= (ch_getEffMaxPL(i)) * .9 && GET_HIT(i) != (ch_getEffMaxPL(i)))
            act("@R...Some slight wounds on $s body.@w", true, i, NULL, ch, TO_VICT);
        else if (IS_NPC(i) && GET_HIT(i) >= (ch_getEffMaxPL(i)) * .8 && GET_HIT(i) < (ch_getEffMaxPL(i)) * .9)
            act("@R...A few wounds on $s body.@w", true, i, NULL, ch, TO_VICT);
        else if (IS_NPC(i) && GET_HIT(i) >= (ch_getEffMaxPL(i)) * .7 && GET_HIT(i) < (ch_getEffMaxPL(i)) * .8)
            act("@R...Many wounds on $s body.@w", true, i, NULL, ch, TO_VICT);
        else if (IS_NPC(i) && GET_HIT(i) >= (ch_getEffMaxPL(i)) * .6 && GET_HIT(i) < (ch_getEffMaxPL(i)) * .7)
            act("@R...Quite a few wounds on $s body.@w", true, i, NULL, ch, TO_VICT);
        else if (IS_NPC(i) && GET_HIT(i) >= (ch_getEffMaxPL(i)) * .5 && GET_HIT(i) < (ch_getEffMaxPL(i)) * .6)
            act("@R...Horrible wounds on $s body.@w", true, i, NULL, ch, TO_VICT);
        else if (IS_NPC(i) && GET_HIT(i) >= (ch_getEffMaxPL(i)) * .4 && GET_HIT(i) < (ch_getEffMaxPL(i)) * .5)
            act("@R...Blood is seeping from the wounds on $s body.@w", true, i, NULL, ch, TO_VICT);
        else if (IS_NPC(i) && GET_HIT(i) >= (ch_getEffMaxPL(i)) * .3 && GET_HIT(i) < (ch_getEffMaxPL(i)) * .4)
            act("@R...$s body is in terrible shape.@w", true, i, NULL, ch, TO_VICT);
        else if (IS_NPC(i) && GET_HIT(i) >= (ch_getEffMaxPL(i)) * .2 && GET_HIT(i) < (ch_getEffMaxPL(i)) * .3)
            act("@R...Is absolutely covered in wounds.@w", true, i, NULL, ch, TO_VICT);
        else if (IS_NPC(i) && GET_HIT(i) >= (ch_getEffMaxPL(i)) * .1 && GET_HIT(i) < (ch_getEffMaxPL(i)) * .2)
            act("@R...Is on $s last leg.@w", true, i, NULL, ch, TO_VICT);
        else if (IS_NPC(i) && GET_HIT(i) < (ch_getEffMaxPL(i)) * .1)
            act("@R...Should be DEAD soon.@w", true, i, NULL, ch, TO_VICT);


        if (GET_EAVESDROP(i) > 0) {
            char eaves[300];
            sprintf(eaves, "@w...$e is spying on everything to the @c%s@w.", dirs[GET_EAVESDIR(i)]);
            act(eaves, true, i, NULL, ch, TO_VICT);
        }
        if (AFF_FLAGGED(i, AFF_FLYING) && GET_ALT(i) == 1)
            act("...$e is in the air!", false, i, NULL, ch, TO_VICT);
        if (AFF_FLAGGED(i, AFF_FLYING) && GET_ALT(i) == 2)
            act("...$e is high in the air!", false, i, NULL, ch, TO_VICT);
        if (AFF_FLAGGED(i, AFF_SANCTUARY) && !GET_SKILL(i, SKILL_AQUA_BARRIER))
            act("...$e has a barrier around $s body!", false, i, NULL, ch, TO_VICT);
        if (AFF_FLAGGED(i, AFF_FIRESHIELD))
            act("...$e has @rf@Rl@Ya@rm@Re@Ys@w around $s body!", false, i, NULL, ch, TO_VICT);
        if (AFF_FLAGGED(i, AFF_SANCTUARY) && GET_SKILL(i, SKILL_AQUA_BARRIER))
            act("...$e has a @Gbarrier@w of @cwater@w and @Cki@w around $s body!", false, i, NULL, ch, TO_VICT);
        if (!IS_NPC(i) && PLR_FLAGGED(i, PLR_SPIRAL))
            act("...$e is spinning in a vortex!", false, i, NULL, ch, TO_VICT);
        if (GET_CHARGE(i))
            act("...$e has a bright %s aura around $s body!", false, i, NULL, ch, TO_VICT);
        if (AFF_FLAGGED(i, AFF_METAMORPH))
            act("@w...$e has a dark, @rred@w aura and menacing presence.", false, i, NULL, ch, TO_VICT);
        if (AFF_FLAGGED(i, AFF_HAYASA))
            act("@w...$e has a soft @cblue@w glow around $s body!", false, i, NULL, ch, TO_VICT);
        if (AFF_FLAGGED(i, AFF_BLIND))
            act("...$e is groping around blindly!", false, i, NULL, ch, TO_VICT);
        if (affected_by_spell(i, SPELL_FAERIE_FIRE))
            act("@m...$e @mis outlined with purple fire!@m", false, i, NULL, ch, TO_VICT);
        if (GET_FEATURE(i)) {
            char woo[MAX_STRING_LENGTH];
            sprintf(woo, "@C%s@n", GET_FEATURE(i));
            act(woo, false, i, NULL, ch, TO_VICT);
        }
    ...
    if (!IS_NPC(i)) {
        if (PLR_FLAGGED(i, PLR_FISHING)) {
            act("@w...$e is @Cfishing@w.@n", true, i, NULL, ch, TO_VICT);
        }
    }
    if (PLR_FLAGGED(i, PLR_AURALIGHT)) {
        ...
        act(bloom, true, i, NULL, ch, TO_VICT);
    }
    if (AFF_FLAGGED(i, AFF_SANCTUARY) && !GET_SKILL(i, SKILL_AQUA_BARRIER))
        act("@w...$e has a @bbarrier@w around $s body!", true, i, NULL, ch, TO_VICT);
    if (AFF_FLAGGED(i, AFF_FIRESHIELD))
        act("@w...$e has @rf@Rl@Ya@rm@Re@Ys@w around $s body!", false, i, NULL, ch, TO_VICT);
    if (AFF_FLAGGED(i, AFF_HEALGLOW))
        act("@w...$e has a serene @Cblue@Y glow@w around $s body.", true, i, NULL, ch, TO_VICT);
    if (AFF_FLAGGED(i, AFF_EARMOR))
        act("@w...$e has ghostly @Ggreen@w ethereal armor around $s body.", true, i, NULL, ch, TO_VICT);
    if (AFF_FLAGGED(i, AFF_SANCTUARY) && GET_SKILL(i, SKILL_AQUA_BARRIER))
        act("@w...$e has a @bbarrier@w of @cwater@w and @CKi@w around $s body!", true, i, NULL, ch, TO_VICT);
    if (AFF_FLAGGED(i, AFF_FLYING) && GET_ALT(i) == 1)
        act("@w...$e is in the air!", true, i, NULL, ch, TO_VICT);
    if (AFF_FLAGGED(i, AFF_FLYING) && GET_ALT(i) == 2)
        act("@w...$e is high in the air!", true, i, NULL, ch, TO_VICT);
    if (GET_KAIOKEN(i) > 0)
        act("@w...@r$e has a red aura around $s body!", true, i, NULL, ch, TO_VICT);
    if (!IS_NPC(i) && PLR_FLAGGED(i, PLR_SPIRAL))
        act("@w...$e is spinning in a vortex!", false, i, NULL, ch, TO_VICT);
    if (IS_TRANSFORMED(i) && !IS_ANDROID(i) && !IS_SAIYAN(i) && !IS_HALFBREED(i))
        act("@w...$e has energy crackling around $s body!", true, i, NULL, ch, TO_VICT);
    if (GET_CHARGE(i) && !IS_SAIYAN(i) && !IS_HALFBREED(i)) {
        ...
        act(aura, true, i, NULL, ch, TO_VICT);
    }
    if (!PLR_FLAGGED(i, PLR_OOZARU) && GET_CHARGE(i) && IS_TRANSFORMED(i) && (IS_SAIYAN(i) || IS_HALFBREED(i)))
        act("@w...$e has a @Ybright @Yg@yo@Yl@yd@Ye@yn@w aura around $s body!", true, i, NULL, ch, TO_VICT);
    if (!PLR_FLAGGED(i, PLR_OOZARU) && GET_CHARGE(i) && !IS_TRANSFORMED(i) && (IS_SAIYAN(i) || IS_HALFBREED(i))) {
        char aura[MAX_INPUT_LENGTH];
        sprintf(aura, "@w...$e has a @Ybright@w %s aura around $s body!", aura_types[GET_AURA(i)]);
        act(aura, true, i, NULL, ch, TO_VICT);
    ...
    if (PLR_FLAGGED(i, PLR_OOZARU) && GET_CHARGE(i) && (IS_SAIYAN(i) || IS_HALFBREED(i)))
        act("@w...$e is in the form of a @rgreat ape@w!", true, i, NULL, ch, TO_VICT);
    if (GET_GENOME(i, 0) == 11)
        act("@w...$e has expanded $s body size@w!", true, i, NULL, ch, TO_VICT);
    if (AFF_FLAGGED(i, AFF_HAYASA))
        act("@w...$e has a soft @cblue@w glow around $s body!", false, i, NULL, ch, TO_VICT);
    if (PLR_FLAGGED(i, PLR_OOZARU) && !GET_CHARGE(i) && (IS_SAIYAN(i) || IS_HALFBREED(i)))
        act("@w...$e has energy crackling around $s @rgreat ape@w body!", true, i, NULL, ch, TO_VICT);
    if (GET_FEATURE(i)) {
        char woo[MAX_STRING_LENGTH];
        sprintf(woo, "@C%s@n", GET_FEATURE(i));
        act(woo, false, i, NULL, ch, TO_VICT);
    ...
    if (GET_RDISPLAY(i)) {
        if (GET_RDISPLAY(i) != "Empty") {
            ...
            act(rdis, false, i, NULL, ch, TO_VICT);
        }
    }
            ...
            if (CONFIG_STACK_MOBS) {
                ...
                for (j = list; j != i; j = j->next_in_room)
                    if ((i->nr == j->nr) &&
                        ...
                        (GET_HIT(i) == (ch_getEffMaxPL(i)) && GET_HIT(j) == (ch_getEffMaxPL(j))) &&
...
*/
        ...
        if (OBJVAL_FLAGGED(obj, CONT_CLOSED))
            ...
            if (GET_OBJ_TYPE(obj) == ITEM_CONTAINER &&
                ...
                act("$n looks in $p.", true, ch, obj, NULL, TO_ROOM);
            }
            switch (bits) {
                case FIND_OBJ_INV:
    ...
    if (!viewport) {
        ...
        if (target_room == NOWHERE) {
            ...
            if (viewport->action_description)
                act(viewport->action_description, true, ch, viewport, NULL, TO_CHAR);
            else
                act("$n looks out the window.", true, ch, NULL, NULL, TO_ROOM);
...
ACMD(do_score) {
    ...
    if (view == full || view == personal) {
        ...
        send_to_char(ch, "  @D|  @CRace@D: @W%10s@D,  @CSensei@D: @W%15s@D,     @CArt@D: @W%-17s@D|@n\n", TRUE_RACE(ch),
                     sen_getName(ch->chclass), sen_style(ch->chclass));
        ...
        for (d = descriptor_list; d; d = d->next) {
            ...
            if (short_list) {
                send_to_char(ch, "               @B[@W%3d @Y%s @C%s@B]@W %-12.12s@n%s@n",
                             GET_LEVEL(tch), RACE_ABBR(tch), sen_getAbbr(tch->chclass), GET_NAME(tch),
                             ((!(++num_can_see % 4)) ? "\r\n" : ""));
            } else {
                num_can_see++;
    ...
    if (!*argument) {
        ...
        if (load_char(argument, victim) >= 0) {
            ...
            if (GET_ADMLEVEL(victim) >= ADMLVL_IMMORT) {
                ...
                send_to_char(ch,
                             "@cName  @D: @w%s\r\n@cSensei@D: @w%s\r\n@cRace  @D: @w%s\r\n@cTitle @D: @w%s@n\r\n@cClan  @D: @w%s@n\r\n",
                             GET_NAME(victim), sen_getName(victim->chclass), race_getName(victim->race),
...
static void search_in_direction(struct char_data *ch, int dir) {
    ...
    act("$n searches the area intently.", true, ch, NULL, NULL, TO_ROOM);
