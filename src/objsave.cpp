...
int Crash_clean_file(char *name) {
    ...
    if (!feof(fl)) {
        ...
        if ((rentcode == RENT_CRASH) ||
            ...
            if (timed < time(NULL) - (CONFIG_CRASH_TIMEOUT * SECS_PER_REAL_DAY)) {
                const char *filetype;
            ...
            if (timed < time(NULL) - (CONFIG_RENT_TIMEOUT * SECS_PER_REAL_DAY)) {
                Crash_delete_file(name);
...
void Crash_crashsave(struct char_data *ch) {
    ...
    fprintf(fp, "%d %d %d %d %d %d\r\n", RENT_CRASH, (int) time(NULL), 0, GET_GOLD(ch),
            GET_BANK_GOLD(ch), 0);
...
void Crash_idlesave(struct char_data *ch) {
    ...
    fprintf(fp, "%d %d %d %d %d %d\r\n", RENT_TIMEDOUT, (int) time(NULL), cost,
...
void Crash_rentsave(struct char_data *ch, int cost) {
    ...
    fprintf(fp, "%d %d %d %d %d %d\r\n", RENT_RENTED, (int) time(NULL), cost,
            GET_GOLD(ch), GET_BANK_GOLD(ch), 0);
...
static void Crash_cryosave(struct char_data *ch, int cost) {
    ...
    fprintf(fp, "%d %d %d %d %d %d\r\n", RENT_CRYO, (int) time(NULL), 0, GET_GOLD(ch),
            GET_BANK_GOLD(ch), 0);
...
static void Crash_rent_deadline(struct char_data *ch, struct char_data *recep,
    ...
    act(buf, false, recep, NULL, ch, TO_VICT);
}
...
static int Crash_offer_rent(struct char_data *ch, struct char_data *recep,
    ...
    if (display) {
        ...
        if (totalcost > GET_GOLD(ch) + GET_BANK_GOLD(ch)) {
            act("$n tells you, '...which I see you can't afford.'", false, recep, NULL, ch, TO_VICT);
            return (0);
...
static int gen_receptionist(struct char_data *ch, struct char_data *recep,
    ...
    if (!CAN_SEE(recep, ch)) {
        act("$n says, 'I don't deal with people I can't see!'", false, recep, NULL, NULL, TO_ROOM);
        return (true);
    }
    ...
    if (CMD_IS("rent")) {
        ...
        if (mode == RENT_FACTOR) {
            act("$n stores your belongings and helps you into your private chamber.", false, recep, NULL, ch,
                TO_VICT);
        ...
        act("$n helps $N into $S private chamber.", false, recep, NULL, ch, TO_NOTVICT);
...
int Crash_load(struct char_data *ch) {
    ...
    if (rentcode == RENT_RENTED || rentcode == RENT_TIMEDOUT) {
        num_of_days = (float) (time(NULL) - timed) / SECS_PER_REAL_DAY;
        cost = 0;
        save_char(ch);
    }
