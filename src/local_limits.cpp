...
static void barrier_shed(struct char_data *ch) {
    ...
    if (GET_BARRIER(ch) <= 0) {
        GET_BARRIER(ch) = 0;
        act("@cYour barrier disappears.@n", true, ch, NULL, NULL, TO_CHAR);
        act("@c$n@c's barrier disappears.@n", true, ch, NULL, NULL, TO_ROOM);
    } else {
        act("@cYour barrier loses some energy.@n", true, ch, NULL, NULL, TO_CHAR);
        send_to_char(ch, "@D[@C%s@D]@n\r\n", add_commas(loss));
        act("@c$n@c's barrier sends some sparks into the air as it seems to get a bit weaker.@n", true, ch, NULL,
            NULL, TO_ROOM);
    }

    if (recharge > 0 && ((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch)) {
        int64_t result;
        result = ch_incCurKI(ch, recharge, true);
...
int64_t hit_gain(struct char_data *ch) {
    ...
    if (IS_NPC(ch)) {
        /* Neat and fast */
        gain = (ch_getEffMaxPL(ch)) / 70;
    } else {
        if (ROOM_FLAGGED(IN_ROOM(ch), ROOM_REGEN) ||
            (GET_BONUS(ch, BONUS_DESTROYER) > 0 && ROOM_DAMAGE(IN_ROOM(ch)) >= 75)) {
            if (IS_HUMAN(ch)) {
                gain = (ch_getEffMaxPL(ch)) / 20;
            }
            if (IS_ARLIAN(ch)) {
                gain = (ch_getEffMaxPL(ch)) / 30;
            }
            if (IS_NAMEK(ch)) {
                gain = (ch_getEffMaxPL(ch)) / 2;
            }
            if (IS_MUTANT(ch)) {
                gain = (ch_getEffMaxPL(ch)) / 11;
            }
            if (!IS_HUMAN(ch) && !IS_NAMEK(ch) && !IS_MUTANT(ch)) {
                gain = (ch_getEffMaxPL(ch)) / 10;
            }
        } else if (!ROOM_FLAGGED(IN_ROOM(ch), ROOM_REGEN)) {
            if (IS_HUMAN(ch)) {
                gain = (ch_getEffMaxPL(ch)) / 30;
            ...
            if (IS_NAMEK(ch)) {
                gain = (ch_getEffMaxPL(ch)) / 4;
            }
            if (IS_MUTANT(ch)) {
                gain = (ch_getEffMaxPL(ch)) / 16;
            }
            if (IS_ARLIAN(ch)) {
                gain = (ch_getEffMaxPL(ch)) / 40;
            }
            if (!IS_HUMAN(ch) && !IS_NAMEK(ch) && !IS_MUTANT(ch)) {
                gain = (ch_getEffMaxPL(ch)) / 15;
            }
            if (ROOM_FLAGGED(IN_ROOM(ch), ROOM_BEDROOM)) {
                gain += gain * 0.25;
            }
...
static int64_t move_gain(struct char_data *ch) {
    ...
    if (ROOM_FLAGGED(IN_ROOM(ch), ROOM_AURA)) {
        gain = GET_MAX_MOVE(ch) - ((ch_getCurST(ch)));
    }
    if (cook_element(IN_ROOM(ch)) == 1)
        gain *= 2;
...
static void update_flags(struct char_data *ch) {
    ...
    if (GET_BONUS(ch, BONUS_LATE) && GET_POS(ch) == POS_SLEEPING && rand_number(1, 3) == 3) {
        if (GET_HIT(ch) >= (ch_getEffMaxPL(ch)) && ((ch_getCurST(ch))) >= GET_MAX_MOVE(ch) &&
    ...
    if (!IS_NPC(ch) && !PLR_FLAGGED(ch, PLR_STAIL) && !PLR_FLAGGED(ch, PLR_NOGROW) &&
        ...
        if (GET_TGROWTH(ch) >= 10) {
            ...
            act("$n@w's tail grows back.@n", true, ch, NULL, NULL, TO_ROOM);
            ch_gainTail(ch);
            GET_TGROWTH(ch) = 0;
        }
    }
    ...
    if (AFF_FLAGGED(ch, AFF_FROZEN) && rand_number(1, 2) == 2) {
        ...
        act("$n@W breaks out of the ice holding $m prisoner!", true, ch, NULL, NULL, TO_ROOM);
    ...
    if (AFF_FLAGGED(ch, AFF_WITHER) && rand_number(1, 6 + sick_fail) == 2) {
        ...
        act("$n@W's looks more fit now.", true, ch, NULL, NULL, TO_ROOM);
        ch->real_abils.str += 3;
        ch->real_abils.cha += 3;
        save_char(ch);
        REMOVE_BIT_AR(AFF_FLAGS(ch), AFF_WITHER);
    ...
    if (gain > 0) {
        ...
        if (GET_LEVEL(ch) < 100) {
            if (MINDLINK(ch) && gain > 0 && LINKER(ch) == 0) {
                if (GET_LEVEL(ch) + 20 < GET_LEVEL(MINDLINK(ch)) || GET_LEVEL(ch) - 20 > GET_LEVEL(MINDLINK(ch))) {
                    ...
                    act("@GYou've absorbed some new experiences from @W$n@G!@n", false, ch, NULL, MINDLINK(ch),
                        TO_VICT);
                    int read = gain * 0.12;
                    gain -= read;
                    if (read == 0)
        ...
        if (GET_LEVEL(ch) == 100 && GET_ADMLEVEL(ch) < 1) {
            ...
            if (rand_number(1, 5) >= 2) {
                if (IS_HUMAN(ch)) {
                    int64_t result;
                    result = ch_gainBasePL(ch, diff * 0.8, false);
                } else {
                    int64_t result;
                    result = ch_gainBasePL(ch, diff, false);
                }
            ...
            if (rand_number(1, 5) >= 2) {
                if (IS_HALFBREED(ch)) {
                    int64_t result;
                    result = ch_gainBaseST(ch, diff * 0.85, false);
                } else {
                    int64_t result;
                    result = ch_gainBaseST(ch, diff, false);
                }
                send_to_char(ch, "@D[@G+@Y%s @gSTA@D]@n ", add_commas(diff));
            }
            if (rand_number(1, 5) >= 2) {
                int64_t result;
                result = ch_gainBaseKI(ch, diff, false);
    ...
    if (PLR_FLAGGED(ch, PLR_WRITING))
                ...
                if (GET_COND(ch, condition) + value > 48) {
                    ...
                    if (condition != DRUNK && prior >= 48 && !IS_MAJIN(ch)) {
                        ...
                        if (pukeroll > GET_CON(ch) + 19) {
                            ...
                            act("@m@6$n@r@6 pukes violently! It seems $e stuffed $mself too much!@n", true, ch, NULL,
                                NULL, TO_ROOM);
                            SET_BIT_AR(AFF_FLAGS(ch), AFF_PUKED);
                            if (!IS_NAMEK(ch)) {
                            ...
                            act("@r@6You puke a little! Your constitution couldn't handle being that stuffed!@n", true,
                                ch, NULL, NULL, TO_CHAR);
                            act("@m@6$n@r@6 pukes a little! It seems $e stuffed $mself too much!@n", true, ch, NULL,
                                NULL, TO_ROOM);
                            SET_BIT_AR(AFF_FLAGS(ch), AFF_PUKED);
                            if (!IS_NAMEK(ch)) {
        ...
        if (!AFF_FLAGGED(ch, AFF_SPIRIT) &&
            ...
            switch (condition) {
                case HUNGER:
                    switch (GET_COND(ch, condition)) {
                        case 0: {
                            if (((ch_getCurST(ch))) >= GET_MAX_MOVE(ch) / 3) {
                                send_to_char(ch, "@RYou are starving to death!@n\r\n");
                                int64_t result;
                                result = ch_decCurSTPercent(ch, .33, 0);
                            } else if ((ch_getCurST(ch)) < GET_MAX_MOVE(ch) / 3) {
                                send_to_char(ch, "@RYou are starving to death!@n\r\n");
                                int64_t result;
                                result = ch_decCurSTPercent(ch, 1, 0);
                                int64_t result1;
                                result1 = ch_decCurHealthPercent(ch, .34, 0);
                            }
                        }
                            break;
                        case 1:
            ...
            if (GET_HIT(ch) <= 0 && GET_COND(ch, HUNGER) == 0) {
                ...
                int64_t result;
                result = ch_decCurSTPercent(ch, 1, 0);
                act("@W$n@W falls down dead before you...@n", false, ch, NULL, NULL, TO_ROOM);
                die(ch, NULL);
...
static void check_idling(struct char_data *ch) {
    ...
    if (++(ch->timer) > CONFIG_IDLE_VOID) {
        if (GET_WAS_IN(ch) == NOWHERE && IN_ROOM(ch) != NOWHERE) {
            ...
            act("$n disappears into the void.", true, ch, NULL, NULL, TO_ROOM);
    ...
    if (healrate > 0) {
        if (GET_LIMBCOND(ch, 1) > 0 && GET_LIMBCOND(ch, 1) < 50) {
            if (GET_LIMBCOND(ch, 1) + healrate >= 50) {
                act("You realize your right arm is no longer broken.", true, ch, NULL, NULL, TO_CHAR);
        ...
        if (GET_LIMBCOND(ch, 2) > 0 && GET_LIMBCOND(ch, 2) < 50) {
            if (GET_LIMBCOND(ch, 2) + healrate >= 50) {
                act("You realize your left arm is no longer broken.", true, ch, NULL, NULL, TO_CHAR);
        ...
        if (GET_LIMBCOND(ch, 3) > 0 && GET_LIMBCOND(ch, 3) < 50) {
            if (GET_LIMBCOND(ch, 3) + healrate >= 50) {
                act("You realize your right leg is no longer broken.", true, ch, NULL, NULL, TO_CHAR);
        ...
        if (GET_LIMBCOND(ch, 4) > 0 && GET_LIMBCOND(ch, 4) < 50) {
            if (GET_LIMBCOND(ch, 4) + healrate >= 50) {
                act("You realize your left leg is no longer broken.", true, ch, NULL, NULL, TO_CHAR);
...
void point_update() {
    ...
    for (i = character_list; i; i = next_char) {
        ...
        if (GET_POS(i) >= POS_STUNNED) {
            ...
            update_flags(i);
            if (!IS_NPC(i)) {
                if (!ch_isFullVitals(i)) {
                    change = true;
                }
            }

            if (PLR_FLAGGED(i, PLR_AURALIGHT)) {
                if (((ch_getMaxKI(i) * i->energy)) > (mana_gain(i) + ch_getPercentOfMaxKI(i, .05))) {
                    send_to_char(i, "You send more energy into your aura to keep the light active.\r\n");
                    int64_t amt = mana_gain(i) + ch_getPercentOfMaxKI(i, .05);
                    int64_t result;
                    result = ch_decCurKI(i, amt, 0);
                } else {
            ...
            if (GET_KAIOKEN(i) > 0) {
                ...
                if ((GET_SKILL(i, SKILL_KAIOKEN) < rand_number(1, x) || ((ch_getCurST(i))) <= GET_MAX_MOVE(i) / 10)) {
                    ch_remove_kaioken(i, 2);
                }
            }
            ...
            if (AFF_FLAGGED(i, AFF_BURNED)) {
                if (rand_number(1, 5) >= 4) {
                    ...
                    act("$n@w's burns are now healed.@n", true, i, NULL, NULL, TO_ROOM);
                    REMOVE_BIT_AR(AFF_FLAGS(i), AFF_BURNED);
                }
            }

            int64_t amt4 = hit_gain(i);
            int64_t result4;
            result4 = ch_incCurHealth(i, amt4, true);
            int64_t amt1 = move_gain(i);
            int64_t result1;
            result1 = ch_incCurST(i, amt1, true);
            int64_t amt2 = mana_gain(i);
            int64_t result2;
            result2 = ch_incCurKI(i, amt2, true);
            ...
            if (SECT(IN_ROOM(i)) == SECT_WATER_NOSWIM && !CARRIED_BY(i) && !IS_KANASSAN(i)) {
                if (((ch_getCurST(i))) >= ((ch_getCurCarriedWeight(i)))) {
                    act("@bYou swim in place.@n", true, i, NULL, NULL, TO_CHAR);
                    act("@C$n@b swims in place.@n", true, i, NULL, NULL, TO_ROOM);
                    int64_t amt = ch_getCurCarriedWeight(i);
                    int64_t result;
                    result = ch_decCurST(i, amt, 0);

                } else {
                    int64_t amt = ch_getCurCarriedWeight(i);
                    int64_t result;
                    result = ch_decCurST(i, amt, 0);
                    act("@RYou are drowning!@n", true, i, NULL, NULL, TO_CHAR);
                    act("@C$n@b gulps water as $e struggles to stay above the water line.@n", true, i, NULL, NULL,
                        TO_ROOM);
                    if (GET_HIT(i) - ((ch_getEffMaxPL(i)) / 3) <= 0) {
                        act("@rYou drown!@n", true, i, NULL, NULL, TO_CHAR);
                        act("@R$n@r drowns!@n", true, i, NULL, NULL, TO_ROOM);
                        die(i, NULL);
                    } else {
                        int64_t amt3 = (ch_getEffMaxPL(i)) / 3;
                        int64_t result3;
                        result3 = ch_decCurHealth(i, amt3, 0);
                    }
                }
            }
            if (!has_o2(i) && SUNKEN(IN_ROOM(i)) && !ROOM_FLAGGED(IN_ROOM(i), ROOM_SPACE)) {
                if ((((ch_getMaxKI(i) * i->energy)) - mana_gain(i)) > GET_MAX_MANA(i) / 200) {
                    send_to_char(i, "Your ki holds an atmosphere around you.\r\n");
                    int64_t amt = mana_gain(i) + ch_getPercentOfMaxKI(i, .005);
                    int64_t result;
                    result = ch_decCurKI(i, amt, 0);
                } else {
                    if ((GET_HIT(i) - hit_gain(i)) > (ch_getEffMaxPL(i)) * 0.05) {
                        send_to_char(i, "You struggle trying to hold your breath!\r\n");
                        int64_t amt = hit_gain(i) + ch_getPercentOfMaxHealth(i, .05);
                        int64_t result;
                        result = ch_decCurHealth(i, amt, 0);
                    } else if (GET_HIT(i) <= (ch_getEffMaxPL(i)) / 20) {
                        send_to_char(i, "You have drowned!\r\n");
                        act("@W$n@W drowns right in front of you.@n", false, i, NULL, NULL, TO_ROOM);
                        die(i, NULL);
                    }
                }
            }
            if (!has_o2(i) && ROOM_FLAGGED(IN_ROOM(i), ROOM_SPACE)) {
                if ((((ch_getMaxKI(i) * i->energy)) - mana_gain(i)) > GET_MAX_MANA(i) * 0.005) {
                    send_to_char(i, "Your ki holds an atmosphere around you.\r\n");
                    int64_t amt = mana_gain(i) + ch_getPercentOfMaxKI(i, .005);
                    int64_t result;
                    result = ch_decCurKI(i, amt, 0);
                    ...
                    if ((GET_HIT(i) - hit_gain(i)) > (ch_getEffMaxPL(i)) * 0.05) {
                        ...
                        int64_t amt = hit_gain(i) + ch_getPercentOfMaxHealth(i, .05);
                        int64_t result3;
                        result3 = ch_decCurHealth(i, amt, 0);
                    } else if (GET_HIT(i) <= (ch_getEffMaxPL(i)) / 20) {
                        send_to_char(i, "You have drowned!\r\n");
                        int64_t result;
                        result = ch_decCurHealthPercent(i, 1, 1);
                        act("@W$n@W drowns right in front of you.@n", false, i, NULL, NULL, TO_ROOM);
                        die(i, NULL);
                    }
                }
            ...
            if (!AFF_FLAGGED(i, AFF_FLYING) && ROOM_EFFECT(IN_ROOM(i)) == 6 && !MOB_FLAGGED(i, MOB_NOKILL) &&
                !IS_DEMON(i)) {
                act("@rYour legs are burned by the lava!@n", true, i, NULL, NULL, TO_CHAR);
                act("@R$n@r's legs are burned by the lava!@n", true, i, NULL, NULL, TO_ROOM);
                if (IS_NPC(i) && IS_HUMANOID(i) && rand_number(1, 2) == 2) {
                    do_fly(i, NULL, 0, 0);
                }
                int64_t result;
                result = ch_decCurHealthPercent(i, .05, 0);
                if (GET_HIT(i) < 0) {
                    act("@rYou have burned to death!@n", true, i, NULL, NULL, TO_CHAR);
                    act("@R$n@r has burned to death!@n", true, i, NULL, NULL, TO_ROOM);
                    die(i, NULL);
                }
            }
            if (change && !AFF_FLAGGED(i, AFF_POISON)) {
                if (PLR_FLAGGED(i, PLR_HEALT) && SITS(i) != NULL) {
                    ...
                    if (HCHARGE(SITS(i)) == 0) {
                        ...
                        act("You step out of the now empty healing tank.", true, i, NULL, NULL, TO_CHAR);
                        act("@C$n@w steps out of the now empty healing tank.@n", true, i, NULL, NULL, TO_ROOM);
            ...
            if (AFF_FLAGGED(i, AFF_POISON)) {
                ...
                if (GET_HIT(i) - (ch_getEffMaxPL(i)) * cost > 0) {
                    send_to_char(i, "You puke as the poison burns through your blood.\r\n");
                    act("$n shivers and then pukes.", true, i, NULL, NULL, TO_ROOM);
                    int64_t amt = ch_getEffMaxPL(i) * cost;
                    int64_t result;
                    result = ch_decCurHealth(i, amt, 0);
                } else {
                    send_to_char(i, "The poison claims your life!\r\n");
                    act("$n pukes up blood and falls down dead!", true, i, NULL, NULL, TO_ROOM);
                    if (i->poisonby) {
        ...
        if (((ch_getMaxKI(i) * i->energy)) >= GET_MAX_MANA(i) * 0.5 && GET_CHARGE(i) < GET_MAX_MANA(i) * 0.1 &&
    ...
    for (j = object_list; j; j = next_thing) {
        ...
        if (OBJ_FLAGGED(j, ITEM_NORENT) && j->worn_by == NULL && j->carried_by == NULL && obj_selling != j &&
            ...
            diff = time(NULL) - GET_LAST_LOAD(j);
            ...
            if (!strstr(j->name, "android") && !strstr(j->name, "Android") && !OBJ_FLAGGED(j, ITEM_BURIED)) {
                if (GET_OBJ_TIMER(j) == 5) {
                    if ((IN_ROOM(j) != NOWHERE) && (world[IN_ROOM(j)].people)) {
                        act("@DFlies start to gather around $p@D.@n", true, world[IN_ROOM(j)].people, j, NULL,
                            TO_CHAR);
                        act("@DFlies start to gather around $p@D.@n", true, world[IN_ROOM(j)].people, j, NULL,
                            TO_ROOM);
                    }
                ...
                if (GET_OBJ_TIMER(j) == 3) {
                    if ((IN_ROOM(j) != NOWHERE) && (world[IN_ROOM(j)].people)) {
                        act("@DA cloud of flies has formed over $p@D.@n", true, world[IN_ROOM(j)].people, j, NULL,
                            TO_CHAR);
                        act("@DA cloud of flies has formed over $p@D.@n", true, world[IN_ROOM(j)].people, j, NULL,
                            TO_ROOM);
                    }
            ...
            if (!GET_OBJ_TIMER(j)) {
                ...
                if (j->carried_by) {
                    if (!strstr(j->name, "android")) {
                        act("$p decays in your hands.", false, j->carried_by, j, NULL, TO_CHAR);
                        if ((IN_ROOM(j) != NOWHERE) && (world[IN_ROOM(j)].people)) {
                            act("A quivering horde of maggots consumes $p.",
                                true, world[IN_ROOM(j)].people, j, NULL, TO_ROOM);
                            act("A quivering horde of maggots consumes $p.",
                                true, world[IN_ROOM(j)].people, j, NULL, TO_CHAR);
                        }
                    } else {
                        act("$p decays in your hands.", false, j->carried_by, j, NULL, TO_CHAR);
                        if ((IN_ROOM(j) != NOWHERE) && (world[IN_ROOM(j)].people)) {
