...
void send_to_sense(int type, char *messg, struct char_data *ch) {
    ...
    for (i = descriptor_list; i; i = i->next) {
        ...
        if (IN_ROOM(ch) == IN_ROOM(tch)) {
            ...
            char *pmsg;
            int64_t chpower = GET_HIT(ch), tchpower = GET_HIT(tch);
            if (chpower > GET_HIT(tch) * 10) {
                pmsg = ", who is @Runbelievably stronger@Y than you";
            } else if (chpower > tchpower * 5) {
                pmsg = ", who is much @Rstronger@Y than you";
            } else if (chpower > tchpower * 2) {
                pmsg = ", who is more than twice as @Rstrong@Y as you";
            } else if (chpower > tchpower) {
                pmsg = ", who is somewhat @mstronger@Y than you";
            } else if (chpower * 10 < tchpower) {
                pmsg = ", who is @Munbelievably weaker@Y than you";
            } else if (chpower * 5 < tchpower) {
                pmsg = ", who is much @Mweaker@Y than you";
            } else if (chpower * 2 < tchpower) {
                pmsg = ", who is more than twice as @Mweak@Y as you";
            } else if (chpower < tchpower) {
                pmsg = ", who is somewhat @Wweaker@Y than you";
            } else {
                pmsg = ", who is close to @Cequal@Y with you";
            }
            snprintf(power, MAX_INPUT_LENGTH, "%s", pmsg);
