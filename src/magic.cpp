    ...
    switch (spellnum) {
        ...
        case SPELL_DISPEL_EVIL:
            ...
            if (IS_EVIL(ch)) {
                ...
                act("The gods protect $N.", false, ch, NULL, victim, TO_CHAR);
                return (0);
            }
            break;
...
void mag_summons(int level, struct char_data *ch, struct obj_data *obj, int spellnum, char *arg) {
    ...
    for (i = 0; i < num; i++) {
        ...
        act(mag_summon_msgs[msg], false, ch, NULL, mob, TO_ROOM);
        load_mtrigger(mob);
        add_follower(mob, ch);
...
void mag_points(int level, struct char_data *ch, struct char_data *victim,
    ...
    switch (spellnum) {
        ...
        case SPELL_SENSU:
            ...
            ch_restore(victim, true);
            break;

        case ART_WHOLENESS_OF_BODY:
            healing = (ch_getEffMaxPL(victim)) - GET_HIT(victim);
...
void mag_alter_objs(int level, struct char_data *ch, struct obj_data *obj,
    ...
    if (to_room != NULL)
        ...
        act(to_char, true, ch, obj, NULL, TO_ROOM);
