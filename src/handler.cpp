...
/* external functions */
struct obj_data *find_vehicle_by_vnum(obj_vnum vnum);
...
void obj_from_char(struct obj_data *object) {
    ...
    int64_t previous = (ch_getEffMaxPL(object->carried_by));
...
void equip_char(struct char_data *ch, struct obj_data *obj, int pos) {
    ...
    if (invalid_align(ch, obj) || invalid_class(ch, obj) || invalid_race(ch, obj)) {
        act("You stop wearing $p as something prevents you.", false, ch, obj, NULL, TO_CHAR);
        act("$n stops wearing $p as something prevents $m.", false, ch, obj, NULL, TO_ROOM);
...
void obj_to_room(struct obj_data *object, room_rnum room) {
    ...
    if (!object || room == NOWHERE || room > top_of_world)
        ...
        IN_ROOM(object) = room;
        object->carried_by = NULL;
        GET_LAST_LOAD(object) = time(NULL);
        ...
        if (EXIT(object, 5) &&
            ...
            act("$p @Bsinks to deeper waters.@n", true, NULL, object, NULL, TO_ROOM);
        ...
        if (EXIT(object, 5) && SECT(IN_ROOM(object)) == SECT_FLYING &&
            ...
            act("$p @Cfalls down.@n", true, NULL, object, NULL, TO_ROOM);
            int numb = GET_ROOM_VNUM(EXIT(object, 5)->to_room);
            obj_from_room(object);
            obj_to_room(object, real_room(numb));
            if (SECT(IN_ROOM(object)) != SECT_FLYING) {
                act("$p @Cfalls down and smacks the ground.@n", true, NULL, object, NULL, TO_ROOM);
            }
        }
        if (GET_OBJ_VAL(object, 0) != 0) {
            if (GET_OBJ_VNUM(object) == 16705 || GET_OBJ_VNUM(object) == 16706 || GET_OBJ_VNUM(object) == 16707) {
        ...
        if (GET_EQ(ch, i)) {
            if (GET_OBJ_TYPE(GET_EQ(ch, i)) == ITEM_LIGHT && GET_OBJ_VAL(GET_EQ(ch, i), VAL_LIGHT_HOURS) > 0 &&
                ...
                if (j == 1) {
                    ...
                    act("$n's light begins to flicker and fade.", false, ch, NULL, NULL, TO_ROOM);
                } else if (j == 0) {
                    send_to_char(ch, "Your light sputters out and dies.\r\n");
                    act("$n's light sputters out and dies.", false, ch, NULL, NULL, TO_ROOM);
                    world[IN_ROOM(ch)].light--;
                }
...
void extract_char_final(struct char_data *ch) {
    ...
    if (GRAPPLING(ch)) {
        act("@WYou stop grappling with @C$N@W!@n", true, ch, NULL, GRAPPLING(ch), TO_CHAR);
        act("@C$n@W stops grappling with @c$N@W!@n", true, ch, NULL, GRAPPLING(ch), TO_ROOM);
    ...
    if (GRAPPLED(ch)) {
        act("@WYou stop being grappled with by @C$N@W!@n", true, ch, NULL, GRAPPLED(ch), TO_CHAR);
        act("@C$n@W stops being grappled with by @c$N@W!@n", true, ch, NULL, GRAPPLED(ch), TO_ROOM);
    ...
    if (DRAGGED(ch)) {
        act("@WYou stop being dragged by @C$N@W!@n", true, ch, NULL, DRAGGED(ch), TO_CHAR);
        act("@C$n@W stops being dragged by @c$N@W!@n", true, ch, NULL, DRAGGED(ch), TO_ROOM);
...
void item_check(struct obj_data *object, struct char_data *ch) {
    ...
    if (IS_HUMANOID(ch) && !(mob_index[GET_MOB_RNUM(ch)].func == shop_keeper)) {
        ...
        switch (GET_OBJ_TYPE(object)) {
            ...
            case ITEM_ARMOR:
            case ITEM_WORN:
                where = find_eq_pos(ch, object, NULL);
                if (!GET_EQ(ch, where)) {
