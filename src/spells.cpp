    ...
    if (GET_OBJ_TYPE(obj) == ITEM_DRINKCON) {
        if ((GET_OBJ_VAL(obj, VAL_DRINKCON_LIQUID) != LIQ_WATER) && (GET_OBJ_VAL(obj, VAL_DRINKCON_HOWFULL) != 0)) {
            ...
            if (water > 0) {
                ...
                act("$p is filled.", false, ch, obj, NULL, TO_CHAR);
            }
        }
    }
}
...
ASPELL(spell_charm) {
    ...
    if (victim == ch)
        ...
        act("Isn't $n just such a nice fellow?", false, ch, NULL, victim, TO_VICT);
        if (IS_NPC(victim))
...
ASPELL(spell_detect_poison) {
    if (victim) {
        if (victim == ch) {
            ...
            if (AFF_FLAGGED(victim, AFF_POISON))
                act("You sense that $E is poisoned.", false, ch, NULL, victim, TO_CHAR);
            else
                act("You sense that $E is healthy.", false, ch, NULL, victim, TO_CHAR);
        }
    }
        ...
        switch (GET_OBJ_TYPE(obj)) {
            ...
            case ITEM_FOOD:
                if (GET_OBJ_VAL(obj, VAL_FOOD_POISON))
                    act("You sense that $p has been contaminated.", false, ch, obj, NULL, TO_CHAR);
                else
                    act("You sense that $p is safe for consumption.", false, ch, obj, NULL,
                        TO_CHAR);
                break;
            default:
...
ASPELL(art_abundant_step) {
    ...
    act("$n is suddenly absent.", true, ch, NULL, NULL, TO_ROOM);

    char_from_room(ch);
    char_to_room(ch, r);

    act("$n is suddenly present.", true, ch, NULL, NULL, TO_ROOM);
