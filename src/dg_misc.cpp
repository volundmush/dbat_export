...
void send_char_pos(struct char_data *ch, int dam) {
    switch (GET_POS(ch)) {
        case POS_MORTALLYW:
            act("$n is mortally wounded, and will die soon, if not aided.", true, ch, NULL, NULL, TO_ROOM);
            send_to_char(ch, "You are mortally wounded, and will die soon, if not aided.\r\n");
            break;
        case POS_INCAP:
            act("$n is incapacitated and will slowly die, if not aided.", true, ch, NULL, NULL, TO_ROOM);
            send_to_char(ch, "You are incapacitated and will slowly die, if not aided.\r\n");
            break;
        case POS_STUNNED:
            act("$n is stunned, but will probably regain consciousness again.", true, ch, NULL, NULL, TO_ROOM);
        ...
        default:                        /* >= POSITION SLEEPING */
            if (dam > ((ch_getEffMaxPL(ch)) >> 2))
                act("That really did HURT!", false, ch, NULL, NULL, TO_CHAR);
            if (GET_HIT(ch) < ((ch_getEffMaxPL(ch)) >> 2))
...
void script_damage(struct char_data *vict, int dam) {
    ...
    int64_t result;
    result = ch_decCurHealth(vict, dam, 0);
