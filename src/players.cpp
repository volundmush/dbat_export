...
void build_player_index() {
    ...
    snprintf(index_name, 40, "%s%s", LIB_PLRFILES, INDEX_FILE);
...
void save_player_index() {
    ...
    snprintf(index_name, 50, "%s%s", LIB_PLRFILES, INDEX_FILE);
    ...
    if ((id = get_ptable_by_name(name)) < 0)
        ...
        while (get_line(fl, line)) {
            ...
            switch (*tag) {
                ...
                default:
                    snprintf(buf, 128, "SYSERR: Unknown tag %s in pfile %s", tag, name);
            }
        }
    }
...
void load_char_pets(struct char_data *ch) {
    ...
    for (foll = ch->followers; foll; foll = foll->next) {
        ...
        act("You are joined by $N.", false, ch, NULL, foll->follower, TO_CHAR);
    }
}
...
void save_char(struct char_data *ch) {
    ...
    if (ch->desc) {
        ...
        if (STATE(ch->desc) == CON_PLAYING) {
            ch->time.played += time(NULL) - ch->time.logon;
            ch->time.logon = time(NULL);
        }
    }
    ...
    if ((ch_getCurKI(ch))	   != PFDEF_MANA || GET_MAX_MANA(ch) != PFDEF_MAXMANA)
      fprintf(fl, "Mana: %" I64T "/%" I64T "\n", (ch_getCurKI(ch)), GET_MAX_MANA(ch));
    if ((ch_getCurST(ch))	   != PFDEF_MOVE || GET_MAX_MOVE(ch) != PFDEF_MAXMOVE)
    ...
    */
    ...
    if (((ch_getBasePL(ch))) != PFDEF_BASEPL) {
        fprintf(fl, "Bpl : %" I64T "\n", ((ch_getBasePL(ch))));
    }
    if (((ch->baseki)) != PFDEF_BASEKI) {
        fprintf(fl, "Bki : %" I64T "\n", ((ch->baseki)));
    }
    if (((ch_getBaseST(ch))) != PFDEF_BASEST) {
        fprintf(fl, "Bst : %" I64T "\n", ((ch_getBaseST(ch))));
    }
