...
const struct command_info cmd_info[] = {
        {"RESERVED",      "",        0,                 NULL,            0,  ADMLVL_NONE,    0},     /* this must be first -- for specprocs */
        ...
        {"\n",            "zzzzzzz", 0,                 NULL,            0,  ADMLVL_NONE,    0}};    /* this must be last */
    ...
    for (x = start; x < finish; x++) { /* Save the new spots */
        if (placed == false) { /* They Haven't Placed */
            if (strcasecmp(topname[x], GET_NAME(ch))) { /* Name doesn't match */
                if ((ch_getEffMaxPL(ch)) > toppoint[x]) {
                    free(topname[x]);
                    toppoint[x] = (ch_getEffMaxPL(ch));
...
static void display_races_sub(struct descriptor_data *d) {
    int i = 0;
    for (int x = 0; x < NUM_RACES; x++) {
        if(race_isPC(x)) continue;
        send_to_char(d->character, "@C%-15s@D[@R%i RPP@D]@n%s", race_getName(x), race_getRPPCost(x),
                     !(++i % 2) ? "\r\n" : "   ");
    }
}
...
static void display_classes_sub(struct descriptor_data *d) {
    int i = 0, x = 0;
    for (x = 0; x < NUM_CLASSES; x++)
        send_to_char(d->character, "@C%s@n%s", sen_getName(x), !(++i % 2) ? "\r\n" : "	");
}
  ...
  */
    ...
    switch (mode) {
        case RECON:
            ...
            d->character->time.logon = time(NULL);
            act("$n has reconnected.", true, d->character, NULL, NULL, TO_ROOM);
...
void payout(int num) {
    ...
    struct descriptor_data *k;
    if (LASTPAYOUT == 0) {
        LASTPAYOUT = time(NULL) + 86400;
        LASTPAYTYPE = num;
    } else if (num > LASTPAYTYPE) {
        LASTPAYOUT = time(NULL) + 86400;
        LASTPAYTYPE = num;
    } else if (LASTPAYOUT <= time(NULL)) {
        LASTPAYOUT = time(NULL) + 86400;
        LASTPAYTYPE = num;
    }
    for (k = descriptor_list; k; k = k->next) {
    ...
    switch (STATE(d)) {
	  ...
	  */
        ...
        case CON_RACE_HELP:
            switch (strlen(arg)) {
                ...
                case 1:
                    ...
                    break;
                default:
                    chosen_race = race_findName(arg);
                    if (!race_isPC(chosen_race)) {
                        write_to_output(d, "\r\nThat's not a race.\r\nRace: ");
                        return;
                    }
                    ...
                    switch (STATE(d)) {
                        case CON_QRACE:
                            if (race_getRPPCost(chosen_race)) {
                                write_to_output(d, "\r\n%i RPP will be paid upon your first level up.\r\n",
                                                race_getRPPCost(chosen_race));
                            }
                            d->character->race = chosen_race;
                            break;
                        case CON_RACE_HELP:
                            show_help(d, race_getName(chosen_race));
                            chosen_sensei = CLASS_UNDEFINED;
                            return;
                    }
            }
        ...
        case CON_QX:
            switch (*arg) {
                case '1':
                    d->character->time.birth = time(NULL) - (8 * SECS_PER_MUD_YEAR);
                    break;
                case '2':
                    d->character->time.birth = time(NULL) - (10 * SECS_PER_MUD_YEAR);
                    break;
                case '3':
                    d->character->time.birth = time(NULL) - (12 * SECS_PER_MUD_YEAR);
                    break;
                case '4':
                    d->character->time.birth = time(NULL) - (14 * SECS_PER_MUD_YEAR);
                    break;
                case '5':
                    d->character->time.birth = time(NULL) - (16 * SECS_PER_MUD_YEAR);
                    break;
                case '6':
                    d->character->time.birth = time(NULL) - (18 * SECS_PER_MUD_YEAR);
                    break;
                case '7':
                    d->character->time.birth = time(NULL) - (20 * SECS_PER_MUD_YEAR);
                    break;
                case '8':
                    d->character->time.birth = time(NULL) - (22 * SECS_PER_MUD_YEAR);
                    break;
                case '9':
                    d->character->time.birth = time(NULL) - (24 * SECS_PER_MUD_YEAR);
                    break;
                case 'A':
                case 'a':
                    d->character->time.birth = time(NULL) - (26 * SECS_PER_MUD_YEAR);
                    break;
                case 'B':
                case 'b':
                    d->character->time.birth = time(NULL) - (28 * SECS_PER_MUD_YEAR);
                    break;
                case 'C':
                case 'c':
                    d->character->time.birth = time(NULL) - (30 * SECS_PER_MUD_YEAR);
                    break;
                case 'D':
                case 'd':
                    d->character->time.birth = time(NULL) - (40 * SECS_PER_MUD_YEAR);
                    break;
                case 'E':
                case 'e':
                    d->character->time.birth = time(NULL) - (50 * SECS_PER_MUD_YEAR);
                    break;
                case 'F':
                case 'f':
                    d->character->time.birth = time(NULL) - (60 * SECS_PER_MUD_YEAR);
                    break;
                case 'G':
                case 'g':
                    d->character->time.birth = time(NULL) - (65 * SECS_PER_MUD_YEAR);
                    break;
                case 'H':
                case 'h':
                    if (IS_KAI(d->character) || IS_DEMON(d->character) || IS_MAJIN(d->character)) {
                        d->character->time.birth = time(NULL) - (500 * SECS_PER_MUD_YEAR);
                    } else if (IS_NAMEK(d->character)) {
                        d->character->time.birth = time(NULL) - (250 * SECS_PER_MUD_YEAR);
                    } else {
                        d->character->time.birth = time(NULL) - (70 * SECS_PER_MUD_YEAR);
                    }
                    break;
                case 'I':
                case 'i':
                    if (IS_KAI(d->character) || IS_DEMON(d->character) || IS_MAJIN(d->character)) {
                        d->character->time.birth = time(NULL) - (800 * SECS_PER_MUD_YEAR);
                    } else if (IS_NAMEK(d->character)) {
                        d->character->time.birth = time(NULL) - (400 * SECS_PER_MUD_YEAR);
                    } else {
                        d->character->time.birth = time(NULL) - (75 * SECS_PER_MUD_YEAR);
                    }
                    break;
                default:
                    write_to_output(d, "That is not an acceptable option.\r\n");
        ...
        case CON_ALIGN:
            ...
            if (moveon == true) {
                ...
                int64_t result;
                result = ch_loseBasePLPercent(d->character, d->character->basepl * .2, false);
        ...
        case CON_CLASS_HELP:
            switch (strlen(arg)) {
                case 1:
                    ...
                    break;
                default:
                    chosen_sensei = sen_findName(arg);
                    if (chosen_sensei == CLASS_UNDEFINED) {
                        write_to_output(d, "\r\nThat's not a sensei.\r\nSensei: ");
                        return;
                    }
                    ...
                    switch (STATE(d)) {
                        case CON_CLASS_HELP:
                            show_help(d, sen_getName(chosen_sensei));
                            chosen_sensei = CLASS_UNDEFINED;
                            return;
                        case CON_QCLASS:
                            if (chosen_sensei == CLASS_KABITO && !IS_KAI(d->character) &&
        ...
        case CON_QROLLSTATS:
            ...
            mudlog(CMP, ADMLVL_GOD, true, "New player: %s [%s %s]",
                   ...
                   sen_getName(d->character->chclass));
            break;
            ...
            switch (*arg) {
                ...
                case '1':
                    if (lockRead(GET_NAME(d->character)) && d->level <= 0) {
                        ...
                        act("$n has entered the game.", true, d->character, NULL, NULL, TO_ROOM);
