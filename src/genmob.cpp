...
int write_mobile_espec(mob_vnum mvnum, struct char_data *mob, FILE *fd) {
    ...
    if (get_size(mob) != race_getSize(mob->race))
        fprintf(fd, "Size: %d\n", get_size(mob));
    if (GET_ATTACK(mob) != 0)
    ...
    if (mob_proto + real_mobile(mvnum) != mob) { /* Not saving a prototype */
        fprintf(fd,
                "Hit: %" I64T "\nMaxHit: %" I64T "\nMana: %" I64T "\nMaxMana: %" I64T "\nMoves: %" I64T "\nMaxMoves: %" I64T "\n",
                GET_HIT(mob), (ch_getEffMaxPL(mob)), ((ch_getMaxKI(mob) * mob->energy)), GET_MAX_MANA(mob),
                ((ch_getCurST(mob))), GET_MAX_MOVE(mob));
...
int write_mobile_record(mob_vnum mvnum, struct char_data *mob, FILE *fd) {
    ...
    fprintf(fd, "%s %s %s %s %s %s %s %s %d E\n"
            ...
            GET_HIT(mob), ((ch_getMaxKI(mob) * mob->energy)), ((ch_getCurST(mob))), GET_NDD(mob), GET_SDD(mob),
            GET_DAMAGE_MOD(mob)
    );
    fprintf(fd, "%d 0 %d %d\n"
                "%d %d %d\n",
...
void check_mobile_string(mob_vnum i, char **string, const char *dscr) {
    if (*string == NULL || **string == '\0') {
        mudlog(BRF, ADMLVL_GOD, true, "GenOLC: Mob #%d has an invalid %s.", i, dscr);
        if (*string)
            free(*string);
