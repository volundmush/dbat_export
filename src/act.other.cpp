...
void bring_to_cap(struct char_data *ch) {
    ...
    bool p_trans = (race_canTransform(ch->race) && !race_canRevert(ch->race));
    int64_t cap = ch_calc_soft_cap(ch);

    switch (ch_getSoftType(ch)) {
        case 0: {
            if (ch_getBasePL(ch) < cap) {
                int64_t amt = cap - ch_getBasePL(ch) - 1;
                int64_t result;
                result = ch_gainBasePL(ch, amt, p_trans);
            }
        }
            if (ch->baseki < cap) {
                int64_t amt = cap - ch->baseki - 1;
                int64_t result;
                result = ch_gainBaseKI(ch, amt, p_trans);
            }
            if (ch_getBaseST(ch) < cap) {
                int64_t amt = cap - ch_getBaseST(ch) - 1;
                int64_t result;
                result = ch_gainBaseST(ch, amt, p_trans);
            }
    }
}
    ...
    if (selection > 2) {
        ...
        if (selection == 11) {     /* Simple Soft-cap Reward*/
            ...
            if (GET_RP(ch) < pay) {
                ...
                if (ch_is_soft_cap(ch, 0, 1.0) && ch_is_soft_cap(ch, 1, 1.0) && ch_is_soft_cap(ch, 2, 1.0)) {
...
ACMD(do_commune) {
    ...
    if (((ch_getCurST(ch))) >= GET_MAX_MOVE(ch)) {
    ...
    if (prob < perc) {
        ...
        act("@W$n closes $s eyes for a moment. Then $e reopens them and frowns.@n", true, ch, NULL, NULL,
            TO_ROOM);
        ...
        act("@cYou close your eyes and commune with the Eldritch Star spiritually. You feel your stamina replenish some.@n",
            true, ch, NULL, NULL, TO_CHAR);
        act("@W$n closes $s eyes for a moment. Then $e reopens them and smiles.@n", true, ch, NULL, NULL,
            TO_ROOM);
        WAIT_STATE(ch, PULSE_2SEC);
        return;
    }
...
ACMD(do_grapple) {
  ...
  return;*/
    ...
    if (GRAPPLING(ch) != NULL) {
        act("@RYou stop grappling with @r$N@R!@n", true, ch, NULL, GRAPPLING(ch), TO_CHAR);
        act("@r$n@R stops grappling with @rYOU!!@n", true, ch, NULL, GRAPPLING(ch), TO_VICT);
        act("@r$n@R stops grappling with @r$N@R!@n", true, ch, NULL, GRAPPLING(ch), TO_NOTVICT);
    ...
    if (!strcasecmp("hold", arg2) || !strcasecmp("choke", arg2) || !strcasecmp("grab", arg2) ||
        ...
        if (((!IS_NPC(vict) && IS_ICER(vict) && rand_number(1, 30) >= 28) || AFF_FLAGGED(vict, AFF_ZANZOKEN)) &&
            ...
            if (!AFF_FLAGGED(ch, AFF_ZANZOKEN) || (AFF_FLAGGED(ch, AFF_ZANZOKEN) && GET_SPEEDI(ch) + rand_number(1, 5) <
                                                                                    ...
                                                                                    rand_number(1, 5))) {
                reveal_hiding(ch, 0);
                act("@C$N@c disappears, avoiding your grapple attempt before reappearing!@n", false, ch, NULL, vict,
                    TO_CHAR);
                act("@cYou disappear, avoiding @C$n's@c grapple attempt before reappearing!@n", false, ch, NULL,
                    vict, TO_VICT);
                act("@C$N@c disappears, avoiding @C$n's@c grapple attempt before reappearing!@n", false, ch, NULL,
                    vict, TO_NOTVICT);
        ...
        if ((GET_HIT(ch) * 0.02) * GET_STR(ch) < (GET_HIT(vict) * 0.01) * GET_STR(vict)) {
            reveal_hiding(ch, 0);
            act("@RYou try to grapple with @r$N@R, but $E manages to overpower you!@n", true, ch, NULL, vict,
                TO_CHAR);
            act("@r$n@R tries to grapple with YOU, but you manage to overpower $m!@n", true, ch, NULL, vict,
                TO_VICT);
            act("@r$n@R tries to grapple with @r$N@R, but $E manages to overpower @r$n@R!@n", true, ch, NULL, vict,
                TO_NOTVICT);
            int64_t result;
            result = ch_decCurST(ch, cost, 0);
            ...
            reveal_hiding(ch, 0);
            act("@RYou try to grapple with @r$N@R, but $E manages to avoid it!@n", true, ch, NULL, vict, TO_CHAR);
            act("@r$n@R tries to grapple with YOU, but you manage to avoid it!@n", true, ch, NULL, vict, TO_VICT);
            act("@r$n@R tries to grapple with @r$N@R, but $E manages to avoid it!@n", true, ch, NULL, vict,
                TO_NOTVICT);
            int64_t result;
            result = ch_decCurST(ch, cost, 0);
            improve_skill(ch, SKILL_GRAPPLE, 1);
            ...
            reveal_hiding(ch, 0);
            act("@RYou rush at @r$N@R and manage to get $M in a hold from behind!@n", true, ch, NULL, vict, TO_CHAR);
            act("@r$n@R rushes at YOU and manages to get you in a hold from behind!@n", true, ch, NULL, vict,
                TO_VICT);
            act("@r$n@R rushes at @r$N@R and manages to get $M in a hold from behind!@n", true, ch, NULL, vict,
                TO_NOTVICT);
            ...
            reveal_hiding(ch, 0);
            act("@RYou rush at @r$N@R and manage to grab $S throat with both hands!@n", true, ch, NULL, vict,
                TO_CHAR);
            act("@r$n@R rushes at YOU and manages to grab your throat with both hands!@n", true, ch, NULL, vict,
                TO_VICT);
            act("@r$n@R rushes at @r$N@R and manages to grab $S throat with both hands!@n", true, ch, NULL, vict,
                TO_NOTVICT);
            ...
            reveal_hiding(ch, 0);
            act("@RYou rush at @r$N@R and manage to lock your arm onto $S!@n", true, ch, NULL, vict, TO_CHAR);
            act("@r$n@R rushes at YOU and manages to lock $s arm onto your's!@n", true, ch, NULL, vict, TO_VICT);
            act("@r$n@R rushes at @r$N@R and manages to lock $s arm onto @r$N's@R!@n", true, ch, NULL, vict,
                TO_NOTVICT);
...
ACMD(do_trip) {
    ...
    int cost = (ch_getEffMaxPL(ch)) / 200;

    if (cost > ((ch_getCurST(ch)))) {
    ...
    if (vict != NULL) {
        ...
        if (((!IS_NPC(vict) && IS_ICER(vict) && rand_number(1, 30) >= 28) || AFF_FLAGGED(vict, AFF_ZANZOKEN)) &&
            ...
            if (!AFF_FLAGGED(ch, AFF_ZANZOKEN) || (AFF_FLAGGED(ch, AFF_ZANZOKEN) && GET_SPEEDI(ch) + rand_number(1, 5) <
                                                                                    ...
                                                                                    rand_number(1, 5))) {
                reveal_hiding(ch, 0);
                act("@C$N@c disappears, avoiding your trip before reappearing!@n", false, ch, NULL, vict, TO_CHAR);
                act("@cYou disappear, avoiding @C$n's@c trip before reappearing!@n", false, ch, NULL, vict, TO_VICT);
                act("@C$N@c disappears, avoiding @C$n's@c trip before reappearing!@n", false, ch, NULL, vict,
                    TO_NOTVICT);
                if (AFF_FLAGGED(ch, AFF_ZANZOKEN)) {
                    REMOVE_BIT_AR(AFF_FLAGS(ch), AFF_ZANZOKEN);
                }
                ...
                reveal_hiding(ch, 0);
                act("@C$N@c disappears, trying to avoid your trip but your zanzoken is faster!@n", false, ch, NULL,
                    vict, TO_CHAR);
                act("@cYou zanzoken to avoid the trip but @C$n's@c zanzoken is faster!@n", false, ch, NULL, vict,
                    TO_VICT);
        ...
        if (perc < prob) { /* Fail! */
            reveal_hiding(ch, 0);
            act("@mYou move to trip $N@m, but you screw up and $E keeps $S footing!@n", true, ch, NULL, vict,
                TO_CHAR);
            act("@m$n@m moves to trip YOU, but $e screws up and you manage to keep your footing!@n", true, ch, NULL,
                vict, TO_VICT);
            act("@m$n@m moves to trip $N@m, but $e screws up and $N@m manages to keep $S footing!@n", true, ch, NULL,
                vict, TO_NOTVICT);
            ...
            reveal_hiding(ch, 0);
            act("@mYou move to trip $N@m, and manage to knock $M off $S feet!@n", true, ch, NULL, vict, TO_CHAR);
            act("@m$n@m moves to trip YOU, and manages to knock you off your feet!@n", true, ch, NULL, vict,
                TO_VICT);
            act("@m$n@m moves to trip $N@m, and manages to knock $N@m off $S feet!@n", true, ch, NULL, vict,
                TO_NOTVICT);
            improve_skill(ch, SKILL_TRIP, 0);
            int64_t result;
            result = ch_decCurST(ch, cost, 0);
...
ACMD(do_train) {
    ...
    weight = ch_getCurCarriedWeight(ch);
...
*/
    ...
    if (GET_ROOM_VNUM(IN_ROOM(ch)) == sen_location(ch->chclass)) {
        ...
        if (GET_LEVEL(ch) >= 100)
            ...
            total *= 150;
        sensei = ch->chclass;
        send_to_char(ch, "@G%s begins to instruct you in training technique.@n\r\n", sen_getName(ch->chclass));
    }

    if (total > (ch_getEffMaxPL(ch)) * 2) {
        bonus = 5;
    } else if (total > (ch_getEffMaxPL(ch))) {
        bonus = 4;
    } else if (total > ((ch_getEffMaxPL(ch)) / 2)) {
        bonus = 3;
    } else if (total > ((ch_getEffMaxPL(ch)) / 4)) {
        bonus = 2;
    } else if (total > ((ch_getEffMaxPL(ch)) / 8)) {
        bonus = 1;
    }
    ...
    switch (stat_id) {
        case 1:
            switch (msg_case) {
                case 1:
                    act("@WYou throw a flurry of punches into the air at an invisible opponent.@n", true, ch, NULL,
                        NULL, TO_CHAR);
                    act("@W$n throws a flurry of punches into the air.@n", true, ch, NULL, NULL, TO_ROOM);
                    break;
                case 2:
                    act("@WYou leap into the air and throw a wild kick at an invisible opponent@n", true, ch, NULL,
                        NULL, TO_CHAR);
                    act("@W$n leaps into the air and throws a wild kick at nothing.@n", true, ch, NULL, NULL,
                        TO_ROOM);
                    break;
                case 3:
        ...
        case 2:
            switch (msg_case) {
                case 1:
                    act("@WYou dash quickly around the surrounding area as fast as you can!@n", true, ch, NULL,
                        NULL, TO_CHAR);
                    act("@W$n dashes quickly around the surrounding area as fast as $e can!@n", true, ch, NULL,
                        NULL, TO_ROOM);
                    break;
                case 2:
                    act("@WYou dodge to the side as fast as you can!@n", true, ch, NULL, NULL, TO_CHAR);
                    act("@W$n dodges to the side as fast as $e can!@n", true, ch, NULL, NULL, TO_ROOM);
                    break;
                case 3:
                    act("@WYou dash backwards as fast as you can!@n", true, ch, NULL, NULL, TO_CHAR);
                    act("@W$n dashes backwards as fast as $e can!@n", true, ch, NULL, NULL, TO_ROOM);
                    break;
            }
            break;
        case 3:
            switch (msg_case) {
                case 1:
                    ...
                    break;
                case 2:
                    act("@WYou leap into the air and then slam into the ground with your fists!@n", true, ch, NULL,
                        NULL, TO_CHAR);
                    act("@W$n leaps into the air and then slams into the ground with $s fists!?@n", true, ch, NULL,
                        NULL, TO_ROOM);
                    break;
                case 3:
                    act("@WYou leap into the air and then slam into the ground with your body!@n", true, ch, NULL,
                        NULL, TO_CHAR);
                    act("@W$n leaps into the air and then slams into the ground with $s body!?@n", true, ch, NULL,
                        NULL, TO_ROOM);
                    break;
            }
            break;
        case 4:
            switch (msg_case) {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    act("@WYou flip to the side off one hand and then land on your feet.@n", true, ch, NULL, NULL,
                        TO_CHAR);
                    act("@W$n flips to the side off one hand and then lands on $s feet.@n", true, ch, NULL, NULL,
                        TO_ROOM);
                    break;
            }
            break;
            ...
            switch (msg_case) {
                ...
                case 2:
                    act("@WYou look around and contemplate battle tactics for an imaginary scenario.@n", true, ch,
                        NULL, NULL, TO_CHAR);
                    act("@W$n looks around and appears to be imagining things that aren't there.@n", true, ch, NULL,
                        NULL, TO_ROOM);
                    break;
                case 3:
                    act("@WYou invent a battle plan for a battle that doesn't exist!@n", true, ch, NULL, NULL,
                        TO_CHAR);
                    act("@W$n seems to have thought of something.@n", true, ch, NULL, NULL, TO_ROOM);
                    break;
            }
            break;
    }
...
ACMD(do_rip) {
    ...
    if (ch != vict && GET_POS(ch) > POS_SLEEPING) {
        if (((ch_getCurST(ch))) < GET_MAX_MOVE(ch) / 20) {
            send_to_char(ch, "You are too tired to manage to grab their tail!\r\n");
            return;
        } else if (GET_SPEEDI(ch) > GET_SPEEDI(vict)) {
            int64_t amt = ch_getMaxST(ch) / 20;
            int64_t result;
            result = ch_decCurST(ch, amt, 0);
            if (GET_HIT(ch) > GET_HIT(vict) * 2) {
                reveal_hiding(ch, 0);
                act("@rYou rush at @R$N@r and grab $S tail! With a powerful tug you pull it off!@n", true, ch, NULL,
                    vict, TO_CHAR);
            ...
            result = ch_decCurST(ch, amt, 0);
            reveal_hiding(ch, 0);
            act("@rYou rush at @R$N@r and try to grab $S tail, but fail!@n", true, ch, NULL, vict, TO_CHAR);
            act("@R$n@r rushes at YOU and tries to grab your tail, but fails!@n", true, ch, NULL, vict, TO_VICT);
            act("@R$n@R rushes at @R$N@r and tries to grab $S tail, but fails!@n", true, ch, NULL, vict, TO_NOTVICT);
            return;
        }
    } else if (ch == vict) {
        reveal_hiding(ch, 0);
        act("@rYou grab your own tail and yank it off!@n", true, ch, NULL, NULL, TO_CHAR);
        act("@R$n@r grabs $s own tail and yanks it off!@n", true, ch, NULL, NULL, TO_ROOM);
        ch_loseTail(vict);
    } else {
        if (((ch_getCurST(ch))) < GET_MAX_MOVE(ch) / 20) {
            send_to_char(ch, "You are too tired to manage to grab their tail!\r\n");
        ...
        int64_t amt = ch_getMaxKI(ch) / 20;
        int64_t result;
        result = ch_decCurST(ch, amt, 0);
        reveal_hiding(ch, 0);
        act("@rYou reach and grab @R$N's@r tail! With a powerful tug you pull it off!@n", true, ch, NULL, vict,
            TO_CHAR);
        act("@RYou feel your tail pulled off!@n", true, ch, NULL, vict, TO_VICT);
        act("@R$n@R reaches and grabs @R$N's@r tail! With a powerful tug $e pulls it off!@n", true, ch, NULL, vict,
            TO_NOTVICT);
        ch_loseTail(vict);
        return;
    }
...
ACMD(do_infuse) {
    ...
    if (AFF_FLAGGED(ch, AFF_INFUSE)) {
        act("You stop infusing ki into your attacks.", true, ch, NULL, NULL, TO_CHAR);
        act("$n stops infusing ki into $s attacks.", true, ch, NULL, NULL, TO_ROOM);
    ...
    reveal_hiding(ch, 0);
    act("You start infusing ki into your attacks.", true, ch, NULL, NULL, TO_CHAR);
    act("$n starts infusing ki into $s attacks.", true, ch, NULL, NULL, TO_ROOM);
    SET_BIT_AR(AFF_FLAGS(ch), AFF_INFUSE);
    int64_t amt = ch_getMaxKI(ch) / 100;
    int64_t result;
...
ACMD(do_paralyze) {
    ...
    if (((ch_getMaxKI(ch) * ch->energy)) < GET_HIT(vict) / 10 + (GET_MAX_MANA(ch) / 20)) {
    ...
    if (GET_BONUS(vict, BONUS_INSOMNIAC)) {
        int64_t amt = GET_HIT(vict) / 6 + (GET_MAX_MANA(ch) / 20);
        int64_t result;
        result = ch_decCurKI(ch, amt, 0);
...
ACMD(do_taisha) {
    ...
    if (((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) / 3) {
...
ACMD(do_kura) {
    ...
    if (((ch_getMaxKI(ch) * ch->energy)) >= GET_MAX_MANA(ch)) {
    ...
    if (skill <= axion_dice(0)) {
        ...
        result = ch_decCurST(ch, cost, 0);
        int64_t result1;
        result1 = ch_incCurKI(ch, bonus, true);
        reveal_hiding(ch, 0);
...
ACMD(do_candy) {
    ...
    int64_t ch_max = ch_getMaxPLTrans(ch);
    int64_t vict_max = ch_getMaxPLTrans(vict);
    ...
    if (((ch_getMaxKI(ch) * ch->energy)) < ch_max / 15) {
        send_to_char(ch, "You do not have enough ki.\r\n");
        return;
    }
    if (rand_number(1, 6) == 6) {
        int64_t amt = ch_getMaxKI(ch) / 15;
        int64_t result;
        result = ch_decCurKI(ch, amt, 0);
        reveal_hiding(ch, 0);
        act("@cYou aim your forelock at @R$N@c and fire a beam of energy but it is dodged!@n", true, ch, NULL, vict,
            TO_CHAR);
        act("@C$n@c aims $s forelock at @R$N@c and fires a beam of energy but the beam is dodged!@n", true, ch, NULL,
            vict, TO_NOTVICT);
        if (!FIGHTING(ch)) {
            set_fighting(ch, vict);
        }
...
ACMD(do_future) {
    ...
    if (((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) / 40) {
        send_to_char(ch, "You do not have enough ki.\r\n");
        return;
    }
    ...
    if (vict != ch) {
        ...
        GET_PRACTICES(ch, GET_CLASS(ch)) -= 100;
        reveal_hiding(ch, 0);
        act("@CYou focus your energy into your mind and awaken your latent Future Sight powers!@n", true, ch, NULL,
            vict, TO_CHAR);
        act("@C$n focuses $s energy while closing $s eyes for a moment.@n", true, ch, NULL, vict, TO_VICT);
        act("@C$n focuses $s energy while closing $s eyes for a moment.@n", true, ch, NULL, vict, TO_NOTVICT);
...
ACMD(do_drag) {
    ...
    if (DRAGGING(ch)) {
        ...
        DRAGGING(ch) = NULL;
        DRAGGED(vict) = NULL;
        act("@wYou stop dragging @C$N@W.@n", true, ch, NULL, vict, TO_CHAR);
        act("@C$n@W stops dragging @c$N@W.@n", true, ch, NULL, vict, TO_ROOM);
        return;
    }
    ...
    if (GET_POS(vict) != POS_SLEEPING) {
        reveal_hiding(ch, 0);
        act("@wYou try to grab and pull @C$N@W with you, but $E resists!@n", true, ch, NULL, vict, TO_CHAR);
        act("@C$n@W tries to grab and pull you! However you resist!@n", true, ch, NULL, vict, TO_VICT);
        act("@C$n@W tries to grab and pull @c$N@W but $E resists!@n", true, ch, NULL, vict, TO_NOTVICT);
        ...
        reveal_hiding(ch, 0);
        act("@wYou try to grab and pull @C$N@W with you, but $E is too heavy!@n", true, ch, NULL, vict, TO_CHAR);
        act("@C$n@W tries to grab and pull @c$N@W but $E is too heavy!@n", true, ch, NULL, vict, TO_ROOM);
        return;
    } else {
        reveal_hiding(ch, 0);
        act("@wYou grab and start dragging @C$N@W.@n", true, ch, NULL, vict, TO_CHAR);
        act("@C$n@W grabs and starts dragging @c$N@W.@n", true, ch, NULL, vict, TO_NOTVICT);
        DRAGGING(ch) = vict;
        DRAGGED(vict) = ch;
...
ACMD(do_stop) {
    ...
    if (!FIGHTING(ch)) {
        ...
        act("@CYou move out of your fighting posture.@n", true, ch, NULL, NULL, TO_CHAR);
        act("@c$n@C moves out of $s fighting posture.@n", true, ch, NULL, NULL, TO_ROOM);
        stop_fighting(ch);
        WAIT_STATE(ch, PULSE_2SEC);
        return;
    }
...
ACMD(do_suppress) {
    ...
    if (!strcasecmp(arg, "release")) {
        if (GET_SUPPRESS(ch)) {
            reveal_hiding(ch, 0);
            act("@GYou stop suppressing your current powerlevel!@n", true, ch, NULL, NULL, TO_CHAR);
            act("@G$n smiles as a rush of power erupts around $s body briefly.@n", true, ch, NULL, NULL, TO_ROOM);
            GET_SUPPRESS(ch) = 0;
            return;
        } else {
    ...
    int64_t max = (ch_getEffMaxPL(ch));
    int64_t amt = ((max * 0.01) * num);

    reveal_hiding(ch, 0);

    if (GET_SUPPRESS(ch) != 0) {
        act("@GYou alter your suppression level!@n", true, ch, NULL, NULL, TO_CHAR);
        act("@G$n seems to concentrate for a moment.@n", true, ch, NULL, NULL, TO_ROOM);

    } else {
        act("@GYou suppress your current powerlevel!@n", true, ch, NULL, NULL, TO_CHAR);
        act("@G$n seems to concentrate for a moment.@n", true, ch, NULL, NULL, TO_ROOM);
    }
    GET_SUPPRESS(ch) = num;
    return;
...
ACMD(do_hass) {
    ...
    if (((ch_getCurST(ch))) < GET_MAX_MOVE(ch) / 30) {
    ...
    if (perc < prob) {
        ...
        reveal_hiding(ch, 0);
        act("@WYou concentrate and start to move your arms at incredible speeds.@n", true, ch, NULL, NULL,
            TO_CHAR);
        act("@C$n@W concentrates and starts to move $s arms at incredible speeds.@n", true, ch, NULL, NULL,
            TO_ROOM);
...
ACMD(do_pose) {
    ...
    if (((ch_getCurST(ch))) < GET_MAX_MOVE(ch) / 40) {
    ...
    if (prob < perc) {
        reveal_hiding(ch, 0);
        act("@WYou attempt to strike an awe inspiring pose, but end up falling on your face!@n", true, ch, NULL,
            NULL, TO_CHAR);
        act("@C$n@W attempts to strike an awe inspiring pose, but ends up falling on $s face!@n", true, ch, NULL,
            NULL, TO_ROOM);
        int64_t amt1 = ch_getMaxST(ch) / 40;
        int64_t result1;
        result1 = ch_decCurST(ch, amt1, 0);
        ...
        ch->real_abils.dex += 8;
        save_char(ch);
        int64_t before = ((ch_getMaxLF(ch)));
        SET_BIT_AR(PLR_FLAGS(ch), PLR_POSE);
        int64_t amt = ((ch_getMaxLF(ch))) - before;
        int64_t result;
        result = ch_incCurLF(ch, amt, true);
        int64_t amt1 = ch_getMaxST(ch) / 40;
        int64_t result1;
        result1 = ch_decCurST(ch, amt1, 0);
        improve_skill(ch, SKILL_POSE, 0);
...
ACMD(do_fury) {
    ...
    if (!*arg) {
        if (GET_HIT(ch) < (ch_getEffMaxPL(ch))) {
            if (((ch_getCurLF(ch))) >= ((ch_getMaxLF(ch))) * 0.2) {
                ch_restoreHealth(ch, false);
                int64_t result;
                result = ch_decCurLFPercent(ch, .2, 0);
            } else {
                int64_t amt = ((ch_getCurLF(ch)));
                int64_t result1;
                result1 = ch_incCurHealth(ch, amt, true);
                int64_t result;
                result = ch_decCurLFPercent(ch, 2, -1);
            }
        }
        GET_FURY(ch) = 0;
    ...
    reveal_hiding(ch, 0);
    act("You release your fury! Your very next attack is guaranteed to rip your foes a new one!", true, ch, NULL,
        NULL, TO_CHAR);
    act("$n screams furiously as a look of anger appears on $s face!", true, ch, NULL, NULL, TO_ROOM);
...
ACMD(do_telepathy) {
    ...
    if (!(strcmp(arg, "link"))) {
        ...
        if (MINDLINK(ch)) {
            act("@CYou remove the link your mind had with @w$N.@n", true, ch, NULL, MINDLINK(ch), TO_CHAR);
            act("@w$n@C removes the link $s mind had with yours.@n", true, ch, NULL, MINDLINK(ch), TO_VICT);
            ...
            act("@R$n@r tried to link $s mind with yours, but you manage to force a break in the link!@n", false, ch,
                NULL, vict, TO_VICT);
            act("@R$N@r manages to sense the intrusion and with $S intelligence push you out!@n", false, ch, NULL,
                vict, TO_CHAR);
            return;
        } else {
            act("@CYou link your mind with @w$N.@n", true, ch, NULL, vict, TO_CHAR);
            act("@w$n@C links $s mind with yours. You can speak your thoughts to $m with 'think'.@n", true, ch, NULL,
                vict, TO_VICT);
        ...
        if (!(vict = get_char_vis(ch, arg2, NULL, FIND_CHAR_ROOM))) {
            ...
            if (axion_dice(0) > GET_SKILL(ch, SKILL_TELEPATHY)) {
                ...
                int64_t result;
                result = ch_decCurKI(ch, amt, 0);
                act("@wYou attempt to read $N's@w mind, but fail to see it clearly.@n", true, ch, NULL, vict,
                    TO_CHAR);
                if (rand_number(1, 15) >= 14 && !AFF_FLAGGED(ch, AFF_SHOCKED)) {
                    act("@MYour mind has been shocked!@n", true, ch, NULL, NULL, TO_CHAR);
                ...
                act("@wYou fail to read @c$N's@w mind and they seemed to have noticed the attempt!@n", true, ch,
                    NULL, vict, TO_CHAR);
                act("@C$n@w attempts to read your mind, but you resist and force $m out!@n", true, ch, NULL, vict,
                    TO_VICT);
                improve_skill(ch, SKILL_TELEPATHY, 0);
                return;
            } else {
...
ACMD(do_potential) {
    ...
    if (IS_MAJIN(vict)) {
        ...
        int64_t amt2 = (ch_getBasePL(vict) / 100) * boost;
        int64_t result2;
        result2 = ch_gainBasePL(vict, amt2, false);
        if (IS_HALFBREED(vict)) {
            int64_t amt = (vict->baseki / 100) * boost;
            int64_t result;
            result = ch_gainBaseKI(vict, amt, false);
            int64_t amt1 = (ch_getBaseST(vict) / 100) * boost;
            int64_t result1;
            result1 = ch_gainBaseST(vict, amt1, false);
        }
        reveal_hiding(ch, 0);
...
ACMD(do_majinize) {
    ...
    if (MAJINIZED(vict) > 0 && MAJINIZED(vict) != GET_ID(ch)) {
        ...
        return;
    } else if ((ch_getEffMaxPL(vict)) > (ch_getEffMaxPL(ch)) * 4) {
        send_to_char(ch,
        ...
        act("You remove $N's majinization, freeing them from your influence, but also weakening them.", true, ch,
            NULL, vict, TO_CHAR);
        act("$n removes your majinization, freeing you from their influence, and weakening you!", true, ch, NULL,
            vict, TO_VICT);
        act("$n waves a hand at $N, and instantly the glowing M on $S forehead disappears!", true, ch, NULL, vict,
            TO_NOTVICT);
        MAJINIZED(vict) = 0;
        GET_BOOSTS(ch) += 1;

        if (GET_MAJINIZED(vict) == 0) {
            GET_MAJINIZED(vict) = (((ch_getBasePL(vict))) * .4);
        }
        int64_t result1;
        result1 = ch_loseBasePL(vict, GET_MAJINIZED(vict), false);
        return;
        ...
        GET_MAJINIZED(vict) = ((ch_getBasePL(vict))) * .4;
        int64_t result;
        result = ch_gainBasePLPercent(vict, .4, true);
        return;
    }
...
ACMD(do_spit) {
    ...
    if (AFF_FLAGGED(vict, AFF_PARALYZE)) {
        act("$N has already been turned to stone.", true, ch, NULL, vict, TO_CHAR);
        return;
    }
    if (FIGHTING(vict)) {
    ...
    if (GET_SKILL(ch, SKILL_SPIT) < axion_dice(0)) {
        ...
        if (AFF_FLAGGED(vict, AFF_ZANZOKEN) && ((ch_getCurST(vict))) >= 1 && GET_POS(vict) != POS_SLEEPING) {
            int64_t result;
            result = ch_decCurKI(ch, cost, 0);
            reveal_hiding(ch, 0);
            act("@C$N@c disappears, avoiding your spit before reappearing!@n", false, ch, NULL, vict, TO_CHAR);
            act("@cYou disappear, avoiding @C$n's@c @rstone spit@c before reappearing!@n", false, ch, NULL, vict,
                TO_VICT);
            act("@C$N@c disappears, avoiding @C$n's@c @rstone spit@c before reappearing!@n", false, ch, NULL, vict,
                TO_NOTVICT);
            REMOVE_BIT_AR(AFF_FLAGS(vict), AFF_ZANZOKEN);
            WAIT_STATE(ch, PULSE_2SEC);
            improve_skill(ch, SKILL_SPIT, 1);
   ...
   */
    ...
    if (!(strcmp(arg, "food"))) {
        ...
        if (((ch_getMaxKI(ch) * ch->energy)) < cost) {
            ...
            reveal_hiding(ch, 0);
            GET_COOLDOWN(ch) = 10;
            act("You hold out your hand and create $p out of your ki!", true, ch, obj, NULL, TO_CHAR);
            act("$n holds out $s hand and creates $p out of thin air!", true, ch, obj, NULL, TO_ROOM);
            int64_t result;
            result = ch_decCurKI(ch, cost, 0);
            return;
        }
        ...
        if (((ch_getMaxKI(ch) * ch->energy)) < cost) {
            ...
            if (((ch_getCurST(ch))) < GET_MAX_MOVE(ch)) {
                ...
                result1 = ch_decCurKI(ch, cost, 0);
                int64_t result2;
                result2 = ch_decCurHealth(ch, cost2, 0);
                int64_t result;
                result = ch_decCurSTPercent(ch, 1, 1);
    ...
    if (!PLR_FLAGGED(ch, PLR_REPAIR)) {
        ...
        if (((ch_getMaxKI(ch) * ch->energy)) < cost) {
            ...
            if (((ch_getCurST(ch))) >= GET_MAX_MOVE(ch)) {
                ...
                reveal_hiding(ch, 0);
                GET_COOLDOWN(ch) = 10;
                act("You focus your ki into your energy reserves, recharging them some.", true, ch, NULL, NULL,
                    TO_CHAR);
                act("$n stops and glows green briefly.", true, ch, NULL, NULL, TO_ROOM);
                int64_t result1;
                result1 = ch_decCurKI(ch, cost, 0);
                if ((ch_getCurST(ch)) + (cost * 2) < GET_MAX_MOVE(ch)) {
                    int64_t result;
                    result = ch_incCurST(ch, cost * 2, true);
                } else {
                    ch_restoreST(ch, false);
    ...
    if (!IS_NPC(ch) && !PLR_FLAGGED(ch, PLR_REPAIR)) {
        ...
        if (((ch_getCurST(ch))) < cost) {
            ...
            act("You repair some of your outer casings and internal systems, with the small nano-robots contained in your body.",
                true, ch, NULL, NULL, TO_CHAR);
            act("$n stops a moment as small glowing particles move across $s body.", true, ch, NULL, NULL,
                TO_ROOM);
            ...
            if (ch_incCurHealth(ch, heal, true) == ch_getEffMaxPL(ch)) {
                send_to_char(ch, "You are fully repaired now.\r\n");
            }

            if (!IS_NPC(ch) && rand_number(1, 3) == 2 && ((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch)) {
                send_to_char(ch,
                             "@GThe repairs have managed to relink power reserves and boost your current energy level.@n\r\n");
                int64_t result1;
                result1 = ch_incCurKI(ch, cost, true);
            }
            WAIT_STATE(ch, PULSE_2SEC);
        }
    }
...
ACMD(do_upgrade) {
    ...
    if (!strcasecmp("augment", arg)) {
        ...
        if (!(obj = get_obj_in_list_vis(ch, "Augmentation", NULL, ch->carrying))) {
            ...
            switch (GET_LEVEL(ch)) { /* R: Original was GET_LEVEL(ch) */
                ...
                case 89:
                case 90:
                    gain = (ch_getEffMaxPL(ch)) * 0.005; /* R: Original was GET_LEVEL(ch) * 2000; */
                    break;
                case 91:
                case 92:
                case 93:
                ...
                case 98:
                case 99:
                    gain = (ch_getEffMaxPL(ch)) * 0.005; /* R: Original was GET_LEVEL(ch) * 4000; */
                    break;
                case 100:
                    gain = (ch_getEffMaxPL(ch)) * 0.005; /* R: Original was GET_LEVEL(ch) * 100000 */
                    if (gain > 10000000) {
                        gain = 10000000;
                    }
                    break;
            ...
            if (!strcasecmp("powerlevel", arg2)) {
                obj_from_char(obj);
                extract_obj(obj);
                act("@WYou install the circuits and upgrade your maximum powerlevel.@n", true, ch, NULL, NULL,
                    TO_CHAR);
                act("@C$n@W installs some circuits and upgrades $s systems.@n", true, ch, NULL, NULL, TO_ROOM);
                int64_t result1;
                result1 = ch_gainBasePL(ch, gain, true);
                send_to_char(ch, "@gGain @D[@G+%s@D]\r\n", add_commas(gain));
                return;
            } else if (!strcasecmp("ki", arg2)) {
                obj_from_char(obj);
                extract_obj(obj);
                act("@WYou install the circuits and upgrade your maximum ki.@n", true, ch, NULL, NULL, TO_CHAR);
                act("@C$n@W installs some circuits and upgrades $s systems.@n", true, ch, NULL, NULL, TO_ROOM);
                int64_t result;
                result = ch_gainBaseKI(ch, gain, true);
                send_to_char(ch, "@gGain @D[@G+%s@D]\r\n", add_commas(gain));
                return;
            } else if (!strcasecmp("stamina", arg2)) {
                obj_from_char(obj);
                extract_obj(obj);
                act("@WYou install the circuits and upgrade your maximum stamina.@n", true, ch, NULL, NULL,
                    TO_CHAR);
                act("@C$n@W installs some circuits and upgrades $s systems.@n", true, ch, NULL, NULL, TO_ROOM);
                int64_t result;
                result = ch_gainBaseST(ch, gain, true);
                send_to_char(ch, "@gGain @D[@G+%s@D]\r\n", add_commas(gain));
                return;
            } else {
    ...
    if (!strcasecmp("powerlevel", arg)) {
        ...
        if (cost > GET_UP(ch)) {
            ...
            if (ch_is_soft_cap(ch, bonus, 1.0)) {
                ...
                int64_t result;
                result = ch_gainBasePL(ch, bonus, true);
            }
        }
        ...
        if (cost > GET_UP(ch)) {
            ...
            if (ch_is_soft_cap(ch, bonus, 1.0)) {
                ...
                send_to_char(ch, "You upgrade your system and gain %s %s!", add_commas(bonus), arg);
                int64_t result;
                result = ch_gainBaseKI(ch, bonus, true);
            }
        }
        ...
        if (cost > GET_UP(ch)) {
            ...
            if (ch_is_soft_cap(ch, bonus, 1.0)) {
                ...
                send_to_char(ch, "You upgrade your system and gain %s %s!", add_commas(bonus), arg);
                int64_t result;
                result = ch_gainBaseST(ch, bonus, true);
            }
        }
    } else {
...
ACMD(do_ingest) {
    ...
    if (IS_MAJIN(ch)) {
        ...
        if ((ch_getEffMaxPL(vict)) >= ((ch_getBasePL(ch))) * 3) {
        ...
        reveal_hiding(ch, 0);
        if (AFF_FLAGGED(vict, AFF_ZANZOKEN) && ((ch_getCurST(vict))) >= 1 && GET_POS(vict) != POS_SLEEPING) {
            act("@C$N@c disappears, avoiding your attempted ingestion!@n", false, ch, NULL, vict, TO_CHAR);
            act("@cYou disappear, avoiding @C$n's@c attempted @ringestion@c before reappearing!@n", false, ch, NULL,
                vict, TO_VICT);
        ...
        if (GET_SPEEDI(ch) + rand_number(1, 5) < GET_SPEEDI(ch) + rand_number(1, 5)) {
            act("@WYou fling a piece of goo at @c$N@W, and try to ingest $M! $E manages to avoid your blob of goo though!@n",
                true, ch, NULL, vict, TO_CHAR);
            act("@C$n@W flings a piece of goo at you, you manage to avoid it though!@n", true, ch, NULL, vict,
                TO_VICT);
            act("@C$n@w flings a piece of goo at @c$N@W, but the goo misses $M@W!@n", true, ch, NULL, vict,
                TO_NOTVICT);
            WAIT_STATE(ch, PULSE_3SEC);
            return;
        } else {
            ...
            act("@C$n@w flings a piece of goo at @c$N@W! The goo engulfs $M and then return to @C$n@W!@n", true, ch,
                NULL, vict, TO_NOTVICT);
            GET_ABSORBS(ch) += 1;
            int64_t pl = ((ch_getBasePL(vict))) / 6;
            int64_t stam = ((ch_getBaseST(vict))) / 6;
            int64_t ki = ((vict->baseki)) / 6;
            int64_t result2;
            result2 = ch_gainBasePL(ch, pl, true);
            int64_t result1;
            result1 = ch_gainBaseST(ch, stam, true);
            int64_t result;
            result = ch_gainBaseKI(ch, ki, true);
...
ACMD(do_absorb) {
    ...
    if (ABSORBING(ch)) {
        act("@WYou stop absorbing from @c$N@W!@n", true, ch, NULL, ABSORBING(ch), TO_CHAR);
        act("$n stops absorbing from you!", true, ch, NULL, ABSORBING(ch), TO_VICT);
        act("$n stops absorbing from $N!", true, ch, NULL, ABSORBING(ch), TO_NOTVICT);
    ...
    if (IS_ANDROID(ch)) {
        ...
        if ((ch_getEffMaxPL(vict)) > (ch_getEffMaxPL(ch)) * 2) {
            send_to_char(ch, "They are too strong for you to absorb from.\r\n");
            return;
        }
        if ((ch_getEffMaxPL(vict)) * 20 < (ch_getEffMaxPL(ch))) {
            send_to_char(ch, "They are too weak for you to bother absorbing from.\r\n");
            return;
        }
        if (((ch_getCurST(vict))) < (GET_MAX_MOVE(vict) / 20) && ((ch_getMaxKI(vict) * vict->energy)) < (GET_MAX_MANA(vict) / 20)) {
            send_to_char(ch, "They have nothing to absorb right now, they are drained...\r\n");
            return;
        }
        reveal_hiding(ch, 0);
        if (init_skill(ch, SKILL_ABSORB) < axion_dice(0)) {
            act("@WYou rush at @c$N@W and try to absorb from them, but $E manages to avoid you!@n", true, ch, NULL,
                vict, TO_CHAR);
            act("@C$n@W rushes at you and tries to grab you, but you manage to avoid $m!@n", true, ch, NULL, vict,
                TO_VICT);
            ...
            act("@WYou rush at @c$N@W and try to absorb from them, and manage to grab on!@n", true, ch, NULL, vict,
                TO_CHAR);
            act("@C$n@W rushes at you and tries to grab you, and manages to grab on!@n", true, ch, NULL, vict,
                TO_VICT);
            act("@C$n@w rushes at @c$N@W and tries to grab $M, and manages to grab on!@n", true, ch, NULL, vict,
                TO_NOTVICT);
            improve_skill(ch, SKILL_ABSORB, 1);
            ABSORBING(ch) = vict;
            ABSORBBY(vict) = ch;
        ...
        if (GET_SKILL(ch, SKILL_ABSORB) < axion_dice(0)) {
            ...
            int64_t stam = ((ch_getBaseST(vict))) / 5;
            int64_t ki = ((vict->baseki)) / 5;
            int64_t pl = ((ch_getBasePL(vict))) / 5;
        ...
        if ((ch_getEffMaxPL(vict)) >= (ch_getEffMaxPL(ch))) {
            send_to_char(ch, "You are too weak to absorb them into your cellular structure!\r\n");
            return;
        }
        if ((ch_getEffMaxPL(vict)) < (ch_getEffMaxPL(ch)) / 5) {
        ...
        if (GET_SKILL(ch, SKILL_ABSORB) < failthresh) {
            ...
            act("@C$n@w rushes at @c$N@W and stabs $M with $s tail! $e quickly sucks out all the bio extract and leaves the empty husk of @c$N@W behind!@n",
                true, ch, NULL, vict, TO_NOTVICT);
            int64_t stam = ((ch_getBaseST(vict))) / 2000;
            int64_t ki = ((vict->baseki)) / 2000;
            int64_t pl = ((ch_getBasePL(vict))) / 2000;
            ...
            result = ch_gainBaseKI(ch, ki, true);
            int64_t result3;
            result3 = ch_incCurLFPercent(ch, .05, true);
            send_to_char(ch, "@D[@gABSORB@D] @rPL@W: @D(@y%s@D) @cKi@W: @D(@y%s@D) @gSt@W: @D(@y%s@D)@n\r\n",
                         add_commas(pl), add_commas(ki), add_commas(stam));
            improve_skill(ch, SKILL_ABSORB, 0);
            WAIT_STATE(ch, PULSE_4SEC);
...
ACMD(do_escape) {
    ...
    if (ABSORBBY(ch)) {
        ...
        if (num > skill) {
            act("@c$N@W manages to break loose of @C$n's@W hold!@n", true, ABSORBBY(ch), NULL, ch, TO_NOTVICT);
            ...
            act("@c$N@W struggles to break loose of @C$n's@W hold!@n", true, ABSORBBY(ch), NULL, ch, TO_NOTVICT);
            act("@WYou struggle to break loose of @C$n's@W hold!@n", true, ABSORBBY(ch), NULL, ch, TO_VICT);
            act("@c$N@W struggles to break loose of your hold!@n", true, ABSORBBY(ch), NULL, ch, TO_CHAR);
            if (rand_number(1, 3) == 3) {
                int64_t dmg = (ch_getEffMaxPL(ch)) * 0.025;
        ...
        if (num > skill) {
            if (GRAPTYPE(GRAPPLED(ch)) == 4) {
                act("@c$N@M flexes with all $S might and causes your body to explode outward into gooey chunks!@n",
                    true, GRAPPLED(ch), NULL, ch, TO_CHAR);
                act("@MYou flex with all your might and cause @C$n's@M body to explode outward into gooey chunks!@n",
                    true, GRAPPLED(ch), NULL, ch, TO_VICT);
                act("@c$N@M flexes with all $S might and causes @C$n's@M body to explode outward into gooey chunks!@n",
                    true, GRAPPLED(ch), NULL, ch, TO_NOTVICT);

                act("@MYou reform your body mere moments later.@n", true, GRAPPLED(ch), NULL, NULL, TO_CHAR);
                act("@C$n@M reforms $s body mere moments later.", true, GRAPPLED(ch), NULL, NULL, TO_ROOM);
            } else {
                act("@c$N@W manages to break loose of @C$n's@W hold!@n", true, GRAPPLED(ch), NULL, ch, TO_NOTVICT);
                act("@WYou manage to break loose of @C$n's@W hold!@n", true, GRAPPLED(ch), NULL, ch, TO_VICT);
                act("@c$N@W manages to break loose of your hold!@n", true, GRAPPLED(ch), NULL, ch, TO_CHAR);
            }
            if (FIGHTING(ch) == NULL) {
            ...
            act("@c$N@W struggles to break loose of @C$n's@W hold!@n", true, GRAPPLED(ch), NULL, ch, TO_NOTVICT);
            act("@WYou struggle to break loose of @C$n's@W hold!@n", true, GRAPPLED(ch), NULL, ch, TO_VICT);
            act("@c$N@W struggles to break loose of your hold!@n", true, GRAPPLED(ch), NULL, ch, TO_CHAR);
            if (rand_number(1, 3) == 3) {
                int64_t dmg = (ch_getEffMaxPL(ch)) * 0.025;
                hurt(0, 0, ch, GRAPPLED(ch), NULL, dmg, 0);
                if (GET_POS(GRAPPLED(ch)) == POS_SLEEPING) {
                    act("@c$N@W manages to break loose of @C$n's@W hold!@n", true, GRAPPLED(ch), NULL, ch,
                        TO_NOTVICT);
                    act("@WYou manage to break loose of @C$n's@W hold!@n", true, GRAPPLED(ch), NULL, ch, TO_VICT);
                    act("@c$N@W manages to break loose of your hold!@n", true, GRAPPLED(ch), NULL, ch, TO_CHAR);
                    GRAPTYPE(GRAPPLED(ch)) = -1;
...
ACMD(do_regenerate) {
    ...
    if (GET_SUPPRESS(ch) > 0 && GET_HIT(ch) >= (((ch_getEffMaxPL(ch)) / 100) * GET_SUPPRESS(ch))) {
        send_to_char(ch, "You do not need to regenerate, you are at full health.\r\n");
        return;
    }
    ...
    amt = ((ch_getEffMaxPL(ch)) * 0.01) * num;
    if (amt > 1)
        amt /= 2;
    ...
    int64_t life = (((ch_getCurLF(ch))) - amt * 0.8), energy = (((ch_getMaxKI(ch) * ch->energy)) - amt * 0.2);
    ...
    int64_t result2;
    result2 = ch_incCurHealth(ch, amt * 2, true);

    if (!IS_NPC(ch)) {
        int64_t result;
        result = ch_decCurLF(ch, amt * .8, 0);
    }

    int64_t result1;
    result1 = ch_decCurKI(ch, amt * .2, 0);

    reveal_hiding(ch, 0);

    if (GET_HIT(ch) >= (ch_getEffMaxPL(ch))) {
        act("You concentrate your ki and regenerate your body completely.", true, ch, NULL, NULL, TO_CHAR);
        act("$n concentrates and regenerates $s body completely.", true, ch, NULL, NULL, TO_ROOM);
    } else if (amt < (ch_getEffMaxPL(ch)) / 10) {
        act("You concentrate your ki and regenerate your body a little.", true, ch, NULL, NULL, TO_CHAR);
        act("$n concentrates and regenerates $s body a little.", true, ch, NULL, NULL, TO_ROOM);
    } else if (amt < (ch_getEffMaxPL(ch)) / 5) {
        act("You concentrate your ki and regenerate your body some.", true, ch, NULL, NULL, TO_CHAR);
        act("$n concentrates and regenerates $s body some.", true, ch, NULL, NULL, TO_ROOM);
    } else if (amt < (ch_getEffMaxPL(ch)) / 2) {
        act("You concentrate your ki and regenerate your body a great deal.", true, ch, NULL, NULL, TO_CHAR);
        act("$n concentrates and regenerates $s body a great deal.", true, ch, NULL, NULL, TO_ROOM);
    } else if (GET_HIT(ch) < (ch_getEffMaxPL(ch))) {
        act("You concentrate your ki and regenerate you nearly completely.", true, ch, NULL, NULL, TO_CHAR);
        act("$n concentrates and regenerates $s body nearly completely.", true, ch, NULL, NULL, TO_ROOM);
    }
    ...
    if (!IS_NPC(ch)) {
        if (GET_LIMBCOND(ch, 1) <= 0) {
            act("You regrow your right arm!", true, ch, NULL, NULL, TO_CHAR);
            act("$n regrows $s right arm!", true, ch, NULL, NULL, TO_ROOM);
            GET_LIMBCOND(ch, 1) = 100;
        } else if (GET_LIMBCOND(ch, 1) >= 0 && GET_LIMBCOND(ch, 1) < 50) {
            act("Your broken right arm mends itself!", true, ch, NULL, NULL, TO_CHAR);
            act("$n regenerates $s broken right arm!", true, ch, NULL, NULL, TO_ROOM);
            GET_LIMBCOND(ch, 1) = 100;
        }
        if (GET_LIMBCOND(ch, 2) <= 0) {
            GET_LIMBCOND(ch, 2) = 100;
            act("You regrow your left arm!", true, ch, NULL, NULL, TO_CHAR);
            act("$n regrows $s left arm!", true, ch, NULL, NULL, TO_ROOM);
        } else if (GET_LIMBCOND(ch, 2) > 0 && GET_LIMBCOND(ch, 2) < 50) {
            act("Your broken left arm mends itself!", true, ch, NULL, NULL, TO_CHAR);
            act("$n regenerates $s broken left arm!", true, ch, NULL, NULL, TO_ROOM);
            GET_LIMBCOND(ch, 2) = 100;
        }
        if (GET_LIMBCOND(ch, 4) <= 0) {
            GET_LIMBCOND(ch, 4) = 100;
            act("You regrow your left leg!", true, ch, NULL, NULL, TO_CHAR);
            act("$n regrows $s left leg!", true, ch, NULL, NULL, TO_ROOM);
        } else if (GET_LIMBCOND(ch, 4) > 0 && GET_LIMBCOND(ch, 4) < 50) {
            act("Your broken left leg mends itself!", true, ch, NULL, NULL, TO_CHAR);
            act("$n regenerates $s broken left leg!", true, ch, NULL, NULL, TO_ROOM);
            GET_LIMBCOND(ch, 4) = 100;
        }
        if (GET_LIMBCOND(ch, 3) <= 0) {
            GET_LIMBCOND(ch, 3) = 100;
            act("You regrow your right leg!", true, ch, NULL, NULL, TO_CHAR);
            act("$n regrows $s right leg!", true, ch, NULL, NULL, TO_ROOM);
        } else if (GET_LIMBCOND(ch, 3) > 0 && GET_LIMBCOND(ch, 3) < 50) {
            GET_LIMBCOND(ch, 3) = 100;
            act("Your broken right leg mends itself!", true, ch, NULL, NULL, TO_CHAR);
            act("$n regenerates $s broken right leg!", true, ch, NULL, NULL, TO_ROOM);
        }
        if (!PLR_FLAGGED(ch, PLR_TAIL) && IS_BIO(ch)) {
            SET_BIT_AR(PLR_FLAGS(ch), PLR_TAIL);
            act("You regrow your tail!", true, ch, NULL, NULL, TO_CHAR);
            act("$n regrows $s tail!", true, ch, NULL, NULL, TO_ROOM);
        }
        improve_skill(ch, SKILL_REGENERATE, 0);
    }
}
    ...
    if (AFF_FLAGGED(ch, AFF_CURSE)) {
        ...
        if (!*name) {
            if (AFF_FLAGGED(ch, AFF_STONESKIN)) {
                ...
                if (((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) / 20) {
                    ...
                    result = ch_decCurKI(ch, amt, 0);
                    reveal_hiding(ch, 0);
                    act("You focus ki into your skin, but fail in making it tough!", true, ch, NULL, NULL, TO_CHAR);
                    act("$n focuses ki into $s skin, but fails in making it tough!", true, ch, NULL, NULL, TO_ROOM);
                    return;
                } else {
                    ...
                    result = ch_decCurKI(ch, amt, 0);
                    reveal_hiding(ch, 0);
                    act("You focus ki into your skin, making it tough!", true, ch, NULL, NULL, TO_CHAR);
                    act("$n focuses ki into $s skin, making it tough!", true, ch, NULL, NULL, TO_ROOM);
                    return;
                }
            }
            ...
            if (!can_kill(ch, vict, NULL, 2)) {
                ...
                if (AFF_FLAGGED(vict, AFF_STONESKIN)) {
                    ...
                    if (((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) / 20) {
                        ...
                        result = ch_decCurKI(ch, amt, 0);
                        reveal_hiding(ch, 0);
                        act("You focus ki into $N's skin, but fail in making it tough!", true, ch, NULL, vict, TO_CHAR);
                        act("$n focuses ki into your skin, but fails in making it tough!", true, ch, NULL, vict,
                            TO_VICT);
                        act("$n focuses ki into $N's skin, but fails in making it tough!", true, ch, NULL, vict,
                            TO_NOTVICT);
                        return;
                    } else {
                        ...
                        result = ch_decCurKI(ch, amt, 0);
                        reveal_hiding(ch, 0);
                        act("You focus ki into $N's skin, making it tough!", true, ch, NULL, vict, TO_CHAR);
                        act("$n focuses ki into your skin, making it tough!", true, ch, NULL, vict, TO_VICT);
                        act("$n focuses ki into $N's skin, making it tough!", true, ch, NULL, vict, TO_NOTVICT);
                        return;
                    }
                }
            }
        ...
        if (!*name) {
            if (AFF_FLAGGED(ch, AFF_MIGHT)) {
                ...
                if (((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) / 20) {
                    ...
                    result = ch_decCurKI(ch, amt, 0);
                    reveal_hiding(ch, 0);
                    act("You focus ki into your muscles, but fail in making them mighty!", true, ch, NULL, NULL,
                        TO_CHAR);
                    act("$n focuses ki into $s muscles, but fails in making them mighty!", true, ch, NULL, NULL,
                        TO_ROOM);
                    return;
                } else {
                    ...
                    reveal_hiding(ch, 0);
                    act("You focus ki into your muscles, making them mighty!", true, ch, NULL, NULL, TO_CHAR);
                    act("$n focuses ki into $s muscles, making them mighty!", true, ch, NULL, NULL, TO_ROOM);
                    return;
                }
            }
            ...
            if (!can_kill(ch, vict, NULL, 2)) {
                ...
                if (AFF_FLAGGED(vict, AFF_MIGHT)) {
                    ...
                    if (((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) / 20) {
                        ...
                        result = ch_decCurKI(ch, amt, 0);
                        reveal_hiding(ch, 0);
                        act("You focus ki into $N's muscles, but fail in making them mighty!", true, ch, NULL, vict,
                            TO_CHAR);
                        act("$n focuses ki into your muscles, but fails in making them mighty!", true, ch, NULL, vict,
                            TO_VICT);
                        act("$n focuses ki into $N's muscles, but fails in making them mighty!", true, ch, NULL, vict,
                            TO_NOTVICT);
                        return;
                    } else {
                        int64_t amt = ch_getMaxKI(ch) / 20;
                        ...
                        reveal_hiding(ch, 0);
                        act("You focus ki into $N's muscles, making them mighty!", true, ch, NULL, vict, TO_CHAR);
                        act("$n focuses ki into your muscles, making them mighty!", true, ch, NULL, vict, TO_VICT);
                        act("$n focuses ki into $N's muscles, making them mighty!", true, ch, NULL, vict, TO_NOTVICT);
                        return;
                    }
                }
            }
        ...
        if (AFF_FLAGGED(vict, AFF_WITHER)) {
            ...
            if (((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) / 20) {
                ...
                result = ch_decCurKI(ch, amt, 0);
                reveal_hiding(ch, 0);
                act("You focus ki into $N's body, but fail in withering it!", true, ch, NULL, vict, TO_CHAR);
                act("$n focuses ki into your body, but fails in withering it!", true, ch, NULL, vict, TO_VICT);
                act("$n focuses ki into $N's body, but fails in withering it!", true, ch, NULL, vict, TO_NOTVICT);
                return;
            } else {
                ...
                save_char(vict);
                reveal_hiding(ch, 0);
                act("You focus ki into $N's body, and succeed in withering it!", true, ch, NULL, vict, TO_CHAR);
                act("$n focuses ki into your body, and succeeds in withering it!", true, ch, NULL, vict, TO_VICT);
                act("$n focuses ki into $N's body, and succeeds in withering it!", true, ch, NULL, vict, TO_NOTVICT);
                return;
            }
        }
    } // End of wither
        ...
        if (!*name) {
            if (AFF_FLAGGED(ch, AFF_ENLIGHTEN)) {
                ...
                if (((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) / 20) {
                    ...
                    result = ch_decCurKI(ch, amt, 0);
                    reveal_hiding(ch, 0);
                    act("You focus ki into your mind, but fail in awakening it to cosmic wisdom!", true, ch, NULL,
                        NULL, TO_CHAR);
                    act("$n focuses ki into $s mind, but fails in awakening it to cosmic wisdom!", true, ch, NULL,
                        NULL, TO_ROOM);
                    return;
                } else {
                    int duration = roll_aff_duration(GET_INT(ch), 2);
                    ...
                    result = ch_decCurKI(ch, amt, 0);
                    reveal_hiding(ch, 0);
                    act("You focus ki into your mind, awakening it to cosmic wisdom!", true, ch, NULL, NULL, TO_CHAR);
                    act("$n focuses ki into $s mind, awakening it to cosmic wisdom!", true, ch, NULL, NULL, TO_ROOM);
            ...
            if (!can_kill(ch, vict, NULL, 2)) {
                ...
                if (AFF_FLAGGED(vict, AFF_ENLIGHTEN)) {
                    ...
                    if (((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) / 20) {
                        ...
                        result = ch_decCurKI(ch, amt, 0);
                        reveal_hiding(ch, 0);
                        act("You focus ki into $N's mind, but fail in awakening it to cosmic wisdom!", true, ch, NULL,
                            vict, TO_CHAR);
                        act("$n focuses ki into your mind, but fails in awakening it to cosmic wisdom!", true, ch, NULL,
                            vict, TO_VICT);
                        act("$n focuses ki into $N's mind, but fails in awakening it to cosmic wisdom!", true, ch, NULL,
                            vict, TO_NOTVICT);
                        return;
                    } else {
                        int duration = roll_aff_duration(GET_INT(ch), 2);
                        ...
                        result = ch_decCurKI(ch, amt, 0);
                        reveal_hiding(ch, 0);
                        act("You focus ki into $N's mind, awakening it to cosmic wisdom!", true, ch, NULL, vict,
                            TO_CHAR);
                        act("$n focuses ki into your mind, awakening it to cosmic wisdom!", true, ch, NULL, vict,
                            TO_VICT);
                        act("$n focuses ki into $N's mind, awakening it to cosmic wisdom!", true, ch, NULL, vict,
                            TO_NOTVICT);
        ...
        if (!*name) {
            if (AFF_FLAGGED(ch, AFF_GENIUS)) {
                ...
                if (((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) / 20) {
                    ...
                    result = ch_decCurKI(ch, amt, 0);
                    reveal_hiding(ch, 0);
                    act("You focus ki into your mind, but fail in making it work faster!", true, ch, NULL, NULL,
                        TO_CHAR);
                    act("$n focuses ki into $s muscles, but fails in making it work faster!", true, ch, NULL, NULL,
                        TO_ROOM);
                    return;
                } else {
                    int duration = roll_aff_duration(GET_INT(ch), 2);
                    ...
                    result = ch_decCurKI(ch, amt, 0);
                    reveal_hiding(ch, 0);
                    act("You focus ki into your mind, making it work faster!", true, ch, NULL, NULL, TO_CHAR);
                    act("$n focuses ki into $s mind, making it work faster!", true, ch, NULL, NULL, TO_ROOM);
                    return;
                }
            }
            ...
            if (!can_kill(ch, vict, NULL, 2)) {
                ...
                if (AFF_FLAGGED(vict, AFF_GENIUS)) {
                    ...
                    if (((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) / 20) {
                        ...
                        result = ch_decCurKI(ch, amt, 0);
                        reveal_hiding(ch, 0);
                        act("You focus ki into $N's mind, but fail in making it work faster!", true, ch, NULL, vict,
                            TO_CHAR);
                        act("$n focuses ki into your mind, but fails in making it work faster!", true, ch, NULL, vict,
                            TO_VICT);
                        act("$n focuses ki into $N's mind, but fails in making it work faster!", true, ch, NULL, vict,
                            TO_NOTVICT);
                        return;
                    } else {
                        int duration = roll_aff_duration(GET_INT(ch), 2);;
                        ...
                        result = ch_decCurKI(ch, amt, 0);
                        reveal_hiding(ch, 0);
                        act("You focus ki into $N's mind, making it work faster!", true, ch, NULL, vict, TO_CHAR);
                        act("$n focuses ki into your mind, making it work faster!", true, ch, NULL, vict, TO_VICT);
                        act("$n focuses ki into $N's mind, making it work faster!", true, ch, NULL, vict, TO_NOTVICT);
        ...
        if (!*name) {
            if (AFF_FLAGGED(ch, AFF_FLEX)) {
                ...
                if (((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) / 20) {
                    ...
                    result = ch_decCurKI(ch, amt, 0);
                    reveal_hiding(ch, 0);
                    act("You focus ki into your limbs, but fail in making them more flexible!", true, ch, NULL, NULL,
                        TO_CHAR);
                    act("$n focuses ki into $s muscles, but fails in making them more flexible!", true, ch, NULL,
                        NULL, TO_ROOM);
                    return;
                } else {
                    int64_t amt = ch_getMaxKI(ch) / 20;
                    ...
                    reveal_hiding(ch, 0);
                    act("You focus ki into your limbs, making them more flexible!", true, ch, NULL, NULL, TO_CHAR);
                    act("$n focuses ki into $s limbs, making them more flexible!", true, ch, NULL, NULL, TO_ROOM);
                    return;
                }
            }
        } // End of no vict FLEX
            ...
            if (!can_kill(ch, vict, NULL, 2)) {
                ...
                if (AFF_FLAGGED(vict, AFF_FLEX)) {
                    ...
                    if (((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) / 20) {
                        ...
                        result = ch_decCurKI(ch, amt, 0);
                        reveal_hiding(ch, 0);
                        act("You focus ki into $N's limbs, but fail in making them more flexible!", true, ch, NULL, vict,
                            TO_CHAR);
                        act("$n focuses ki into your limbs, but fails in making them more flexible!", true, ch, NULL,
                            vict, TO_VICT);
                        act("$n focuses ki into $N's limbs, but fails in making them more flexible!", true, ch, NULL,
                            vict, TO_NOTVICT);
                        return;
                    } else {
                        int64_t amt = ch_getMaxKI(ch) / 20;
                        ...
                        reveal_hiding(ch, 0);
                        act("You focus ki into $N's limbs, making them more flexible!", true, ch, NULL, vict, TO_CHAR);
                        act("$n focuses ki into your limbs, making them more flexible!", true, ch, NULL, vict, TO_VICT);
                        act("$n focuses ki into $N's limbs, making them more flexible!", true, ch, NULL, vict,
                            TO_NOTVICT);
                        if ((vict->master == ch || ch->master == vict || ch->master == vict->master) &&
                            AFF_FLAGGED(ch, AFF_GROUP) && AFF_FLAGGED(vict, AFF_GROUP)) {
                            if (IS_KAI(ch) && level_exp(ch, GET_LEVEL(ch) + 1) - GET_EXP(ch) > 0 &&
            ...
            if (AFF_FLAGGED(ch, AFF_BLESS)) {
                ...
                if (((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) / 20) {
                    ...
                    act("You focus ki while chanting spiritual words. Your blessing does nothing though, you must have messed up!",
                        true, ch, NULL, NULL, TO_CHAR);
                    act("$n focuses ki while chanting spiritual words. $n seems disappointed.", true, ch, NULL, NULL,
                        TO_ROOM);
                    return;
                } else {
            ...
            if (!can_kill(ch, vict, NULL, 2)) {
                ...
                if (AFF_FLAGGED(vict, AFF_BLESS)) {
                    ...
                    if (((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) / 20) {
                        ...
                        result = ch_decCurKI(ch, amt, 0);
                        reveal_hiding(ch, 0);
                        act("You focus ki while chanting spiritual words. Your blessing fails!", true, ch, NULL, NULL,
                            TO_CHAR);
            ...
            if (AFF_FLAGGED(ch, AFF_CURSE)) {
                ...
                if (((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) / 20) {
                    ...
                    act("You focus ki while chanting demonic words. Your cursing does nothing though, you must have messed up!",
                        true, ch, NULL, NULL, TO_CHAR);
                    act("$n focuses ki while chanting demonic words. $n seems disappointed.", true, ch, NULL, NULL,
                        TO_ROOM);
                    return;
                } else {
                    int duration = roll_aff_duration(GET_INT(ch), 3);;
                    ...
                    act("You focus ki while chanting demonic words. You feel your body recovering at below normal speed!",
                        true, ch, NULL, NULL, TO_CHAR);
                    act("$n focuses ki while chanting demonic words. $n grins after finishing $s chant.", true, ch, NULL,
                        NULL, TO_ROOM);
                    if (AFF_FLAGGED(ch, AFF_BLESS)) {
            ...
            if (!can_kill(ch, vict, NULL, 0)) {
                ...
                if (AFF_FLAGGED(vict, AFF_CURSE)) {
                    ...
                    if (((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) / 20) {
                        ...
                        result = ch_decCurKI(ch, amt, 0);
                        reveal_hiding(ch, 0);
                        act("You focus ki while chanting demonic words. Your cursing fails!", true, ch, NULL, NULL,
                            TO_CHAR);
        ...
        if (!can_kill(ch, vict, NULL, 0)) {
            ...
            if (AFF_FLAGGED(vict, AFF_SLEEP)) {
                ...
                if (((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) / 20) {
                    ...
                    result = ch_decCurKI(ch, amt, 0);
                    reveal_hiding(ch, 0);
                    act("You focus ki while moving your hands in lulling patterns, putting $N to sleep!", true, ch, NULL,
                        vict, TO_CHAR);
                    act("$n focuses ki while moving $s hands in a lulling pattern, before you realise it you are asleep!",
                        true, ch, NULL, vict, TO_VICT);
                    act("$n focuses ki while moving $s hands in a lulling pattern, putting $N to sleep!", true, ch, NULL,
                        vict, TO_NOTVICT);
                    GET_POS(vict) = POS_SLEEPING;
                    if (AFF_FLAGGED(vict, AFF_FLYING)) {
        ...
        if (!*name) {
            if (((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) / 10) {
                send_to_char(ch, "You do not have enough ki to use vigor.\r\n");
                return;
            } else if (GET_SKILL(ch, SKILL_VIGOR) < axion_dice(0)) {
                int64_t amt1 = ch_getMaxKI(ch) / 10;
                int64_t result1;
                result1 = ch_decCurKI(ch, amt1, 0);
                reveal_hiding(ch, 0);
                act("You focus ki into your very cells, but fail at re-engerizing them!", true, ch, NULL, NULL,
                    TO_CHAR);
                act("$n focuses ki and glows green for a moment, $e then frowns.", true, ch, NULL, NULL, TO_ROOM);
                WAIT_STATE(ch, PULSE_2SEC);
                return;
            } else {
                if (((ch_getCurST(ch))) >= GET_MAX_MOVE(ch)) {
                    ...
                    if (GET_BONUS(ch, BONUS_HEALER) > 0) {
                        int64_t amt = ch_getMaxKI(ch) / 8;
                        int64_t result;
                        result = ch_incCurST(ch, amt, true);
                        int64_t amt1 = ch_getMaxKI(ch) / 8;
                        int64_t result1;
                        result1 = ch_decCurKI(ch, amt1, 0);
                    } else {
                        int64_t amt = ch_getMaxKI(ch) / 10;
                        int64_t result;
                        result = ch_incCurST(ch, amt, true);
                        int64_t amt1 = ch_getMaxKI(ch) / 10;
                        int64_t result1;
                    ...
                    reveal_hiding(ch, 0);
                    act("You focus ki into your very cells, and manage to re-energize them!", true, ch, NULL, NULL,
                        TO_CHAR);
                    act("$n focuses ki and glows green for a moment, $e then smiles.", true, ch, NULL, NULL, TO_ROOM);
                    WAIT_STATE(ch, PULSE_2SEC);
                    return;
                }
            }
            ...
            if (!can_kill(ch, vict, NULL, 2)) {
                ...
                if (IS_NPC(vict)) {
                    ...
                    if (((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) / 10) {
                        ...
                        if (((ch_getCurST(vict))) >= GET_MAX_MOVE(vict)) {
                            ...
                            result1 = ch_decCurKI(ch, amt1, 0);
                            reveal_hiding(ch, 0);
                            act("You focus ki into $N's very cells, and fail at re-energizing them!", true, ch, NULL, vict,
                                TO_CHAR);
                            act("$n focuses ki into your very cells, but nothing happens!", true, ch, NULL, vict, TO_VICT);
                            act("$n focuses ki and $N glows green for a moment, $N frowns.", true, ch, NULL, vict,
                                TO_NOTVICT);
                            WAIT_STATE(ch, PULSE_2SEC);
                            return;
                        } else {
                            if (GET_BONUS(ch, BONUS_HEALER) > 0) {
                                int64_t amt = ch_getMaxKI(vict) / 8;
                                int64_t result;
                                result = ch_incCurST(vict, amt, true);
                                int64_t amt1 = ch_getMaxKI(ch) / 8;
                                int64_t result1;
                                result1 = ch_decCurKI(ch, amt1, 0);
                            } else {
                                int64_t amt = ch_getMaxKI(vict) / 10;
                                int64_t result;
                                result = ch_incCurST(vict, amt, true);
                                int64_t amt1 = ch_getMaxKI(ch) / 10;
                                int64_t result1;
                            ...
                            reveal_hiding(ch, 0);
                            act("You focus ki into $N's very cells, and manage to re-energize them!", true, ch, NULL, vict,
                                TO_CHAR);
                            act("$n focuses ki into your very cells, and manages to re-energize them!", true, ch, NULL, vict,
                                TO_VICT);
                            act("$n focuses ki and $N glows green for a moment, $N smiles.", true, ch, NULL, vict,
                                TO_NOTVICT);
                            WAIT_STATE(ch, PULSE_2SEC);
                            return;
                        }
        ...
        if (!*name) {
            if (!AFF_FLAGGED(ch, AFF_POISON)) {
                ...
                if (((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) / 20) {
                    ...
                    result = ch_decCurKI(ch, amt, 0);
                    reveal_hiding(ch, 0);
                    act("You focus ki and aim a pulsing light at your body. Nothing happens!", true, ch, NULL, NULL,
                        TO_CHAR);
                    act("$n focuses ki and aims a pulsing light at $s body. Nothing seems to happen.", true, ch, NULL,
                        NULL, TO_ROOM);
                    return;
                } else {
                    ...
                    act("You focus ki and aim a pulsing light at your body. You feel the poison in your blood disappear!",
                        true, ch, NULL, NULL, TO_CHAR);
                    act("$n focuses ki and aims a pulsing light at $s body. $n smiles.", true, ch, NULL, NULL,
                        TO_ROOM);
            ...
            if (!can_kill(ch, vict, NULL, 2)) {
                ...
                if (!AFF_FLAGGED(vict, AFF_POISON)) {
                    ...
                    if (((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) / 20) {
                        ...
                        result = ch_decCurKI(ch, amt, 0);
                        reveal_hiding(ch, 0);
                        act("You focus ki and aim a pulsing light at $N's body. Nothing happens.", true, ch, NULL, vict,
                            TO_CHAR);
                        ...
                        result = ch_decCurKI(ch, amt, 0);
                        reveal_hiding(ch, 0);
                        act("You focus ki and aim a pulsing light at $N's body. $e is cured.", true, ch, NULL, vict,
                            TO_CHAR);
                        act("$n focuses ki and aims a pulsing light at your body. You have been cured of your poison!",
                            true, ch, NULL, vict, TO_VICT);
                        act("$n focuses ki and aims a pulsing light at $N's body. $N smiles.", true, ch, NULL, vict,
                            TO_NOTVICT);
        ...
        if (!can_kill(ch, vict, NULL, 0)) {
            ...
            if (AFF_FLAGGED(vict, AFF_POISON)) {
                ...
                if (((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(ch) / 20) {
                    ...
                    result = ch_decCurKI(ch, amt, 0);
                    reveal_hiding(ch, 0);
                    act("You focus ki and fling poison at $N. You missed!", true, ch, NULL, vict, TO_CHAR);
                    act("$n focuses ki and flings poison at you, but misses!", true, ch, NULL, vict, TO_VICT);
                    act("$n focuses ki and flings poison at $N, but misses!", true, ch, NULL, vict, TO_NOTVICT);
                    return;
                } else {
                    int64_t amt = ch_getMaxKI(ch) / 20;
                    int64_t result;
                    result = ch_decCurKI(ch, amt, 0);
                    reveal_hiding(ch, 0);
                    act("You focus ki and fling poison at $N! The poison burns into $s skin!", true, ch, NULL, vict,
                        TO_CHAR);
                    act("$n focuses ki and flings poison at you! The poison burns into your skin!", true, ch, NULL, vict,
                        TO_VICT);
                    act("$n focuses ki and flings poison at $N! The poison burns into $s skin!", true, ch, NULL, vict,
                        TO_NOTVICT);
                    if (IS_NPC(vict)) {
                        set_fighting(vict, ch);
                    }
                    if (IS_MUTANT(vict) && (GET_GENOME(vict, 0) == 7 || GET_GENOME(vict, 1) == 7)) {
                        act("However $N seems unaffected by the poison.", true, ch, NULL, vict, TO_CHAR);
                        act("Your natural immunity to poison prevents it from affecting you.", true, ch, NULL, vict,
                            TO_VICT);
                        act("However $N seems unaffected by the poison.", true, ch, NULL, vict, TO_NOTVICT);
                    } else {
                        vict->poisonby = ch;
                        if (GET_CHARGE(ch) > 0) {
...
ACMD(do_kaioken) {
    ...
    int64_t cost_unit = ch_getMaxKI(ch) / 50;
    ...
    if (init_skill(ch, SKILL_KAIOKEN) < roll) {
        ...
        act("$n tries to use kaioken but messes up somehow.", true, ch, NULL, NULL, TO_ROOM);
        WAIT_STATE(ch, PULSE_1SEC);
        return;
    }

    if (ch_getMaxHealth(ch) < kaioken_levels[x-1]) {
...
ACMD(do_plant) {
    ...
    if (roll <= detect && roll <= fail) {
        ...
        if (GET_OBJ_WEIGHT(obj) + ((ch_getCurCarriedWeight(vict))) > CAN_CARRY_W(vict)) {
            reveal_hiding(ch, 0);
            act("@C$n@w tries to plant $p@w on you!@n", true, ch, obj, vict, TO_VICT);
            act("@C$n@w tries to plant $p@w on @c$N@w!@n", true, ch, obj, vict, TO_NOTVICT);
...
ACMD(do_forgery) {
    ...
    reveal_hiding(ch, 0);
    act("@c$n@w looks at $p, begins to work on forging a fake copy of it.@n", true, ch, obj2, NULL, TO_ROOM);
    ...
    if (GET_SKILL(ch, SKILL_FORGERY) < axion_dice(0)) {
        ...
        send_to_char(ch, "You start to make a forgery of %s but screw up and waste your forgery kit..\r\n",
                     obj2->short_description);
        act("@c$n@w tried to duplicate $p but screws up somehow.@n", true, ch, obj2, NULL, TO_ROOM);
        obj_from_char(obj4);
        extract_obj(obj4);
        WAIT_STATE(ch, PULSE_2SEC);
        return;
    ...
    act("@c$n@w makes a perfect forgery of $p.@n", true, ch, obj2, NULL, TO_ROOM);
    WAIT_STATE(ch, PULSE_2SEC);
}
...
ACMD(do_appraise) {
    ...
    reveal_hiding(ch, 0);
    act("@c$n@w looks at $p, turning it over in $s hands.@n", true, ch, obj, NULL, TO_ROOM);
    ...
    if (GET_SKILL(ch, SKILL_APPRAISE) < axion_dice(-10)) {
        ...
        act("@c$n@w looks stumped about $p.@n", true, ch, obj, NULL, TO_ROOM);
        WAIT_STATE(ch, PULSE_2SEC);
        return;
    }
    ...
    if (OBJ_FLAGGED(obj, ITEM_BROKEN)) {
        ...
        act("@c$n@w looks at $p and frowns.@n", true, ch, obj, NULL, TO_ROOM);
        WAIT_STATE(ch, PULSE_2SEC);
        return;
    }
    ...
    if (OBJ_FLAGGED(obj, ITEM_FORGED)) {
        ...
        act("@c$n@w looks at $p with an angry face.@n", true, ch, obj, NULL, TO_ROOM);
        WAIT_STATE(ch, PULSE_2SEC);
        return;
    }
    found = false;
    ...
    act("@c$n@w looks at $p and nods, a satisfied look on $s face.@n", true, ch, obj, NULL, TO_ROOM);
    int percent = false;
...
ACMD(do_disguise) {
    ...
    if (PLR_FLAGGED(ch, PLR_DISGUISED)) {
        ...
        act("@C$n @wpulls off $s disguise and reveals $mself!", true, ch, NULL, NULL, TO_ROOM);
        return;
    }
    ...
    if (((ch_getCurST(ch))) < ((ch_getCurST(ch))) / 50) {
    ...
    if (skill > roll) {
        ...
        act("@C$n @wmanages to disguise $mself with some skilled manipulation of $s headwear.", true, ch, NULL,
            NULL, TO_ROOM);
        ...
        act("@C$n @wattempts and fails to disguise $mself properly and must try again.", true, ch, NULL, NULL,
            TO_ROOM);
        int64_t amt = ch_getMaxST(ch) / 50;
        int64_t result;
        result = ch_decCurST(ch, amt, 0);
        return;
    }
...
ACMD(do_zanzoken) {
    ...
    act("@wYou focus your ki, preparing to move at super speeds if necessary.@n", true, ch, NULL, NULL, TO_CHAR);
    int64_t result1;
    result1 = ch_decCurKI(ch, cost, 0);
...
ACMD(do_block) {
    ...
    if (!*arg) {
        ...
        if (BLOCKS(ch)) {
            act("@wYou stop blocking @c$N@w.@n", true, ch, NULL, BLOCKS(ch), TO_CHAR);
            act("@C$n@w stops blocking you.@n", true, ch, NULL, BLOCKS(ch), TO_VICT);
            act("@C$n@w stops blocking @c$N@w.@n", true, ch, NULL, BLOCKS(ch), TO_NOTVICT);
            vict = BLOCKS(ch);
            BLOCKED(vict) = NULL;
            BLOCKS(ch) = NULL;
            return;
    ...
    if (BLOCKS(ch)) {
        ...
        BLOCKED(vict) = ch;
        reveal_hiding(ch, 0);
        act("@wYou start blocking @c$N's@w escape.@n", true, ch, NULL, BLOCKS(ch), TO_CHAR);
        act("@C$n@w starts blocking your escape.@n", true, ch, NULL, BLOCKS(ch), TO_VICT);
        act("@C$n@w starts blocking @c$N's@w escape.@n", true, ch, NULL, BLOCKS(ch), TO_NOTVICT);
        return;
    } else {
        BLOCKS(ch) = vict;
        BLOCKED(vict) = ch;
...
ACMD(do_eyec) {
    ...
    if (PLR_FLAGGED(ch, PLR_EYEC)) {
        ...
        act("@wYou open your eyes.@n", true, ch, NULL, NULL, TO_CHAR);
        act("@C$n@w opens $s eyes.@n", true, ch, NULL, NULL, TO_ROOM);
    } else if (!PLR_FLAGGED(ch, PLR_EYEC)) {
        SET_BIT_AR(PLR_FLAGS(ch), PLR_EYEC);
        act("@wYou close your eyes.@n", true, ch, NULL, NULL, TO_CHAR);
        act("@C$n@w closes $s eyes.@n", true, ch, NULL, NULL, TO_ROOM);
    }
...
ACMD(do_solar) {
    ...
    for (vict = world[IN_ROOM(ch)].people; vict; vict = next_v) {
        ...
        if (vict == ch)
            ...
            act("@W$N@W is @YBLINDED@W!@n", true, ch, NULL, vict, TO_CHAR);
            act("@RYou are @YBLINDED@R!@n", true, ch, NULL, vict, TO_VICT);
            act("@W$N@W is @YBLINDED@W!@n", true, ch, NULL, vict, TO_NOTVICT);
        }
    }
    improve_skill(ch, SKILL_SOLARF, 0);
    int64_t result1;
...
ACMD(do_heal) {
    ...
    if (prob >= 100) {
        cost = GET_MAX_MANA(ch) / 20;
        heal = (ch_getEffMaxPL(vict)) / 5;
    } else if (prob >= 90) {
        cost = GET_MAX_MANA(ch) / 16;
        heal = (ch_getEffMaxPL(vict)) / 10;
    } else if (prob >= 75) {
        cost = GET_MAX_MANA(ch) / 14;
        heal = (ch_getEffMaxPL(vict)) / 12;
    } else if (prob >= 50) {
        cost = GET_MAX_MANA(ch) / 12;
        heal = (ch_getEffMaxPL(vict)) / 15;
    } else if (prob >= 25) {
        cost = GET_MAX_MANA(ch) / 10;
        heal = (ch_getEffMaxPL(vict)) / 20;
    } else if (prob < 25) {
    ...
    if (GET_HIT(vict) >= (ch_getEffMaxPL(vict))) {
        if (vict != ch) {
    ...
    if (GET_SUPPRESS(vict) > 0 && GET_HIT(vict) >= (((ch_getEffMaxPL(vict)) / 100) * GET_SUPPRESS(vict))) {
        send_to_char(ch, "They are already at full health.\r\n");
        return;
    }
    ...
    if (prob < perc) {
        if (vict != ch) {
            act("@WYou place your hands near @c$N@W, but fail to concentrate enough to heal them!@n", true, ch, NULL,
                vict, TO_CHAR);
            act("@C$n@W places $s hands near you, but nothing happens!@n", true, ch, NULL, vict, TO_VICT);
            act("@C$n@W places $s hands near @c$N@W, but nothing happens.", true, ch, NULL, vict, TO_NOTVICT);
            int64_t result;
            result = ch_decCurKI(ch, cost, 0);
        ...
        if (vict == ch) {
            act("@WYou place your hands on your body, but fail to concentrate to heal yourself!@n", true, ch, NULL,
                vict, TO_CHAR);
            act("@C$n@W places $s hands on $s body, but nothing happens.", true, ch, NULL, vict, TO_NOTVICT);
            int64_t result;
            result = ch_decCurKI(ch, cost, 0);
            improve_skill(ch, SKILL_HEAL, 0);
            WAIT_STATE(ch, PULSE_2SEC);
    ...
    if (vict != ch) {
        ...
        act("@WYou place your hands near @c$N@W and an orange glow surrounds $M!@n", true, ch, NULL, vict, TO_CHAR);
        act("@C$n@W places $s hands near you and an orange glow surrounds you!@n", true, ch, NULL, vict, TO_VICT);
        act("@C$n@W places $s hands near @c$N@W and an orange glow surrounds $M.", true, ch, NULL, vict, TO_NOTVICT);
        int64_t result1;
        result1 = ch_decCurKI(ch, cost, 0);
        int64_t result2;
        result2 = ch_incCurHealth(vict, heal, true);

        if (IS_NAIL(ch)) {
            if (GET_SKILL(ch, SKILL_HEAL) >= 100) {
                int64_t result;
                result = ch_incCurST(ch, heal * .4, true);
                send_to_char(vict, "@GYou feel some of your stamina return as well!@n\r\n");
            } else if (GET_SKILL(ch, SKILL_HEAL) >= 60) {
                int64_t result;
                result = ch_incCurST(ch, heal * .2, true);
                send_to_char(vict, "@GYou feel some of your stamina return as well!@n\r\n");
            } else if (GET_SKILL(ch, SKILL_HEAL) >= 40) {
                int64_t result;
                result = ch_incCurST(ch, heal * .1, true);
                send_to_char(vict, "@GYou feel some of your stamina return as well!@n\r\n");
            }
        }
        ...
        if (AFF_FLAGGED(vict, AFF_BURNED)) {
            ...
            act("$n@w's burns are now healed.@n", true, vict, NULL, NULL, TO_ROOM);
        ...
        if (AFF_FLAGGED(vict, AFF_HYDROZAP)) {
            ...
            act("$n@w no longer looks as if they could drink an ocean.@n", true, vict, NULL, NULL, TO_ROOM);
        ...
        if (((ch_getCurLF(vict))) <= ((ch_getMaxLF(vict))) * 0.5 && !IS_ANDROID(vict)) {
            int64_t amt = ((ch_getMaxLF(ch))) * .35;
            int64_t result;
            result = ch_incCurLF(vict, amt, true);
    ...
    if (vict == ch) {
        ...
        act("@WYou place your hands on your body and an orange glow surrounds you!@n", true, ch, NULL, vict,
            TO_CHAR);
        act("@C$n@W places $s hands on $s body and an orange glow surrounds $m.", true, ch, NULL, vict, TO_NOTVICT);
        int64_t result1;
        result1 = ch_decCurKI(ch, cost, 0);
        int64_t result2;
        result2 = ch_incCurHealth(vict, heal, true);
...
ACMD(do_barrier) {
    ...
    if (AFF_FLAGGED(ch, AFF_SANCTUARY) && !strcasecmp("release", arg)) {
        act("@BYou dispel your barrier, releasing its energy.@n", true, ch, NULL, NULL, TO_CHAR);
        act("@B$n@B dispels $s barrier, releasing its energy.@n", true, ch, NULL, NULL, TO_ROOM);
        GET_BARRIER(ch) = 0;
    ...
    if (size > prob) {
        ...
        if (GET_SKILL(ch, SKILL_BARRIER)) {
            act("@BYou shout as you form a barrier of ki around your body!@n", true, ch, NULL, NULL, TO_CHAR);
            act("@B$n@B shouts as $e forms a barrier of ki around $s body!@n", true, ch, NULL, NULL, TO_ROOM);
        } else {
            act("@BYou shout as you form a barrier of ki and raging waters around your body!@n", true, ch, NULL,
                NULL, TO_CHAR);
            act("@B$n@B shouts as $e forms a barrier of ki and raging waters around $s body!@n", true, ch, NULL,
                NULL, TO_ROOM);
        }
...
ACMD(do_instant) {
    ...
    if (((ch_getMaxKI(ch) * ch->energy)) - cost < 0) {
    ...
    if (tar != NULL) {
        ...
        act("@wPlacing two fingers on your forehead you close your eyes and concentrate. Accelerating to such a speed that you move through the molecules of the universe faster than the speed of light. You stop as you arrive at $N@w!@n",
            true, ch, NULL, tar, TO_CHAR);
        act("@w$n@w appears in an instant out of nowhere right next to you!@n", true, ch, NULL, tar, TO_VICT);
        act("@w$n@w places two fingers on $s forehead and disappears in an instant!@n", true, ch, NULL, tar,
            TO_NOTVICT);
        ...
        act("@wPlacing two fingers on your forehead you close your eyes and concentrate. Accelerating to such a speed that you move faster than light and arrive almost instantly at your destination. Having located the planet by its collective population's ki.@n",
            true, ch, NULL, NULL, TO_CHAR);
        act("@w$n@w places two fingers on $s forehead and disappears in an instant!@n", true, ch, NULL, NULL,
            TO_NOTVICT);
...
ACMD(do_transform) {
    ...
    if (!*arg) {
        ch_displayForms(ch);
        if (trans_req(ch, 1) > 0) {
            ch_displayTransReq(ch);
        }
        return;
    }/* End of No Argument */

    struct transform_bonus cur_form = ch_getCurForm(ch);
    bool can_revert = race_canRevert(ch->race);

    // If we are in kaioken or something weird like that, prevent transforming.
    if (!ch_checkCanTransform(ch)) {
        return;
    }
    ...
    if (!strcasecmp("revert", arg)) {
        ...
        ch_echoRevert(ch, ch_transFlagToTier(cur_form.flag));
    ...
    struct transform_bonus trans = ch_findForm(ch, arg);
    if (!trans.flag) {
        send_to_char(ch, "You don't have that form.\r\n");
        return;
    }

    int to_tier = ch_transFlagToTier(trans.flag);
    ...
    if (!npc && ((ch_getBasePL(ch))) < trans_req(ch, to_tier)) {
        send_to_char(ch, "You are not strong enough to handle that transformation!\r\n");
        return;
    }

    if (!npc && ((ch_getCurST(ch))) <= GET_MAX_MOVE(ch) * trans.drain) {
        send_to_char(ch, "You do not have enough stamina!");
        return;
    }
    ...
    if (!npc) {
        ...
        if (!ch_checkTransUnlock(ch, to_tier)) {
            return;
        }
    }
    ...
    int zone = 0;
    if (race_NoisyTransforms(ch->race)) {
...
ACMD(do_situp) {
    ...
    if (AFF_FLAGGED(ch, AFF_FLYING)) {
        ...
        if (ROOM_GRAVITY(IN_ROOM(ch)) == 0) {
            bonus = 1;
            act("@gYou do a situp.@n", true, ch, NULL, NULL, TO_CHAR);
            act("@g$n does a situp.@n", true, ch, NULL, NULL, TO_ROOM);
        } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 10) {
            bonus = rand_number(3, 7);
            act("@gYou do a situp, feeling the strain of gravity.@n", true, ch, NULL, NULL, TO_CHAR);
            act("@g$n does a situp, while sweating.@n", true, ch, NULL, NULL, TO_ROOM);
        } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 20) {
            bonus = rand_number(8, 14);
            act("@gYou do a situp, and feel gravity's pull.@n", true, ch, NULL, NULL, TO_CHAR);
            act("@g$n does a situp, while sweating.@n", true, ch, NULL, NULL, TO_ROOM);
        } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 30) {
            bonus = rand_number(14, 20);
            act("@gYou do a situp, and feel the burn.@n", true, ch, NULL, NULL, TO_CHAR);
            act("@g$n does a situp, while sweating.@n", true, ch, NULL, NULL, TO_ROOM);
            ...
            act("@gYou do a situp, and really strain against the gravity.@n", true, ch, NULL, NULL, TO_CHAR);
            act("@g$n does a situp, while sweating.@n", true, ch, NULL, NULL, TO_ROOM);
            ...
            act("@gYou do a situp, and it was a really hard one to finish.@n", true, ch, NULL, NULL, TO_CHAR);
            act("@g$n does a situp, while sweating profusely.@n", true, ch, NULL, NULL, TO_ROOM);
        } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 5000) {
...
ACMD(do_meditate) {
    ...
    if (!strcasecmp(arg, "expand")) {
        ...
        if (MINDLINK(ch) == NULL) {
            ...
            if (((ch_getMaxKI(ch) * ch->energy)) < GET_MAX_MANA(MINDLINK(ch)) * 0.05) {
                ...
                act("@rYou manage to break the mind link between you and @R$N@r!@n", false, ch, NULL, MINDLINK(ch),
                    TO_CHAR);
                act("$n closes their eyes for a few seconds.", false, ch, NULL, MINDLINK(ch), TO_ROOM);
                ...
                act("@rYou struggle to free your mind of @R$N's@r link, but fail!@n", false, ch, NULL, MINDLINK(ch),
                    TO_CHAR);
                act("$n closes their eyes for a few seconds, and appears to struggle quite a bit.", false, ch, NULL,
                    MINDLINK(ch), TO_ROOM);
                send_to_char(MINDLINK(ch), "@rYour mind linked target struggles to free their mind, but fails!@n\r\n");

                int64_t amt = GET_MAX_MANA(MINDLINK(ch)) * .05;
                int64_t result;
                result = ch_decCurKI(ch, amt, 0);
                return;
            }
    ...
    if (((ch_getMaxKI(ch) * ch->energy)) < cost) {
        ...
        /* Meditate Message */
        act("@cYou close your eyes and concentrate, lifting $p@c with your ki.@n", true, ch, obj, NULL, TO_CHAR);
        act("@c$n closes $s eyes and lifts $p@c with $s ki.@n", true, ch, obj, NULL, TO_ROOM);
        ...
        if (bonus != 1 && IS_DEMON(ch) && rand_number(1, 100) >= 80) {
            send_to_char(ch, "Your spirit magnifies the strength of your body! @D[@G+%s@D]@n\r\n",
                         add_commas(bonus / 2));
            int64_t result;
            result = ch_gainBasePL(ch, bonus / 2, false);
        }
...
ACMD(do_pushup) {
    ...
    if (AFF_FLAGGED(ch, AFF_FLYING)) {
        ...
        if (ROOM_GRAVITY(IN_ROOM(ch)) == 0) {
            bonus = 1;
            act("@gYou do a pushup.@n", true, ch, NULL, NULL, TO_CHAR);
            act("@g$n does a pushup.@n", true, ch, NULL, NULL, TO_ROOM);
        } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 10) {
            bonus = rand_number(3, 7);
            act("@gYou do a pushup, feeling the strain of gravity.@n", true, ch, NULL, NULL, TO_CHAR);
            act("@g$n does a pushup, while sweating.@n", true, ch, NULL, NULL, TO_ROOM);
        } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 20) {
            bonus = rand_number(8, 14);
            act("@gYou do a pushup, and feel gravity's pull.@n", true, ch, NULL, NULL, TO_CHAR);
            act("@g$n does a pushup, while sweating.@n", true, ch, NULL, NULL, TO_ROOM);
        } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 30) {
            bonus = rand_number(14, 20);
            act("@gYou do a pushup, and feel the burn.@n", true, ch, NULL, NULL, TO_CHAR);
            act("@g$n does a pushup, while sweating.@n", true, ch, NULL, NULL, TO_ROOM);
        } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 40) {
            bonus = rand_number(20, 35);
            act("@gYou do a pushup, and feel the burn.@n", true, ch, NULL, NULL, TO_CHAR);
            ...
            bonus = rand_number(180, 250);
            act("@gYou do a pushup, and really strain against the gravity.@n", true, ch, NULL, NULL, TO_CHAR);
            act("@g$n does a pushup, while sweating.@n", true, ch, NULL, NULL, TO_ROOM);
        } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 200) {
            bonus = rand_number(400, 600);
            act("@gYou do a pushup, and really strain against the gravity.@n", true, ch, NULL, NULL, TO_CHAR);
            ...
            bonus = rand_number(9000, 10000);
            act("@gYou do a pushup, and it was a really hard one to finish.@n", true, ch, NULL, NULL, TO_CHAR);
            act("@g$n does a pushup, while sweating profusely.@n", true, ch, NULL, NULL, TO_ROOM);
        } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 5000) {
            bonus = rand_number(15000, 20000);
            act("@gYou do a pushup, and it was a really hard one to finish.@n", true, ch, NULL, NULL, TO_CHAR);
            act("@g$n does a pushup, while sweating profusely.@n", true, ch, NULL, NULL, TO_ROOM);
        ...
        if (cost >= ch_getMaxPLTrans(ch) / 2) {
            send_to_char(ch, "This gravity is a great challenge for you!\r\n");
            bonus *= 10;
        } else {
            if (cost >= ch_getMaxPLTrans(ch) / 4) {
                send_to_char(ch, "This gravity is an awesome challenge for you!\r\n");
                bonus *= 8;
            } else if (cost >= ch_getMaxPLTrans(ch) / 10) {
                send_to_char(ch, "This gravity is a good challenge for you!\r\n");
                bonus *= 6;
            } else if (cost < ch_getMaxPLTrans(ch) / 1000) {
                send_to_char(ch, "This gravity is so easy to you, you could do it in your sleep....\r\n");
                bonus /= 8;
            } else if (cost < ch_getMaxPLTrans(ch) / 100) {
                send_to_char(ch, "This gravity is the opposite of a challenge for you....\r\n");
                bonus /= 5;
            } else if (cost < ch_getMaxPLTrans(ch) / 50) {
                send_to_char(ch, "This gravity is definitely not a challenge for you....\r\n");
                bonus /= 4;
            } else if (cost < ch_getMaxPLTrans(ch) / 30) {
                send_to_char(ch, "This gravity is barely a challenge for you....\r\n");
                bonus /= 3;
            } else if (cost < ch_getMaxPLTrans(ch) / 20) {
                send_to_char(ch, "This gravity is hardly a challenge for you....\r\n");
                bonus /= 2;
            } else {
                send_to_char(ch, "This gravity is just perfect for you....\r\n");
...
ACMD(do_spar) {
    if (IS_NPC(ch)) {
        ...
        if (PLR_FLAGGED(ch, PLR_SPAR)) {
            act("@wYou cease your sparring stance.@n", false, ch, NULL, NULL, TO_CHAR);
            act("@C$n@w ceases $s sparring stance.@n", false, ch, NULL, NULL, TO_ROOM);
        }
        if (!PLR_FLAGGED(ch, PLR_SPAR)) {
            act("@wYou move into your sparring stance.@n", false, ch, NULL, NULL, TO_CHAR);
            act("@C$n@w moves into $s sparring stance.@n", false, ch, NULL, NULL, TO_ROOM);
        }
...
static void check_eq(struct char_data *ch) {
    ...
    for (i = 0; i < NUM_WEARS; i++) {
        if (GET_EQ(ch, i)) {
            obj = GET_EQ(ch, i);
            if (OBJ_FLAGGED(obj, ITEM_BROKEN)) {
                act("@W$p@W falls apart and you remove it.@n", false, ch, obj, NULL, TO_CHAR);
                act("@W$p@W falls apart and @C$n@W remove it.@n", false, ch, obj, NULL, TO_ROOM);
                perform_remove(ch, i);
                return;
            }
            if (obj == GET_EQ(ch, WEAR_WIELD1) && GET_LIMBCOND(ch, 1) <= 0) {
                act("@WWithout your right arm you let go of @c$p@W!@n", false, ch, obj, NULL, TO_CHAR);
                act("@C$n@W lets go of @c$p@W!@n", false, ch, obj, NULL, TO_ROOM);
                perform_remove(ch, i);
                return;
            }
            if (obj == GET_EQ(ch, WEAR_WIELD2) && GET_LIMBCOND(ch, 2) <= 0) {
                act("@WWithout your left arm you let go of @c$p@W!@n", false, ch, obj, NULL, TO_CHAR);
                act("@C$n@W lets go of @c$p@W!@n", false, ch, obj, NULL, TO_ROOM);
                perform_remove(ch, i);
                return;
            }
...
void base_update() {
    ...
    if (INTERESTTIME != 0 && INTERESTTIME <= time(NULL) && time(NULL) != 0) {
        INTERESTTIME = time(NULL) + 86400;
        LASTINTEREST = time(NULL);
        save_mud_time(&time_info);
        cash = true;
        countch = true;
    }
    ...
    for (d = descriptor_list; d; d = d->next) {
        ...
        if (IS_NPC(d->character)) {
            ...
            if (IS_ANDROID(d->character) && ABSORBING(d->character) && rand_number(1, 10) >= 7) {
                ...
                if (((ch_getCurST(drained))) - drain2 < 0) {
                    drain2 = ((ch_getCurST(drained)));
                }
                if (((ch_getMaxKI(drained) * drained->energy)) - drain1 < 0) {
                    drain1 = ((ch_getMaxKI(drained) * drained->energy));
                }
                int64_t result;
                result = ch_incCurST(d->character, drain2, true);
                int64_t result1;
                result1 = ch_incCurKI(d->character, drain1, true);
                int64_t result2;
                result2 = ch_incCurHealth(d->character, drain1 * .5, true);

                if (ch_isFullKI(d->character) && ch_isFullST(d->character)) {
        ...
        int64_t forty_lf = ((ch_getMaxLF(d->character))) * 0.4;
        if (AFF_FLAGGED(d->character, AFF_CURSE) && ((ch_getCurLF(d->character))) > forty_lf) {
            int64_t result;
            result = ch_decCurLFPercent(d->character, .01, 0);
            demon_refill_lf(d->character, ((ch_getMaxLF(d->character))) * 0.01);

            if (((ch_getCurLF(d->character))) < forty_lf) {
                int64_t amt = forty_lf - ch_getCurLF(d->character);
                int64_t result1;
                result1 = ch_incCurLF(d->character, amt, true);
            }
        }
        ...
        if (PLR_FLAGGED(d->character, PLR_GOOP) && d->character->gooptime == 60) {
            if (IS_BIO(d->character)) {
                act("@GConciousness slowly returns to you. You realize quickly that some of your cells have survived. You take control of your regenerative processes and focus on growing a new body!@n",
                    true, d->character, NULL, NULL, TO_CHAR);
            } else {
                act("@MSlowly you regain conciousness. The various split off chunks of your body begin to likewise stir.@n",
                    true, d->character, NULL, NULL, TO_CHAR);
                act("@MYou think you notice the chunks of @m$n@M's moving slightly.@n", true, d->character, NULL,
                    NULL, TO_ROOM);
            }
            ...
            if (IS_BIO(d->character)) {
                ch_restoreHealth(d->character, true);
                ...
                zenkaiPL = ((ch_getBasePL(d->character))) * 1.03;
                zenkaiKi = ((d->character->baseki)) * 1.015;
                zenkaiSt = ((ch_getBaseST(d->character))) * 1.015;
                ...
                if (!IN_ARENA(d->character)) {
                    int64_t result2;
                    result2 = ch_gainBasePL(d->character, zenkaiPL, false);
                    int64_t result;
                    result = ch_gainBaseKI(d->character, zenkaiKi, false);
                    int64_t result1;
                    result1 = ch_gainBaseST(d->character, zenkaiSt, false);
        ...
        if (!IS_NPC(d->character) && ROOM_EFFECT(IN_ROOM(d->character)) >= 1 && rand_number(1, 100) >= 96) {
            if (ROOM_EFFECT(IN_ROOM(d->character)) <= 4) {
                switch (rand_number(1, 4)) {
                    case 1:
                        act("@RLava spews up violently from the cracks in the ground!@n", false, d->character, NULL,
                            NULL, TO_ROOM);
                        act("@RLava spews up violently from the cracks in the ground!@n", false, d->character, NULL,
                            NULL, TO_CHAR);
                        break;
                    case 2:
                        act("@RThe lava bubbles and gives off tremendous heat!@n", false, d->character, NULL,
                            NULL, TO_ROOM);
                        act("@RThe lava bubbles and gives off tremendous heat!@n", false, d->character, NULL,
                            NULL, TO_CHAR);
                        break;
                    case 3:
                        act("@RNoxious fumes rise from the bubbling lava!@n", false, d->character, NULL, NULL,
                            TO_ROOM);
                        act("@RNoxious fumes rise from the bubbling lava!@n", false, d->character, NULL, NULL,
                            TO_CHAR);
                        break;
                ...
                act("@RLava covers the entire area now!@n", false, d->character, NULL, NULL, TO_ROOM);
                act("@RLava covers the entire area now!@n", false, d->character, NULL, NULL, TO_CHAR);
                ROOM_EFFECT(IN_ROOM(d->character)) += 1;
            }
        }
        if (ABSORBING(d->character) && IN_ROOM(d->character) != IN_ROOM(ABSORBING(d->character))) {
        ...
        if (IS_ANDROID(d->character) && ABSORBING(d->character)) {
            if (((ch_getCurST(((d->character)->absorbing)))) < (GET_MAX_MOVE(d->character) / 15) &&
                ((ch_getMaxKI(((d->character)->absorbing)) * ((d->character)->absorbing)->energy)) < (GET_MAX_MANA(d->character) / 15)) {
                act("@WYou stop absorbing stamina and ki from @c$N as they don't have enough for you to take@W!@n",
                    true, d->character, NULL, ABSORBING(d->character), TO_CHAR);
                act("@C$n@W stops absorbing stamina and ki from you!@n", true, d->character, NULL,
                    ABSORBING(d->character), TO_VICT);
                act("@C$n@W stops absorbing stamina and ki from @c$N@w!@n", true, d->character, NULL,
        ...
        if (IS_ANDROID(d->character) && ABSORBING(d->character) && rand_number(1, 9) >= 6) {
            if (((ch_getCurST(((d->character)->absorbing)))) > (GET_MAX_MOVE(d->character) / 15) ||
                ...
                int64_t amt3 = ch_getMaxKI(d->character) * .08;
                int64_t result4;
                result4 = ch_incCurKI(d->character, amt3, true);
                int64_t amt1 = ch_getMaxST(d->character) * .08;
                int64_t result2;
                result2 = ch_incCurST(d->character, amt1, true);

                int64_t amt2 = ch_getMaxKI(d->character) / 20;
                int64_t result3;
                result3 = ch_decCurKI(ABSORBING(d->character), amt2, 1);
                int64_t amt = ch_getMaxST(d->character) / 20;
                int64_t result1;
                result1 = ch_decCurST(ABSORBING(d->character), amt, 1);

                act("@WYou absorb stamina and ki from @c$N@W!@n", true, d->character, NULL, ABSORBING(d->character),
                    TO_CHAR);
                act("@C$n@W absorbs stamina and ki from you!@n", true, d->character, NULL, ABSORBING(d->character),
                    TO_VICT);
                send_to_char(ABSORBING(d->character), "@wTry 'escape'!@n\r\n");
                act("@C$n@W absorbs stamina and ki from @c$N@w!@n", true, d->character, NULL,
                    ABSORBING(d->character), TO_NOTVICT);
                if (GET_HIT(d->character) < (ch_getEffMaxPL(d->character))) {
                    int64_t amt4 = ch_getMaxKI(d->character) * .04;
                    int64_t result;
                    result = ch_incCurHealth(d->character, amt4, true);
                    send_to_char(d->character,
                                 "@CYou convert a portion of the absorbed energy into refilling your powerlevel.@n\r\n");
                }

                if (ch_isFullST(d->character) && ch_isFullKI(d->character)) {

                    act("@WYou stop absorbing stamina and ki from @c$N as you are full@W!@n", true, d->character,
                        NULL, ABSORBING(d->character), TO_CHAR);
                    act("@C$n@W stops absorbing stamina and ki from you!@n", true, d->character, NULL,
                        ABSORBING(d->character), TO_VICT);
                    act("@C$n@W stops absorbing stamina and ki from @c$N@w!@n", true, d->character, NULL,
                        ABSORBING(d->character), TO_NOTVICT);
                ...
                bool sum = !ch_is_soft_cap(d->character, 0, 1.0);
                bool mum = !ch_is_soft_cap(d->character, 2, 1.0);
                bool ium = !ch_is_soft_cap(d->character, 1, 1.0);
                ...
                if (sum) {
                    if (rand_number(1, 8) >= 6) {
                        ...
                        int64_t result;
                        result = ch_gainBasePL(d->character, gain, false);
                    }
                }
                if (mum) {
                    if (rand_number(1, 8) >= 6) {
                        ...
                        int64_t result;
                        result = ch_gainBaseST(d->character, gain, false);
                    }
                }
                if (ium) {
                    if (rand_number(1, 8) >= 6) {
                        ...
                        int64_t result;
                        result = ch_gainBaseKI(d->character, gain, false);
                    }
                }
                if (!sum) {
                    if (rand_number(1, 8) >= 6) {
...
ACMD(do_snet) {
    ...
    if (channel > 0) {
        ...
        act("@wYou push some buttons on $p@w and change its channel.", true, ch, obj, NULL, TO_CHAR);
        act("@C$n@w pushes some buttons on $p@w and changes its channel.", true, ch, obj, NULL, TO_ROOM);
        return;
    } else {
        ...
        if (call <= -1) {
            if (global == false) {
                ...
                *hist = '\0';
                ...
                act(over, true, ch, NULL, NULL, TO_ROOM);
...
ACMD(do_scouter) {
    ...
    if (!obj) {
        ...
        if (IS_ANDROID(vict)) {
            act("$n points $s scouter at you.", false, ch, NULL, vict, TO_VICT);
            act("$n points $s scouter at $N.", false, ch, NULL, vict, TO_NOTVICT);
            send_to_char(ch, "@D,==================================|@n\r\n");
            ...
            if (OBJ_FLAGGED(obj, ITEM_BSCOUTER) && GET_HIT(vict) >= 150000) {
                ...
                act("$n's scouter explodes!", false, ch, NULL, NULL, TO_ROOM);
                extract_obj(obj);
                save_char(ch);
                return;
                ...
                int64_t stam = ((ch_getCurST(vict))), mstam = GET_MAX_MOVE(vict);
                if (stam <= 0)
                    stam = 1;
                if (mstam <= 0)
                    mstam = 1;
...
ACMD(do_quit) {
    ...
    if (subcmd != SCMD_QUIT)
        ...
        act("$n has left the game.", true, ch, NULL, NULL, TO_ROOM);
...
ACMD(do_steal) {
    ...
    if (((ch_getCurST(ch))) < (GET_MAX_MOVE(ch) / 40) + IS_CARRYING_W(ch)) {
        send_to_char(ch, "You do not have enough stamina.\r\n");
        return;
    }
    ...
    if (axion_dice(0) > 100 && GET_POS(vict) != POS_SLEEPING) { /* Unlucky! */
        reveal_hiding(ch, 0);
        act("@r$N@R just happens to glance in your direction! What terrible luck!@n", true, ch, NULL, vict, TO_CHAR);
        act("@RYou just happen to glance behind you and spot @r$n@R trying to STEAL from you!@n", true, ch, NULL,
            vict, TO_VICT);
        act("@r$N@R just happens to glance in @r$n's@R direction and catches $m trying to STEAL!@n", true, ch, NULL,
            vict, TO_NOTVICT);
        prob = -1000;
    }
    ...
    if (prob + 20 < perc && GET_POS(vict) != POS_SLEEPING) { /* Critical failure! */
        reveal_hiding(ch, 0);
        act("@rYou are caught trying to stick your hand in @R$N's@r possessions!@n", true, ch, NULL, vict, TO_CHAR);
        act("@rYou catch @R$n@r trying to rummage through your possessions!@n", true, ch, NULL, vict, TO_VICT);
        act("@R$n@R is caught by @R$N@r as $e sticks $s hand in @R$N's@r possessions!@n", true, ch, NULL, vict,
            TO_NOTVICT);
        WAIT_STATE(ch, PULSE_3SEC);
        if (IS_NPC(vict)) {
            set_fighting(vict, ch);
        ...
        if (!strcasecmp(arg, "zenni")) { /* MOOLA! */
            if (prob > perc) {
                if (GET_GOLD(vict) > 0) {
                    ...
                    if (axion_dice(0) > prob) {
                        ...
                        reveal_hiding(ch, 0);
                        act("@R$n@r just stole zenni from @R$N@r!@n", true, ch, NULL, vict, TO_ROOM);
                ...
                reveal_hiding(ch, 0);
                act("@rYou are caught trying to steal zenni from @R$N@r!@n", true, ch, NULL, vict, TO_CHAR);
                act("@rYou catch @R$n's@r hand trying to snatch your zenni!@n", true, ch, NULL, vict, TO_VICT);
                act("@R$N@r catches @R$n's@r hand trying to snatch $S zenni!@n", true, ch, NULL, vict, TO_NOTVICT);
                WAIT_STATE(ch, PULSE_3SEC);
                if (IS_NPC(vict)) {
                    set_fighting(vict, ch);
                }
            ...
            if (!(obj = get_obj_in_list_vis(ch, arg, NULL, vict->carrying))) {
                ...
                if (!obj) {
                    act("$E isn't wearing that item.", false, ch, NULL, vict, TO_CHAR);
                    return;
                } else if (GET_POS(vict) > POS_SLEEPING) {
                    send_to_char(ch,
                    ...
                    if (GET_OBJ_WEIGHT(obj) + ((ch_getCurCarriedWeight(ch))) > CAN_CARRY_W(ch)) {
...
ACMD(do_visible) {
    ...
    if (AFF_FLAGGED(ch, AFF_ETHEREAL) && affectedv_by_spell(ch, ART_EMPTY_BODY)) {
        ...
        if (AFF_FLAGGED(ch, AFF_ETHEREAL)) {
            ...
            if (!appeared)
                act("$n flashes into existence.", false, ch, NULL, NULL, TO_ROOM);
        }
        appeared = 1;
    }
...
perform_group(struct char_data *ch, struct char_data *vict, int highlvl, int lowlvl, int64_t highpl, int64_t lowpl) {
    ...
    if (GET_BONUS(vict, BONUS_LONER) > 0) {
        act("$n is the loner type and refuses to be in your group.", true, vict, NULL, ch, TO_VICT);
        return (0);
    }
...
static void print_group(struct char_data *ch) {
    ...
    if (!AFF_FLAGGED(ch, AFF_GROUP))
        ...
        if (AFF_FLAGGED(k, AFF_GROUP)) {
            send_to_char(ch, "@D----------------@n\r\n");
            if (GET_HIT(k) > (ch_getEffMaxPL(k)) / 10) {
                snprintf(buf, sizeof(buf),
                         "@gL@D: @w$N @W- @D[@RPL@Y: @c%s @CKi@Y: @c%s @GST@Y: @c%s@D] [@w%2d %s %s@D]@n",
                         add_commas(GET_HIT(k)), add_commas(((ch_getMaxKI(k) * k->energy))), add_commas(((ch_getCurST(k)))), GET_LEVEL(k),
                         CLASS_ABBR(k), RACE_ABBR(k));
            }
            if (GET_HIT(k) <= ((ch_getEffMaxPL(k)) - ((ch_getCurCarriedWeight(k)))) / 10) {
                snprintf(buf, sizeof(buf),
                         "@gL@D: @w$N @W- @D[@RPL@Y: @r%s @CKi@Y: @c%s @GST@Y: @c%s@D] [@w%2d %s %s@D]@n",
                         add_commas(GET_HIT(k)), add_commas(((ch_getMaxKI(k) * k->energy))), add_commas(((ch_getCurST(k)))), GET_LEVEL(k),
                         CLASS_ABBR(k), RACE_ABBR(k));
            }
            act(buf, false, ch, NULL, k, TO_CHAR);
        }
        ...
        for (f = k->followers; f; f = f->next) {
            if (!AFF_FLAGGED(f->follower, AFF_GROUP))
                continue;
            send_to_char(ch, "@D----------------@n\r\n");
            if (GET_HIT(f->follower) > ((ch_getEffMaxPL(f->follower)) - ((ch_getCurCarriedWeight(f->follower)))) / 10) {
                snprintf(buf, sizeof(buf),
                         "@gF@D: @w$N @W- @D[@RPL@Y: @c%s @CKi@Y: @c%s @GST@Y: @c%s@D] [@w%2d %s %s@D]",
                         add_commas(GET_HIT(f->follower)), add_commas(((ch_getMaxKI(f->follower) * f->follower->energy))), add_commas(
                                ((ch_getCurST(f->follower)))),
            ...
            act(buf, false, ch, NULL, f->follower, TO_CHAR);
        }
        send_to_char(ch, "@D----------------@n\r\n");
    }
}
...
ACMD(do_group) {
    ...
    lowlvl = GET_LEVEL(ch);
    highpl = (ch_getEffMaxPL(ch));
    lowpl = (ch_getEffMaxPL(ch));
    ...
    if (!(vict = get_char_vis(ch, buf, NULL, FIND_CHAR_ROOM)))
        ...
        act("$N must follow you to enter your group.", false, ch, NULL, vict, TO_CHAR);
    else {
        if (!AFF_FLAGGED(vict, AFF_GROUP)) {
            ...
            if (ch != vict)
                act("$N is no longer a member of your group.", false, ch, NULL, vict, TO_CHAR);
            act("You have been kicked out of $n's group!", false, ch, NULL, vict, TO_VICT);
            act("$N has been kicked out of $n's group!", false, ch, NULL, vict, TO_NOTVICT);
...
ACMD(do_ungroup) {
    ...
    act("$N has been kicked out of $n's group!", false, ch, NULL, tch, TO_NOTVICT);
...
ACMD(do_report) {
    ...
    snprintf(buf, sizeof(buf), "$n reports: %" I64T "/%" I64T "H, %" I64T "/%" I64T "M, %" I64T "/%" I64T "V\r\n",
             GET_HIT(ch), (ch_getEffMaxPL(ch)),
             ((ch_getMaxKI(ch) * ch->energy)), GET_MAX_MANA(ch),
             ((ch_getCurST(ch))), GET_MAX_MOVE(ch));
                ...
                switch (GET_OBJ_VNUM(mag_item)) {
                    case 381: {
                        ...
                        if (GET_SKILL(ch, SKILL_FIRST_AID) > 0) {
                            ...
                            int64_t amt = ch_getMaxST(ch) * .25;
                            int64_t result;
                            result = ch_incCurST(ch, amt, true);
                        } else {
                            int64_t amt = ch_getMaxST(ch) * .1;
                            int64_t result;
                            result = ch_incCurST(ch, amt, true);
                        }
                    ...
                    case 382:
                        if (AFF_FLAGGED(ch, AFF_BURNED)) {
                            act("@WYou gently apply the salve to your burns.@n", true, ch, mag_item, NULL, TO_CHAR);
                            act("@C$n@W gently applies a burn salve to $s burns.@n", true, ch, mag_item, NULL,
                                TO_ROOM);
                    ...
                    case 385:
                        act("@WYou drink the contents of the vial before disposing of it.@n", true, ch, mag_item,
                            NULL, TO_CHAR);
                        act("@C$n@W dinks a $p and then disposes of it.@n", true, ch, mag_item, NULL, TO_ROOM);
                        if (AFF_FLAGGED(ch, AFF_BLIND)) {
                            act("@WYour eyesight has returned!@n", true, ch, mag_item, NULL, TO_CHAR);
                            act("@C$n@W eyesight seems to have returned.@n", true, ch, mag_item, NULL, TO_ROOM);
                            null_affect(ch, AFF_BLIND);
                        }
                        refreshed = false;

                        if (GET_HIT(ch) <= (ch_getEffMaxPL(ch)) * 0.99) {
                            int64_t amt2 = large_rand((ch_getEffMaxPL(ch)) * 0.08, (ch_getEffMaxPL(ch)) * 0.16);
                            int64_t result2;
                            result2 = ch_incCurHealth(ch, amt2, true);
                            refreshed = true;
                        } else {
                            if (((ch_getMaxKI(ch) * ch->energy)) <= (ch_getEffMaxPL(ch)) * 0.99) {
                                int64_t amt1 = large_rand(GET_MAX_MANA(ch) * 0.08, GET_MAX_MANA(ch) * 0.16);
                                int64_t result1;
                                result1 = ch_incCurKI(ch, amt1, true);
                                refreshed = true;
                            } else {
                                if (((ch_getCurST(ch))) <= GET_MAX_MOVE(ch) * 0.99) {
                                    int64_t amt = large_rand(GET_MAX_MOVE(ch) * 0.08, GET_MAX_MOVE(ch) * 0.16);
                                    int64_t result;
                                    result = ch_incCurST(ch, amt, true);
                                    refreshed = true;
                                }
        ...
        switch (subcmd) {
            case SCMD_WIMPY:
                ...
                if ((value_lev = atoi(arg)) != 0) {
                    ...
                    else if (value_lev > (ch_getEffMaxPL(ch)))
                        send_to_char(ch, "That doesn't make much sense, now does it?\r\n");
                    else if (value_lev > ((ch_getEffMaxPL(ch)) * 0.5))
    ...
    switch (subcmd) {
        ...
        case SCMD_AFK:
            ...
            if (PRF_FLAGGED(ch, PRF_AFK))
                act("$n has gone AFK.", true, ch, NULL, NULL, TO_ROOM);
            else {
                act("$n has come back from AFK.", true, ch, NULL, NULL, TO_ROOM);
...
ACMD(do_break) {
    ...
    act("$n ruins $p.", false, ch, obj, NULL, TO_ROOM);
...
ACMD(do_fix) {
    ...
    if (self == false) {
        if (GET_SKILL(ch, SKILL_REPAIR) < axion_dice(0)) {
            act("You try to repair $p but screw up..", true, ch, obj, NULL, TO_CHAR);
            act("$n tries to repair $p but screws up..", true, ch, obj, NULL, TO_ROOM);
            extract_obj(obj4);
        ...
        if (GET_OBJ_VAL(obj, VAL_ALL_HEALTH) + GET_SKILL(ch, SKILL_REPAIR) < 100) {
            ...
            act("$n repairs $p a bit.", false, ch, obj, NULL, TO_ROOM);
            ...
            act("$n repairs $p completely.", false, ch, obj, NULL, TO_ROOM);
        ...
        if (GET_HIT(ch) >= (ch_getEffMaxPL(ch))) {
            ...
            act("You try to repair your body but screw up..", true, ch, NULL, NULL, TO_CHAR);
            act("$n tries to repair $s body but screws up..", true, ch, NULL, NULL, TO_ROOM);
            extract_obj(obj4);
            improve_skill(ch, SKILL_REPAIR, 1);
            WAIT_STATE(ch, PULSE_5SEC);
            return;
        } else {
            act("You use the repair kit to fix part of your body...", true, ch, NULL, NULL, TO_CHAR);
            act("$n works on their body with a repair kit.", true, ch, NULL, NULL, TO_ROOM);
            int64_t mult = GET_SKILL(ch, SKILL_REPAIR);
            int64_t add = (((ch_getEffMaxPL(ch)) * 0.005) + 10) * mult;

            extract_obj(obj4);
            if (ch_incCurHealth(ch, add, true) == ch_getEffMaxPL(ch)) {
...
ACMD(do_aid) {
    ...
    if (num == 47) {
        ...
        if (!AFF_FLAGGED(vict, AFF_SPIRIT) && !PLR_FLAGGED(vict, PLR_BANDAGED)) {
            ...
            act("$n attempts to bandage $N's wounds.", true, ch, NULL, vict, TO_ROOM);
            dc = axion_dice(0);
            if ((GET_SKILL(ch, SKILL_FIRST_AID) + 1) > dc) {
                send_to_char(ch, "You bandage %s's wounds.\r\n", GET_NAME(vict));
                int64_t roll = (((ch_getEffMaxPL(vict)) / 100) * (GET_WIS(ch) / 4)) + (ch_getEffMaxPL(vict)) * 0.25;
                if (GET_BONUS(ch, BONUS_HEALER) > 0) {
                    roll += roll * .1;
                }
                int64_t result;
                result = ch_incCurHealth(vict, roll, true);

                send_to_char(vict, "Your wounds are bandaged by %s!\r\n", GET_NAME(ch));
                act("$n's wounds are stablized by $N!", true, vict, NULL, ch, TO_NOTVICT);
    ...
    if (!strcasecmp(arg, "light")) {
        if (GET_SKILL(ch, SKILL_FOCUS) < 75 || GET_SKILL(ch, SKILL_CONCENTRATION) < 75) {
            ...
            if (PLR_FLAGGED(ch, PLR_AURALIGHT)) {
                ...
                act("$n's aura fades as they stop shining light on the area.", true, ch, NULL, NULL, TO_ROOM);
                REMOVE_BIT_AR(PLR_FLAGS(ch), PLR_AURALIGHT);
                world[IN_ROOM(ch)].light--;

            } else {
                if (((ch_getMaxKI(ch) * ch->energy)) > GET_MAX_MANA(ch) * 0.12) {
                    if (IN_ROOM(ch) != NOWHERE) {
                        world[IN_ROOM(ch)].light++;
                    }
                    reveal_hiding(ch, 0);
                    int64_t result;
                    result = ch_decCurKIPercent(ch, .12, 0);
                    send_to_char(ch,
