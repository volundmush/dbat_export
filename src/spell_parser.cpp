...
int call_magic(struct char_data *caster, struct char_data *cvict,
    ...
    if (ROOM_FLAGGED(IN_ROOM(caster), ROOM_PEACEFUL) && GET_ADMLEVEL(caster) < ADMLVL_IMPL &&
        ...
        act("White light from no particular source suddenly fills the room, then vanishes.", false, caster, NULL,
            NULL, TO_ROOM);
        return (0);
    }
...
void mag_objectmagic(struct char_data *ch, struct obj_data *obj,
    ...
    switch (GET_OBJ_TYPE(obj)) {
        case ITEM_STAFF:
            act("You tap $p three times on the ground.", false, ch, obj, NULL, TO_CHAR);
            if (obj->action_description)
                act(obj->action_description, false, ch, obj, NULL, TO_ROOM);
            else
                act("$n taps $p three times on the ground.", false, ch, obj, NULL, TO_ROOM);

            if (GET_OBJ_VAL(obj, VAL_STAFF_CHARGES) <= 0) {
                send_to_char(ch, "It seems powerless.\r\n");
                act("Nothing seems to happen.", false, ch, obj, NULL, TO_ROOM);
            } else {
