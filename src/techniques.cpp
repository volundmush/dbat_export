...
bool tech_handle_zanzoken(struct char_data *ch, struct char_data *vict, const char* name) {
    ...
    if (((!IS_NPC(vict) && IS_ICER(vict) && rand_number(1, 30) >= 28) || AFF_FLAGGED(vict, AFF_ZANZOKEN)) &&
        ...
        if (!AFF_FLAGGED(ch, AFF_ZANZOKEN) || (AFF_FLAGGED(ch, AFF_ZANZOKEN) && GET_SPEEDI(ch) + rand_number(1, 5) <
            ...
            snprintf(msg, MAX_STRING_LENGTH, "@C$N@c disappears, avoiding your %s before reappearing!@n", name);
            act(msg, true, ch, NULL, vict, TO_CHAR);
            snprintf(msg, MAX_STRING_LENGTH, "@cYou disappear, avoiding @C$n's@c %s before reappearing!@n", name);
            act(msg, true, ch, NULL, vict, TO_VICT);
            snprintf(msg, MAX_STRING_LENGTH, "@C$N@c disappears, avoiding @C$n's@c %s before reappearing!@n", name);
            ...
            act("@C$N@c disappears, trying to avoid your attack but your zanzoken is faster!@n", false, ch, NULL,
                vict, TO_CHAR);
            act("@cYou zanzoken to avoid the attack but @C$n's@c zanzoken is faster!@n", false, ch, NULL, vict,
                TO_VICT);
...
void tech_handle_posmodifier(struct char_data *vict, int* pry, int* blk, int* dge, int* prob) {
    switch (GET_POS(vict)) {
        case POS_SLEEPING:
            *pry = 0;
            *blk = 0;
...
bool tech_handle_charge(struct char_data *ch, char *arg, double minimum, double *attperc) {
    if (*arg) {
...
bool tech_handle_targeting(struct char_data *ch, char *arg, struct char_data **vict, struct obj_data **obj) {
    *vict = NULL;
    *obj = NULL;
...
void tech_handle_fireshield(struct char_data *ch, struct char_data *vict, const char* part) {
    static char msg[MAX_STRING_LENGTH];
    if (GET_HIT(vict) > 0 && !AFF_FLAGGED(vict, AFF_SPIRIT) && AFF_FLAGGED(vict, AFF_FIRESHIELD) &&
        !GET_BONUS(ch, BONUS_FIREPROOF) && !IS_DEMON(ch)) {
        snprintf(msg, MAX_STRING_LENGTH, "@c$N's@W fireshield burns your %s!@n", part);
        act(msg, true, ch, NULL, vict, TO_CHAR);
        snprintf(msg, MAX_STRING_LENGTH, "@C$n's@W %s is burned by your fireshield!@n", part);
        act(msg, true, ch, NULL, vict, TO_VICT);
        snprintf(msg, MAX_STRING_LENGTH, "@c$n's@W %s is burned by @C$N's@W fireshield!@n", part);
        act(msg, true, ch, NULL, vict, TO_NOTVICT);
...
bool tech_handle_android_absorb(struct char_data *ch, struct char_data *vict) {
    if (IS_ANDROID(vict) && HAS_ARMS(vict) && GET_SKILL(vict, SKILL_ABSORB) > rand_number(1, 140)) {
        ...
        act("@WYou absorb @C$n's@W ki attack and all $s charged ki with your hand!@n", true, ch, NULL, vict,
            TO_VICT);
        act("@C$N@W absorbs @c$n's@W ki attack and all $s charged ki with $S hand!@n", true, ch, NULL, vict,
            TO_NOTVICT);
        ...
        if (GET_CHARGE(vict) + amot > GET_MAX_MANA(vict)) {
            int64_t amt = ch_getMaxKI(vict) - GET_CHARGE(vict);
            int64_t result;
            result = ch_incCurKI(vict, amt, true);
...
bool tech_handle_crashdown(struct char_data *ch, struct char_data *vict) {
    if (AFF_FLAGGED(vict, AFF_FLYING)) {
        act("@w$N@w is knocked out of the air!@n", true, ch, NULL, vict, TO_CHAR);
        act("@wYou are knocked out of the air!@n", true, ch, NULL, vict, TO_VICT);
        act("@w$N@w is knocked out of the air!@n", true, ch, NULL, vict, TO_NOTVICT);
