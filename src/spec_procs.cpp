...
SPECIAL(dump) {
    ...
    if (value) {
        ...
        act("$n has been awarded for being a good citizen.", true, ch, NULL, NULL, TO_ROOM);
...
SPECIAL(mayor) {
    ...
    switch (path[path_index]) {
        ...
        case 'W':
            ...
            act("$n awakens and groans loudly.", false, ch, NULL, NULL, TO_ROOM);
            break;

        case 'S':
            GET_POS(ch) = POS_SLEEPING;
            act("$n lies down and instantly falls asleep.", false, ch, NULL, NULL, TO_ROOM);
            break;

        case 'a':
            act("$n says 'Hello Honey!'", false, ch, NULL, NULL, TO_ROOM);
            act("$n smirks.", false, ch, NULL, NULL, TO_ROOM);
            break;
        ...
        case 'd':
            act("$n says 'Good day, citizens!'", false, ch, NULL, NULL, TO_ROOM);
            break;

        case 'e':
            act("$n says 'I hereby declare the bazaar open!'", false, ch, NULL, NULL, TO_ROOM);
            break;

        case 'E':
            act("$n says 'I hereby declare Midgaard closed!'", false, ch, NULL, NULL, TO_ROOM);
            break;
...
SPECIAL(gauntlet_room)  /* Jamdog - 13th Feb 2006 */
{
    ...
    if (CMD_IS("surrender")) {
        if (FIGHTING(ch)) {
            ...
            act("$n suddenly appears looking relieved after $s trial in the Gauntlet", false, ch, NULL, ch,
                TO_NOTVICT);
            act("You are returned to the start of the Gauntlet", false, ch, NULL, ch, TO_VICT);

            /* Hit point penalty for surrendering */
            int64_t result;
            result = ch_decCurHealth(ch, 2000, 0);
    ...
    for (i = 0; gauntlet_info[i][0] != -1; i++) {
        if (world[ch->in_room].number == gauntlet_info[i][1]) {
            if (cmd == gauntlet_info[i][2]) {
                ...
                if (proceed == 0) {
                    send_to_char(ch, buf);
                    act(buf2, false, ch, NULL, ch, TO_ROOM);
                    return true;
                }
            }
        }
...
void npc_steal(struct char_data *ch, struct char_data *victim) {
    ...
    if (AWAKE(victim) && (rand_number(0, GET_LEVEL(ch)) == 0)) {
        ...
        act("$n tries to steal zenni from $N.", true, ch, NULL, victim, TO_NOTVICT);
    } else {
...
SPECIAL(snake) {
    ...
    act("$n bites $N!", 1, ch, NULL, FIGHTING(ch), TO_NOTVICT);
    act("$n bites you!", 1, ch, NULL, FIGHTING(ch), TO_VICT);
    call_magic(ch, FIGHTING(ch), NULL, SPELL_POISON, GET_LEVEL(ch), CAST_SPELL, NULL);
    return (true);
}
...
SPECIAL(fido) {
    ...
    for (i = world[IN_ROOM(ch)].contents; i; i = i->next_content) {
        ...
        act("$n savagely devours a corpse.", false, ch, NULL, NULL, TO_ROOM);
...
SPECIAL(cityguard) {
    ...
    for (tch = world[IN_ROOM(ch)].people; tch; tch = tch->next_in_room) {
        ...
        if (!IS_NPC(tch) && PLR_FLAGGED(tch, PLR_KILLER)) {
            act("$n screams 'HEY!!!  You're one of those PLAYER KILLERS!!!!!!'", false, ch, NULL, NULL, TO_ROOM);
            return (true);
        }

        if (!IS_NPC(tch) && PLR_FLAGGED(tch, PLR_THIEF)) {
            act("$n screams 'HEY!!!  You're one of those PLAYER THIEVES!!!!!!'", false, ch, NULL, NULL, TO_ROOM);
            return (true);
        }
    ...
    if (evil && GET_ALIGNMENT(FIGHTING(evil)) >= 0) {
        act("$n screams 'PROTECT THE INNOCENT!  BANZAI!  CHARGE!  ARARARAGGGHH!'", false, ch, NULL, NULL,
            TO_ROOM);
        return (true);
    }
    ...
    if (CMD_IS("cancel")) {
        ...
        for (obj = world[auct_room].contents; obj; obj = next_obj) {
            ...
            if (obj && GET_AUCTER(obj) == GET_ID(ch)) {
                ...
                if (GET_CURBID(obj2) != -1 && GET_AUCTIME(obj2) + 518400 > time(NULL)) {
                    send_to_char(ch,
                                 "Unable to cancel. Someone has already bid on it and their bid license hasn't expired.\r\n");
                    time_t remain = (GET_AUCTIME(obj2) + 518400) - time(NULL);
        ...
        for (obj = world[auct_room].contents; obj; obj = next_obj) {
            ...
            if (obj && GET_CURBID(obj) == GET_ID(ch)) {
                ...
                if (GET_AUCTIME(obj2) + 86400 > time(NULL)) {
                    time_t remain = (GET_AUCTIME(obj2) + 86400) - time(NULL);
                    int hour = (int) ((remain % 86400) / 3600);
                    int minu = (int) ((remain % 3600) / 60);
                    send_to_char(ch,
        ...
        if (OBJ_FLAGGED(obj2, ITEM_BROKEN)) {
            act("$P is broken and we will not accept it.", false, ch, NULL, obj2, TO_CHAR);
            return (true);
        }

        if (OBJ_FLAGGED(obj2, ITEM_NODONATE)) {
            act("$P is junk and we will not accept it.", false, ch, NULL, obj2, TO_CHAR);
            return (true);
        }
    ...
    if (CMD_IS("htank")) {
        ...
        if (!strcasecmp("enter", arg)) {
            ...
            if (!PLR_FLAGGED(ch, PLR_HEALT)) {
                ...
                act("@wThe healing tank drains and you exit it shortly after.", true, ch, NULL, NULL, TO_CHAR);
                act("@C$n@w exits the healing tank after letting it drain.@n", true, ch, NULL, NULL, TO_ROOM);
