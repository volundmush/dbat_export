...
ACMD(do_newsedit) {
    ...
    *(tmstr + strlen(tmstr) - 1) = '\0';
    ...
    char *backstr = NULL;
    act("$n begins to edit the news.", true, ch, NULL, NULL, TO_ROOM);
...
ACMD(do_transobj) {
    ...
    if (!(obj = get_obj_in_list_vis(ch, arg, NULL, ch->carrying))) {
        ...
        act("You send $p to everyone in the game.", true, ch, obj, NULL, TO_CHAR);
...
ACMD(do_recall) {
    if (GET_ADMLEVEL(ch) < 1) {
        ...
        act("$n disappears in a burst of light!", false, ch, NULL, NULL, TO_ROOM);
...
ACMD(do_echo) {
    ...
    if (!*argument)
        ...
        if (NoName == true) {
            ...
            act(blom, false, ch, NULL, NULL, TO_ROOM);
        }
        if (found == false) {
            ...
            act(buf, false, ch, NULL, tch, TO_NOTVICT);
...
ACMD(do_trans) {
    ...
    if (!*buf)
        ...
        if (!(victim = get_char_vis(ch, buf, NULL, FIND_CHAR_WORLD)))
            ...
            act("$n disappears in a mushroom cloud.", false, victim, NULL, NULL, TO_ROOM);
            char_from_room(victim);
            char_to_room(victim, IN_ROOM(ch));
            act("$n arrives from a puff of smoke.", false, victim, NULL, NULL, TO_ROOM);
            act("$n has transferred you!", false, ch, NULL, victim, TO_VICT);
...
ACMD(do_teleport) {
    ...
    if (!*buf)
        ...
        act("$n arrives from a puff of smoke.", false, victim, NULL, NULL, TO_ROOM);
        act("$n has teleported you!", false, ch, NULL, (char *) victim, TO_VICT);
        look_at_room(IN_ROOM(victim), victim, 0);
        enter_wtrigger(&world[IN_ROOM(victim)], victim, -1);
    }
}
...
static void do_stat_character(struct char_data *ch, struct char_data *k) {
    ...
    if (CONFIG_ALLOW_MULTICLASS) {
        ...
        snprintf(buf, sizeof(buf), "%s", sen_getName(k->chclass));
    }
    snprintf(buf2, sizeof(buf2), "%s", race_getName(k->race));
    ...
    send_to_char(ch, "PL :[@g%12s@n]  KI :[@g%12s@n]  ST :[@g%12s@n]\r\n",
                 add_commas(GET_HIT(k)), add_commas(((ch_getMaxKI(k) * k->energy))), add_commas(((ch_getCurST(k)))));
    send_to_char(ch, "MPL:[@g%12s@n]  MKI:[@g%12s@n]  MST:[@g%12s@n]\r\n",
                 add_commas((ch_getEffMaxPL(k))), add_commas(GET_MAX_MANA(k)), add_commas(GET_MAX_MOVE(k)));
    send_to_char(ch, "BPL:[@g%12s@n]  BKI:[@g%12s@n]  BST:[@g%12s@n]\r\n",
                 add_commas(((ch_getBasePL(k)))), add_commas(((k->baseki))), add_commas(((ch_getBaseST(k)))));
    send_to_char(ch, "LF :[@g%12s@n]  MLF:[@g%12s@n]  LFP:[@g%3d@n]\r\n",
                 add_commas(((ch_getCurLF(k)))), add_commas(((ch_getMaxLF(k)))), GET_LIFEPERC(k));
...
ACMD(do_load) {
    ...
    if (is_abbrev(buf, "mob")) {
        ...
        for (i = 0; i < n; i++) {
            ...
            act("$n makes a quaint, magical gesture with one hand.", true, ch, NULL, NULL, TO_ROOM);
...
static void perform_immort_invis(struct char_data *ch, int level) {
    ...
    for (tch = world[IN_ROOM(ch)].people; tch; tch = tch->next_in_room) {
        ...
        if (GET_ADMLEVEL(tch) >= GET_INVIS_LEV(ch) && GET_ADMLEVEL(tch) < level)
            act("You blink and suddenly realize that $n is gone.", false, ch, NULL,
                tch, TO_VICT);
        if (GET_ADMLEVEL(tch) < GET_INVIS_LEV(ch) && GET_ADMLEVEL(tch) >= level)
            act("You suddenly realize that $n is standing beside you.", false, ch, NULL,
                tch, TO_VICT);
    }
...
ACMD(do_last) {
    ...
    send_to_char(ch, "[%5d] [%2d %s %s] %-12s : %-18s : %-20s\r\n",
                 ...
                 race_getAbbr(vict->race), CLASS_ABBR(vict),
    ...
    if (!*arg)
        ...
        switch (subcmd) {
            ...
            case SCMD_FREEZE:
                ...
                act("A sudden cold wind conjured from nowhere freezes $n!", false, vict, NULL, NULL, TO_ROOM);
            ...
            case SCMD_THAW:
                ...
                act("A sudden fireball conjured from nowhere thaws $n!", false, vict, NULL, NULL, TO_ROOM);
                break;
            case SCMD_UNAFFECT:
...
static int perform_set(struct char_data *ch, struct char_data *vict, int mode,
    ...
    affect_total(vict);
       break; */
        ...
        case 38: SET_OR_REMOVE(PLR_FLAGS(vict), PLR_DELETED);
            break;
        case 39:
            if ((chosen_sensei = sen_findName(val_arg)) == CLASS_UNDEFINED) {
     ...
     */
            vict->time.birth = time(NULL) - (value * SECS_PER_MUD_YEAR);
            break;
        ...
        case 52:
            chosen_race = race_findName(val_arg);
            if(chosen_race == RACE_UNDEFINED) {
                send_to_char(ch, "That is not a valid race.\r\n");
                return (0);
            }
            if(!race_isValidSex(chosen_race, GET_SEX(vict))) {
                send_to_char(ch, "That is not a valid race for them.\r\n");
                return (0);
            }
            if(!IS_NPC(vict) && !race_isPC(chosen_race)) {
                send_to_char(ch, "That is not a valid race for them.\r\n");
                return (0);
            }
...
ACMD(do_plist) {
    ...
    for (i = 0; i <= top_of_p_table; i++) {
        ...
        time_away = *real_time_passed(time(NULL), player_table[i].last);
...
ACMD(do_raise) {
    ...
    if (GET_ADMLEVEL(ch) <= 0) {
        ch_resurrect(vict, Basic);
    } else {
        ch_resurrect(vict, Costless);
    }
