    ...
    if (!*field) {
        if (vd)
            ...
            if (!strcasecmp(var, "self")) {
                ...
                snprintf(str, slen, "%ld", time(NULL));
            ...
            switch (LOWER(*field)) {
                ...
                case 'c':
                    if (!strcasecmp(field, "canbeseen")) {
                        ...
                        if (!IS_NPC(c))
                            snprintf(str, slen, "%s", sen_getName(c->chclass));
                        else
                ...
                case 'h':
                    if (!strcasecmp(field, "has_item")) {
                        ...
                        if (subfield && *subfield) {
                            ...
                            if (addition > 0) {
                                int64_t result1;
                                result1 = ch_incCurHealth(c, addition, true);
                            } else {
                                int64_t result;
                                result = ch_decCurHealth(c, addition, 0);
                            }
                ...
                case 'm':
                    if (!strcasecmp(field, "maxhitp")) {
                        ...
                        snprintf(str, slen, "%" I64T "", (ch_getEffMaxPL(c)));
                    } else if (!strcasecmp(field, "mana")) {
                        if (subfield && *subfield) {
                            int64_t addition = atoll(subfield);
                            if (addition > 0) {
                                int64_t result1;
                                result1 = ch_incCurKI(c, addition, true);
                            } else {
                                int64_t result;
                                result = ch_decCurKI(c, addition, 0);
                            }
                        }
                        snprintf(str, slen, "%" I64T "", ((ch_getMaxKI(c) * c->energy)));
                        ...
                        if (subfield && *subfield) {
                            ...
                            if (addition > 0) {
                                int64_t result1;
                                result1 = ch_incCurST(c, addition, true);
                            } else {
                                int64_t result;
                                result = ch_decCurST(c, addition, 0);
                            }

                        }
                        snprintf(str, slen, "%" I64T "", ((ch_getCurST(c))));
...
/* see note in dg_scripts.h */
                        ...
                        snprintf(str, slen, "%s", race_getName(c->race));
                    }
#endif
