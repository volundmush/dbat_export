...
void medit_disp_race(struct descriptor_data *d) {
    ...
    for (int i = 0; i < NUM_RACES; i++) {
        sprintf(buf, "@g%2d@n) %-20.20s  %s", i, race_getName(i),
                !(++columns % 2) ? "\r\n" : "");
        write_to_output(d, buf);
    }
...
void medit_disp_menu(struct descriptor_data *d) {
    ...
    write_to_output(d,
                    ...
                    GET_NDD(mob), GET_SDD(mob), GET_HIT(mob), ((ch_getMaxKI(mob) * mob->energy)),
                    ((ch_getCurST(mob))), GET_ARMOR(mob), GET_EXP(mob), GET_GOLD(mob)
    );
    ...
    write_to_output(d,
                    ...
                    flags, flag2, sen_getName(mob->chclass),
                    TRUE_RACE(mob),
