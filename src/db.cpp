...
static void setup_dir(FILE *fl, room_rnum room, int dir);
...
/* load the rooms */
static void parse_room(FILE *fl, int virtual_nr) {
    static room_rnum room_nr = 0, zone = 0;
    int t[10], i, retval;
...
struct char_data *read_mobile(mob_vnum nr, int type) /* and mob_rnum */
{
    ...
    *mob = mob_proto[i];
    ...
    mob->time.birth = time(NULL) - birth_age(mob);
    mob->time.created = mob->time.logon = time(NULL); /* why not */
    mob->time.maxage = mob->time.birth + max_age(mob);
    mob->time.played = 0;
    mob->time.logon = time(NULL);
...
void init_char(struct char_data *ch) {
    ...
    ch->time.logon = ch->time.created = time(NULL);
