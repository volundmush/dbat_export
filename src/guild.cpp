...
ACMD(do_teach) {
    ...
    if (!vict->master) {
        ...
        act(tochar, true, ch, NULL, vict, TO_CHAR);
        act(tovict, true, ch, NULL, vict, TO_VICT);
        act(toother, true, ch, NULL, vict, TO_NOTVICT);
...
int rpp_to_level(struct char_data *ch) {
    ...
    switch (GET_LEVEL(ch)) {
        case 2:
            ...
            return race_getRPPCost(ch->race);
        case 91:
        case 92:
        case 93:
        case 94:
...
void handle_exp(struct char_data *keeper, int guild_nr, struct char_data *ch, char *argument) {
    ...
    if (GET_EXP(ch) > level_exp(ch, GET_LEVEL(ch) + 1) && GET_LEVEL(ch) != 100) {
        ...
        act("@c$n@W spends time training you in $s fighting style.@n", true, keeper, NULL, ch, TO_VICT);
        act("@c$n@W spends time training @C$N@W in $s fighting style.@n", true, keeper, NULL, ch, TO_NOTVICT);
...
void handle_study(struct char_data *keeper, int guild_nr, struct char_data *ch, char *argument) {
    ...
    act("@c$N@W spends time lecturing you on various subjects.@n", true, ch, NULL, keeper, TO_CHAR);
    act("@c$N@W spends time lecturing @C$n@W on various subjects.@n", true, ch, NULL, keeper, TO_ROOM);
...
void handle_learn(struct char_data *keeper, int guild_nr, struct char_data *ch, char *argument) {
    ...
    if (!feat_is_available(ch, feat_num, 0, NULL) || !feat_list[feat_num].in_game ||
