...
void damage_weapon(struct char_data *ch, struct obj_data *obj, struct char_data *vict) {
    ...
    if (GET_SKILL(ch, SKILL_HANDLING) >= axion_dice(10)) {
        act("@GYour superior handling prevents @C$p@G from being damaged.@n", true, ch, obj, NULL, TO_CHAR);
        act("@g$n's@G superior handling prevents @C$p@G from being damaged.@n", true, ch, obj, NULL, TO_ROOM);
        result = 0;
    }
...
void handle_multihit(struct char_data *ch, struct char_data *vict) {
    ...
    if (perc >= prob) {
        ...
        act("@Y...in a lightning flash of speed you attack @y$N@Y again!@n", true, ch, NULL, vict, TO_CHAR);
        act("@Y...in a lightning flash of speed @y$n@Y attacks YOU again!@n", true, ch, NULL, vict, TO_VICT);
        act("@Y...in a lightning flash of speed @y$n@Y attacks @y$N@Y again!@n", true, ch, NULL, vict, TO_NOTVICT);
        ch->throws += 1;
...
int handle_defender(struct char_data *vict, struct char_data *ch) {
    ...
    if (GET_DEFENDER(vict)) {
        ...
        if (GET_SPEEDI(def) + defnum > GET_SPEEDI(ch) + chnum && IN_ROOM(def) == IN_ROOM(vict) &&
            ...
            act("@YYou move to and manage to intercept the attack aimed at @y$N@Y!@n", true, def, NULL, vict,
                TO_CHAR);
            act("@y$n@Y moves to and manages to intercept the attack aimed at YOU!@n", true, def, NULL, vict,
                TO_VICT);
            act("@y$n@Y moves to and manages to intercept the attack aimed at @y$N@Y!@n", true, def, NULL, vict,
                TO_NOTVICT);
            result = true;
        } else if (IN_ROOM(def) == IN_ROOM(vict) && GET_POS(def) > POS_SITTING) {
            act("@YYou move to intercept the attack aimed at @y$N@Y, but just not fast enough!@n", true, def, NULL,
                vict, TO_CHAR);
            act("@y$n@Y moves to intercept the attack aimed at YOU, but $e wasn't fast enough!@n", true, def, NULL,
                vict, TO_VICT);
...
void combine_attacks(struct char_data *ch, struct char_data *vict) {
    ...
    act(chbuf, true, ch, NULL, vict, TO_CHAR);
    act(victbuf, true, ch, NULL, vict, TO_VICT);
    act(rmbuf, true, ch, NULL, vict, TO_NOTVICT);
    ...
    for (f = ch->followers; f; f = f->next) {
        if (!AFF_FLAGGED(f->follower, AFF_GROUP)) {
            ...
            sprintf(folbuf2, "@WYou time and merge your @B'@R%s@B'@W into the group attack!@n",
                    attack_names_comp[GET_COMBINE(f->follower)]);
            act(folbuf, true, f->follower, NULL, NULL, TO_ROOM);
            act(folbuf2, true, f->follower, NULL, NULL, TO_CHAR);
        }
    }
    ...
    if (GET_COMBINE(ch) != 5) {
        if (attspd + attsk < GET_SKILL(vict, SKILL_DODGE) + (GET_CHA(ch) / 10)) {
            act("@GYou manage to dodge nimbly through the combined attack of your enemies!@n", true, vict, NULL,
                NULL, TO_CHAR);
            act("@r$n@G manages to dodge nimbly through the combined attack!@n", true, vict, NULL, NULL, TO_ROOM);
            return;
...
int check_ruby(struct char_data *ch) {
    ...
    if (found > 0) {
        act("@RYour $p@R flares up and disappears. Your fire attack has been aided!@n", true, ch, ruby, NULL,
            TO_CHAR);
        act("@R$n's@R $p@R flares up and disappears!@n", true, ch, ruby, NULL, TO_ROOM);
        extract_obj(ruby);
        return (1);
    } else {
        return (0);
...
void handle_knockdown(struct char_data *ch) {
    ...
    if (chance > axion_dice(0)) {
        ...
        act("@mYou are knocked off your feet!@n", true, ch, NULL, NULL, TO_CHAR);
        act("@W$n@m is knocked off $s feet!@n", true, ch, NULL, NULL, TO_ROOM);
...
int64_t gun_dam(struct char_data *ch, int wlvl) {
    ...
    if (dmg_prior <= (ch_getEffMaxPL(ch)) * 0.4)
        dmg = dmg_prior;
    else
        dmg = (ch_getEffMaxPL(ch)) * 0.4;
...
void club_stamina(struct char_data *ch, struct char_data *vict, int wlvl, int64_t dmg) {
    ...
    drained = dmg * drain;
    int64_t result;
    result = ch_decCurST(vict, drained, 0);
...
int backstab(struct char_data *ch, struct char_data *vict, int wlvl, int64_t dmg) {
    ...
    if (chance >= roll_to_beat) {
        ...
        if (attacker_roll > defender_roll) {
            act("@RYou manage to sneak behind @r$N@R and stab $M in the back!@n", true, ch, NULL, vict, TO_CHAR);
            act("@RYou feel @r$n's@R dagger thrust into your back unexpectantly!@n", true, ch, NULL, vict, TO_VICT);
            act("@r$n@R sneaks up behind @r$N@R and stabs $M in the back!@n", true, ch, NULL, vict, TO_NOTVICT);
            dmg += dmg * bonus;
...
void cut_limb(struct char_data *ch, struct char_data *vict, int wlvl, int hitspot) {
    ...
    if (decapitate == true) {
        act("@R$N's@r head is cut off in the attack!@n", true, ch, NULL, vict, TO_CHAR);
        act("@RYOUR head is cut off in the attack!@n", true, ch, NULL, vict, TO_VICT);
        act("@R$N's@rhead is cut off in the attack!@n", true, ch, NULL, vict, TO_NOTVICT);
        ...
        if (!IS_NPC(vict)) {
            if (HAS_ARMS(vict) && rand_number(1, 2) == 2) {
                if (GET_LIMBCOND(vict, 2) > 0) {
                    ...
                    act("@R$N@r loses $s left arm!@n", true, ch, NULL, vict, TO_CHAR);
                    act("@RYOU lose your left arm!@n", true, ch, NULL, vict, TO_VICT);
                    act("@R$N@r loses $s left arm!@n", true, ch, NULL, vict, TO_NOTVICT);
                    remove_limb(vict, 2);
                    ...
                    act("@R$N@r loses $s right arm!@n", true, ch, NULL, vict, TO_CHAR);
                    act("@RYOU lose your right arm!@n", true, ch, NULL, vict, TO_VICT);
                    act("@R$N@r loses $s right arm!@n", true, ch, NULL, vict, TO_NOTVICT);
                    remove_limb(vict, 1);
                }
            } else { /* It's a leg */
                if (GET_LIMBCOND(vict, 4) > 0) {
                    ...
                    act("@R$N@r loses $s left leg!@n", true, ch, NULL, vict, TO_CHAR);
                    act("@RYOU lose your left leg!@n", true, ch, NULL, vict, TO_VICT);
                    act("@R$N@r loses $s left leg!@n", true, ch, NULL, vict, TO_NOTVICT);
                    remove_limb(vict, 4);
                    ...
                    act("@R$N@r loses $s right leg!@n", true, ch, NULL, vict, TO_CHAR);
                    act("@RYOU lose your right leg!@n", true, ch, NULL, vict, TO_VICT);
                    act("@R$N@r loses $s right leg!@n", true, ch, NULL, vict, TO_NOTVICT);
                    remove_limb(vict, 3);
                }
            }
        } else { /* It's a npc */
        ...
        if (rate > 0.00) {
            ...
            if (GET_CHARGE(ch) + add > GET_MAX_MANA(ch)) {
                if (GET_CHARGE(ch) < GET_MAX_MANA(ch)) {
                    ...
                    act("@MYou leech some of the energy away!@n", true, ch, NULL, NULL, TO_CHAR);
                    act("@m$n@M leeches some of the energy away!@n", true, ch, NULL, NULL, TO_ROOM);
                } else {
                    send_to_char(ch,
    ...
    if (GET_BONUS(ch, BONUS_INTOLERANT)) {
        ...
        if (rate > 0.00) {
            if (GET_CHARGE(ch) > 0 && rand_number(1, 100) <= 10) {
                ...
                act("@m$n@M suffers shock from their own charged energy!@n", true, ch, NULL, NULL, TO_ROOM);
                int64_t result;
                result = ch_decCurHealth(ch, GET_CHARGE(ch) / 4, 1);
            }
            add = dmg * rate;
        }
...
void hurt_limb(struct char_data *ch, struct char_data *vict, int chance, int area, int64_t power) {
    ...
    if (power > (ch_getEffMaxPL(vict)) * 0.5) {
        dmg = rand_number(25, 40);
    } else if (power > (ch_getEffMaxPL(vict)) * 0.25) {
        dmg = rand_number(15, 24);
    } else if (power > (ch_getEffMaxPL(vict)) * 0.10) {
        dmg = rand_number(8, 14);
    } else if (power > (ch_getEffMaxPL(vict)) * 0.05) {
        dmg = rand_number(4, 7);
    } else {
        dmg = rand_number(1, 3);;
    }
    ...
    if (!is_sparring(ch)) {
        if (area == 0) { /* Arms */
            if (GET_LIMBCOND(vict, 2) - dmg <= 0) {
                act("@RYour attack @YDESTROYS @r$N's@R left arm!@n", true, ch, NULL, vict, TO_CHAR);
                act("@r$n's@R attack @YDESTROYS@R YOUR left arm!@n", true, ch, NULL, vict, TO_VICT);
                act("@r$n's@R attack @YDESTROYS @r$N's@R left arm!@n", true, ch, NULL, vict, TO_NOTVICT);
                GET_LIMBCOND(vict, 2) = 0;
                ...
                act("@RYour attack hurts @r$N's@R left arm!@n", true, ch, NULL, vict, TO_CHAR);
                act("@r$n's@R attack hurts YOUR left arm!@n", true, ch, NULL, vict, TO_VICT);
                act("@r$n's@R attack hurts @r$N's@R left arm!@n", true, ch, NULL, vict, TO_NOTVICT);
            } else if (GET_LIMBCOND(vict, 1) - dmg <= 0) {
                act("@RYour attack @YDESTROYS @r$N's@R right arm!@n", true, ch, NULL, vict, TO_CHAR);
                act("@r$n's@R attack @YDESTROYS@R YOUR right arm!@n", true, ch, NULL, vict, TO_VICT);
                act("@r$n's@R attack @YDESTROYS @r$N's@R right arm!@n", true, ch, NULL, vict, TO_NOTVICT);
                ...
                act("@RYour attack hurts @r$N's@R right arm!@n", true, ch, NULL, vict, TO_CHAR);
                act("@r$n's@R attack hurts YOUR right arm!@n", true, ch, NULL, vict, TO_VICT);
                act("@r$n's@R attack hurts @r$N's@R right arm!@n", true, ch, NULL, vict, TO_NOTVICT);
            }
        } else if (area == 1) { /* Legs */
            if (GET_LIMBCOND(vict, 4) - dmg <= 0) {
                act("@RYour attack @YDESTROYS @r$N's@R left leg!@n", true, ch, NULL, vict, TO_CHAR);
                act("@r$n's@R attack @YDESTROYS@R YOUR left leg!@n", true, ch, NULL, vict, TO_VICT);
                act("@r$n's@R attack @YDESTROYS @r$N's@R left leg!@n", true, ch, NULL, vict, TO_NOTVICT);
                ...
                act("@RYour attack hurts @r$N's@R left leg!@n", true, ch, NULL, vict, TO_CHAR);
                act("@r$n's@R attack hurts YOUR left leg!@n", true, ch, NULL, vict, TO_VICT);
                act("@r$n's@R attack hurts @r$N's@R left leg!@n", true, ch, NULL, vict, TO_NOTVICT);
            } else if (GET_LIMBCOND(vict, 3) - dmg <= 0) {
                act("@RYour attack @YDESTROYS @r$N's@R right leg!@n", true, ch, NULL, vict, TO_CHAR);
                act("@r$n's@R attack @YDESTROYS@R YOUR right leg!@n", true, ch, NULL, vict, TO_VICT);
                act("@r$n's@R attack @YDESTROYS @r$N's@R right leg!@n", true, ch, NULL, vict, TO_NOTVICT);
                ...
                act("@RYour attack hurts @r$N's@R right leg!@n", true, ch, NULL, vict, TO_CHAR);
                act("@r$n's@R attack hurts YOUR right leg!@n", true, ch, NULL, vict, TO_VICT);
                act("@r$n's@R attack hurts @r$N's@R right leg!@n", true, ch, NULL, vict, TO_NOTVICT);
            }
        }
    }
}
...
void damage_eq(struct char_data *vict, int location) {
    ...
    if (GET_EQ(vict, location) && rand_number(1, 20) >= 19 && !AFF_FLAGGED(vict, AFF_SANCTUARY)) {
        ...
        if (GET_OBJ_VAL(eq, VAL_ALL_HEALTH) <= 0) {
            ...
            act("@WYour $p@W completely breaks!@n", false, NULL, eq, vict, TO_VICT);
            act("@C$N's@W $p@W completely breaks!@n", false, NULL, eq, vict, TO_NOTVICT);
            ...
            act("@WYour $p@W rips a little!@n", false, NULL, eq, vict, TO_VICT);
            act("@C$N's@W $p@W rips a little!@n", false, NULL, eq, vict, TO_NOTVICT);
            if (AFF_FLAGGED(vict, AFF_BLESS)) {
                send_to_char(vict, "@c...But your blessing seems to have partly mended this damage.@n\r\n");
                act("@c...but @C$N's@c body glows blue for a moment and the damage mends a little.@n", true, NULL,
                    NULL, vict, TO_NOTVICT);
                ...
                act("@c...but @C$N's@c body glows red for a moment and the damage grow three times worse!@n", true,
                    NULL, NULL, vict, TO_NOTVICT);
            }
        } else {
            act("@WYour $p@W cracks a little!@n", false, NULL, eq, vict, TO_VICT);
            act("@C$N's@W $p@W cracks a little!@n", false, NULL, eq, vict, TO_NOTVICT);
            if (AFF_FLAGGED(vict, AFF_BLESS)) {
                send_to_char(vict, "@c...But your blessing seems to have partly mended this damage.@n\r\n");
                act("@c...but @C$N's@c body glows blue for a moment and the damage mends a little.@n", true, NULL,
                    NULL, vict, TO_NOTVICT);
...
void huge_update() {
    ...
    for (k = object_list; k; k = k->next) {
        if (GET_AUCTER(k) > 0 && GET_AUCTIME(k) + 604800 <= time(NULL)) {
            ...
            if (KITYPE(k) == 497) {
                if (IN_ROOM(TARGET(k)) == IN_ROOM(k)) {
                    ...
                    act("@WThe large @cS@Cp@wi@cr@Ci@wt @cB@Co@wm@cb@W descends on YOU! It eclipses everything above you as it crushes down into you! You struggle against it with all your might!@n",
                        true, TARGET(k), NULL, NULL, TO_CHAR);
                    act("@WThe large @cS@Cp@wi@cr@Ci@wt @cB@Co@wm@cb@W descends on @C$n@W! It completely obscures $m from view as it crushes into $s body! It appears to be facing some resistance from $m!@n",
                        true, TARGET(k), NULL, NULL, TO_ROOM);
                    ...
                    if (GET_HIT(TARGET(k)) * bonus < KICHARGE(k) * 5) {
                        ...
                        for (vict = world[IN_ROOM(k)].people; vict; vict = next_v) {
                            ...
                            if (((!IS_NPC(vict) && IS_ICER(vict) && rand_number(1, 30) >= 28) ||
                                ...
                                act("@C$N@c disappears, avoiding the explosion!@n", false, ch, NULL, vict, TO_CHAR);
                                act("@cYou disappear, avoiding the explosion!@n", false, ch, NULL, vict, TO_VICT);
                                act("@C$N@c disappears, avoiding the explosion!@n", false, ch, NULL, vict,
                                    TO_NOTVICT);
                        ...
                        int64_t result;
                        result = ch_decCurST(TARGET(k), KICHARGE(k) / 4, 0);
                        extract_obj(k);
                        continue;
                    }
                    ...
                    for (vict = world[IN_ROOM(k)].people; vict; vict = next_v) {
                        ...
                        if (((!IS_NPC(vict) && IS_ICER(vict) && rand_number(1, 30) >= 28) ||
                            ...
                            act("@C$N@c disappears, avoiding the explosion!@n", false, ch, NULL, vict, TO_CHAR);
                            act("@cYou disappear, avoiding the explosion!@n", false, ch, NULL, vict, TO_VICT);
                            act("@C$N@c disappears, avoiding the explosion!@n", false, ch, NULL, vict, TO_NOTVICT);
                            REMOVE_BIT_AR(AFF_FLAGS(vict), AFF_ZANZOKEN);
                            pcost(vict, 0, (ch_getEffMaxPL(vict)) / 200);
                            hurt(0, 0, ch, vict, NULL, 0, 1);
                            continue;
        ...
        act("$p@W descends slowly towards the ground!@n", true, NULL, k, NULL, TO_ROOM);
        KIDIST(k)--;
    }
        ...
        if (GET_OBJ_VNUM(k) != 80 && GET_OBJ_VNUM(k) != 81 && GET_OBJ_VNUM(k) != 84) {
            ...
            if (GET_OBJ_VNUM(k) == 80) { // Tsuihidan
                if (IN_ROOM(k) != IN_ROOM(vict)) {
                    act("@wThe $p@w pursues after you!@n", true, vict, k, NULL, TO_CHAR);
                    act("@wThe $p@W pursues after @C$n@w!@n", true, vict, k, NULL, TO_ROOM);
                    obj_from_room(k);
                    obj_to_room(k, IN_ROOM(vict));
                    continue;
                } else {
                    act("@RThe $p@R makes a tight turn and rockets straight for you!@n", true, vict, k, NULL,
                        TO_CHAR);
                    act("@RThe $p@R makes a tight turn and rockets straight for @r$n@n", true, vict, k, NULL,
                        TO_ROOM);
                            ...
                            if (dmg > (ch_getEffMaxPL(vict)) / 5 && (!IS_MAJIN(vict) && !IS_BIO(vict))) {
                                ...
                                if (GET_SKILL(vict, SKILL_REGENERATE) > rand_number(1, 101) &&
                                    ...
                                    if (GET_SKILL(vict, SKILL_REGENERATE) > rand_number(1, 101) &&
                                        ...
                                        act("@R$N@r is cut in half by the attack!@n", true, ch, NULL, vict, TO_CHAR);
                                        act("@rYou are cut in half by the attack!@n", true, ch, NULL, vict, TO_VICT);
                                        act("@R$N@r is cut in half by the attack!@n", true, ch, NULL, vict,
                                            TO_NOTVICT);
                                        die(vict, ch);
                                        if (AFF_FLAGGED(ch, AFF_GROUP)) {
                                            group_gain(ch, vict);
...
int limb_ok(struct char_data *ch, int type) {
    ...
    if (type == 0) {
        ...
        if (AFF_FLAGGED(ch, AFF_ENSNARED) && rand_number(1, 100) <= 90) {
            ...
            act("You manage to break the silk ensnaring your arms!", true, ch, NULL, NULL, TO_CHAR);
            act("$n manages to break the silk ensnaring $s arms!", true, ch, NULL, NULL, TO_ROOM);
...
parry_ki(double attperc, struct char_data *ch, struct char_data *vict, char sname[1000], int prob, int perc, int skill,
    ...
    for (tch = world[IN_ROOM(ch)].people; tch; tch = next_v) {
        ...
        if (rand_number(1, 101) >= 90 && foundv == false) {
            if (handle_parry(tch) > rand_number(1, 140)) {
                ...
                act(buf, true, ch, NULL, tch, TO_CHAR);
                act(buf2, true, ch, NULL, tch, TO_VICT);
                act(buf3, true, ch, NULL, tch, TO_NOTVICT);
                foundv = false;
            } else {
                foundv = true;
                sprintf(buf,
                ...
                sprintf(buf3,
                        ...
                        sname, sname);
                act(buf, true, vict, NULL, tch, TO_CHAR);
                act(buf2, true, vict, NULL, tch, TO_VICT);
                act(buf3, true, vict, NULL, tch, TO_NOTVICT);
    ...
    for (tob = world[IN_ROOM(ch)].contents; tob; tob = next_obj) {
        ...
        if (rand_number(1, 101) >= 80) {
            ...
            sprintf(buf2,
                    ...
                    sname);
            act(buf, true, vict, tob, NULL, TO_CHAR);
            act(buf2, true, vict, tob, NULL, TO_ROOM);
    ...
    if ((foundo == false || foundv == false) && !ROOM_FLAGGED(IN_ROOM(vict), ROOM_SPACE)) {
        ...
        act(buf, true, vict, NULL, NULL, TO_CHAR);
        act(buf2, true, vict, NULL, NULL, TO_ROOM);
        if (SECT(IN_ROOM(vict)) != SECT_INSIDE && SECT(IN_ROOM(vict)) != SECT_UNDERWATER &&
            ...
            switch (rand_number(1, 8)) {
                case 1:
                    act("Debris is thrown into the air and showers down thunderously!", true, ch, NULL, vict,
                        TO_CHAR);
                    act("Debris is thrown into the air and showers down thunderously!", true, ch, NULL, vict,
                        TO_ROOM);
                    break;
                ...
                case 3:
                    ...
                    break;
                case 4:
                    act("The surrounding area roars and shudders from the impact!", true, ch, NULL, vict, TO_CHAR);
                    act("The surrounding area roars and shudders from the impact!", true, ch, NULL, vict, TO_ROOM);
                    break;
                case 5:
                    act("The ground shatters apart from the stress of the impact!", true, ch, NULL, vict, TO_CHAR);
                    act("The ground shatters apart from the stress of the impact!", true, ch, NULL, vict, TO_ROOM);
                    break;
                case 6:
        ...
        if (SECT(IN_ROOM(vict)) == SECT_UNDERWATER) {
            switch (rand_number(1, 3)) {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    act("The water collapses in on the hole created!", true, ch, NULL, vict, TO_CHAR);
        ...
        if (SECT(IN_ROOM(vict)) == SECT_WATER_SWIM || SECT(IN_ROOM(vict)) == SECT_WATER_NOSWIM) {
            switch (rand_number(1, 3)) {
                case 1:
                    act("A huge column of water erupts from the impact!", true, ch, NULL, vict, TO_CHAR);
                    act("A huge column of water erupts from the impact!", true, ch, NULL, vict, TO_ROOM);
                    break;
                case 2:
                    act("The impact briefly causes a swirling vortex of water!", true, ch, NULL, vict, TO_CHAR);
                    act("The impact briefly causes a swirling vortex of water!", true, ch, NULL, vict, TO_ROOM);
                    break;
                case 3:
        ...
        if (SECT(IN_ROOM(vict)) == SECT_INSIDE) {
            ...
            switch (rand_number(1, 8)) {
                case 1:
                    ...
                    break;
                case 2:
                    act("The structure of the surrounding room cracks and quakes from the blast!", true, ch, NULL,
                        vict, TO_CHAR);
                    act("The structure of the surrounding room cracks and quakes from the blast!", true, ch, NULL,
                        vict, TO_ROOM);
                    break;
                case 3:
                    act("Parts of the ceiling collapse, crushing into the floor!", true, ch, NULL, vict, TO_CHAR);
                    act("Parts of the ceiling collapse, crushing into the floor!", true, ch, NULL, vict, TO_ROOM);
                    break;
                case 4:
                    act("The surrounding area roars and shudders from the impact!", true, ch, NULL, vict, TO_CHAR);
                    act("The surrounding area roars and shudders from the impact!", true, ch, NULL, vict, TO_ROOM);
                ...
                case 5:
                    ...
                    break;
                case 6:
                    act("The walls of the surrounding room crack in the same instant!", true, ch, NULL, vict,
                        TO_CHAR);
                    act("The walls of the surrounding room crack in the same instant!", true, ch, NULL, vict,
                        TO_ROOM);
                    break;
...
void dodge_ki(struct char_data *ch, struct char_data *vict, int type, int type2, int skill, int skill2) {
    ...
    if (type == 1) {
        if (rand_number(1, 3) != 2) {
            act("@RIt turns around at the last second and begins to pursue @r$N@R!@n", true, ch, NULL, vict,
                TO_CHAR);
            act("@RIt turns around at the last second and begins to pursue YOU!@n", true, ch, NULL, vict, TO_VICT);
            act("@RIt turns around at the last second and begins to pursue @r$N@R!@n", true, ch, NULL, vict,
                TO_NOTVICT);
            struct obj_data *obj;
            int num = 0;
            ...
            act("@RIt fails to follow after @r$N@R!@n", true, ch, NULL, vict, TO_CHAR);
            act("@RIt fails to follow after YOU!@n", true, ch, NULL, vict, TO_VICT);
            act("@RIt fails to follow after @r$N@R!@n", true, ch, NULL, vict, TO_NOTVICT);
        }
    }
    if (type == 2 && (skill2 != 481 || IS_FRIEZA(ch))) {
        ...
        act("@RYou turn it around and send it back after @r$N@R!@n", true, ch, NULL, vict, TO_CHAR);
        act("@W$n @Rturns it around and sends it back after YOU!@n", true, ch, NULL, vict, TO_VICT);
        act("@W$n @Rturns it around and sends it back after @r$N@R!@n", true, ch, NULL, vict, TO_NOTVICT);
        struct obj_data *obj;
        int num = 0;
...
int64_t damtype(struct char_data *ch, int type, int skill, double percent) {
    ...
    if (!IS_NPC(ch)) {
        ...
        switch (type) {
            ...
            case 26: /* Tribeam */
                ...
                if (!IS_NPC(ch) && percent > 0.15) {
                    ...
                    int64_t amount = ch_getEffMaxPL(ch) * hitperc;
                    int64_t difference = GET_HIT(ch) - amount;

                    int64_t result;
                    result = ch_decCurHealth(ch, amount, 1);
        ...
        dam = (GET_HIT(ch) * 0.05) + ((ch_getEffMaxPL(ch)) * 0.025);
    ...
    if (IS_NPC(ch)) {
        ...
        int64_t max_hit = (ch_getEffMaxPL(ch));
    ...
    if (PLR_FLAGGED(ch, PLR_FURY) &&
        (type == 0 || type == 1 || type == 2 || type == 3 || type == 4 || type == 5 || type == 6 || type == 8 ||
         type == 51 || type == 52)) {
        dam *= 1.5;
        act("Your rage magnifies your attack power!", true, ch, NULL, NULL, TO_CHAR);
        act("Swirling energy flows around $n as $e releases $s rage in the attack!", true, ch, NULL, NULL,
            TO_ROOM);
    ...
    if ((type == -1 || type == 0 || type == 1 || type == 2 || type == 3 || type == 4 || type == 5 || type == 6 ||
        ...
        if (!IS_NPC(ch))
            dam -= dam * 0.08;
        if (!IS_NPC(ch) && dam > (ch_getEffMaxPL(ch)) * 0.1)
            dam *= 0.6;
    } else {
...
void saiyan_gain(struct char_data *ch, struct char_data *vict) {
    ...
    if ((ch_getEffMaxPL(vict)) < (ch_getEffMaxPL(ch)) / 10) {
        weak = true;
    }
    ...
    if (rand_number(1, 22) >= 18 && (GET_LEVEL(ch) == 100 || level_exp(ch, GET_LEVEL(ch) + 1) - (GET_EXP(ch)) > 0)) {
        if (weak) {
            ...
            switch (stats[rand_number(0, y)]) {
                case 0: {
                    int64_t result1;
                    result1 = ch_gainBasePL(ch, gain, false);
                }
                ...
                case 1: {
                    int64_t result;
                    result = ch_gainBaseKI(ch, gain, false);
                }
                ...
                case 2: {
                    int64_t result;
                    result = ch_gainBaseST(ch, gain, false);
                }
...
void spar_gain(struct char_data *ch, struct char_data *vict, int type, int64_t dmg) {
    ...
    if (ch != NULL && !IS_NPC(ch)) {
        if (dmg > (ch_getEffMaxPL(vict)) / 10) {
            chance = rand_number(20, 100);
        } else if (dmg <= (ch_getEffMaxPL(vict)) / 10) {
    ...
    if (vict != NULL && !IS_NPC(vict) && !IS_NPC(ch)) {
        if (dmg > (ch_getEffMaxPL(vict)) / 4) {
            chance = rand_number(1, 100);
        } else if (dmg <= (ch_getEffMaxPL(vict)) / 4) {
        ...
        if (chance >= rand_number(60, 75)) {
            ...
            if (GET_PRACTICES(vict, GET_CLASS(vict)) >= pscost) {
                ...
                if (rand_number(1, 5) >= 4) {
                    ...
                    int64_t result;
                    result = ch_gainBaseST(vict, st, false);
                }
...
int can_grav(struct char_data *ch) {
    /* Gravity Related */
    if (ROOM_GRAVITY(IN_ROOM(ch)) == 10 && (ch_getEffMaxPL(ch)) < 5000 && !IS_BARDOCK(ch) && !IS_NPC(ch)) {
        send_to_char(ch, "You are hardly able to move in this gravity!\r\n");
        return 0;
    } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 20 && (ch_getEffMaxPL(ch)) < 20000) {
        send_to_char(ch, "You are hardly able to move in this gravity!\r\n");
        return 0;
    } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 30 && (ch_getEffMaxPL(ch)) < 50000) {
        send_to_char(ch, "You are hardly able to move in this gravity!\r\n");
        return 0;
    } else {
        if (ROOM_GRAVITY(IN_ROOM(ch)) == 40 && (ch_getEffMaxPL(ch)) < 100000) {
            send_to_char(ch, "You are hardly able to move in this gravity!\r\n");
            return 0;
        } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 50 && (ch_getEffMaxPL(ch)) < 200000) {
            send_to_char(ch, "You are hardly able to move in this gravity!\r\n");
            return 0;
        } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 100 && (ch_getEffMaxPL(ch)) < 400000) {
            send_to_char(ch, "You are hardly able to move in this gravity!\r\n");
            return 0;
        } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 200 && (ch_getEffMaxPL(ch)) < 1000000) {
            send_to_char(ch, "You are hardly able to move in this gravity!\r\n");
            return 0;
        } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 300 && (ch_getEffMaxPL(ch)) < 5000000) {
            send_to_char(ch, "You are hardly able to move in this gravity!\r\n");
            return 0;
        } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 400 && (ch_getEffMaxPL(ch)) < 8000000) {
            send_to_char(ch, "You are hardly able to move in this gravity!\r\n");
            return 0;
        } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 500 && (ch_getEffMaxPL(ch)) < 15000000) {
            send_to_char(ch, "You are hardly able to move in this gravity!\r\n");
            return 0;
        } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 1000 && (ch_getEffMaxPL(ch)) < 25000000) {
            send_to_char(ch, "You are hardly able to move in this gravity!\r\n");
            return 0;
        } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 5000 && (ch_getEffMaxPL(ch)) < 100000000) {
            send_to_char(ch, "You are hardly able to move in this gravity!\r\n");
            return 0;
        } else if (ROOM_GRAVITY(IN_ROOM(ch)) == 10000 && (ch_getEffMaxPL(ch)) < 200000000) {
            send_to_char(ch, "You are hardly able to move in this gravity!\r\n");
            return 0;
        } else {
            return 1;
    ...
    if (vict) {
        ...
        if (ROOM_FLAGGED(IN_ROOM(ch), ROOM_PEACEFUL)) {
            ...
            return 0;
                ...
                (ch_getEffMaxPL(vict)) > (ch_getEffMaxPL(ch)) + ((ch_getEffMaxPL(ch)) * .5) &&
                   !FIGHTING(ch)) {
            ...
            act("@GYou stretch your limbs toward @g$N@G in an attempt to hit $M!@n", true, ch, NULL, vict, TO_CHAR);
            act("@g$n@G stretches $s limbs toward @RYOU@G in an attempt to land a hit!@n", true, ch, NULL, vict,
                TO_VICT);
            act("@g$n@G stretches $s limbs toward @g$N@G in an attempt to hit $M!@n", true, ch, NULL, vict,
                TO_NOTVICT);
            return 1;
...
void pcost(struct char_data *ch, double ki, int64_t st) {
    ...
    if (GET_LEVEL(ch) > 1 && !IS_NPC(ch)) {
        if (ki == 0) {
            before = ((ch_getCurST(ch)));
            if (ch_calcGravCost(ch, 0)) {
                if (before > ((ch_getCurST(ch)))) {
        ...
        int64_t result;
        result = ch_decCurST(ch, st, 0);
    }
    if (IS_NPC(ch)) {
        int64_t result1;
        result1 = ch_decCurKI(ch, ki, 0);
        int64_t result;
        result = ch_decCurST(ch, st, 0);
    }
}
...
hurt(int limb, int chance, struct char_data *ch, struct char_data *vict, struct obj_data *obj, int64_t dmg, int type) {
   ...
   if (PLR_FLAGGED(ch, PLR_TRANS3)) {
    ...
    if (AFF_FLAGGED(ch, AFF_INFUSE) && !AFF_FLAGGED(ch, AFF_HASS) && type <= 0) {
        int64_t infuse_cost = ch_getPercentOfMaxKI(ch, .005);
        if (dmg > 0) {
            if (ch_getMaxKI(ch) * ch->energy - infuse_cost) {
                int64_t result;
                result = ch_decCurKI(ch, infuse_cost, 0);
                send_to_room(IN_ROOM(ch), "@CA swirl of ki explodes from the attack!@n\r\n");
            } else {
                act("@wYou can no longer infuse ki into your attacks!@n", true, ch, NULL, NULL, TO_CHAR);
                act("@c$n@w can no longer infuse ki into $s attacks!@n", true, ch, NULL, NULL, TO_ROOM);
    ...
    if (vict) {
        ...
        if (IS_MUTANT(vict) && (GET_GENOME(vict, 0) == 8 || GET_GENOME(vict, 1) == 8) && type == 0) {
            ...
            dmg -= drain;
            int64_t result;
            result = ch_decCurST(ch, drain, 1);
            act("@Y$N's rubbery body makes hitting it tiring!@n", true, ch, NULL, vict, TO_CHAR);
            act("@Y$n's stamina is sapped a bit by hitting your rubbery body!@n", true, ch, NULL, vict, TO_VICT);
        }
        ...
        if (AFF_FLAGGED(vict, AFF_SANCTUARY)) {
            ...
            if (GET_BARRIER(vict) - dmg > 0) {
                act("@c$N's@C barrier absorbs the damage!@n", true, ch, NULL, vict, TO_CHAR);
                char barr[MAX_INPUT_LENGTH];
                sprintf(barr, "@CYour barrier absorbs the damage! @D[@B%s@D]@n", add_commas(dmg));
                act(barr, true, ch, NULL, vict, TO_VICT);
                act("@c$N's@C barrier absorbs the damage!@n", true, ch, NULL, vict, TO_NOTVICT);
                GET_BARRIER(vict) -= dmg;
                dmg = 0;
            } else if (GET_BARRIER(vict) - dmg <= 0) {
                dmg -= GET_BARRIER(vict);
                GET_BARRIER(vict) = 0;
                act("@c$N's@C barrier bursts!@n", true, ch, NULL, vict, TO_CHAR);
                act("@CYour barrier bursts!@n", true, ch, NULL, vict, TO_VICT);
                act("@c$N's@C barrier bursts!@n", true, ch, NULL, vict, TO_NOTVICT);
        ...
        if (AFF_FLAGGED(vict, AFF_FIRESHIELD) && rand_number(1, 200) < GET_SKILL(vict, SKILL_FIRESHIELD)) {
            act("@c$N's@C fireshield repels the damage!@n", true, ch, NULL, vict, TO_CHAR);
            act("@CYour fireshield repels the damage!@n", true, ch, NULL, vict, TO_VICT);
            act("@c$N's@C fireshield repels the damage!@n", true, ch, NULL, vict, TO_NOTVICT);
            if (rand_number(1, 3) == 3) {
                act("@c$N's@C fireshield disappears...@n", true, ch, NULL, vict, TO_CHAR);
                act("@CYour fireshield disappears...@n", true, ch, NULL, vict, TO_VICT);
                act("@c$N's@C fireshield disappears...@n", true, ch, NULL, vict, TO_NOTVICT);
        ...
        if (type == 0) {
            if ((ch_getEffMaxPL(vict)) < conlimit) {
                index += ((ch_getEffMaxPL(vict)) / 1500) * (GET_CON(vict) / 2);
            } else if ((ch_getEffMaxPL(vict)) < conlimit * 2) {
                index += ((ch_getEffMaxPL(vict)) / 2500) * (GET_CON(vict) / 2);
            } else if ((ch_getEffMaxPL(vict)) < conlimit * 3) {
                index += ((ch_getEffMaxPL(vict)) / 3500) * (GET_CON(vict) / 2);
            } else if ((ch_getEffMaxPL(vict)) < conlimit * 5) {
                index += ((ch_getEffMaxPL(vict)) / 6000) * (GET_CON(vict) / 2);
            } else if ((ch_getEffMaxPL(vict)) < conlimit * 10) {
                index += ((ch_getEffMaxPL(vict)) / 8500) * (GET_CON(vict) / 2);
            } else if ((ch_getEffMaxPL(vict)) < conlimit * 15) {
                index += ((ch_getEffMaxPL(vict)) / 10000) * (GET_CON(vict) / 2);
            } else if ((ch_getEffMaxPL(vict)) < conlimit * 20) {
                index += ((ch_getEffMaxPL(vict)) / 12500) * (GET_CON(vict) / 2);
            } else if ((ch_getEffMaxPL(vict)) < conlimit * 25) {
                index += ((ch_getEffMaxPL(vict)) / 16000) * (GET_CON(vict) / 2);
            } else if ((ch_getEffMaxPL(vict)) < conlimit * 30) {
                index += ((ch_getEffMaxPL(vict)) / 22000) * (GET_CON(vict) / 2);
            } else if ((ch_getEffMaxPL(vict)) > conlimit * 30) {
                index += ((ch_getEffMaxPL(vict)) / 25000) * (GET_CON(vict) / 2);
            }
        }
        ...
        if (GET_CLAN(vict) != NULL && !strcasecmp(GET_CLAN(vict), "Heavenly Kaios")) {
            if (((ch_getMaxKI(vict) * vict->energy)) >= GET_MAX_MANA(vict) / 2) {
                dmg -= (dmg / 100) * 20;
                act("@wYou are covered in a pristine @Cglow@w.@n", true, vict, NULL, NULL, TO_CHAR);
                act("@w$n is covered in a pristine @Cglow@w!@n", true, vict, NULL, NULL, TO_ROOM);
            }
        }
        ...
        if (!AFF_FLAGGED(vict, AFF_KNOCKED) && (GET_POS(vict) == POS_SITTING || GET_POS(vict) == POS_RESTING) &&
            ...
            int64_t rollcost = (ch_getMaxHealth(vict) / 300) * (GET_STR(ch) / 2);
            if (((ch_getCurST(vict))) >= rollcost) {
                act("@GYou roll to your feet in an agile fashion!@n", true, vict, NULL, NULL, TO_CHAR);
                act("@G$n rolls to $s feet in an agile fashion!@n", true, vict, NULL, NULL, TO_ROOM);
                do_stand(vict, NULL, 0, 0);
                int64_t result;
                result = ch_decCurST(vict, rollcost, 0);
            }
        }
        ...
        if (IS_NPC(vict) && GET_HIT(vict) > ((ch_getEffMaxPL(vict))) / 4) {
            LASTHIT(vict) = GET_IDNUM(ch);
        }
        if (AFF_FLAGGED(vict, AFF_SLEEP) && rand_number(1, 2) == 2) {
            affect_from_char(vict, SPELL_SLEEP);
            act("@c$N@W seems to be more aware now.@n", true, ch, NULL, vict, TO_CHAR);
            act("@WYou are no longer so sleepy.@n", true, ch, NULL, vict, TO_VICT);
            act("@c$N@W seems to be more aware now.@n", true, ch, NULL, vict, TO_NOTVICT);
        }
        if (AFF_FLAGGED(vict, AFF_KNOCKED) && rand_number(1, 12) >= 11) {
            ch_cureStatusKnockedOut(vict, true);
            if (IS_NPC(vict) && rand_number(1, 20) >= 12) {
                act("@W$n@W stands up.@n", false, vict, NULL, NULL, TO_ROOM);
        ...
        if (IS_NPC(vict) && dmg > ch_getMaxHealth(vict) * .7 && GET_BONUS(ch, BONUS_SADISTIC) > 0) {
            GET_EXP(vict) /= 2;
        } else {
            if (IS_NPC(vict) && dmg > ch_getCurHealth(vict) && ch_isFullHealth(vict) * .5 &&
        ...
        if (CARRYING(vict) && dmg > (((ch_getEffMaxPL(vict))) * 0.01) && rand_number(1, 10) >= 8) {
            carry_drop(vict, 2);
        }

        if (GET_POS(vict) == POS_SITTING && IS_NPC(vict) && ch_getCurHealth(vict) >= ((ch_getEffMaxPL(vict))) * .98) {
            do_stand(vict, NULL, 0, 0);
        }
        int suppresso = false;
        if (is_sparring(ch) && is_sparring(vict) && (GET_SUPP(vict) + ch_getCurHealth(vict)) - dmg <= 0) {
            if (!IS_NPC(vict)) {
                act("@c$N@w falls down unconscious, and you stop sparring with $M.@n", true, ch, NULL, vict,
                    TO_CHAR);
                act("@C$n@w stops sparring with you as you fall unconscious.@n", true, ch, NULL, vict, TO_VICT);
                act("@c$N@w falls down unconscious, and @C$n@w stops sparring with $M.@n", true, ch, NULL, vict,
                    TO_NOTVICT);
                int64_t result;
                result = ch_setCurHealth(vict, 1);
                if (GET_SUPP(vict) > 0) {
                    GET_SUPP(vict) = 0;
                    GET_SUPPRESS(vict) = 0;
                }
                ...
                act("@c$N@w admits defeat to you, stops sparring, and stumbles away.@n", true, ch, NULL, vict,
                    TO_CHAR);
                act("@c$N@w admits defeat to $n, stops sparring, and stumbles away.@n", true, ch, NULL, vict,
                    TO_NOTVICT);
                solo_gain(ch, vict);
                ...
                if (founded == 1) {
                    act("@c$N@w leaves a reward behind out of respect.@n", true, ch, NULL, vict, TO_CHAR);
                }
                int64_t result;
                result = ch_setCurHealth(vict, 0);
                extract_char(vict);
                return;
            }
        } else if (is_sparring(ch) && (GET_SUPP(vict) + GET_HIT(vict)) - dmg <= 0) {
            act("@c$N@w falls down unconscious, and you spare $S life.@n", true, ch, NULL, vict, TO_CHAR);
            act("@C$n@w spares your life as you fall unconscious.@n", true, ch, NULL, vict, TO_VICT);
            act("@c$N@w falls down unconscious, and @C$n@w spares $S life.@n", true, ch, NULL, vict, TO_NOTVICT);
            int64_t result;
            result = ch_setCurHealth(vict, 1);
            if (FIGHTING(vict)) {
                stop_fighting(vict);
            ...
            act("@w$n@w stops sparring!@n", true, ch, NULL, vict, TO_ROOM);
        ...
        if (PLR_FLAGGED(vict, PLR_IMMORTAL) && !is_sparring(ch) && ch_getCurHealth(vict) - dmg <= 0) {
            if (IN_ARENA(vict)) {
                ...
                int64_t result3;
                result3 = ch_setCurHealth(vict, 1);
                ...
                act("@c$N@w disappears right before dying. $N appears to be immortal.@n", true, ch, NULL, vict,
                    TO_CHAR);
                act("@CYou disappear right before death, having been saved by your immortality.@n", true, ch, NULL,
                    vict, TO_VICT);
                act("@c$N@w disappears right before dying. $N appears to be immortal.@n.", true, ch, NULL, vict,
                    TO_NOTVICT);
                int64_t result2;
                result2 = ch_decCurHealthPercent(vict, 1, 1);
                int64_t result;
                result = ch_decCurSTPercent(vict, 1, 1);
                int64_t result1;
                result1 = ch_decCurKIPercent(vict, 1, 1);
                ...
                char_from_room(vict);
                char_to_room(vict, real_room(sen_startRoom(vict->chclass)));
            }
            return;
        }

        if (GRAPPLING(vict) && GRAPPLING(vict) != ch && type == 1) {
            act("@YThe attack hurts YOU as well because you are grappling with $M!@n", true, vict, NULL,
                GRAPPLING(vict), TO_VICT);
            act("@YThe attack hurts @y$N@Y as well because $n is grappling with $m!@n", true, vict, NULL,
                GRAPPLING(vict), TO_NOTVICT);
            maindmg = maindmg / 2;
            hurt(0, 0, ch, GRAPPLING(vict), NULL, maindmg, 3);
        }
        if (GRAPPLED(vict) && GRAPPLED(vict) != ch && type == 1) {
            act("@YThe attack hurts YOU as well because you are being grappled by $M!@n", true, vict, NULL,
                GRAPPLED(vict), TO_VICT);
            act("@YThe attack hurts @y$N@Y as well because $n is being grappled by $m!@n", true, vict, NULL,
        ...
        if (!is_sparring(ch) && !PLR_FLAGGED(vict, PLR_IMMORTAL) && GET_HIT(vict) - dmg <= 0) {
            if (GET_HIT(vict) - dmg <= 0 && suppresso == false) {
                int64_t result2;
                result2 = ch_decCurHealthPercent(vict, 1, 0);
                if (!IS_NPC(vict) && ((ch_getCurLF(vict))) - (dmg - GET_HIT(vict)) >= 0) {
                    act("@c$N@w barely clings to life!@n", true, ch, NULL, vict, TO_CHAR);
                    act("@CYou barely cling to life!@n", true, ch, NULL, vict, TO_VICT);
                    act("@c$N@w barely clings to life!@n.", true, ch, NULL, vict, TO_NOTVICT);
                    int64_t lifeloss = dmg - GET_HIT(vict);
                    int64_t result1;
                    result1 = ch_decCurLF(vict, lifeloss, 0);
                    send_to_char(vict, "@D[@CLifeforce@D: @R-%s@D]\n", add_commas(lifeloss));
                    if (((ch_getCurLF(vict))) >= ((ch_getMaxLF(vict))) * 0.05) {
                        send_to_char(vict, "@YYou recover a bit thanks to your strong life force.@n\r\n");
                        int64_t amt = ((ch_getMaxLF(vict))) * .05;
                        int64_t result3;
                        result3 = ch_incCurHealth(vict, amt, true);
                        int64_t result;
                        result = ch_decCurLFPercent(vict, .05, 0);
                    } else {
                        int64_t result;
                        result = ch_incCurHealth(vict, GET_LEVEL(vict) * 100, true);
                    }
                    return;
                }
                ...
                if (type <= 0 && (!IS_NPC(vict) || !MOB_FLAGGED(vict, MOB_DUMMY))) {
                    ...
                    act("@R$N@w self destructs with a mild explosion!@n", true, ch, NULL, vict, TO_CHAR);
                    act("@R$N@w self destructs with a mild explosion!@n", true, ch, NULL, vict, TO_ROOM);
                }
                if (dmg > 1) {
                    if (type <= 0 && GET_HIT(ch) >= ((ch_getEffMaxPL(ch)) * 0.5)) {
                        int64_t raise = (GET_MAX_MANA(ch) * 0.005) + 1;
                        int64_t result;
                        result = ch_incCurKI(ch, raise, true);
                    }
                    send_to_char(ch, "@D[@GDamage@W: @R%s@D]@n\r\n", add_commas(dmg));
                    send_to_char(vict, "@D[@rDamage@W: @R%s@D]@n\r\n", add_commas(dmg));
                    int64_t healhp = (long double) ((ch_getEffMaxPL(vict))) * 0.12;
                    if (AFF_FLAGGED(ch, AFF_METAMORPH) && GET_HIT(ch) <= (ch_getEffMaxPL(ch))) {
                        act("@RYour dark aura saps some of @r$N's@R life energy!@n", true, ch, NULL, vict, TO_CHAR);
                        act("@r$n@R's dark aura saps some of your life energy!@n", true, ch, NULL, vict, TO_VICT);
                        int64_t result;
                        result = ch_incCurHealth(ch, healhp, true);
                    }
                    if (IS_MUTANT(ch) && (GET_GENOME(ch, 0) == 10 || GET_GENOME(ch, 1) == 10)) {
                        int64_t result;
                        result = ch_incCurKI(ch, dmg * .05, true);
                    }
                ...
                int64_t result3;
                result3 = ch_decCurHealthPercent(vict, 1, 0);
            ...
            if (suppresso == false) {
                int64_t result;
                result = ch_decCurHealth(vict, dmg, 0);
            }
            if (FIGHTING(ch) == NULL) {
        ...
        if (OBJ_FLAGGED(obj, ITEM_UNBREAKABLE)) {
            act("$p@w seems unaffected.@n", true, ch, obj, NULL, TO_CHAR);
            act("$p@w seems unaffected.@n", true, ch, obj, NULL, TO_ROOM);
        } else if (GET_OBJ_VNUM(obj) == 79) {
            if (GET_OBJ_WEIGHT(obj) - dmg > 0) {
                if (type <= 0) {
                    if (AFF_FLAGGED(ch, AFF_INFUSE))
                        dmg *= 10;
                    act("$p@w cracks some.@n", true, ch, obj, NULL, TO_CHAR);
                    act("$p@w cracks some.@n", true, ch, obj, NULL, TO_ROOM);
                    ...
                    dmg *= 30;
                    act("$p@w melts some.@n", true, ch, obj, NULL, TO_CHAR);
                    act("$p@w melts some.@n", true, ch, obj, NULL, TO_ROOM);
                    GET_OBJ_WEIGHT(obj) -= dmg;
                    if (GET_FELLOW_WALL(obj)) {
                        struct obj_data *wall;
                        wall = GET_FELLOW_WALL(obj);
                ...
                if (type <= 0) {
                    act("$p@w breaks completely apart and then melts away.@n", true, ch, obj, NULL, TO_CHAR);
                    act("$p@w breaks completely apart and then melts away.@n", true, ch, obj, NULL, TO_ROOM);
                    extract_obj(obj);
                } else {
                    act("$p@w is blown away into snow and water!@n", true, ch, obj, NULL, TO_CHAR);
                    act("$p@w is blown away into snow and water!@n", true, ch, obj, NULL, TO_ROOM);
                    extract_obj(obj);
                }
            }
            ...
            if (type <= 0) {
                act("$p@w shatters apart!@n", true, ch, obj, NULL, TO_CHAR);
                act("$p@w shatters apart!@n", true, ch, obj, NULL, TO_ROOM);
                ...
                act("$p@w is disintegrated!@n", true, ch, obj, NULL, TO_CHAR);
                act("$p@w is disintegrated!@n", true, ch, obj, NULL, TO_ROOM);
                extract_obj(obj);
            }
        }
    } else {
...
void handle_spiral(struct char_data *ch, struct char_data *vict, int skill, int first) {
    ...
    if (vict == NULL && FIGHTING(ch)) {
        ...
        act("@WHaving lost your target you slow down until your vortex disappears, and end your attack.@n", true, ch,
            NULL, NULL, TO_CHAR);
        act("@C$n@W slows down until $s vortex disappears.@n", true, ch, NULL, NULL, TO_ROOM);
        ...
        if (prob < perc) {
            if (((ch_getCurST(vict))) > 0) {
                if (blk > rand_number(1, 130)) {
                    act("@C$N@W moves quickly and blocks your Spiral Comet blast!@n", false, ch, NULL, vict,
                        TO_CHAR);
                    act("@WYou move quickly and block @C$n's@W Spiral Comet blast!@n", false, ch, NULL, vict,
                        TO_VICT);
                    act("@C$N@W moves quickly and blocks @c$n's@W Spiral Comet blast!@n", false, ch, NULL, vict,
                        TO_NOTVICT);
                    pcost(ch, amount, 0);
                    ...
                    act("@WYou can't believe it but your Spiral Comet blast misses, flying through the air harmlessly!@n",
                        false, ch, NULL, vict, TO_CHAR);
                    act("@C$n@W fires a Spiral Comet blast at you, but misses!@n ", false, ch, NULL, vict, TO_VICT);
                    act("@c$n@W fires a Spiral Comet blast at @C$N@W, but somehow misses!@n ", false, ch, NULL, vict,
                        TO_NOTVICT);
                    pcost(ch, amount, 0);
                    hurt(0, 0, ch, vict, NULL, 0, 1);
                    return;
...
void handle_death_msg(struct char_data *ch, struct char_data *vict, int type) {
    if (type == 0) {
        if (!SUNKEN(IN_ROOM(vict)) && SECT(IN_ROOM(vict)) != SECT_WATER_SWIM &&
            ...
            switch (rand_number(1, 5)) {
                case 1:
                    act("@R$N@r coughs up blood before falling to the ground dead.@n", true, ch, NULL, vict,
                        TO_CHAR);
                    act("@rYou cough up blood before falling to the ground dead.@n", true, ch, NULL, vict, TO_VICT);
                    act("@R$N@r coughs up blood before falling down dead.@n", true, ch, NULL, vict, TO_NOTVICT);
                    break;
                case 2:
                    act("@R$N@r crumples to the ground dead.@n", true, ch, NULL, vict, TO_CHAR);
                    act("@rYou crumple to the ground dead.@n", true, ch, NULL, vict, TO_VICT);
                    act("@R$N@r crumples to the ground dead.@n", true, ch, NULL, vict, TO_NOTVICT);
                    break;
                case 3:
                    act("@R$N@r cries out $S last breath before dying.@n", true, ch, NULL, vict, TO_CHAR);
                    act("@rYou cry out your last breath before dying.@n", true, ch, NULL, vict, TO_VICT);
                    act("@R$N@r cries out $S last breath before dying.@n", true, ch, NULL, vict, TO_NOTVICT);
                    break;
                case 4:
                    act("@R$N@r writhes on the ground screaming in pain before finally dying!@n", true, ch, NULL,
                        vict, TO_CHAR);
                    act("@rYou writhe on the ground screaming in pain before finally dying!@n", true, ch, NULL, vict,
                        TO_VICT);
                    act("@R$N@r writhes on the ground screaming in pain before finally dying!@n", true, ch, NULL,
                        vict, TO_NOTVICT);
                    if (GET_DEATH_TYPE(vict) != DTYPE_HEAD) {
            ...
            switch (rand_number(1, 5)) {
                ...
                case 2:
                    act("@R$N@r crumples down to the water, with the signs of life leaving $S eyes as $E floats in the water.@n",
                        true, ch, NULL, vict, TO_CHAR);
                    act("@rYou crumple down to the water and die.@n", true, ch, NULL, vict, TO_VICT);
                    act("@R$N@r crumples down to the water, with the signs of life leaving $S eyes as $E floats in the water.@n",
                        true, ch, NULL, vict, TO_NOTVICT);
                    break;
                case 3:
                    ...
                    break;
                case 4:
                    act("@R$N@r writhes in the water screaming in pain before finally dying!@n", true, ch, NULL,
                        vict, TO_CHAR);
                    act("@rYou writhe in the water screaming in pain before finally dying!@n", true, ch, NULL, vict,
                        TO_VICT);
                    act("@R$N@r writhes in the water screaming in pain before finally dying!@n", true, ch, NULL,
                        vict, TO_NOTVICT);
                    if (GET_DEATH_TYPE(vict) != DTYPE_HEAD) {
            ...
            switch (rand_number(1, 5)) {
                case 1:
                    ...
                    break;
                case 2:
                    act("@R$N@r dies and leaves $S corpse floating freely in space.@n", true, ch, NULL, vict,
                        TO_CHAR);
                    act("@rYou die and leave your corpse floating freely in space.@n", true, ch, NULL, vict,
                        TO_VICT);
                    act("@R$N@r dies and leaves $S corpse floating freely in space.@n", true, ch, NULL, vict,
                        TO_NOTVICT);
                    break;
                case 3:
                    ...
                    break;
                case 4:
                    act("@R$N@r writhes in space trying to scream in pain before $e finally dies!@n", true, ch, NULL,
                        vict, TO_CHAR);
                    act("@rYou writhe in space trying to scream in pain before you finally die!@n", true, ch, NULL,
                        vict, TO_VICT);
                    act("@R$N@r writhes in space trying to scream in pain before $e finally dies!@n", true, ch, NULL,
                        vict, TO_NOTVICT);
                    if (GET_DEATH_TYPE(vict) != DTYPE_HEAD) {
            ...
            switch (rand_number(1, 5)) {
                case 1:
                    ...
                    break;
                case 2:
                    act("@R$N@r dies and $S corpse begins to fall to the ground below.@n", true, ch, NULL, vict,
                        TO_CHAR);
                    act("@rYou die and your corpse begins to fall to the ground below.@n", true, ch, NULL, vict,
                        TO_VICT);
                    act("@R$N@r dies and $S corpse begins to fall to the ground below.@n", true, ch, NULL, vict,
                        TO_NOTVICT);
                    break;
                case 3:
                    ...
                    break;
                case 4:
                    act("@R$N@r writhes in midair screaming in pain before finally dying!@n", true, ch, NULL, vict,
                        TO_CHAR);
                    act("@rYou writhe in midair screaming in pain before finally dying!@n", true, ch, NULL, vict,
                        TO_VICT);
                    act("@R$N@r writhes in midair screaming in pain before finally dying!@n", true, ch, NULL, vict,
                        TO_NOTVICT);
                    if (GET_DEATH_TYPE(vict) != DTYPE_HEAD) {
            ...
            switch (rand_number(1, 5)) {
                case 1:
                    ...
                    break;
                case 2:
                    act("@R$N@r dies and $S corpse begins to float limply in the water.@n", true, ch, NULL, vict,
                        TO_CHAR);
                    act("@rYou die and your corpse begins to float limply in the water.@n", true, ch, NULL, vict,
                        TO_VICT);
                    act("@R$N@r dies and $S corpse begins to float limply in the water.@n", true, ch, NULL, vict,
                        TO_NOTVICT);
                    break;
        ...
        if (!SUNKEN(IN_ROOM(vict)) && SECT(IN_ROOM(vict)) != SECT_WATER_SWIM &&
            ...
            switch (rand_number(1, 5)) {
                case 1:
                    act("@R$N@r explodes and chunks of $M shower to the ground.@n", true, ch, NULL, vict, TO_CHAR);
                    act("@rYou explode leaving only chunks behind.@n", true, ch, NULL, vict, TO_VICT);
                    act("@R$N@r explodes and chunks of $M shower to the ground.@n", true, ch, NULL, vict,
                        TO_NOTVICT);
                    GET_DEATH_TYPE(vict) = DTYPE_VAPOR;
                    break;
                case 2:
                    act("@rThe bottom half of @R$N@r is all that remains as $E dies.@n", true, ch, NULL, vict,
                        TO_CHAR);
                    act("@rThe bottom half of your body is all that remains as you die.@n", true, ch, NULL, vict,
                        TO_VICT);
                    act("@rThe bottom half of @R$N@r is all that remains as $E dies.@n", true, ch, NULL, vict,
                        TO_NOTVICT);
                    ...
                    break;
                case 3:
                    act("@R$N@r is completely disintegrated in the attack!@n", true, ch, NULL, vict, TO_CHAR);
                    act("@rYour body completely disintegrates in the attack!@n", true, ch, NULL, vict, TO_VICT);
                    act("@R$N@r is completely disintegrated in the attack!@n", true, ch, NULL, vict, TO_NOTVICT);
                    GET_DEATH_TYPE(vict) = DTYPE_VAPOR;
                    break;
                case 4:
                    act("@R$N@r falls down as a smoldering corpse!@n", true, ch, NULL, vict, TO_CHAR);
                    act("@rYour body falls down as a smoldering corpse!@n", true, ch, NULL, vict, TO_VICT);
                    act("@R$N@r falls down as a smoldering corpse!@n", true, ch, NULL, vict, TO_NOTVICT);
                    break;
                case 5:
                    act("@rWhat's left of @R$N@r's body slams into the ground as $E dies!@n", true, ch, NULL, vict,
                        TO_CHAR);
                    act("@rWhat's left of your body slams into the ground as you die!@n", true, ch, NULL, vict,
                        TO_VICT);
                    act("@rWhat's left of @R$N@r's body slams into the ground as $E dies!@n", true, ch, NULL, vict,
                        TO_NOTVICT);
                    break;
            ...
            switch (rand_number(1, 5)) {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    act("@R$N@r floats away as a smoldering corpse!@n", true, ch, NULL, vict, TO_CHAR);
                    act("@rYour body floats away as a smoldering corpse!@n", true, ch, NULL, vict, TO_VICT);
                    act("@R$N@r floats away as a smoldering corpse!@n", true, ch, NULL, vict, TO_NOTVICT);
                    break;
                case 5:
                    act("@rWhat's left of @R$N@r's body floats away through space!@n", true, ch, NULL, vict,
                        TO_CHAR);
                    act("@rWhat's left of your body floats away through space!@n", true, ch, NULL, vict, TO_VICT);
                    act("@rWhat's left of @R$N@r's body floats away through space!@n", true, ch, NULL, vict,
                        TO_NOTVICT);
                    break;
            }
            ...
            switch (rand_number(1, 5)) {
                case 1:
                    act("@R$N@r explodes and chunks of $M shower towards the ground far below.@n", true, ch, NULL,
                        vict, TO_CHAR);
                    act("@rYou explode leaving only chunks behind.@n", true, ch, NULL, vict, TO_VICT);
                    act("@R$N@r explodes and chunks of $M shower toward the ground far below.@n", true, ch, NULL,
                        vict, TO_NOTVICT);
                    GET_DEATH_TYPE(vict) = DTYPE_VAPOR;
                    break;
                case 2:
                ...
                case 3:
                    ...
                    break;
                case 4:
                    act("@R$N@r falls down toward the ground as a smoldering corpse!@n", true, ch, NULL, vict,
                        TO_CHAR);
                    act("@rYour body falls down toward the ground as a smoldering corpse!@n", true, ch, NULL, vict,
                        TO_VICT);
                    act("@R$N@r falls down toward the ground as a smoldering corpse!@n", true, ch, NULL, vict,
                        TO_NOTVICT);
                    break;
                case 5:
                    act("@rWhat's left of @R$N@r's body falls toward the ground as $E dies!@n", true, ch, NULL, vict,
                        TO_CHAR);
                    act("@rWhat's left of yor body falls toward the ground as you die!@n", true, ch, NULL, vict,
                        TO_VICT);
                    act("@rWhat's left of @R$N@r's body falls toward the ground as $E dies!@n", true, ch, NULL, vict,
                        TO_NOTVICT);
                    break;
            ...
            switch (rand_number(1, 5)) {
                case 1:
                    act("@R$N@r explodes and chunks of $M float freely through the water.@n", true, ch, NULL, vict,
                        TO_CHAR);
                    act("@rYou explode leaving only chunks behind.@n", true, ch, NULL, vict, TO_VICT);
                    act("@R$N@r explodes and chunks of $M float freely through the water.@n", true, ch, NULL, vict,
                        TO_NOTVICT);
                ...
                case 3:
                    ...
                    break;
                case 4:
                    act("@R$N@r falls back as a smoldering corpse!@n", true, ch, NULL, vict, TO_CHAR);
                    act("@rYour body falls back as a smoldering corpse!@n", true, ch, NULL, vict, TO_VICT);
                    act("@R$N@r falls back as a smoldering corpse!@n", true, ch, NULL, vict, TO_NOTVICT);
                    break;
                case 5:
                    act("@rWhat's left of @R$N@r's body floats limply as $E dies!@n", true, ch, NULL, vict, TO_CHAR);
                    act("@rWhat's left of yor body floats limply as you die!@n", true, ch, NULL, vict, TO_VICT);
                    act("@rWhat's left of @R$N@r's body floats limply as $E dies!@n", true, ch, NULL, vict,
                        TO_NOTVICT);
                    break;
            }
        }
