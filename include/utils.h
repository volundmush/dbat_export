...
extern int Q_FIELD(bitvector_t x);
extern bitvector_t Q_BIT(bitvector_t x);
extern bool IS_SET_AR(const bitvector_t field[], int bit);
extern bool SET_BIT_AR(bitvector_t field[], int bit);
extern bool REMOVE_BIT_AR(bitvector_t field[], int bit);
extern bool TOGGLE_BIT_AR(bitvector_t field[], int bit);
...
extern bool ROOM_FLAGGED(room_rnum loc, int flag);
...
extern bool DEAD(struct char_data *ch);
...
extern room_vnum GET_ROOM_VNUM(room_rnum rnum);

#define GET_ROOM_SPEC(room) \
    (VALID_ROOM_RNUM(room) ? world[(room)].func : 0)

/* Minor Planet Defines */
extern bool PLANET_ZENITH(room_rnum room);
...
extern const char* GET_NAME(struct char_data *ch);
extern const char* GET_TITLE(struct char_data *ch);
#define GET_USER_TITLE(d) ((d)->title)
#define GET_PHASE(ch)   ((ch)->starphase)
#define GET_MIMIC(ch)   ((ch)->mimic ? (ch)->mimic+1 : 0)
...
extern const char* GET_USER(struct char_data *ch);
...
extern uint8_t GET_PC_HEIGHT(struct char_data *ch);
extern uint8_t GET_PC_WEIGHT(struct char_data *ch);
...
#define GET_SPEEDINT(ch) (IS_BIO(ch) ? ((GET_CHA(ch) * GET_DEX(ch)) * (ch_getEffMaxPL(ch) / 1200) / 1200) + (GET_CHA(ch) * (GET_KAIOKEN(ch) * 100)) : ((GET_CHA(ch) * GET_DEX(ch)) * (ch_getEffMaxPL(ch) / 1000) / 1000) + (GET_CHA(ch) * (GET_KAIOKEN(ch) * 100)))
...
#define GET_HIT(ch)      (ch_getCurPL(ch))
#define GET_MAX_MOVE(ch)  (ch_getMaxST(ch))
#define GET_MAX_MANA(ch)  (ch_getMaxKI(ch))
...
extern mob_vnum GET_MOB_VNUM(struct char_data *mob);
...
#define CAN_CARRY_W(ch) (ch_getMaxCarryWeight(ch))
...
extern bool IS_INMENU(struct descriptor_data *d);
...
extern bool CAN_WEAR(struct obj_data *obj, int part);
...
extern const char* HSHR(struct char_data *ch);
extern const char* HSSH(struct char_data *ch);
extern const char* HMHR(struct char_data *ch);
extern const char* MAFE(struct char_data *ch);
extern const char* ANA(struct obj_data *obj);
extern const char* SANA(struct obj_data *obj);


/* Various macros building up to CAN_SEE */

extern bool LIGHT_OK(struct char_data *sub);
...
extern bool ISWIZ(struct char_data *ch, struct char_data *vict);
...
extern bool CAN_GO(struct char_data *ch, int door);

#define RACE(ch)      (ch_juggleRaceName(ch, true))
#define LRACE(ch)     (ch_juggleRaceName(ch, false))
#define TRUE_RACE(ch) (race_getName((ch)->race))

#define CLASS_ABBR(ch) (sen_getAbbr((ch)->chclass))
#define RACE_ABBR(ch) (race_getAbbr((ch)->race))
...
extern bool IS_NEUTER(struct char_data *ch);
...
extern bool DIRT_ROOM(struct char_data *ch);
...
extern double min_double(double a, double b);
extern double max_double(double a, double b);
extern double abs_double(double a);
extern int64_t max_i64(int64_t a, int64_t b);
extern int64_t min_i64(int64_t a, int64_t b);
extern int64_t abs_i64(int64_t a);
