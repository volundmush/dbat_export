...
struct obj_data {
    ...
    IDXTYPE value[NUM_OBJ_VAL_POSITIONS];   /* Values of the item (see list)    */
...
enum CharStatusCategory {
    CSTAT_PRIMARYFORM = 0,
    CSTAT_SECONDARYFORM = 1,
    CSTAT_BUFF = 2,
    CSTAT_CONSUMABLE = 3,
    CSTAT_EQUIPMENT = 4,
    CSTAT_QUEST = 5,
...
struct cstatus_data {
    int category; // this is for a CharStatusCategory
    int id; // the ID of the specific status. These are in an array elsewhere.
    int duration; // -1 for indefinite.
    int modifier; // A modifier like skill level it was applied with.
    struct cstatus_data *next; // Next node in the chain.
};

struct cstatus_holder {
    int category; // the stat category
    int id; // the stat id
    int mode; // the kind of value being added like premod or mult
    double value; // the value to modify by.
    struct cstatus_holder *next;
};
